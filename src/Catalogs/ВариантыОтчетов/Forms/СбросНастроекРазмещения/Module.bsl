...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Варианты) <> Тип("Массив") Тогда
		ТекстОшибки = НСтр("ru = 'Не указаны варианты отчетов.'; en = 'Not specified reporting options.'");
		Возврат;
	КонецЕсли;
...
Процедура КомандаСбросить(Команда)
	...
	Если КоличествоВыбранныхВариантов = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не указаны варианты отчетов.'; en = 'Not specified reporting options.'"));
		Возврат;
	КонецЕсли;
	...
	Если КоличествоВариантов = 1 И КоличествоВыбранныхВариантов = 1 Тогда
		...
		ОповещениеЗаголовок = НСтр("ru = 'Сброшены настройки размещения варианта отчета'; en = 'Reset network settings option of the report'");
...
"- Выбраны варианты дополнительных или внешних отчетов.'; en = 'Resetting the settings of selected report variants is not required for one or several reasons:"
"- Custom report variants are selected."
"- Reporting variants marked on deletion have been selected."
"- Variants for additional or external reports are selected.'");
			Возврат;
		КонецЕсли;
	КонецЕсли;
