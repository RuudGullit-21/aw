...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Отказ Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Курс валюты можно связать только с курсом независимой валюты.'; en = 'Currency rate can be linked only with course of independent currency.'"));
	КонецЕсли;
	...
	Если Не ЭтоНовый()
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Валюта не может быть подчиненной, так как она является основной для других валют.'; en = 'Currency cannot be subordinated, as it is the basis for other currencies.'"));
		Отказ = Истина;
	КонецЕсли;
...
Функция ИспользуемыеВалютыПриРасчетеКурса()
	...
	Если РезультатЗапроса.Пустой() Тогда
		ТекстОшибки = НСтр("ru = 'В формуле должна быть использована хотя бы одна основная валюта.'; en = 'At least one main currency is to be used in the formula.'");
