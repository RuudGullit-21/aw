...
Процедура ПередЗаписью(Отказ)
	...
	Если ЗначениеЗаполнено(Родитель) И Родитель.Автор <> Автор Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Нельзя указывать группу другого пользователя.'; en = 'You cannot specify group of another user.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Не ЭтоГруппа Тогда 
		...
		Если ПустаяСтрока(Тема) Тогда 
			Тема = "<" + НСтр("ru = 'Пустая заметка'; en = 'Blank note'") + ">";
		КонецЕсли;
