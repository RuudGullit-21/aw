...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ДополнительныеСвойства.Свойство("ЗаполнениеПредопределенных") Тогда
		ВызватьИсключение НСтр("ru = 'Запись в справочник ""Предопределенные варианты отчетов"" запрещена. Его данные заполняются автоматически.'; en = 'Record in catalog ""Predefined versions of reports"" is prohibited. Its data is filled in automatically.'");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьЗаполнениеПредопределенного(Отказ)
	...
	ВызватьИсключение НСтр("ru = 'Не заполнено поле ""Отчет""'; en = 'Not filled in field ""Report""'");
