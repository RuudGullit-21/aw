...
"Каждую категорию разрешено вводить только один раз.'; en = 'Category [%КатегорияНовостей%] is repeated several times."
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЗагруженоССервера = Истина Тогда
		...
		Если (ПометкаУдаления = Истина)
			...
			Иначе
				ТекстСообщения = НСтр("ru = 'Нельзя помечать на удаление ленты новостей, загруженные автоматически с сервера новостей.'; en = 'Cannot mark news feeds for deletion that were downloaded from the news server automatically.'");
				Сообщение = Новый СообщениеПользователю();
