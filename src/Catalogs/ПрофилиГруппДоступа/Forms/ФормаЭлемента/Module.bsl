		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНесуществующаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Роль ""%1"" не найдена в метаданных.'; en = 'Role ""%1"" was not found in metadata.'"), Строка.Синоним),
					"Роли",
			...
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Роль ""%1"" недоступна для назначения профиля.'; en = 'Role ""%1"" is not available for profile assignment.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
...
Процедура ВосстановитьПоНачальномуЗаполнению(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("ru = 'Восстановить профиль по содержимому начального заполнения?'; en = 'Restore the profile to its initial state?'"),
...
Процедура ВыбратьНазначение(Команда)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("ru = 'Выбор назначения профиля групп доступа'; en = 'Select a purpose of an access group profile'"),,, ОписаниеОповещения);
КонецПроцедуры
...
"группы доступа профиля обновлены.'; en = 'Profile ""%1"" is restored to its initial state,"
"access groups of the profile are updated.'");
	Иначе
		Текст =
			НСтр("ru = 'Профиль ""%1"" восстановлен по содержимому начального заполнения,"
"группы доступа профиля не обновлены.'; en = 'Profile ""%1"" is restored to its initial state,"
...
"значениями доступа и добавлены недостающие виды доступа.'; en = 'Do you want to update the access groups that use this profile?"
""
"Additional types of access will be deleted with the specified types of access"
"access values and missing access types.'");
