...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ru = 'Не совместимы с текущей версией'; en = 'Not compatible with the current version'");
		...
		Дело.ЕстьДела      = КоличествоНесовместимыхПрофилейГруппДоступа > 0;
		Дело.Важное        = Истина;
		Дело.Представление = НСтр("ru = 'Профили групп доступа'; en = 'Access group profiles'");
		Дело.Количество    = КоличествоНесовместимыхПрофилейГруппДоступа;
	...
	Попытка
		...
		Иначе
			ПрофильПоНаименованию = ПрофильПоНаименованию(
				НСтр("ru = 'Администратор'; en = 'Administrator'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ЗаполнитьПрофильАдминистратор(ОписаниеПрофиляАдминистратор, КромеОписания = Ложь)
	...
	ОписаниеПрофиляАдминистратор.Наименование  = НСтр("ru = 'Администратор'; en = 'Administrator'", ОбщегоНазначения.КодОсновногоЯзыка());
...
"""%4"".'; en = 'Role ""%3"" that does not satisfy profile"
"purpose:"
"""%4"""
...
Процедура ПодготовитьВидыДоступаПоставляемогоПрофиля(СвойстваПрофиля, ОписаниеПрофиля,
	...
	Для Каждого ЭлементСписка Из ОписаниеПрофиля.ВидыДоступа Цикл
		...
		Если СвойстваВидовДоступа.ПоИменам.Получить(ИмяВидаДоступа) = Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании профиля ""%1"""
"указан несуществующий вид доступа ""%2"".'; en = 'Non-existing access kind ""%2"""
...
"""%3"".'; en = 'Access kind ""%2"" for access"
"value ""%3"" in profile description"
"""%1"""
"does not exist.'"),
				?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
				  ОписаниеПрофиля.Имя,
...
"""%3"".'; en = 'Non-reference access value type ""%3"""
"is specified in profile description"
"""%1"" for access kind"
"""%2"".'"),
				?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
...
"типа, который не указан в свойствах вида доступа ""%2"".'; en = 'Access value ""%3"" of the type"
"that is not specified in properties of access kind ""%2"""
"is specified in profile description ""%1"".'"),
				?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
				  ОписаниеПрофиля.Имя,
				  ОписаниеПрофиля.Идентификатор),
...
"""%3"".'; en = 'Access value ""%3"" is specified"
"repeatedly in profile description ""%1"""
"for access kind"
"""%2"".'"),
				?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
				  ОписаниеПрофиля.Имя,
...
Функция ПредставлениеНазначенияПрофиля(НазначениеПрофиля)
	...
	Если НазначениеПрофиля = "СовместноДляПользователейИВнешнихПользователей" Тогда
		Возврат НСтр("ru = 'Совместно для пользователей и внешних пользователей'; en = 'Shared by users and external users'");
		
	ИначеЕсли НазначениеПрофиля = "ДляВнешнихПользователей" Тогда
		Возврат НСтр("ru = 'Для внешних пользователей'; en = 'For external users'");
	КонецЕсли;
	
	Возврат НСтр("ru = 'Для пользователей'; en = 'For users'");
