...
"Можно использовать символы подстановки * и ?'; en = 'Full-text search is enabled."
"It is allowed to use substitution symbols * and ?'");
	Иначе
		Элементы.СтрокаПоиска.Подсказка = НСтр("ru = 'Полнотекстовый поиск отключен."
"Доступен поиск только точных фраз'; en = 'Full-text search is disabled."
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Строка поиска пустая'; en = 'The search string is empty'");
	...
	Если (ЭтотОбъект.ПоискДатаОТ <> '00010101') И (ЭтотОбъект.ПоискДатаДО <> '00010101') Тогда
		Если (ЭтотОбъект.ПоискДатаОТ > ЭтотОбъект.ПоискДатаДО) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Неверно задан интервал поиска по датам: дата начала позже даты окончания'; en = 'Search interval by dates is specified incorrectly: start date is greater than end date'");
