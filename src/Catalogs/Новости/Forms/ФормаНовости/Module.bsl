...
"Используйте загрузку новостей из лент новостей.'; en = 'You cannot create news manually in the news catalog. "
	...
	Если Объект.ПриОткрытииСразуПереходитьПоСсылке = Истина Тогда
		Если НЕ ПустаяСтрока(НавигационнаяСсылка) Тогда
			...
			ИначеЕсли СтрНайти(ВРег(НавигационнаяСсылка), ВРег("1C:")) = 1 Тогда // 1C - "С" - английская
				...
				ОповеститьИВыйти = Истина;
			Иначе
				ТекстСообщения = НСтр("ru = 'Неизвестная ссылка: %НавигационнаяСсылка%'; en = 'Unknown link: %НавигационнаяСсылка%'");
...
Процедура УправлениеФормойНаКлиенте()
	...
	Если Пометка = 0 Тогда
		...
		Элементы.Пометка.Подсказка = НСтр("ru = 'Новость не отмечена'; en = 'News is not marked'");
	Иначе
		...
		Элементы.Пометка.Подсказка = НСтр("ru = 'Новость отмечена'; en = 'News marked'");
	КонецЕсли;
