...
"или закройте окно вручную, до выхода из программы.'; en = 'It is necessary to recalculate the service data."
"Click the [Service Data Recalculating] button"
...
"Пользователю с ролью РедактированиеНовостей можно редактировать разрешенные Администратором локальные ленты новостей.'; en = 'You cannot add news if there are no local news feeds where editing is allowed."
...
"Пользователю с ролями АдминистраторСистемы и ПолныеПрава можно редактировать все локальные ленты новостей."
"Пользователю с ролью РедактированиеНовостей можно редактировать разрешенные Администратором локальные ленты новостей.'; en = 'It is allowed to change news only in the local news feed where editing is allowed."
"User with the АдминистраторСистемы and ПолныеПрава roles can edit all local news feeds."
"User with the РедактированиеНовостей role can edit the local news feeds allowed by Administrator.'");
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = ТекстСообщения;
...
Процедура ПересчетСлужебныхДанныхНаСервере()
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Пересчет служебных данных по новостям'; en = 'Recalculation of service data on news'"));
...
""
"По шагам:"
"%4'; en = 'Recalculation of all service data on news"
"Start time (ms): %1"
"End time (ms): %2"
"Duration (ms): %3"
""
...
"%4'")
		...
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ПересчетВсехСлужебныхДанныхПоНовостям'; en = 'Новости. Сервис и регламент. ПересчетВсехСлужебныхДанныхПоНовостям'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
