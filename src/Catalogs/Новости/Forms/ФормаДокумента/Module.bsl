...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '(все время указано для часового пояса %1 (%2))'; en = '(all the time listed for time zone %1 (%2))'"),
		ЧасовойПояс(),
	...
	Если (ЭтотОбъект.БезАвторизации = Истина)
		...
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("ru = 'Для редактирования этой новости необходимо авторизоваться в новостном центре.'; en = 'To edit this news, authorize in the news center.'");
	...
	ИначеЕсли (ЭтотОбъект.БезАвторизации = Ложь)
		...
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("ru = 'Новость была успешно опубликована. Чтобы изменить ее, необходимо вначале отменить публикацию.'; en = 'News was successfully published. To change it, it is required to cancel publication first.'");
		Элементы.ДекорацияВозможностьИзмененияНовости.Видимость = Истина;
		ЭтотОбъект.ТолькоПросмотр = Истина;
	ИначеЕсли (ЭтотОбъект.БезАвторизации = Ложь)
			И (ЭтотОбъект.СтатусНовостиНаСервере = 3) Тогда // Есть на сервере, опубликована, нельзя повторно публиковать.
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("ru = 'Новость была успешно опубликована. Так как новость была опубликована в каналах, по которым запрещена перепубликация (емейл, смс), то изменить ее уже нельзя, но можно скопировать эту новость и править уже ее.'; en = 'News was successfully published. As the news was published in channels where republication is prohibited (email, SMS), it cannot be changed but it can be copied and then changed.'");
		Элементы.ДекорацияВозможностьИзмененияНовости.Видимость = Истина;
		ЭтотОбъект.ТолькоПросмотр = Истина;
	КонецЕсли;
...
"Невозможно отредактировать новость.'; en = 'No news feed allowed for editing. "
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для С=0 По Объект.Действия.Количество()-1 Цикл
		...
		Если НайденныеПовторения.Количество() > 1 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Идентификатор действия %УИНДействия% повторяется %КоличествоПовторений%'; en = 'Action ID %УИНДействия% is repeated %КоличествоПовторений%'");
		...
		Если Лев(ВРег(ТекущаяСтрока.УИНДействия), "3") <> ВРег("1C:") Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Идентификатор действия должен начинаться с 1C:'; en = 'Action ID should start with 1C:'");
			Сообщение.Поле  = "Объект.Действия[" + С + "].УИНДействия";
			Сообщение.Сообщить();
		КонецЕсли;
		...
		Для ТекущийСимвол=4 По СтрДлина(ТекущаяСтрока.УИНДействия) Цикл
			...
			Если (СтрНайти(РазрешенныеСимволы, Символ) = 0) Тогда
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru = 'Идентификатор действия должен начинаться с 1C: и состоять только из английских символов, цифр и знака подчеркивания'; en = 'Action ID should start with 1C: and contain only English letters, digits and an underscore'");
				Сообщение.Поле  = "Объект.Действия[" + С + "].УИНДействия";
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЦикла;
		...
		Если ПустаяСтрока(ТекущаяСтрока.Действие) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Пустое действие'; en = 'Empty action'");
		...
		Иначе
			...
			Иначе
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru = 'Неправильное действие. Выберите значение действия из списка'; en = 'Incorrect action. Select action value from the list'");
				Сообщение.Поле  = "Объект.Действия[" + С + "].Действие";
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЕсли;
	...
	Для С=0 По Объект.ПараметрыДействий.Количество()-1 Цикл
		...
		Если НайденныеВладельцы.Количество() = 0 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Параметр не привязан ни к одному действию (должно быть хотя бы одно действие с таким же идентификатором [%УИНДействия%]). Рекомендуется удалить строку.'; en = 'Parameter is not linked to any action (there should be at least one action with the same ID [%УИНДействия%]). It is recommended to remove the line.'");
			Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%УИНДействия%", ТекущаяСтрока.УИНДействия);
		...
		ИначеЕсли НайденныеВладельцы.Количество() > 1 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'У параметра должен быть только один владелец (одно действие)'; en = 'Parameter must have only one owner (one action)'");
			Сообщение.Поле  = "Объект.ПараметрыДействий[" + С + "].УИНДействия";
			Сообщение.Сообщить();
		Иначе // =1
			...
			Если ТекущийВладелец.Действие = "Переход по навигационной ссылке" Тогда
				...
				Иначе
					Отказ = Истина;
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("ru = 'У действия [%Действие%] параметр [%Параметр%] неправильный. Выберите значение из списка'; en = 'Parameter [%Параметр%] of action [%Действие%] is incorrect. Select a value from the list'");
			...
			ИначеЕсли ТекущийВладелец.Действие = "Показать картинку" Тогда
				Если ТекущаяСтрока.Параметр = "УИН" Тогда
					...
					Если НайденныеБинарныеДанные.Количество() = 0 Тогда
						Отказ = Истина;
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = НСтр("ru = 'У действия [%Действие%] параметр [%Параметр%] указывает на несуществующие бинарные данные. Выберите правильное значение бинарных данных.'; en = 'Parameter [%Параметр%] of action [%Действие%] indicates non-existing binary data.Select a correct value of binary data.'");
						Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Действие%", ТекущийВладелец.Действие);
						Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Параметр%", ТекущаяСтрока.Параметр);
						Сообщение.Поле  = "Объект.ПараметрыДействий[" + С + "].Параметр";
						Сообщение.Сообщить();
	...
	Если ОтменаРедактирования <> Истина Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Если ТекущиеДанные.ВерсияОТ > ТекущиеДанные.ВерсияДО Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru = 'Версия ОТ [ВерсияОТ] не должна быть больше, чем Версия ДО [ВерсияДО].'; en = 'Version FROM [ВерсияОТ] should not be later than Version BEFORE [ВерсияДО].'");
...
""
"А в поле Действие должно быть значение, выбранное из списка.'; en = 'Hyperlink ID should start with 1C: (where C is an English letter)"
...
"And the value selected from the list should be in the Action field.'");
			ПоказатьПредупреждение(
				...
				0, // Таймаут
				НСтр("ru = 'Ошибка'; en = 'Error'")); // Заголовок
		КонецЕсли;
	КонецЕсли;
...
Процедура КомандаЗагрузитьФайлКакБинарныеДанные(Команда)
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Выберите строку, куда помещать данные'; en = 'Select a line to place the data to'");
		ПоказатьПредупреждение(
			, // ОписаниеОповещенияОЗавершении
			ТекстОшибки, // ТекстПредупреждения
			0, // Таймаут
...
Процедура Подключаемый_КомандаБуферОбмена_Копировать(Команда)
	...
	Если ВРег(Команда.Имя) = ВРег("КомандаБуферОбмена_Копировать_КатегорииПростые")
		...
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ru = 'Выделите хотя бы одну строку'; en = 'Select at least one line'"), , , БиблиотекаКартинок.НовостиВнимание);
		КонецЕсли;
	...
	Если ВРег(Команда.Имя) = ВРег("КомандаБуферОбмена_Вставить_ТекущийБуферОбмена_КатегорииПростые")
		...
		Иначе
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Нет данных в буфере обмена.'; en = 'No data in the clipboard.'"),
				,
				НСтр("ru = 'Вначале скопируйте строки.'; en = 'Copy lines first.'"),
	...
	Если Элементы.ПривязкаКМетаданным.ТекущиеДанные <> Неопределено Тогда
		...
		Исключение
			...
			ПоказатьПредупреждение(
				...
				ТекстОшибки, // ТекстПредупреждения
				0, // Таймаут
				НСтр("ru = 'Предупреждение'; en = 'Warning'")); // Заголовок
		КонецПопытки;
	...
	Если СтрНайти(ВРег(Команда.Имя), ВРег(ШаблонИмениКоманды)) = 1 Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			Если ПустаяСтрока(ПолеТекстовогоДокумента.ВыделенныйТекст) Тогда
				ТекстГиперссылки = НСтр("ru = 'Нажмите здесь'; en = 'Click here'");
			Иначе
...
Процедура КомандаИмпортЭкспортXML(Команда)
	...
	ИначеЕсли Команда.Имя = "КомандаИмпортИзXML" Тогда
		Если Объект.ЛентаНовостей.Пустая() Тогда
			ТекстСообщения = НСтр("ru = 'Необходимо заполнить ленту новостей.'; en = 'It is necessary to fill the news feed.'");
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
...
"Approximate news size (including news text): %ОбщийРазмер% bytes"
"%Предупреждение%'");
	...
	Если Объект.БинарныеДанные.Итог("ДанныеРазмер") > 50000 Тогда
		ЛокальныйТекст = СтрЗаменить(ЛокальныйТекст, "%Предупреждение%", НСтр("ru = 'Новости большого размера могут замедлить работу программы на платформе 1С'; en = 'Big news can slow down the application on 1C platform'"));
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(СписокТабличныхЧастей = Неопределено)
	...
	Если МассивТабличныхЧастей.Найти("БинарныеДанные") <> Неопределено Тогда
		...
		Иначе
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Не удалось загрузить бинарные данные (не совпадает количество строк в форме и в объекте). При сохранении они будут утеряны. Рекомендуется закрыть / открыть форму новости без сохранения'; en = 'Cannot import the binary data (number of lines in form and in object is not the same). It will be lost while saving. It is recommended to close / open the news form without saving'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеКатегорииДляЛентыНовостейСервер()
	...
	Если МассивКатегорий_Простые.Количество() = 0 Тогда
		...
		Элементы.ГруппаКатегорииПростые.Заголовок = НСтр("ru = 'Простые категории (недоступно, так как для этой ленты новостей не включены отборы по простым категориям)'; en = 'Simple categories (not available as filters by simple categories are not enabled for this news feed)'");
	Иначе
		Элементы.КатегорииПростые.ТолькоПросмотр = Ложь;
		Элементы.ГруппаКатегорииПростые.Доступность = Истина;
		Элементы.ГруппаКатегорииПростые.Заголовок = НСтр("ru = 'Простые категории'; en = 'Simple categories'");
	КонецЕсли;
	...
	Если МассивКатегорий_ИнтервалыВерсий.Количество() = 0 Тогда
		...
		Элементы.ГруппаКатегорииИнтервалыВерсий.Заголовок = НСтр("ru = 'Привязка к версиям (недоступна, так как для этой ленты новостей не включены отборы по версиям)'; en = 'Link to versions (not available as filters by versions are not set for this news feed)'");
	Иначе
		Элементы.КатегорииИнтервалыВерсий.ТолькоПросмотр = Ложь;
		Элементы.ГруппаКатегорииИнтервалыВерсий.Доступность = Истина;
		Элементы.ГруппаКатегорииИнтервалыВерсий.Заголовок = НСтр("ru = 'Привязка к версиям'; en = 'Binding to versions'");
	КонецЕсли;
	...
	Если ТипЗнч(РезультатЗакрытия) = ТипСтруктура Тогда
		...
		Если ВРег(РезультатЗакрытия.Действие) = ВРег("Переход по интернет ссылке") Тогда
			...
			Если ПустаяСтрока(ЛокальныйВыделенныйТекст) Тогда
				ЛокальныйВыделенныйТекст = НСтр("ru = 'Нажмите здесь'; en = 'Click here'");
			КонецЕсли;
				...
				#Если НЕ ВебКлиент Тогда
					...
					Попытка
						...
						Если НЕ ПустаяСтрока(ПараметрыСхлопывающегосяБлока.УИНБлока) Тогда
							...
							Для С=1 По 5 Цикл
								ЭлементХТМЛТекст = ДокументХТМЛ.СоздатьТекстовыйУзел(НСтр("ru = 'Строка'; en = 'String'") + " " + С + Символы.ПС);
...
#Область ФорматированиеHTML_БлокиИРазрывы
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_РазрывСтроки");
		...
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Разрыв строки <br/>'; en = 'Line break <br/>'");
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_ГоризонтальнаяЛиния");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Горизонтальная линия <hr/>'; en = 'Horizontal line <hr/>'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_Параграф");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Абзац <p></p>'; en = 'Paragraph <p></p>'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_БлокDIV");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Блок для абзаца <div></div>'; en = 'Block for paragraph <div></div>'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_БлокSPAN");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Блок для текста внутри абзаца <span></span>'; en = 'Block of text within paragraph <span></span>'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматШрифта
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_Жирный");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Жирный шрифт (<b></b>)'; en = 'Bold type (<b></b>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_Курсив");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Курсивный шрифт (<i></i>)'; en = 'Italic (<i></i>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_Подчеркнутый");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Подчеркнутый шрифт (<u></u>)'; en = 'Underlined front (<u></u>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_Зачеркнутый");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Зачеркнутый шрифт (<s></s>)'; en = 'Font strikethrough (<s></s>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_ВерхнийИндекс");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Верхний индекс (<sup></sup>)'; en = 'Superscript (<sup></sup>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_НижнийИндекс");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Нижний индекс (<sub></sub>)'; en = 'Subscript (<sub></sub>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	Для С=1 По 6 Цикл
		НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_Заголовок" + С);
			...
			НоваяКоманда.Заголовок                 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Заголовок %1 уровня (<h%1></h%1>)'; en = 'Title %1 level (<h%1></h%1>)'"),
				С);
			НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
			НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
			НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматАбзаца
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_ВыравниваниеВлево");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Выравнивание влево'; en = 'Left alignment'"); // <p style="text-align: left;"></p>
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_ВыравниваниеВправо");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Выравнивание вправо'; en = 'Right alignment'"); // <p style="text-align: right;"></p>
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_ВыравниваниеПоЦентру");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Выравнивание по центру'; en = 'Center alignment'"); // <p style="text-align: center;"></p>
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_ВыравниваниеПоШирине");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Выравнивание по ширине'; en = 'Distributed'"); // <p style="text-align: justify;"></p>
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_СтилиТекста
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_ДлиннаяЦитата");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Длинная цитата (<blockquote></blockquote>)'; en = 'Long quote (<blockquote></blockquote>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_КороткаяЦитата");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Короткая цитата (<q></q>)'; en = 'Short quotation (<q></q>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_ИсточникЦитаты");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Источник цитаты (<cite></cite>)'; en = 'Source citations (<cite></cite>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_ВводСКлавиатуры");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Ввод с клавиатуры (<kbd></kbd>)'; en = 'Keyboard input (<kbd></kbd>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_ПрограммныйКод");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Программный код (<code></code>)'; en = 'Code (<code></code>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаВставитьТег_АкцентированныйТекст");
		НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
		НоваяКоманда.Заголовок                 = НСтр("ru = 'Акцентированный текст (<em></em>)'; en = 'Accented text (<em></em>)'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_БлокиИРазрывы
	...
	ГруппаКоманд.Заголовок = НСтр("ru = 'Блоки и разрывы'; en = 'Blocks and breaks'");
...
#Область ФорматированиеHTML_ФорматШрифта
	...
	ГруппаКоманд.Вид = ВидГруппыФормы.ГруппаКнопок;
	ГруппаКоманд.Заголовок = НСтр("ru = 'Формат шрифта'; en = 'Font format'");
...
#Область ФорматированиеHTML_ФорматШрифта_Заголовки
	...
	ГруппаКоманд.Заголовок = НСтр("ru = 'Заголовки'; en = 'Headers'");
...
#Область ФорматированиеHTML_СтилиТекста
	...
	ГруппаКоманд.Вид = ВидГруппыФормы.ГруппаКнопок;
	ГруппаКоманд.Заголовок = НСтр("ru = 'Стили текста'; en = 'Text styles'");
...
Процедура СоздатьКомандыБуфераОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаБуферОбмена_Копировать_" + ТекущийБуферОбмена);
			...
			НоваяКоманда.Заголовок                 = НСтр("ru = 'Копировать'; en = 'Copy'");
			НоваяКоманда.ИзменяетСохраняемыеДанные = Ложь;
			НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
			НоваяКоманда.Подсказка                 = НСтр("ru = 'Копировать выделенные строки в буфер обмена'; en = 'Copy the selected lines to clipboard'");
		...
		НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаБуферОбмена_Настроить_" + ТекущийБуферОбмена);
			...
			НоваяКоманда.Заголовок                 = НСтр("ru = 'Управление сохраненными буферами обмена'; en = 'Saved clipboards management'");
			НоваяКоманда.ИзменяетСохраняемыеДанные = Ложь;
			НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
			НоваяКоманда.Подсказка                 = НСтр("ru = 'Управление сохраненными раньше буферами обмена'; en = 'Manage saved before the clipboards'");
		...
		НоваяКоманда = ЭтотОбъект.Команды.Добавить("КомандаБуферОбмена_Вставить_ТекущийБуферОбмена_" + ТекущийБуферОбмена);
			...
			НоваяКоманда.Заголовок                 = НСтр("ru = 'Вставить содержимое буфера обмена'; en = 'Insert the clipboard content'");
			НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
			НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
			НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
Процедура СоздатьКомандыБуфераОбмена_СохраненныеБуферыОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		Для каждого ТекущийЭлементСписка Из ЭтотОбъект["СохраненныеБуферыОбмена_" + ТекущийБуферОбмена] Цикл
			...
			НоваяКоманда.Действие                  = "Подключаемый_КомандаБуферОбмена_Вставить";
			НоваяКоманда.Заголовок                 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Вставить из %1'; en = 'Insert from %1'"),
...
Процедура СоздатьПодменюБуфераОбмена(Подменю, БуферОбмена)
	...
	ГруппаКомандВставить.Заголовок   = "Вставить";
		...
		ГруппаКомандВставитьСохраненный.Заголовок = НСтр("ru = 'Вставить сохраненный'; en = 'Insert saved'");
...
Процедура УстановитьУсловноеОформление()
	...
	// 2. Заменить в поле Объект.Важность число (0, 1, 2) на текст ("Обычная", "Очень важная", "Важная").
		УсловияОтбора = Новый Соответствие;
		УсловияОтбора.Вставить(0, НСтр("ru = 'Обычная'; en = 'Normal'"));
		УсловияОтбора.Вставить(1, НСтр("ru = 'Очень важная'; en = 'Very important'"));
		УсловияОтбора.Вставить(2, НСтр("ru = 'Важная'; en = 'Important'"));
	...
	// 8. Поле "Единственный параметр".
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Несколько параметров настраиваются в таблице [Параметры]'; en = 'Several parameters are set up in the table [Parameters]'"));
		Элемент.Оформление.УстановитьЗначениеПараметра("Доступность", Ложь);
...
" - 13 = %Текст1С_13_Э% / %Текст1С_13_О%,'; en = 'text to external sources:"
"- 10 = %ТекстВИ_10_Э% / %ТекстВИ_10_О%,"
"- 13 = %ТекстВИ_13_Э% / %ТекстВИ_13_О%,"
"Text for 1C:"
"- 10 = %Текст1С_10_Э% / %Текст1С_10_О%,"
"- 13 = %Текст1С_13_Э% / %Текст1С_13_О%,'")
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru = 'Статистика по переводам строк:'; en = 'Newline statistics:'"),
		,
		ТекстПояснения,
			...
			Если СвойствоСписокКатегорийПростых <> Неопределено Тогда
				...
				Для каждого ТекущаяКатегорияПростаяХДТО Из МассивОбъектовХДТО Цикл
					...
					Если НайденнаяКатегория.Пустая() Тогда
						ТекстСообщения = НСтр("ru = 'Категория (простая) с УИН %УИН% и кодом %Код% не найдена.'; en = 'Category (simple) with UIC %УИН% with code %Код% not found.'");
		...
		Если СвойствоКатегорииИнтервалыВерсий <> Неопределено Тогда
			...
			Если СвойствоСписокКатегорийИнтервалыВерсий <> Неопределено Тогда
				...
				Для каждого ТекущаяКатегорияИнтервалыВерсийХДТО Из МассивОбъектовХДТО Цикл
					...
					НайденнаяКатегория = ПланыВидовХарактеристик.КатегорииНовостей.НайтиПоКоду(КодКатегории);
					Если НайденнаяКатегория.Пустая() Тогда
						ТекстСообщения = НСтр("ru = 'Категория (интервал версий) с УИН %УИН% и кодом %Код% не найдена.'; en = 'Category (interval of version) with UIC %УИН% and code %Код% not found.'");
...
Функция ПолучитьПредставлениеНовостиXML()
	...
	Если ПустаяСтрока(ЗаголовокНовостиЛокальный) Тогда
		ЗаголовокНовостиЛокальный = НСтр("ru = 'Заголовок новости'; en = 'News title'");
	КонецЕсли;
