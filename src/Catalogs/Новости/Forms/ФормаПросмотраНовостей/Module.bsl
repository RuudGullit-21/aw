...
Процедура КомандаПоиск(Команда)
	...
	Для каждого ТекущаяПапка Из ЭтотОбъект.Папки.ПолучитьЭлементы() Цикл
		...
		Если ТекущаяПапка.ВариантОтбора = 2 Тогда // Результаты поиска
			...
			Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
				ТекущаяПапка.Представление = НСтр("ru = 'Результаты поиска'; en = 'Search results'");
			Иначе
				ТекущаяПапка.Представление = НСтр("ru = 'Результаты поиска'; en = 'Search results'") + " (" + СокрЛП(ЭтотОбъект.СтрокаПоиска) + ")";
			КонецЕсли;
...
#Область Проверки
	...
	Если ТекущиеДанные.ОбязательныйКанал = Истина Тогда
		ТекстСообщения = Новый Массив;
			ТекстСообщения.Добавить(НСтр("ru = 'Лента новостей'; en = 'News feed'"));
...
"%2"
"%3'; en = 'Last update of the list of news was"
"%1"
"The next update of the list of news will happen"
"%2"
"%3'"),
...
Процедура ОбновитьСписокПапокОтбора()
	...
	НоваяСтрока = КорневойУровень.Добавить();
		...
		НоваяСтрока.Наименование           = НСтр("ru = 'Все новости'; en = 'All news'");
	...
	НоваяСтрока = КорневойУровень.Добавить();
		...
		НоваяСтрока.ЗначениеОтбора         = Неопределено;
		Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
			НоваяСтрока.Наименование = НСтр("ru = 'Результаты поиска'; en = 'Search results'");
		Иначе
			НоваяСтрока.Наименование =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Результаты поиска (%1)'; en = 'Search results (%1)'"),
	...
	НоваяСтрока = КорневойУровень.Добавить();
		...
		НоваяСтрока.ЗначениеОтбора         = Неопределено;
		НоваяСтрока.Наименование           = НСтр("ru = 'Помеченные новости'; en = 'Marked news'");
		НоваяСтрока.КоличествоНепрочтенных = 0; //// ... Всегда = 0, т.к. расчет количества непрочтенных для этой ситуации будет ресурсоемким
		НоваяСтрока.ВыделеннаяСтрока       = Ложь;
	...
	Если НЕ ПараметрыСеанса_ТекущийПользователь.Пустая() Тогда // Неразделенный режим - нельзя прочитать никакие настройки
		...
		Если ЛентыНовостейПодпискаОтключенаПользователем.Количество() > 0 Тогда
			НоваяСтрока = КорневойУровень.Добавить();
				...
				НоваяСтрока.ЗначениеОтбора         = Неопределено;
				НоваяСтрока.Наименование           = НСтр("ru = 'Отключенные ленты новостей'; en = 'Disabled news feeds'");
...
Процедура УстановитьУсловноеОформление()
	...
	// 5. Представление даты: сегодня.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Сегодня'; en = 'Today'"));
	...
	// 6. Представление даты: вчера.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Вчера'; en = 'Yesterday'"));
