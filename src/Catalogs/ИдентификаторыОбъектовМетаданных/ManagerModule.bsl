...
"нажмите кнопку Отключить или программно очистите константу Главный узел.'; en = 'Catalog ""%1"" cannot be used"
"in the infobase with unconfirmed main node cancellation. "
""
"To restore connection with the main node,"
"launch 1C:Enterprise and click ""Restore"" or set programmatically"
"the main node saved in constant ""Main node""."
""
"To confirm cancellation of connection with the main node, launch 1C:Enterprise"
"and click Disable or clear constant ""Main node"" programmatically.'"),
...
"""%1"".'; en = 'An error occurred when importing the metadata object ID."
...
"""%1"".'; en = 'An error occurred when importing the metadata object IDs."
...
"""%1"" не соответствует полному имени ""%2"".'; en = 'An error occurred when importing the metadata object IDs."
"Cannot import an item which key of metadata object ""%1"""
"does not correspond to full name ""%2"".'"),
...
"Прежний тип: ""%1"","
"новый тип: ""%2"".'; en = 'Names of renamed metadata object types are not the same."
"Previous type: ""%1"","
"new type: ""%2"".'"),
			СтароеПолноеИмя,
			НовоеПолноеИмя);
...
"%2.'; en = 'It is not required to describe renaming for metadata object type ""%1"""
...
Функция СвойстваКоллекцийОбъектовМетаданных(ОбъектыРасширений = Ложь) Экспорт
	...
	Строка.Синоним         = НСтр("ru = 'Обработки'; en = 'Data processors'");
	Строка.ИмяВЕдЧисле     = "Обработка";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Обработка'; en = 'Processing'");
	Строка.БезДанных       = Истина;
	Строка.ОбъектыРасширений = Истина;
...
"реквизита ""Пометка удаления"" установлено Ложь недопустимо.'; en = 'Cannot delete object IDs whose value"
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СписокНовыхОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = СписокКритичныхИзменений
				+ НСтр("ru = 'Добавление новых идентификаторов объектов метаданных:'; en = 'Add new metadata object identifiers:'")
		...
		Если Не ТолькоПроверка
		   ...
		   И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
			...
			ИмяСобытия = НСтр("ru = 'Идентификаторы объектов метаданных.Требуется загрузить критичные изменения'; en = 'Metadata object IDs.Import critical changes'",
...
"in the main node of a distributed information base."
"The composition of the required changes see in the event log.'"));
		КонецЕсли;
		...
		Если Не ТолькоПроверка Тогда
			...
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(?(ОбъектыРасширений,
						НСтр("ru = 'Идентификаторы объектов расширений.Выполнены критичные изменения'; en = 'Extension object identifiers.Critical changes made'",
							ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция НазваниеСправочника(ОбъектыРасширений)
	...
	Если ОбъектыРасширений Тогда
		НазваниеСправочника = НСтр("ru = 'Идентификаторы объектов расширений'; en = 'Extension object IDs'");
	Иначе
...
Процедура ВызватьИсключениеПоОшибке(ОбъектыРасширений, ТекстОшибки)
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка при работе со справочником ""%1"".'; en = 'An error occurred when operating with catalog ""%1"".'"),
		НазваниеСправочника(ОбъектыРасширений));
...
"возможно только автоматически при обновлении данных справочника.'; en = 'New object identifier can be created"
...
"установлено только автоматически при обновлении данных справочника.'; en = 'Full name ""%1"" was specified"
"while changing the object ID; this name"
"can be set only automatically when updating data of the catalog.'"),
				Объект.ПолноеИмя));
		Иначе
			...
			Если ЗначениеЗаполнено(НазваниеСправочника) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При изменении идентификатора объекта указано"
"полное имя ""%1"","
"которое уже используется в справочнике ""%2"".'; en = 'Full name ""%1"" specified while changing"
"object ID "
"is already in use in the ""%2"" catalog.'"),
...
"только в главном узле распределенной информационной базы.'; en = 'You can add new items"
...
"только в главном узле распределенной информационной базы.'; en = 'Attribute ""Full name"" can be changed"
"only in the main node of the distributed infobase.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
"не может использоваться в неразделенном режиме.'; en = 'The ""Extension object IDs"" catalog "
...
"регистре сведений ""Идентификаторы объектов версий расширений"".'; en = 'For the ""%1"" metadata object"
"some identifiers are found in catalog ""Metadata object IDs"" and"
"information register ""Object identifiers of extension versions"".'"),
				НСтр("ru = 'Для объекта метаданных ""%1"""
"найдено несколько идентификаторов в справочнике ""Идентификаторы объектов метаданных"".'; en = 'For the ""%1"" metadata object"
"some identifiers are found in catalog ""Metadata object IDs"".'"));
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ПолноеИмяОбъектаМетаданных);
			ДобавитьУточнениеОшибкиПараметровРаботыПрограммыДляРазработчика = Истина;
			Ошибки.Добавить(ОписаниеОшибки);
			Продолжить;
...
"которому соответствует удаленный объект метаданных.'; en = 'Identifier for the ""%1"" metadata object"
...
"to which the deleted metadata object corresponds.'"),
			...
			Иначе
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для объекта метаданных ""%1"""
"найден идентификатор в справочнике ""%2"","
"который соответствует другому объекту метаданных ""%3"".'; en = 'Identifier for the ""%1"" metadata object"
"is found in the ""%2"" catalog"
...
"whose full name is different from the specified in the identifier.'"),
...
"но значение реквизита ""Пометка удаления"" установлено Истина.'; en = 'Identifier ""%1"" was"
"found in catalog ""%2"","
...
"Выполните действие в монопольном режиме.'; en = 'Failed to replace the duplicates of the identifiers of metadata objects."
"After 10 attempts, there are still data that need replacing."
"Perform the action in exclusive mode.'");
		...
		Если Не БезОшибок Тогда
			ТекстОшибки =
				НСтр("ru = 'Не удалось выполнить замену дублей идентификаторов объектов метаданных."
"Подробнее см. ошибки при замене идентификатора в журнале регистрации.'; en = 'Cannot replace duplicates of metadata object IDs."
...
"%2'; en = 'An error occurred when writing object ""%1"": "
"%2'"),
...
"%3'; en = 'An error occurred when writing ""%1"" object movements in ""%2"": "
"%3'"),
								ПолучитьНавигационнуюСсылку(Параметры.Объект.Ссылка),
...
"%3'; en = 'An error occurred in ""%2"" when writing by recorder ""%1"": "
"%3'"),
...
"%2'; en = 'An error occurred when deleting record ""%1"":"
"%2'"),
...
"%2'; en = 'An error occurred when adding entry ""%1"":"
"%2'"),
							ПолучитьНавигационнуюСсылку(МенеджерРегистра.СоздатьКлючЗаписи(СтруктураИзмерений)),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
						ВызватьИсключение;
...
Процедура СообщитьОбОшибке(Знач Описание, ОбъектыРасширений)
	...
	ЗаписьЖурналаРегистрации(
		?(ОбъектыРасширений,
			НСтр("ru = 'Идентификаторы объектов расширений.Замена идентификатора'; en = 'Extension object identifiers.Change identifier'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
			НСтр("ru = 'Идентификаторы объектов метаданных.Замена идентификатора'; en = 'Metadata object IDs.Change identifier'",
				ОбщегоНазначения.КодОсновногоЯзыка())),
