	...
	Для каждого ТекущаяСтрока Из Состав Цикл
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.ВнешнийПользователь) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				"Объект.Состав",
				НомерСтроки,
				НСтр("ru = 'Внешний пользователь в строке %1 не выбран.'; en = 'External user in line %1 was not selected.'"));
			Продолжить;
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если Ссылка = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
		Если Состав.Количество() > 0 Тогда
			ВызватьИсключение
				НСтр("ru = 'Добавление участников в предопределенную группу ""Все внешние пользователи"" запрещено.'; en = 'Add members to predefined group ""All external users"" is prohibited.'");
		КонецЕсли;
	Иначе
		ТекстОшибки = ТекстОшибкиПроверкиНазначения();
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
...
Функция ТекстОшибкиПроверкиРодителя()
	...
	Если Родитель = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
		Возврат
			НСтр("ru = 'Предопределенная группа ""Все внешние пользователи"" не может быть родителем.'; en = 'Predefined group ""All external users"" cannot be a parent group.'");
	КонецЕсли;
	...
	Иначе
		Если Родитель = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			Возврат
				НСтр("ru = 'Невозможно добавить подгруппу к предопределенной группе ""Все внешние пользователи"".'; en = 'Cannot add subgroup to the predefined group ""All external users"".'");
			
		ИначеЕсли Родитель.ВсеОбъектыАвторизации Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно добавить подгруппу к группе ""%1"","
"так как в число ее участников входят все пользователи.'; en = 'Impossible to add subgroup to group ""%1"","
"as number of participants includes all users.'"), Родитель);
		КонецЕсли;
		
		Если ВсеОбъектыАвторизации И ЗначениеЗаполнено(Родитель) Тогда
			Возврат
				НСтр("ru = 'Невозможно переместить группу, в число участников которой входят все пользователи.'; en = 'Cannot move the group that includes all users.'");
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстОшибкиПроверкиНазначения()
	...
	Если Назначение.Количество() = 0 Тогда
		Возврат НСтр("ru = 'Не указан вид участников группы.'; en = 'Group members type not specify.'");
	КонецЕсли;
...
"с предопределенной группой ""Все внешние пользователи"".'; en = 'It is impossible to create a group that matches the assignment"
"with the predefined group ""All internal users"".'");
		КонецЕсли;
...
"в число участников которой входят все пользователи указанных видов.'; en = 'Already exists group ""%1"","
"participants which includes all users of specified types.'"),
...
"группы внешних пользователей ""%1"".'; en = 'Kind of participants in group should be like that of the superior"
"group of external users ""%1"".'"), Родитель);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
"так как у нее имеются подгруппы.'; en = 'It is not possible to change the kind of group members,"
"since it has subgroups.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно изменить вид участников группы,"
"так как у нее имеется подгруппа ""%1"" с другим назначением участников.'; en = 'It is not possible to change type of group members,"
"since it has subgroup of ""%1"" with a different assignment of members.'"),
				Выборка.СсылкаПредставление);
		КонецЕсли;
	КонецЕсли;
