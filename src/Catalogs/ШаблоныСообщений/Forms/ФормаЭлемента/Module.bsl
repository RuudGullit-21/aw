...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не ИнформацияОПроверке.Успех Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Шаблон сообщения не может быть записан.'; en = 'Message template can not be recorded.'")
...
Функция ОбработатьТекстШаблона()
	...
	Если ОшибочныеПараметры.Количество() > 0 Тогда
		ТекстОшибки = ?(ОшибочныеПараметры.Количество() = 1,
			НСтр("ru = 'Некорректный реквизит в шаблоне сообщения:'; en = 'Incorrect props in the message template:'"),
			НСтр("ru = 'Некорректные реквизиты в шаблоне сообщения:'; en = 'Incorrect attributes in the message template:'"));
...
Процедура ПроверитьШаблон(Команда)
	...
	Если ИнформацияОПроверке.Успех Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Шаблон заполнен корректно'; en = 'Template is filled correctly'"));
	Иначе
...
Процедура ИзменитьВложение(Команда)
	...
	Если ТекущиеДанные.Ссылка = ПредопределенноеЗначение("Справочник.ШаблоныСообщенийПрисоединенныеФайлы.ПустаяСсылка") Тогда
		...
		ТекстВопроса = НСтр("ru = 'Свойства файла доступны только после его записи. Записать?'; en = 'File properties are available only after file recording. Record?'");
...
Процедура ОтображениеЭлементовФормы(ФорматаПисьма = "")
	...
	Если Объект.ПредназначенДляSMS Тогда
		СуффиксЗаголовка = НСтр("ru = 'Шаблон сообщения SMS'; en = 'SMS template'");
	...
	Иначе
		СуффиксЗаголовка = НСтр("ru = 'Шаблон сообщения электронного письма'; en = 'Template e-mail message'");
	...
	Иначе
		Заголовок = СуффиксЗаголовка + " (" + НСтр("ru = 'создание'; en = 'create'")+ ")";
	КонецЕсли;
...
Функция СкопироватьВложенияИзИсточника()
	...
	ОписаниеОшибки = НСтр("ru = 'Не удалось скопировать вложение по причине: %1'; en = 'Failed to copy attachment by reason: %1'");
...
Функция ИмяСобытияШаблоныСообщений()
	...
	Возврат НСтр("ru = 'Шаблоны сообщений'; en = 'Message templates'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаполнитьШаблонПоВнешнейОбработке()
		...
		Иначе
			...
			ОписаниеОшибки = НСтр("ru = 'Предмет %1 определенный в внешней обработке не найдено. Внешняя обработка не может быть подключена.'; en = 'Object %1 is defined in the external processing not found. External processing may not be connected.'");
