...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидСообщения = "СообщениеSMS" Тогда
		...
		Заголовок = НСтр("ru = 'Шаблоны сообщений SMS'; en = 'Message templates SMS'");
	Иначе
	...
	Если РежимВыбора Тогда
		...
		Элементы.ФормаСформировать.Заголовок = НСтр("ru = 'Выбрать'; en = 'Select'");
...
Процедура ПослеФормированияИОтправкиСообщения(Результат, ПараметрыОтправки)
	...
	Иначе
		...
		ОписаниеОшибки = Результат.ОписаниеОшибки + Символы.ПС + НСтр("ru = 'Открыть форму отправки сообщения?'; en = 'Open the form to send a message?'");
...
Процедура ЗаполнитьСписокДоступныхШаблонов()
	...
	Если НЕ РежимВыбора И НЕ ПодготовитьШаблон Тогда
		...
		ПерваяСтрока.Представление = НСтр("ru = '<Без шаблона>'; en = '<No template>'");
	КонецЕсли;
