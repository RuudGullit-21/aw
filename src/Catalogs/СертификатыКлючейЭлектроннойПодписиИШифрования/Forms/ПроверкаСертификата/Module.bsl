...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПроверкаПриВыборе Тогда
		...
		Элементы.ФормаПроверить.Заголовок = НСтр("ru = 'Проверить и продолжить'; en = 'Проверить и продолжить'");
	КонецЕсли;
...
"при этом дополнительных проверок не указано.'; en = 'Для проверки сертификата отключены стандартные проверки,"
...
Процедура ПоказатьПредупреждениеОНевозможностиПродолжения()
	...
	ПоказатьПредупреждение(,
		НСтр("ru = 'Не удалось продолжить, т.к. пройдены не все требуемые проверки.'; en = 'Не удалось продолжить, т.к. пройдены не все требуемые проверки.'"));
...
Процедура ПроверитьЗаконныйСертификатНаКлиенте(Контекст)
	...
	Если Не Элементы.ГруппаЗаконныйСертификат.Видимость
	 Или Контекст.СертификатКриптографии.Субъект.Свойство("SN") Тогда
		...
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ru = 'В описании субъекта сертификата не найдено поле ""SN"".'; en = 'В описании субъекта сертификата не найдено поле ""SN"".'");
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Иначе
		УстановитьЭлемент(ЭтотОбъект, "НаличиеСертификата", Ложь,
			...
			+ НСтр("ru = 'Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.'; en = 'Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.'"),
...
Процедура ПроверитьНаСторонеКлиентаПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'Программа для использования закрытого ключа не указана в сертификате.'; en = 'Программа для использования закрытого ключа не указана в сертификате.'");
...
"расшифровки не могут быть выполнены.'; en = 'Проверка подписания, созданной подписи, шифрования и"
	...
	Иначе
		...
		Если (ОбъединятьРезультаты = "ОбъединятьПоИ"
			...
			Если ЗначениеПроверки = Неопределено
				...
				ЭлементКартинка.Подсказка = НСтр("ru = 'Проверка не выполнялась.'; en = 'Проверка не выполнялась.'");
			...
			ИначеЕсли ЗначениеПроверки <> Неопределено
				...
				ЭлементКартинка.Подсказка = НСтр("ru = 'Проверка выполнена успешно.'; en = 'Проверка выполнена успешно.'");
				ЭлементНадпись.Гиперссылка = Ложь;
				ЭлементНадпись.Заголовок = ЗаголовокЭлемента(
					Форма.ДополнительныеПроверки, ЭлементНадпись.Имя);
			Иначе
		...
		Иначе
			...
			ЭлементКартинка.Подсказка = ?(ОписаниеОшибки = Неопределено,
				НСтр("ru = 'Проверка не выполнялась.'; en = 'Проверка не выполнялась.'"),
				НСтр("ru = 'Проверка выполнена успешно.'; en = 'Проверка выполнена успешно.'"));
...
Функция СтандартныйЗаголовокЭлемента(ИмяЭлемента)
	...
	Если ИмяЭлемента = "ЗаконныйСертификатНадпись" Тогда
		Возврат НСтр("ru = 'Соответствие законодательству Российской Федерации'; en = 'Соответствие законодательству Российской Федерации'");
		
	ИначеЕсли ИмяЭлемента = "НаличиеСертификатаНадпись" Тогда
		Возврат НСтр("ru = 'Наличие сертификата в личном списке'; en = 'Наличие сертификата в личном списке'");
		
	ИначеЕсли ИмяЭлемента = "ДанныеСертификатаНадпись" Тогда
		Возврат НСтр("ru = 'Корректность данных сертификата'; en = 'Корректность данных сертификата'");
		
	ИначеЕсли ИмяЭлемента = "НаличиеПрограммыНадпись" Тогда
		Возврат НСтр("ru = 'Наличие программы для подписания и расшифровки'; en = 'Наличие программы для подписания и расшифровки'");
		
	ИначеЕсли ИмяЭлемента = "ПодписаниеНадпись" Тогда
		Возврат НСтр("ru = 'Подписание данных'; en = 'Signing data'");
		
	ИначеЕсли ИмяЭлемента = "ПроверкаПодписиНадпись" Тогда
		Возврат НСтр("ru = 'Проверка созданной подписи'; en = 'Проверка созданной подписи'");
		
	ИначеЕсли ИмяЭлемента = "ШифрованиеНадпись" Тогда
		Возврат НСтр("ru = 'Шифрование данных'; en = 'Data encryption'");
		
	ИначеЕсли ИмяЭлемента = "РасшифровкаНадпись" Тогда
		Возврат НСтр("ru = 'Расшифровка данных'; en = 'Decryption of data'");
	Иначе
		Возврат "";
	КонецЕсли;
