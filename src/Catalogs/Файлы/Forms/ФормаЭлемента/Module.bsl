...
Процедура ОбновитьЗаголовок()
	...
	Если ТипЗнч(Объект.ВладелецФайла) = Тип("СправочникСсылка.ПапкиФайлов") Тогда
		ТипФайла = НСтр("ru = 'Файл'; en = 'File'");
	Иначе
		ТипФайла = НСтр("ru = 'Присоединенный файл'; en = 'Attached file'");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 (%2)'; en = '%1 (%2)'"), Строка(ЭтотОбъект.Объект.Ссылка), ТипФайла);
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 (Создание)'; en = '%1 (Creation)'"), ТипФайла);
	КонецЕсли;
