...
Процедура ПрименитьПравила(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не установлен отбор по владельцу(учетной записи) правил.'; en = 'Filter by the owner (of account) of rules is not set.'"));
		Возврат;
	КонецЕсли;
