...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ШаблонЗаполнения.НайтиСтроки(Новый Структура("ДеньВключенВГрафик", Истина)).Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Не отмечены дни, включаемые в график работы'; en = 'There were no marked days to include in work schedule'"), , "Объект.ШаблонЗаполнения", , Отказ);
	КонецЕсли;
...
Процедура НомерТекущегоГодаПриИзменении(Элемент)
	...
	Если МодифицированностьРезультата Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Записать измененные данные за %1 год?'; en = 'Record modified data in %1 year?'"), Формат(НомерПредыдущегоГода, "ЧГ=0"));
...
Процедура СформироватьШаблонЗаполнения(СпособЗаполнения, ШаблонЗаполнения, Знач ДлинаЦикла, Знач ДатаОтсчета = Неопределено)
	...
	Если СпособЗаполнения = ПредопределенноеЗначение("Перечисление.СпособыЗаполненияГрафикаРаботы.ПоНеделям") Тогда
		ШаблонЗаполнения[0].ПредставлениеДня = НСтр("ru = 'Понедельник'; en = 'Monday'");
		ШаблонЗаполнения[1].ПредставлениеДня = НСтр("ru = 'Вторник'; en = 'Tuesday'");
		ШаблонЗаполнения[2].ПредставлениеДня = НСтр("ru = 'Среда'; en = 'Wednesday'");
		ШаблонЗаполнения[3].ПредставлениеДня = НСтр("ru = 'Четверг'; en = 'Thursday'");
		ШаблонЗаполнения[4].ПредставлениеДня = НСтр("ru = 'Пятница'; en = 'Friday'");
		ШаблонЗаполнения[5].ПредставлениеДня = НСтр("ru = 'Суббота'; en = 'Saturday'");
		ШаблонЗаполнения[6].ПредставлениеДня = НСтр("ru = 'Воскресенье'; en = 'Sunday'");
	Иначе
		ДатаДня = ДатаОтсчета;
...
Функция ПредставлениеРасписанияДня(Форма, НомерДня)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 ч. (%2)'; en = '%1 h. (%2)'"), Часов, ПредставлениеИнтервалов);
...
Функция ПредставлениеВремениСуток(Знач ВремяСуток, ДвенадцатиЧасовойФорматВремени)
	...
	ФорматВремени = ?(ДвенадцатиЧасовойФорматВремени,
		НСтр("ru = 'ДФ=''чч:мм вв'''"), НСтр("ru = 'ДФ=ЧЧ:мм; ДП='; en = 'ДФ=ЧЧ:мм; ДП='"));
...
Функция ПредставлениеПустогоРасписания()
	...
	Возврат НСтр("ru = 'Заполнить расписание'; en = 'Fill schedule'");
...
Процедура УточнитьЗаполненностьГрафика(Форма)
	...
	Если Не ЗначениеЗаполнено(Форма.ДатаЗаполненности) Тогда
		Форма.ЗаполненностьИнформационныйТекст = НСтр("ru = 'График работы не заполнен'; en = 'Timetable is not filled'");
		Форма.ТребуетЗаполнения = Истина;
	Иначе	
		Если Не ЗначениеЗаполнено(Форма.Объект.ГоризонтПланирования) Тогда
			ИнформационныйТекст = НСтр("ru = 'График работы заполнен до %1'; en = 'Work schedule is filled to %1'");
		...
		Иначе
			...
			ИнформационныйТекст = НСтр("ru = 'График работы заполнен до %1, с учетом горизонта планирования график должен быть заполнен до %2'; en = 'Work schedule is filled to %1 with account of planning horizon schedule should be filled to %2'");
	...
	Иначе
		Если Форма.РезультатЗаполненПоШаблону Тогда
			Если ЗначениеЗаполнено(Форма.Объект.ПроизводственныйКалендарь) Тогда
				ИнформационныйТекст = НСтр("ru = 'График работы автоматически обновляется при изменении производственного календаря за текущий год.'; en = 'Work schedule is automatically updated when you change production calendar for current year.'");
...
Процедура ЗаполнитьИнформационныйТекстРучногоРедактирования(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационнаяКартинка = БиблиотекаКартинок.Предупреждение;
		ИнформационныйТекст = НСтр("ru = 'График работы на текущий год изменен вручную. Изменения выделены в результатах заполнения.'; en = 'Work schedule for current year is changed manually. Changes are highlighted in results of filling.'");
	КонецЕсли;
