...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма
		...
		Отказ = Истина;
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Работа с печатными формами не поддерживается.'; en = 'Work printed forms is not supported.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не ПравоДобавления Тогда
		Если ЭтоНовый Тогда
			ВызватьИсключение НСтр("ru = 'Недостаточно прав доступа для добавления дополнительных отчетов или обработок.'; en = 'Insufficient access rights to add additional reports and data processors.'");
		Иначе
	...
	Если ПоказатьДиалогЗагрузкиИзФайлаПриОткрытии И Не Элементы.ЗагрузитьИзФайла.Видимость Тогда
		ВызватьИсключение НСтр("ru = 'Недостаточно прав для загрузки дополнительных отчетов и обработок'; en = 'Insufficient rights to import additional reports and data processors'");
	КонецЕсли;
...
Процедура ВариантыДополнительногоОтчетаПередУдалением(Элемент, Отказ)
	...
	Если НЕ Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Пометка на удаление предопределенного варианта отчета запрещена.'; en = 'Marking for deletion of predefined report version is prohibited.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВариантыДополнительногоОтчетаОткрыть(Команда)
	...
	Если Вариант = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите вариант отчета.'; en = 'Select report variant.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьКоманду(Команда)
	...
	Если Объект.Ссылка.Пустая() Или Модифицированность Тогда
		...
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("ЗаписатьИПродолжить", НСтр("ru = 'Записать и продолжить'; en = 'Write and continue'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ОбновитьИзФайлаПослеПодтверждения(Ответ, ПараметрыРегистрации) Экспорт
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru = 'Выберите файл внешнего отчета или обработки'; en = 'Select a file of external report or data processor'");
	ИначеЕсли Объект.Вид = ВидДополнительныйОтчет Или Объект.Вид = ВидОтчет Тогда
		ПараметрыЗагрузки.Диалог.ИндексФильтра = 1;
		ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru = 'Выберите файл внешнего отчета'; en = 'Select an external report file'");
	Иначе
		ПараметрыЗагрузки.Диалог.ИндексФильтра = 2;
		ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru = 'Выберите файл внешней обработки'; en = 'Select an external data processor file'");
	КонецЕсли;
...
Процедура ОбновитьИзФайлаПослеВыбораФайла(ОписаниеФайла, ПараметрыРегистрации) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = НСтр("ru = 'Расширение файла не соответствует расширению внешнего отчета (ERF) или обработки (EPF).'; en = 'File extension does not match those of the external report (ERF) or processing (EPF).'");
...
Процедура ПоказатьКонфликты(ПараметрыРегистрации)
	...
	Если ПараметрыРегистрации.КоличествоКонфликтующих > 1 Тогда
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("ru = 'Конфликты при загрузке внешнего отчета'; en = 'Conflicts during external report import'");
...
"3. ""[Open]"" - cancel import and show  the list of conflicting reports.'");
		Иначе
			ЗаголовокВопроса = НСтр("ru = 'Конфликты при загрузке внешней обработки'; en = 'Conflicts occurred during import of external data processor'");
...
"3. ""[Open]"" - отменить загрузку и показать список конфликтующих обработок.'; en = 'Internal name of data processor ""[Name]"""
...
"3. ""[Open]"" - cancel import  and show conflicting data processors list.'");
	...
	Иначе
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("ru = 'Конфликт при загрузке внешнего отчета'; en = 'Conflict during external report import'");
			ТекстВопроса = НСтр("ru = 'Внутреннее имя отчета ""[Name]"""
...
"3. ""[Open]"" - open a conflicting report card.'");
		...
		Иначе
			ЗаголовокВопроса = НСтр("ru = 'Конфликт при загрузке внешней обработки'; en = 'Conflict occurred during import of external data processor'");
			ТекстВопроса = НСтр("ru = 'Внутреннее имя обработки ""[Name]"""
...
"3. ""[Open]"" - открыть карточку конфликтующей обработки.'; en = 'Internal name of data processor ""[Name]"""
...
"3. ""[Open]"" - open the conflicting data processor card.'");
		...
		КонецЕсли;
		ПредставлениеКнопкиОткрыть = НСтр("ru = 'Отменить и открыть'; en = 'Cancel and show'");
	КонецЕсли;
...
Процедура ОбновитьИзФайлаРазрешениеКонфликтов(Ответ, ПараметрыРегистрации) Экспорт
	...
	ИначеЕсли Ответ = "ОтменитьИОткрыть" Тогда
		...
		Если ПоказатьСписок Тогда // Форма списка с отбором по конфликтующим.
			...
			ФормаЗаголовок = НСтр("ru = 'Дополнительные отчеты и обработки с внутреннем именем ""%1""'; en = 'Additional reports and data processors with internal name ""%1""'");
...
Процедура ОткрытьВариант()
	...
	Если НЕ ЗначениеЗаполнено(Вариант.Ссылка) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вариант отчета ""%1"" не зарегистрирован.'; en = 'Report variant ""%1"" is not registered.'"), Вариант.Наименование);
...
"не может использоваться в регламентных заданиях.'; en = 'Cannot use the command with startup option ""%1"""
"in scheduled jobs.'");
...
Процедура ПослеОкончанияРедактированияРасписания(Расписание, Контекст) Экспорт
	...
	Иначе
		...
		Иначе
			ЭлементКоманда.РегламентноеЗаданиеПредставление = НСтр("ru = 'Не заполнено'; en = 'Not filled in'");
			Если ЭлементКоманда.РегламентноеЗаданиеИспользование Тогда
				ЭлементКоманда.РегламентноеЗаданиеИспользование = Ложь;
				ПоказатьОповещениеПользователя(
					НСтр("ru = 'Запуск по расписанию отключен'; en = 'Scheduled launch is disabled'"),
					,
					НСтр("ru = 'Расписание не заполнено'; en = 'Schedule is not filled'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция НачатьВыполнениеСервернойКомандыВФоне(ВыполняемаяКоманда, УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Дополнительные отчеты и обработки: Выполнение серверного метода обработки'; en = 'Additional reports and data processors: Running server method of data processor'");
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	Если Объект.Вид = ВидДополнительныйОтчет И Объект.ИспользуетХранилищеВариантов Тогда
		...
		Если Регистрация ИЛИ КоличествоВариантов = 0 Тогда
			...
			Элементы.СтраницаВарианты.Заголовок = НСтр("ru = 'Варианты отчета'; en = 'Report variants'");
		Иначе
			...
			Элементы.СтраницаВарианты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Варианты отчета (%1)'; en = 'Report variants (%1)'"),
...
"профиля безопасности %1, в котором будут разрешены только следующие операции:'; en = 'Additional report or data processor will connect to the application with the use of ""personal"""
...
"требующих разрешений на использование профилей безопасности.'; en = '%1 is not a valid connection mode for additional reports and data processors,"
"requiring permissions to use security profiles.'"),
	...
	Если ПредставлениеНазначения = "" Тогда
		ПредставлениеНазначения = НСтр("ru = 'Не определено'; en = 'Not determined'");
	КонецЕсли;
	...
	Если ЭтоНовый Тогда
		Заголовок = ?(ЭтоОтчет, НСтр("ru = 'Дополнительный отчет (создание)'; en = 'Additional report (creation)'"), НСтр("ru = 'Дополнительная обработка (создание)'; en = 'Additional data processor (creation)'"));
	Иначе
	...
	Для Каждого ЭлементКоманда Из Объект.Команды Цикл
			...
			Если Не ПустаяСтрока(ЭлементКоманда.ЗаменяемыеКоманды) И КомандыПечатиОбъекта <> Неопределено Тогда
				...
				Если КоличествоЗаменяемых > 0 Тогда
					Если КоличествоЗаменяемых = 1 Тогда
						ШаблонКомментария = НСтр("ru = 'Заменяет стандартную команду печати %1'; en = 'Replaces standard print command %1'");
					Иначе
						ШаблонКомментария = НСтр("ru = 'Заменяет стандартные команды печати: %1'; en = 'Replaces standard print commands: %1'");
					КонецЕсли;
		...
		Иначе
			ЭлементКоманда.РегламентноеЗаданиеПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неприменимо для команд с вариантом запуска ""%1""'; en = 'Not applicable for commands with variant of start ""%1""'"),
...
Процедура ВариантыДополнительногоОтчетаЗаполнить()
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Не удалось получить список вариантов отчета из-за ошибки, возникшей при подключении этого отчета:'; en = 'Cannot receive a list of report variants due to the error that occurred when connecting this report:'");
