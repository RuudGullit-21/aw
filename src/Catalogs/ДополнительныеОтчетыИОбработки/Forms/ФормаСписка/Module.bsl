...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокВыбора.Добавить(1, НСтр("ru = 'Только отчеты'; en = 'Only reports'"));
	СписокВыбора.Добавить(2, НСтр("ru = 'Только обработки'; en = 'Only data processors'"));
...
"Выберите дополнительный отчет или обработку.'; en = 'Command cannot be executed for the specified object."
...
"Выберите дополнительный отчет или обработку.'; en = 'Cannot execute the command for the group."
"Select an additional report or data processor.'"));
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
...
Процедура ИзменитьПубликацию(ВариантПубликации)
	...
	Если КоличествоСтрок = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбран ни один дополнительный отчет (обработка)'; en = 'No additional report (data processor) is selected'"));
		Возврат;
	КонецЕсли;
	...
	Если КоличествоСтрок = 1 Тогда
		ТекстСообщения = НСтр("ru = 'Изменена публикация дополнительного отчета (обработки) ""%1""'; en = 'Publication of additional report (data processor) ""%1"" is changed'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Строка(ВыделенныеСтроки[0]));
	Иначе
		ТекстСообщения = НСтр("ru = 'Изменена публикация у дополнительных отчетов (обработок): %1'; en = 'Changed the publication of additional reports (data processor): %1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КоличествоСтрок);
	КонецЕсли;
	
	ПоказатьОповещениеПользователя(НСтр("ru = 'Изменена публикация'; en = 'Publication is changed'"),, ТекстСообщения);
