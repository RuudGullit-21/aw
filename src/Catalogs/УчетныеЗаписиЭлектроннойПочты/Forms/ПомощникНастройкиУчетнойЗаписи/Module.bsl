...
Процедура ПроверитьЗаполнениеНаСтраницеНастройкаУчетнойЗаписи(Отказ)
	...
	Если ПустаяСтрока(АдресЭлектроннойПочты) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Введите адрес электронной почты'; en = 'Enter email address'"), , "АдресЭлектроннойПочты", , Отказ);
	ИначеЕсли Не ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(АдресЭлектроннойПочты, Истина) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Адрес электронной почты введен неверно'; en = 'Email address is invalid'"), , "АдресЭлектроннойПочты", , Отказ);
	КонецЕсли;
...
Процедура НастроитьЭлементыТекущейСтраницы()
	...
	Если Не КонтекстныйРежим И ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		Элементы.КнопкаПерейтиКНастройкам.Заголовок = НСтр("ru = 'Перейти к настройкам'; en = 'Go to settings'");
	Иначе
...
Процедура СоздатьУчетнуюЗапись()
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка при создании учетной записи электронной почты'; en = 'An error occurred when creating an email account'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
"Настройте параметры подключения вручную.'; en = 'Cannot determine the connection settings."
...
"http://buh.ru/articles/documents/42429/#briefly_43166'; en = 'See also recommendations on configuring Gmail email:"
...
Функция НачатьПоискНастроекУчетнойЗаписи()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Поиск настроек почтового сервера'; en = 'Search for mailing server settings'");
...
Функция ПараметрыЗапросаАвторизации()
	...
	Если ЭтоВебКлиент() Тогда
		Если Не ЗначениеЗаполнено(НастройкиАвторизации.АдресРегистрацииУстройства) Тогда
			ПараметрыЗапроса.Вставить("device_name", НСтр("ru = '1С:Предприятие'; en = '1C:Enterprise'"));
		КонецЕсли;
