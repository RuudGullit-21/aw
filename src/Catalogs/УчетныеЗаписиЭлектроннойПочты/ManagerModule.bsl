...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Данные.АдресЭлектроннойПочты) И СтрНайти(Данные.Наименование, Данные.АдресЭлектроннойПочты) = 0 Тогда
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2)'; en = '%1 (%2)'"), 
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент.Наименование                     = НСтр("ru = 'Системная учетная запись'; en = 'Системная учетная запись'", 
		ОбщегоНазначения.КодОсновногоЯзыка());
	Элемент.ИмяПользователя                  = НСтр("ru = '1С:Предприятие'; en = '1C:Enterprise'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты(Профиль, АдресЭлектроннойПочты)
	...
	Тема = НСтр("ru = 'Тестовое сообщение 1С:Предприятие'; en = '1C:Enterprise test message'");
	Тело = НСтр("ru = 'Это сообщение отправлено подсистемой электронной почты 1С:Предприятие'; en = 'This message is sent by subsystem of 1C:Enterprise email'");
	ИмяОтправителяПисем = НСтр("ru = '1С:Предприятие'; en = '1C:Enterprise'");
...
Функция ИнформацияОПрограмме()
	...
	Для Каждого Расширение Из РасширенияКонфигурации.Получить() Цикл
		Если Расширение.Активно Тогда
			Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2)'; en = '%1 (%2)'"),
...
Функция СобытиеПроверкаПодключенияКПочтовомуСерверу()
	Возврат НСтр("ru = 'Проверка подключения к почтовому серверу'; en = 'Check connection to the mail server'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
