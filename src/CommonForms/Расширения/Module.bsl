...
Процедура СохранитьКак(Команда)
	...
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru = 'Выберите файл для сохранения расширения конфигурации'; en = 'Select a file to save the configuration extension to'");
	ПараметрыСохранения.Диалог.Фильтр    = НСтр("ru = 'Файлы расширений конфигурации (*.cfe)|*.cfe|Все файлы (*.*)|*.*'; en = 'Configuration extension files (*.cfe)|*.cfe|All files (*.*)|*.*'");
...
Процедура УдалитьУстаревшиеПараметровРаботыРасширений(Команда)
	...
	ПоказатьПредупреждение(, НСтр("ru = 'Выполнено удаление устаревших версий параметров работы расширений.'; en = 'Obsolete versions of extension parameters are deleted.'"));
...
Процедура ОбновитьСписок(ПослеДобавления = Ложь)
	...
	Для Каждого Расширение Из Расширения Цикл
		...
		Если ПодключенныеРасширения[КлючРасширения] <> Неопределено Тогда
			...
			ЭлементРасширения.СостояниеПодключения = НСтр("ru = 'Подключено'; en = 'Connected'");
		ИначеЕсли ОтключенныеРасширения[КлючРасширения] <> Неопределено Тогда
			ЭлементРасширения.Подключено = 2;
			ЭлементРасширения.СостояниеПодключения = НСтр("ru = 'Отключено'; en = 'Disabled'");
		Иначе
...
Процедура ЗагрузитьРасширениеПослеПодтверждения(Ответ, Контекст) Экспорт
	...
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru = 'Расширение конфигурации'; en = 'Configuration extension'", "ru")+ " (*.cfe)|*.cfe";
	ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru = 'Выберите файл расширения конфигурации'; en = 'Select a configuration extension file'", "ru");
...
Процедура ЗагрузитьРасширениеЗавершение()
	...
	Если Контекст.ИдентификаторРасширения = Неопределено Тогда
		Если Контекст.ПомещенныеФайлы.Количество() > 1 Тогда
			ТекстОповещения = НСтр("ru = 'Расширения конфигурации добавлены'; en = 'Configuration extensions are added'", "ru");
		Иначе
			ТекстОповещения = НСтр("ru = 'Расширение конфигурации добавлено'; en = 'Configuration extension is added'", "ru");
		КонецЕсли;
	Иначе
		ТекстОповещения = НСтр("ru = 'Расширение конфигурации обновлено'; en = 'Configuration extension is updated'", "ru");
	КонецЕсли;
	...
	Если Контекст.ПомещенныеФайлы.Количество() > 1 Тогда
		Если СтрНайти(НеподключенныеРасширения, ",") > 0 Тогда
			ТекстПредупреждения = НСтр("ru = 'Некоторые расширения не подключаются:'; en = 'Some extensions are not connected:'");
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Одно расширение не подключается:'; en = 'One extension not connect:'");
		КонецЕсли;
		ТекстПредупреждения = ТекстПредупреждения + " " + НеподключенныеРасширения;
	Иначе
		ТекстПредупреждения = НСтр("ru = 'Расширение не подключается.'; en = 'Extension is not connected.'");
	КонецЕсли;
...
"%1'; en = 'An error occurred while attempting to restore the changed extension:"
...
"%1'; en = 'When trying to delete added extensions, one more error occurred:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибкеВосстановления));
			КонецЕсли;
		КонецПопытки;
		Если ВосстановлениеВыполнялось
		   И ИнформацияОбОшибкеВосстановления = Неопределено Тогда
			...
			Иначе
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
					+ НСтр("ru = 'Добавленные расширения были удалены.'; en = 'Added extensions were removed.'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
