...
Процедура Подключаемый_Список_ОбработкаВыбора(Элемент, РезультатВыбора, СтандартнаяОбработка)
	...
	Если Дополнение.Всего > 0 Тогда
		Если Дополнение.Всего = 1 Тогда
			ЗаголовокОповещения = НСтр("ru = 'Элемент добавлен в список'; en = 'Item is added to the list'");
		Иначе
			ЗаголовокОповещения = НСтр("ru = 'Элементы добавлены в список'; en = 'Items are added to the list'");
		КонецЕсли;
...
Процедура СтруктураВарианта_СохранитьВФайл(Команда)
	...
	ИмяФайла = НСтр("ru = 'Настройки.xml'; en = 'Settings.xml'");
...
Процедура ОбновитьПоляГруппировки()
	...
	Для Каждого ЭлементНастройки Из СвойствоЭлементаСтруктуры.Элементы Цикл 
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоПолеГруппировкиКомпоновкиДанных") Тогда 
			Строка.Заголовок  = НСтр("ru = 'Авто (по всем полям)'; en = 'Auto (by all fields)'");
...
Процедура ОбновитьПараметрыДанных()
	...
	Раздел.Заголовок = НСтр("ru = 'Параметры'; en = 'Parameters'");
...
Функция ПараметрыГруппировкиОтборов()
	...
	Если Строки.Количество() = 0 Тогда 
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите элементы.'; en = 'Select items.'"));
		Возврат Неопределено;
	КонецЕсли;
	
	Родители = ОбщегоНазначенияКлиентСервер.СвернутьМассив(Родители);
	Если Родители.Количество() > 1 Тогда 
		ПоказатьПредупреждение(, НСтр("ru = 'Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.'; en = 'The selected items cannot be grouped as they have different parents.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПараметрыСдвигаОтборов()
	...
	Родители = ОбщегоНазначенияКлиентСервер.СвернутьМассив(Родители);
	Если Родители.Количество() > 1 Тогда 
		ПоказатьПредупреждение(, НСтр("ru = 'Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.'; en = 'The selected items cannot be transferred as they have different parents.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ОбновитьВыбранныеПоля(Строки = Неопределено, ЭлементыНастроек = Неопределено)
	...
	Если Строки = Неопределено Тогда 
		...
		Раздел.ЭтоРаздел = Истина;
		Раздел.Заголовок = НСтр("ru = 'Поля'; en = 'Fields'");
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл 
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоВыбранноеПолеКомпоновкиДанных") Тогда 
			Строка.Заголовок = НСтр("ru = 'Авто (поля родителя)'; en = 'Auto (parent field)'");
			Строка.Картинка = 18;
			Продолжить;
		КонецЕсли;
...
Процедура ОбновитьСортировку()
	...
	Раздел.ЭтоРаздел = Истина;
	Раздел.Заголовок = НСтр("ru = 'Сортировки'; en = 'Sorting'");
	Раздел.Картинка = ВариантыОтчетовСлужебныйКлиентСервер.ИндексКартинкиПоля(Неопределено);
	Строки = Раздел.ПолучитьЭлементы();
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл 
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоЭлементПорядкаКомпоновкиДанных") Тогда 
			Строка.Заголовок = НСтр("ru = 'Авто (сортировки родителя)'; en = 'Auto (parent sorting)'");
...
Процедура ОформлениеИзменитьЭлемент(ИдентификаторСтроки = Неопределено, Строка = Неопределено)
	...
	ПараметрыФормы.Вставить("Заголовок", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Элемент условного оформления отчета ""%1""'; en = 'Conditional report appearance item ""%1""'"), ВариантНаименование));
...
Процедура ОбновитьСтруктуру()
	...
	Раздел.Представление = НСтр("ru = 'Отчет'; en = 'Report'");
	Раздел.ЭтоРаздел = Истина;
	Раздел.Картинка = -1;
...
Функция СвойстваЭлементаКоллекцииСтруктуры(Элемент)
	...
	ИначеЕсли ТипЭлемента = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда 
		...
		Если Не ЗначениеЗаполнено(СвойстваЭлемента.Представление) Тогда
			СвойстваЭлемента.Представление = НСтр("ru = 'Вложенная группировка'; en = 'Attach grouping'");
		КонецЕсли;
		
	ИначеЕсли ТипЭлемента = Тип("ТаблицаКомпоновкиДанных") Тогда 
		
		СвойстваЭлемента.Представление = НСтр("ru = 'Таблица'; en = 'Table'");
		Если ЗначениеЗаполнено(Элемент.ПредставлениеПользовательскойНастройки) Тогда 
			СвойстваЭлемента.Представление = Элемент.ПредставлениеПользовательскойНастройки;
		КонецЕсли;
	...
	ИначеЕсли ТипЭлемента = Тип("ДиаграммаКомпоновкиДанных") Тогда 
		...
		СвойстваЭлемента.Представление = НСтр("ru = 'Диаграмма'; en = 'Chart'");
		Если ЗначениеЗаполнено(Элемент.ПредставлениеПользовательскойНастройки) Тогда 
			СвойстваЭлемента.Представление = Элемент.ПредставлениеПользовательскойНастройки;
		КонецЕсли;
...
Процедура ДобавитьЭлементСтруктурыНастроек(ТипЭлемента)
	...
	Если Строка.Тип = "ДиаграммаКомпоновкиДанных" Тогда
		...
		Строка.Представление = НСтр("ru = 'Диаграмма'; en = 'Chart'");
		...
		ПодчиненнаяСтрока.Представление = НСтр("ru = 'Точки'; en = 'Points'");
		...
		ПодчиненнаяСтрока.Идентификатор = СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка = -1;
		ПодчиненнаяСтрока.Представление = НСтр("ru = 'Серии'; en = 'Series'");
	ИначеЕсли Строка.Тип = "ТаблицаКомпоновкиДанных" Тогда
		Строка.Представление = НСтр("ru = 'Таблица'; en = 'Table'");
		...
		ПодчиненнаяСтрока.Идентификатор = СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка = -1;
		ПодчиненнаяСтрока.Представление = НСтр("ru = 'Строки'; en = 'Rows'");
		...
		ПодчиненнаяСтрока.Идентификатор = СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка = -1;
		ПодчиненнаяСтрока.Представление = НСтр("ru = 'Колонки'; en = 'Columns'");
	КонецЕсли;
...
Процедура ИзменитьЭлементСтруктуры(Строка, ИмяСтраницы = Неопределено, ИспользоватьФормуВарианта = Неопределено)
	...
	ШаблонЗаголовка = НСтр("ru = 'Настройка %1 отчета ""%2""'; en = 'Setting %1 of report ""%2""'");
	Если Строка.Тип = "ДиаграммаКомпоновкиДанных" Тогда
		ПредставлениеЭлемента = НСтр("ru = 'диаграммы'; en = 'charts'");
	Иначе
		ПредставлениеЭлемента = НСтр("ru = 'группировки'; en = 'groupings'");
	КонецЕсли;
...
Процедура ОпределитьВыделенныеСтроки(Контекст)
	...
	Если Контекст.СтрокиДерева.Количество() = 0 Тогда
		Контекст.ПричинаОтказа = НСтр("ru = 'Выберите элементы.'; en = 'Select items.'");
		Возврат;
	КонецЕсли;
	Если Контекст.ТекущаяСтрока = Неопределено Тогда
		Если Контекст.Действие = "ИзменитьГруппу" Тогда
			Контекст.ПричинаОтказа = НСтр("ru = 'Выберите группу.'; en = 'Select group.'");
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция СтрокаДобавлена(Строки, СтрокаДерева, Особенности)
	...
	Если (Особенности.МогутБытьПараметры И СтрокаДерева.ЭтоПараметр)
		...
		Если Строки.Действие = "Переместить" Тогда
			Строки.ПричинаОтказа = НСтр("ru = 'Параметры не могут быть перемещены.'; en = 'Parameters cannot be transferred.'");
		ИначеЕсли Строки.Действие = "Сгруппировать" Тогда
			Строки.ПричинаОтказа = НСтр("ru = 'Параметры не могут быть участниками групп.'; en = 'Parameters cannot be group participants.'");
		ИначеЕсли Строки.Действие = "Удалить" Тогда
			Строки.ПричинаОтказа = НСтр("ru = 'Параметры не могут быть удалены.'; en = 'Parameters cannot be deleted.'");
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
	Если Особенности.ТребоватьОдногоРодителя Тогда
		...
		ИначеЕсли Строки.ТекущийРодитель <> Родитель Тогда
			Если Строки.Действие = "Переместить" Тогда
				Строки.ПричинаОтказа = НСтр("ru = 'Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.'; en = 'The selected items cannot be transferred as they have different parents.'");
			ИначеЕсли Строки.Действие = "Сгруппировать" Тогда
				Строки.ПричинаОтказа = НСтр("ru = 'Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.'; en = 'The selected items cannot be grouped as they have different parents.'");
			КонецЕсли;
			Возврат Ложь; 
		КонецЕсли;
	КонецЕсли;
...
Функция ДоступныеРежимыОтображения(ПоказыватьРежимыФлажков)
	...
	ДоступныеРежимыОтображения.Добавить("ПоказыватьВШапкеОтчета", НСтр("ru = 'В шапке отчета'; en = 'In report header'"), , БиблиотекаКартинок.БыстрыйДоступ);
	
	Если ПоказыватьРежимыФлажков Тогда
		ДоступныеРежимыОтображения.Добавить("ПоказыватьТолькоФлажокВШапкеОтчета", НСтр("ru = 'Только флажок в шапке отчета'; en = 'Only check box in report title'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	
	ДоступныеРежимыОтображения.Добавить("ПоказыватьВНастройкахОтчета", НСтр("ru = 'В настройках отчета'; en = 'In report settings'"), , БиблиотекаКартинок.Реквизит);
	
	Если ПоказыватьРежимыФлажков Тогда
		ДоступныеРежимыОтображения.Добавить("ПоказыватьТолькоФлажокВНастройкахОтчета", НСтр("ru = 'Только флажок в настройках отчета'; en = 'Only check box in report settings'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
	
	ДоступныеРежимыОтображения.Добавить("НеПоказывать", НСтр("ru = 'Не показывать'; en = 'Do not show'"), , БиблиотекаКартинок.СкрытаяНастройкаОтчета);
...
Процедура ОбновитьСвойстваЭлементовФормы()
	...
	Если ЭтоРасширенныйРежим Тогда
		Элементы.СтраницаОтборы.Заголовок = НСтр("ru = 'Фильтры'; en = 'Filters'");
	Иначе
		Элементы.СтраницаОтборы.Заголовок = НСтр("ru = 'Основное'; en = 'Main'");
	КонецЕсли;
	...
	Если ОтображатьВыбранныеПоля И ОтображатьСортировки Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ru = 'Поля и сортировки'; en = 'Fields and sortings'");
	...
	ИначеЕсли ОтображатьСортировки Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ru = 'Сортировки'; en = 'Sorting'");
	КонецЕсли;
	...
	Иначе
		Элементы.ЗакрытьИСформировать.Заголовок = НСтр("ru = 'Закрыть и сформировать'; en = 'Close and generate'");
...
Функция ПредставлениеКоллекцииЭлементаСтруктуры(ИмяКоллекции)
	...
	ПредставлениеКоллекций.Вставить("Структура", НСтр("ru = 'Структура'; en = 'Structure'"));
	ПредставлениеКоллекций.Вставить("Строки", НСтр("ru = 'Строки'; en = 'Rows'"));
	ПредставлениеКоллекций.Вставить("Колонки", НСтр("ru = 'Колонки'; en = 'Columns'"));
	ПредставлениеКоллекций.Вставить("Точки", НСтр("ru = 'Точки'; en = 'Points'"));
	ПредставлениеКоллекций.Вставить("Серии", НСтр("ru = 'Серии'; en = 'Series'"));
