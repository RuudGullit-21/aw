...
Процедура ПриЗавершенииЗадания(РезультатЗадания, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли Результат.КоличествоНайденных > 20 Тогда
		...
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Показаны первые 20 контрагентов из %1 найденных. Уточните реквизиты для поиска.'; en = 'The first 20 counterparties of %1 found are shown. Refine your search criteria.'"), 
			Результат.КоличествоНайденных));
	ИначеЕсли НЕ ПустаяСтрока(СтрокаПоиска) И Результат.КоличествоНайденных = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Ничего не найдено. Уточните реквизиты для поиска.'; en = 'No results found. Refine your search criteria.'"));
...
"Подключить Интернет-поддержку?'; en = 'To automatically fill in the attributes of the counterparties"
"it is necessary to connect users Internet support."
"Connect Internet support?'");
...
"необходимо подключить Интернет-поддержку пользователей."
"Обратитесь к администратору.'; en = 'To automatically fill in the attributes of the counterparties"
"it is necessary to connect users Internet support."
"Please contact the administrator.'"));
	...
	ИначеЕсли КоличествоНайденных > Контрагенты.Количество() Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Найдено слишком много похожих контрагентов (%1). Уточните реквизиты для поиска.'; en = 'Too many similar counterparties (%1) are found. Specify the search attributes.'"),
			КоличествоНайденных);
...
Процедура ВыполнитьВыборКонтрагента()
	...
	Если ТекДанные = Неопределено Тогда
		Если Контрагенты.Количество() = 0 Тогда
			ТекстПредупреждения = НСтр("ru = 'Ничего не найдено. Уточните реквизиты и нажмите ""Найти"".'; en = 'No results found. Refine your search criteria, and then click ""Search"".'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ТекДанные.ИНН) Тогда
		ТекстПредупреждения = НСтр("ru = 'Выберите контрагента, у которого указан ИНН.'; en = 'Select a counterparty with the specified TIN.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
