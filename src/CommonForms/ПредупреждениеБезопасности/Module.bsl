...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Не ЗначениеЗаполнено(Параметры.Ключ) Тогда
		ТекстОшибки = 
			НСтр("ru = 'Общая форма ""Предупреждение безопасности"" является вспомогательной и открывается из служебных механизмов программы.'; en = 'General form ""Security warning"" is an auxiliary one and is opened from service mechanisms of the application.'");
	...
	Если Параметры.МножественныйВыбор Тогда 
		Элементы.ПредупреждениеПередУдалениемРасширенияСДаннымиТекстУдалить.Заголовок = НСтр("ru = 'Удалить выделенные расширения?'; en = 'Delete selected extensions?'");
	Иначе 
		Элементы.ПредупреждениеПередУдалениемРасширенияСДаннымиТекстУдалить.Заголовок = НСтр("ru = 'Удалить расширение?'; en = 'Delete extension?'");
	КонецЕсли;
	...
	Если ТекущаяСтраница = Элементы.ПередУдалениемРасширенияСДанными
	 ...
	 Или ТекущаяСтраница = Элементы.ПередОтключениемРасширенияСДанными Тогда
		Заголовок = НСтр("ru = 'Предупреждение'; en = 'Warning'");
	КонецЕсли;
...
Процедура ПредложитьПерезапуск()
	...
	Кнопки.Добавить("НеПерезапускать", НСтр("ru = 'Не перезапускать'; en = 'Do not restart'"));
	ТекстВопроса = НСтр("ru = 'Для применения изменений требуется перезапустить программу.'; en = 'To apply the changes, restart the application.'");
