...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		Элементы.ПутьКАрхивуWindows.Заголовок = НСтр("ru = 'Для сервера 1С:Предприятия под управлением Microsoft Windows'; en = 'For the 1C:Enterprise server under Microsoft Windows'"); 
	Иначе
...
Процедура ПутьКАрхивуWindowsНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Диалог.Фильтр                       = НСтр("ru = 'Архивы zip(*.zip)|*.zip'; en = 'Zip archives(*.zip)|*.zip'");
...
"файлами начального образа (файл *.zip)'; en = 'Specify a full name of an archive with"
"initial image files (file *.zip)'");
...
"должен быть в формате UNC (\\servername\resource).'; en = 'Path to the archive with initial image files must have"
...
"начального образа успешно завершено.'; en = 'Placement of files from archive"
"with initial images succeeded.'");
	ПоказатьОповещениеПользователя(НСтр("ru = 'Размещение файлов'; en = 'File placement'"),, ТекстОповещения);
