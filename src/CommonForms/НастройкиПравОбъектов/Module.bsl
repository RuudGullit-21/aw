...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
		ВызватьИсключение НСтр("ru = 'Не указан владелец настроек прав.'; en = 'Not specified owner of rights settings.'");
	КонецЕсли;
...
"для объектов типа ""%1"".'; en = 'Access rights for each object are not configured"
"for objects of type ""%1"".'"),
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.НайтиПоТипу(ТипСсылкиНаОбъект)) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Нет права Просмотр на объекты типа ""%1"".'; en = 'No right View on objects of type ""%1"".'"), Строка(ТипСсылкиНаОбъект));
	КонецЕсли;
...
Процедура Перечитать(Команда)
	...
	Иначе
		ПоказатьВопрос(
			...
			НСтр("ru = 'Данные изменены. Прочитать без сохранения?'; en = 'Data is changed. Read without saving?'"),
...
Процедура ЗаписатьНачало(Закрыть = Ложь)
	...
	Если ПодтвердитьОтказОтУправленияПравами = Истина Тогда
		...
		Кнопки.Добавить("ЗаписатьИЗакрыть", НСтр("ru = 'Записать и закрыть'; en = 'Write and close'"));
		Кнопки.Добавить("Отмена", НСтр("ru = 'Отмена'; en = 'Cancel'"));
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗаписатьПослеПодтверждения", ЭтотОбъект),
			НСтр("ru = 'После записи настройка прав станет недоступной.'; en = 'After writing, you cannot assign access rights.'"),
			Кнопки,, "Отмена");
	Иначе
		Если Закрыть Тогда
			Закрыть();
...
"but also for its subfolders'");
		...
		Элемент.Заголовок   = НСтр("ru = 'Наследуется от'; en = 'Inherited from'");
		Элемент.Подсказка   = НСтр("ru = 'Папка, от которой наследуются настройка прав'; en = 'Folder from which access rights settings are inherited'");
	...
	Пока НЕ Отказ И НомерСтроки >= 0 Цикл
		...
		Если НетЗаполненногоПрава Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Не заполнено ни одно право доступа.'; en = 'No access right is specified.'"),
				,
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока["Пользователь"]) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Не заполнен пользователь или группа.'; en = 'User or group is not entered.'"),
				,
		...
		Если ГруппыПрав.НайтиСтроки(Отбор).Количество() > 1 Тогда
			Если ТипЗнч(Отбор.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстСообщения = НСтр("ru = 'Настройка для пользователя ""%1"" уже есть.'; en = 'Setting for user ""%1"" already exists.'");
			Иначе
				ТекстСообщения = НСтр("ru = 'Настройка для группы пользователей ""%1"" уже есть.'; en = 'Setting for user group ""%1"" already exists.'");
			КонецЕсли;
			ОбщегоНазначенияКлиент.СообщитьПользователю(
...
Процедура ЗаписатьПрава()
	...
	Попытка
		...
		Иначе
			...
			ВызватьИсключение НСтр("ru = 'После записи настройка прав станет недоступной.'; en = 'After writing, you cannot assign access rights.'");
		КонецЕсли;
...
"вышестоящей папки ""%1"".'; en = 'These rights are inherited, they can be changed in setting right form of"
"parent folder ""%1"".'"),
...
Процедура ПроверитьРазрешениеНаУправлениеПравами()
	...
	ВызватьИсключение НСтр("ru = 'Настройка прав недоступна.'; en = 'Right setting is not available.'");
