...
Процедура ДиалогПользователяСНесовместимымиПравилами(ОписаниеСтрокиПриложения, ОписаниеОшибки)
	...
	Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
		...
		ПараметрыФормы.Заголовок = НСтр("ru = 'Синхронизация данных не может быть выполнена'; en = 'Data synchronization cannot be executed'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		
	Иначе
		
		ПараметрыФормы.Заголовок = НСтр("ru = 'Синхронизация данных может быть выполнена некорректно'; en = 'Data synchronization may be executed incorrectly'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
