...
Процедура ЗаполнитьИдентификациюФайлаОбмена_ПослеПолученияУИД(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗавершенныеПоТитулу = ЗавершенныеОтправки.Получить(Объект.ТекущийТитул);
	
	ПоследняяВерсия = 0;
	ВерсияЗафиксирована = Ложь;
	Для Каждого СтрокаВерсии Из ВерсииТитулов Цикл
		Если СтрокаВерсии.Титул = Объект.ТекущийТитул И ПоследняяВерсия <= СтрокаВерсии.НомерВерсии Тогда
			ВерсияЗафиксирована = Ложь; //СтрокаВерсии.ИдентификаторФайла <> "" И ЗначениеЗаполнено(СтрокаВерсии.ДатаВерсии);
	...
	ИначеЕсли Переформировать = Ложь Тогда
		...
		Если Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_ОткМ") Тогда
			ТребуетсяЗаполнить = Ложь;
		ИначеЕсли ДанныеСостоянияЭДО.СуществуетАктуальныйДокумент = Истина Тогда
			Если Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул1") Тогда
				ТребуетсяЗаполнить = Ложь;
			ИначеЕсли ВерсияЗафиксирована = Истина Тогда
				ТребуетсяЗаполнить = Ложь;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.УИДМинтранс) = Ложь
		И Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул2")
		И Объект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул6") Тогда
		ТребуетсяЗаполнить = Истина;	
	КонецЕсли;
...
Процедура УстановитьТекущийШагПомощника(ПредыдущийТекущийТитул = Неопределено)
	...
	Иначе		
		Если ТитулСтраницы = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул2") Тогда			
...
Процедура ТитулМедосмотрВариантМедработникПриИзменении(Элемент)
	...
	Если ВариантМедработник = 0 Тогда
		...
		ОбменСГИСЭПДКлиентСервер.УдалитьРеквизитыПоГруппеДанных(ЭтотОбъект, "ТитулМедосмотрСтороннийМедработник");	
	Иначе
		ХранимыеДанныеТитулМедосмотрШтатныйМедработник = Неопределено;
		ОбменСГИСЭПДКлиентСервер.УдалитьРеквизитыПоГруппеДанных(ЭтотОбъект, "ТитулМедосмотрШтатныйМедработник");	
	КонецЕсли;
...
Процедура ТитулМедосмотрВариантМедработникПослеПриИзменении(Элемент)
	...
	Если ВариантМедработник = 0 Тогда
		...
		ОбменСГИСЭПДКлиентСервер.УдалитьРеквизитыПоГруппеДанных(ЭтотОбъект, "ТитулМедосмотрПослеСтороннийМедработник");	
	Иначе
		ХранимыеДанныеТитулМедосмотрПослеШтатныйМедработник = Неопределено;
		ОбменСГИСЭПДКлиентСервер.УдалитьРеквизитыПоГруппеДанных(ЭтотОбъект, "ТитулМедосмотрПослеШтатныйМедработник");	
	КонецЕсли;
