...
Процедура ПодготовитьПредставлениеФайла(ЭлементСхемыРегламента)
	...
	РедактированиеЗапрещено = ЭлементСхемыРегламента.Статус = Перечисления.СтатусыСообщенийЭДО.Подписан
		...
		ИЛИ ЭлементСхемыРегламента.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорнойДокумент;
...
Процедура ОбернутьДанныеСообщенияВПДФ(Действие)
	...
	Если ЭтоАдресВременногоХранилища(АдресПДФКонтейнера) Тогда
		...
		ОписаниеСообщения = ПолучитьИзВременногоХранилища(Параметры.РезультатПодготовкиДанных.АдресОписанияСообщения);
		
		Обработки.ФорматДоговорнойДокумент101.ПоместитьФайлЭДВФайлПДФ(ДвоичныеДанныеФайлаПДФ,
			ОписаниеСообщения.Данные.Документ);
		
		Параметры.РезультатПодготовкиДанных.АдресОписанияСообщения = ПоместитьВоВременноеХранилище(ОписаниеСообщения,
			Параметры.РезультатПодготовкиДанных.АдресОписанияСообщения);
