...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТолькоПочта Тогда
		ЗаголовокУчастникиПочта =  НСтр("ru = 'Кому, от'; en = 'To, from'");
...
Процедура ДеревоВзаимодействийПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ДеревоВзаимодействий.ВыделенныеСтроки.Количество() > 0 Тогда
		...
		Если ЕстьПомеченныеНаУдаление Тогда
			ТекстВопроса = НСтр("ru = 'Снять с выделенных элементов пометку на удаление?'; en = 'Clear marks for deletion of the selected items?'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Пометить выделенные строки на удаление?'; en = 'Mark the selected lines for deletion?'");
		КонецЕсли;
...
Процедура ОтложитьРассмотрениеВыполнить(Команда)
	...
	ПоказатьВводДаты(ОбработчикОповещенияОЗакрытии, ДатаОтработки, НСтр("ru = 'Отработать после'; en = 'Process after'"));
...
Процедура УправлениеСтраницамиСервер()
	...
	Если ВВидеДерева Тогда
		...
		Команды.ПереключитьРежимПросмотра.Подсказка = НСтр("ru = 'Установить режим просмотра в виде списка'; en = 'Set view mode as a list'");
	...
	Иначе
		...
		Команды.ПереключитьРежимПросмотра.Подсказка = НСтр("ru = 'Установить режим просмотра в виде дерева'; en = 'Set view mode as a tree'");
...
Процедура ЗаполнитьДеревоВзаимодействий()
	...
	ШаблонЗаголовка = НСтр("ru = 'Тип взаимодействий: %1'; en = 'Interaction type: %1'");
