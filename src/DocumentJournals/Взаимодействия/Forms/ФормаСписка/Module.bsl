...
Процедура ПапкиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если ТекущиеДанные.ЕстьПравоНаИзменение = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Недостаточно прав для создания папки.'; en = 'Insufficient rights to create a folder.'"));
		Возврат;
	КонецЕсли;
...
Процедура Ответить(Команда)
	...
	Если ВыборКорректен(Элементы.Список.Имя, Истина) Тогда
		...
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Команда ""Ответить"" может быть выполнена только для входящего письма.'; en = 'You can click ""Reply"" only for incoming messages.'"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
...
Процедура ОтветитьВсем(Команда)
	...
	Если ВыборКорректен(Элементы.Список.Имя, Истина) Тогда
		ТекущееВзаимодействие = Элементы.Список.ТекущиеДанные.Ссылка;
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Команда ""Ответить всем"" может быть выполнена только для входящего письма.'; en = 'You can click ""Reply to all"" only for incoming messages.'"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
...
Процедура Переслать(Команда)
	...
	Если ВыборКорректен(Элементы.Список.Имя, Истина) Тогда
		...
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") 
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Команда ""Переслать"" может быть выполнена только для писем.'; en = 'You can click ""Forward"" only for messages.'"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
...
Процедура ДобавитьВЗакладки(Команда)
	...
	ИмяТекущегоЭлемента = ТекущийЭлемент.Имя;
	Если СтрНачинаетсяС(ИмяТекущегоЭлемента, "Список") И Не ВыборКорректен(ИмяТекущегоЭлемента) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбран элемент для добавления в закладки.'; en = 'Item for adding to tabs is not selected.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не Результат.ЭлементДобавлен Тогда
		...
		Возврат;
	КонецЕсли;
	ПоказатьОповещениеПользователя(НСтр("ru = 'Добавлены в закладки:'; en = 'Added to tabs:'"),
...
Процедура ОтложитьРассмотрениеВыполнить(Команда)
	...
	ПоказатьВводДаты(ОбработчикОповещенияОЗакрытии, ДатаОтработки, НСтр("ru = 'Отработать после'; en = 'Process after'"), ЧастиДаты.ДатаВремя);
...
Процедура ПеренестиВПапку(Команда)
	...
	Если ИмяТекущегоЭлемента = "Папки" Тогда
		Если ТипЗнч(ПапкиТекущиеДанные.Значение) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") 
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Команда не может быть выполнена для данного объекта'; en = 'Command cannot be executed for this object'"));
			Возврат;
		ИначеЕсли ПапкиТекущиеДанные.ЕстьПравоНаИзменение = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Недостаточно прав для изменения папок.'; en = 'Insufficient rights to change folders.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПереслатьКакВложение(Команда)
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Пересылать как вложения можно только отправленные и полученные письма.'; en = 'Forward as attachments can only sent and received emails.'");
...
Процедура ПриИзмененииСтатусаСервер(ОбновлятьПанельНавигации)
	...
	ШаблонЗаголовка = НСтр("ru = 'Статус: %1'; en = 'Status: %1'");
...
Процедура ИзменитьОтборСписок(ИмяТаблицы, ДанныеДляОбработки)
	...
	Если ИмяТаблицы = "Предметы" Тогда
		...
		ИмяОтбора = НСтр("ru = 'Предмет'; en = 'Subject'");
	...
	ИначеЕсли ИмяТаблицы = "Папки" Тогда
			...
			Если ТипЗнч(ДанныеДляОбработки.Значение) = Тип("СправочникСсылка.ПапкиЭлектронныхПисем") Тогда
				...
				ВидСравненияЭлементаОтбора = ВидСравненияКомпоновкиДанных.Равно;
				ПравоеЗначение             = ДанныеДляОбработки.Значение;
				ИмяОтбора = НСтр("ru = 'Папка'; en = 'Folder'");
	...
	ИначеЕсли ИмяТаблицы = "Контакты" Тогда
		...
		ВидСравненияЭлементаОтбора = ВидСравненияКомпоновкиДанных.Равно;
		ПравоеЗначение             = ДанныеДляОбработки.Значение;
		ИмяОтбора = НСтр("ru = 'Контакт'; en = 'Contact'");
		ЗначениеОтбора = ДанныеДляОбработки.Значение;
	...
	ИначеЕсли ИмяТаблицы = "Свойства" Тогда
		...
		Если ТипЗнч(ДанныеДляОбработки.Значение) = Тип("Строка") 
			...
			ПравоеЗначение             = "";
			ЗначениеОтбора = НСтр("ru = 'Не указан'; en = 'Not specified'");
	...
	ИначеЕсли ИмяТаблицы = "Категории" Тогда
		...
		ИмяОтбора      = НСтр("ru = 'Категория'; en = 'Category'");
	...
	ИначеЕсли ИмяТаблицы = "Закладки" Тогда
		...
		ЗаголовокПанелиНавигации   = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонЗаголовка, НСтр("ru = 'Закладка'; en = 'Tab'"), ДанныеДляОбработки.Значение);
...
Процедура УправлениеВидимостьюПриПереключенииПанелиНавигации()
	...
	Если ПанельНавигацииСкрыта Тогда
		...
		КомандаИзменитьОтображениеПанелиНавигации.Подсказка = НСтр("ru = 'Показать панель навигации'; en = 'Show navigation panel'");
		КомандаИзменитьОтображениеПанелиНавигации.Заголовок = НСтр("ru = 'Показать панель навигации'; en = 'Show navigation panel'");
	Иначе
		Элементы.ИзменитьОтображениеПанелиНавигации.Картинка = БиблиотекаКартинок.СтрелкаВлево;
		КомандаИзменитьОтображениеПанелиНавигации.Подсказка = НСтр("ru = 'Скрыть панель навигации'; en = 'Hide navigation panel'");
		КомандаИзменитьОтображениеПанелиНавигации.Заголовок = НСтр("ru = 'Скрыть панель навигации'; en = 'Hide navigation panel'");
	КонецЕсли;
...
Процедура УстановитьЗаголовокВидаПанелиНавигации(ЗначениеОтбора = Неопределено)
	...
	Иначе
		...
		ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаЗакладки Тогда
			...
			Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru = 'По закладкам'; en = 'By tabs'");
			Элементы.УстановитьВариантНавигацииПоЗакладкам.Пометка = Истина;
			
		ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаПредмет Тогда
			
			Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru = 'По предметам'; en = 'By subjects'");
		...
		ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаКонтакт Тогда
			...
			Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru = 'По контактам'; en = 'By contacts'");
			Элементы.УстановитьВариантНавигацииПоКонтакту.Пометка = Истина;
			
		ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаПапки Тогда
			
			Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru = 'По папкам'; en = 'By folders'");
			Элементы.УстановитьВариантНавигацииПоПапкам.Пометка = Истина;
			
		ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаКатегории Тогда
			
			Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru = 'По категориям'; en = 'By categories'");
...
Процедура ЗаполнитьДеревоСвойств(ИмяКоманды = "")
	...
	Запрос.УстановитьПараметр("НеУказанПредставление", НСтр("ru = 'Не указан'; en = 'Not specified'"));
	...
	Для Каждого Строка Из РодительскаяСтрока.Строки Цикл
		...
		Если ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Строка.Значение) Тогда
			...
			НоваяСтрока.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 от %2 %3'"),
				?(ПустаяСтрока(СтрокаДетали.Тема),НСтр("ru = 'Тема не указана'; en = 'Subject is not specified'"), СтрокаДетали.Тема),
...
Процедура ДополнитьМенюПанелиНавигации()
	...
	Пока Выборка.Следующий() Цикл
		...
		ЭлементКнопкаПодменю.Заголовок = НСтр("ru = 'По'; en = 'To'") + " " + Выборка.Представление;
	...
	Если ТаблицаДопРеквизитовСвойствТипаБулево.Количество() > 0 Тогда
		...
		ЭлементКнопкаПодменю.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
		ЭлементКнопкаПодменю.ИмяКоманды = НоваяКоманда.Имя;
		ЭлементКнопкаПодменю.Заголовок = НСтр("ru = 'По категориям'; en = 'By categories'");
...
Функция ДобавитьВЗакладкиСервер(Знач ДанныеДляОбработки, ИмяЭлементаФормы)
	...
	Если СтрНачинаетсяС(ИмяЭлементаФормы, "Список") Тогда
		...
		Если СписокВзаимодействий.Количество() = 0 Тогда
			Результат.ТекстСообщенияОшибка = НСтр("ru = 'Не выбран элемент для добавления в закладки.'; en = 'Item for adding to tabs is not selected.'");
			Возврат Результат;
		КонецЕсли;
		...
		НаименованиеЗакладки = ?(ТолькоПочта, НСтр("ru = 'Избранные письма'; en = 'Selected emails'"), НСтр("ru = 'Избранные взаимодействия'; en = 'Favorite interactions'"));
		Если СписокВзаимодействий.Количество() > 1 Тогда
			Текст = ?(ТолькоПочта, НСтр("ru = 'Выбранные письма (%1)'; en = 'Selected emails (%1)'"), НСтр("ru = 'Выбранные взаимодействия (%1)'; en = 'Selected interactions (%1)'"));
	...
	Иначе
		...
		Если ДанныеДляОбработки.Значение = "ВсеЗначения" Тогда
			Результат.ТекстСообщенияОшибка = НСтр("ru = 'Закладку без отбора создать нельзя.'; en = 'Cannot create a tab without filter.'");
			Возврат Результат;
		КонецЕсли;
		...
		Если ИмяЭлементаФормы = "ПредметыПанельНавигации" Тогда
			...
			Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеДляОбработки.Значение) Тогда
				...
				НаименованиеЗакладки       = НСтр("ru = 'Предмет'; en = 'Subject'") + " = " + Строка(ДанныеДляОбработки.Значение); 
				Текст = ?(ТолькоПочта, НСтр("ru = 'Письма по предмету %1'; en = 'Emails by subject %1'"), НСтр("ru = 'Взаимодействия по предмету %1'; en = 'Interactions on the subject %1'"));
			...
			ИначеЕсли ДанныеДляОбработки.Значение = "НеУказан" Тогда
				...
				НаименованиеЗакладки       = НСтр("ru = 'Предмет не указан'; en = 'Subject is not specified'");
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма без предмета'; en = 'Emails without subject'"), НСтр("ru = 'Взаимодействия без предмета'; en = 'Interaction without subject'"));
				
			Иначе
				
				НаименованиеЗакладки       = НСтр("ru = 'Тип предмета'; en = 'Subject type'") + " " + Строка(ДанныеДляОбработки.ОписаниеТипов.Типы()[0]);
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма по: %1'; en = 'Emails by: %1'"), НСтр("ru = 'Взаимодействия по: %1'; en = 'Interactions on:%1'"));
		...
		ИначеЕсли ИмяЭлементаФормы = "Свойства" Тогда
			...
			Если ТипЗнч(ДанныеДляОбработки.Значение) = Тип("Строка") 
				И ДанныеДляОбработки.Значение = "НеУказан" Тогда
				НаименованиеЗакладки       = ТекущееСвойствоПанелиНавигации.Наименование + " " + НСтр("ru = 'не указан'; en = 'not specified'");
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма'; en = 'Emails'"), НСтр("ru = 'Взаимодействия'; en = 'Interactions'"));
			Иначе
				...
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, 
				                                   НСтр("ru = 'Письма с заданным свойством: %1'; en = 'Emails with the specified property: %1'"), 
				                                   НСтр("ru = 'Взаимодействия с заданным свойством: %1'; en = 'Interactions with specified property: %1'"));
		...
		ИначеЕсли ИмяЭлементаФормы = "Категории" Тогда
			...
			НаименованиеЗакладки       = НСтр("ru = 'Входит в категорию'; en = 'Belongs to the category'") + " " + Строка(ДанныеДляОбработки.Значение);
			Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма из категории: %1'; en = 'Emails from category: %1'"), НСтр("ru = 'Взаимодействия из категории: %1'; en = 'Interactions from the category: %1'"));
			Результат.ПредставлениеЭлемента = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Результат.ПредставлениеЭлемента, ДанныеДляОбработки.Значение);
		...
		ИначеЕсли ИмяЭлементаФормы = "КонтактыПанелиНавигации" Тогда
			...
			Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеДляОбработки.Значение) Тогда
				...
				НаименованиеЗакладки       = НСтр("ru = 'Контакт'; en = 'Contact'") + " = " + Строка(ДанныеДляОбработки.Значение); 
				Текст = ?(ТолькоПочта, НСтр("ru = 'Письма по контакту %1'; en = 'Emails by contact %1'"), НСтр("ru = 'Взаимодействия по контакту %1'; en = 'Interactions by contact %1'"));
				Результат.ПредставлениеЭлемента = 
			...
			ИначеЕсли ДанныеДляОбработки.Значение = НСтр("ru = 'Контакт не подобран'; en = 'Contact is not selected'") Тогда
				
				НаименованиеЗакладки       = НСтр("ru = 'Контакт не указан'; en = 'Contact is not specified'");
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма без подобранных контактов'; en = 'Emails without selected contacts'"), НСтр("ru = 'Взаимодействия без подобранных контактов'; en = 'Interactions without selected contacts'"));
				
			Иначе
				
				НаименованиеЗакладки       = НСтр("ru = 'Тип контакта'; en = 'Contact type'") + " " + Строка(ДанныеДляОбработки.ОписаниеТипов.Типы()[0]);
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма по контактам: %1'; en = 'Emails by contacts: %1'"), НСтр("ru = 'Взаимодействия по контактам: %1'; en = 'Interactions by contacts: %1'"));
				Результат.ПредставлениеЭлемента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Результат.ПредставлениеЭлемента, ДанныеДляОбработки.ОписаниеТипов.Типы()[0]);
		...
		ИначеЕсли ИмяЭлементаФормы = "Папки" Тогда
			...
			НаименованиеЗакладки       = НСтр("ru = 'В папке'; en = 'In folder'") + " " + Строка(ДанныеДляОбработки.Значение);
			Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма в папке: %1'; en = 'Emails are in folder: %1'"), НСтр("ru = 'Взаимодействия в папке: %1'; en = 'Interactions in the folder: %1'"));
	...
	Пока Выборка.Следующий() Цикл
		Если ЗначениеВXML(КомпоновщикНастроек.ПолучитьНастройки()) =  ЗначениеВXML(Выборка.КомпоновщикНастроек.Получить()) Тогда
			Результат.ТекстСообщенияОшибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Закладка с такими настройками уже существует : %1'; en = 'Tab with these settings already exists : %1'"),
...
Функция РезультатПоискаВзаимодействийПолнотекстовымПоиском()
	...
	Если СписокПоиска.СлишкомМногоРезультатов() Тогда
		...
		Результат.ТекстОшибки = НСтр("ru = 'Слишком много результатов, уточните запрос.'; en = 'Too many results. Refine your search criteria.'");
		Возврат Результат;
	...
	Если СписокПоиска.ПолноеКоличество() = 0 Тогда
		...
		Результат.ТекстОшибки         = НСтр("ru = 'Ничего не найдено.'; en = 'None found.'");
		Возврат Результат;
