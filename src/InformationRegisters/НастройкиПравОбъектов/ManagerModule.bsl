	...
	Для Каждого ВозможноеПраво Из ВозможныеПрава Цикл
		...
		Если ОбъектМетаданныхВладельца = Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найден владелец прав ""%1"".'; en = 'Owner of rights ""%1"" is not found.'"),
...
"но обнаружен в описании вида доступа ""%2"".'; en = 'Rights owner type ""%1"""
"cannot be used as an access value type but it is"
"detected in the description of access kind ""%2"".'"),
					Строка(ТипСсылки),
...
"но обнаружен в описании вида доступа ""%2"".'; en = 'Rights owner type ""%1"""
"cannot be used as a type of access value groups but it is"
"detected in the description of access kind ""%2"".'"),
					Строка(ТипСсылки),
					СвойстваВидаДоступа.Имя);
				ВызватьИсключение ТекстОшибки;
...
"повторно определено право ""%2"".'; en = 'The ""%2"" right is defined again"
"for the ""%1"" right owner.'"),
				ВозможноеПраво.ВладелецПрав,
				ВозможноеПраво.Имя);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
"В этом случае отдельных таблиц указывать не нужно.'; en = 'The ""*"" character is specified for the ""%1"" right owner"
"for the ""%2"" right in tables for reading."
...
"В этом случае отдельных таблиц указывать не нужно.'; en = 'The ""*"" character is specified for the ""%1"" right owner"
"for the ""%2"" right in tables for change."
"In this case, do not specify separate tables.'")
				КонецЕсли;
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Если ЗначениеЗаполнено(ОбщиеПрава[Свойство]) Тогда
				Если Свойство = "ЧтениеВТаблицах" Тогда
					ШаблонОшибки =
						НСтр("ru = 'Для владельца прав ""%1"""
"для права ""%2"" в таблицах для чтения указан символ ""*""."
"Однако символ ""*"" уже указан в таблицах для чтения для права ""%3"".'; en = 'The ""*"" character is specified for the ""%1"" right owner"
"for the ""%2"" right in tables for reading."
"Chracter ""*"" is already specified in tables for reading for the ""%3"" right.'")
				Иначе
					ШаблонОшибки =
						НСтр("ru = 'Для владельца прав ""%1"""
"для права ""%2"" в таблицах для изменения указан символ ""*""."
"Однако символ ""*"" уже указан в таблицах для изменения для права ""%3"".'; en = 'The ""*"" character is specified for the ""%1"" right owner"
"for the ""%2"" right in tables for change."
...
"для права ""%2"" не найдена таблица для чтения ""%3"".'; en = 'Table for reading ""%3"" is not found for the ""%1"""
"right owner for the ""%2"" right.'")
			Иначе
				ШаблонОшибки = НСтр("ru = 'Для владельца прав ""%1"""
"для права ""%2"" не найдена таблица для изменения ""%3"".'; en = 'Table for change ""%3"" is not found for the ""%1"""
"right owner for the ""%2"" right.'")
			КонецЕсли;
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
