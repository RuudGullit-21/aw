	...
	Для каждого ТекущаяЗапись Из ЭтотОбъект Цикл
		Если ТипЗнч(ТекущаяЗапись.Список) = ТипКатегорияНовостей Тогда
			Если ТекущаяЗапись.Список.Пустая() Тогда
				...
				Сообщение.Текст = НСтр("ru = 'Значение обновляемой категории должно быть заполнено.'; en = 'Value of the updated category is required.'");
				Сообщение.Сообщить();
			КонецЕсли;
...
"- Код категории новостей (начинается с [Значения категории новостей:])'; en = 'Incorrect value %1."
"Allowed only:"
"- News category list"
"- News feed list"
...
"- Ссылка на план видов характеристик КатегорииНовостей'; en = 'Incorrect value %1."
"Allowed only:"
"- News category list"
"- News feed list"
"- News category code (begins with [News category values:])"
