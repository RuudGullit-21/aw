...
"по заданным в ней правилам. Продолжить?'; en = 'Removing the setting will lead to the termination of files cleanup"
"on specified in it rules. Continue?'");
	...
	ТекстСообщения = НСтр("ru = 'Расширенная настройка очистки файлов не предусмотрена для этого объекта.'; en = 'Advanced setting of cleaning files are not provided for this object.'");
...
Процедура ЗапуститьРегламентноеЗадание()
	...
	Иначе
		...
		ПараметрыЗадания.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Запуск вручную: %1'; en = 'Start manually: %1'"), РегламентноеЗаданиеМетаданные.Синоним);
...
Процедура ДобавитьНастройкиПоВладельцу(ВыбранноеЗначение)
	...
	Для Каждого Настройка Из ВыбранноеЗначение Цикл
		...
		ДетализированнаяНастройка.ПравилоОтбора = НСтр("ru = 'Изменить правило'; en = 'Change rule'");
	КонецЦикла;
