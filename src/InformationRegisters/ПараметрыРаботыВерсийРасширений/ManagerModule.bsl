...
"изменилось имя или версия конфигурации - требуется перезапуск сеанса.'; en = 'Failed to update all the parameters of the extension operation, as"
"changed the name or version of the configuration - requires restart of the session.'");
	КонецЕсли;
...
Процедура ОбновитьПараметрыРаботыРасширений(ПроверяемыеРасширения = Неопределено, НеподключенныеРасширения = "", ТекстДляАсинхронногоВызова = "") Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		ТекстОшибки = НСтр("ru = 'Фоновое задание подготовки расширений не вернуло результат.'; en = 'Background job of preparing extensions did not return result.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
