	...
	Для Каждого текСтрока Из ТаблицаИзмененийТабличныхДокументов Цикл
		...
		Для Индекс = 0 По ВГраница Цикл
			...
			Если ТекСтрока[ИмяКолонки] = "И" Тогда
				...
				ШаблонТекста = НСтр("ru = 'сравнить версию №%1 с версией №%2'; en = 'compare version No.1 with version No.2'");
			...
			ИначеЕсли текСтрока[ИмяКолонки] = "У" Тогда
				...
				Область.Текст = НСтр("ru = 'Сохранение изменений табличного документа отключено'; en = 'Saving table document changes is disabled'");
...
Процедура ВывестиШапку(ТЧОтчета, СписокВерсий, ЧислоВерсий, ОбщийШаблон, СсылкаНаОбъект)
	...
	ОбластьШапка.Параметры.НаименованиеОтчета = НСтр("ru = 'Отчет по изменениям версий объекта'; en = 'Object version change report'");
...
Процедура ДобавитьНомераСтрокВТабличныеЧасти(ТабличныеЧасти)
	...
	Для Каждого Соответствие Из ТабличныеЧасти Цикл
		...
		Если Таблица.Колонки.Найти("НомерСтроки") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Таблица.Колонки.Вставить(0, "НомерСтроки",,НСтр("ru = '№ строки'; en = 'line No.'"));
