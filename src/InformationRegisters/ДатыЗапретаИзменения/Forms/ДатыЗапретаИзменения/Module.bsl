...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ru = 'Даты запрета загрузки данных прошлых периодов из других программ отключены в настройках.'; en = 'ban historical data download date from other programs turned off in the settings.'");
		
		Заголовок = НСтр("ru = 'Даты запрета загрузки данных'; en = 'Data import prohibition dates'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("ДляВсехПользователей").Представление =
			НСтр("ru = 'Для всех информационных баз'; en = 'For all infobases'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("ПоПользователям").Представление =
			НСтр("ru = 'По информационным базам'; en = 'By infobases'");
		
		Элементы.ПользователиПолноеПредставление.Заголовок =
			НСтр("ru = 'Программа: информационная база'; en = 'Application: infobase'");
	...
	Иначе
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ru = 'Даты запрета ввода и редактирования данных прошлых периодов отключены в настройках программы.'; en = 'Dates for prohibiting input and editing of previous periods data are disabled in program settings.'");
		Элементы.ПользователиПолноеПредставление.Заголовок = 
			?(ПолучитьФункциональнуюОпцию("ИспользоватьГруппыПользователей"),
			НСтр("ru = 'Пользователь, группа пользователей'; en = 'User, user group'"), НСтр("ru = 'Пользователь'; en = 'User'"));
		ЗначениеДляВсехПользователей = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехПользователей;
		СписокТиповПользователей.Добавить(
			Тип("СправочникСсылка.Пользователи"),        НСтр("ru = 'Пользователь'; en = 'User'"));
		СписокТиповПользователей.Добавить(
			Тип("СправочникСсылка.ВнешниеПользователи"), НСтр("ru = 'Внешний пользователь'; en = 'External user'"));
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ru = 'Заданные настройки дат запрета будут автоматически скорректированы.'; en = 'Settings date of ban will be automatically adjusted.'") 
		+ Символы.ПС + Символы.ПС + ТекстВопроса + Символы.ПС + Символы.ПС + НСтр("ru = 'Закрыть?'; en = 'Close?'");
...
Процедура ПользователиПередУдалением(Элемент, Отказ)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Представление) И Не ТекущиеДанные.БезДатыЗапрета Тогда
		...
		Если ДополнительныеПараметры.ДатыЗапретаДляВсехПользователей Тогда
			ТекстВопроса = НСтр("ru = 'Отключить даты запрета для всех пользователей?'; en = 'Disable the ban dates for all users?'");
		Иначе
			Если ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить даты запрета для ""%1""?'; en = 'Disable deny date for ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить даты запрета для группы пользователей ""%1""?'; en = 'Disable prohibition date for user groups ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить даты запрета для внешнего пользователя ""%1""?'; en = 'Disable the ban date for an external user ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ГруппыВнешнихПользователей") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить даты запрета для группы внешних пользователей ""%1""?'; en = 'Disable the ban date for group of external user ""%1""?'"), ТекущиеДанные.Пользователь);
			Иначе
				ТекстВопроса = НСтр("ru = 'Отключить даты запрета?'; en = 'Disable deny date?'");
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ТекущиеДанные.ЭтоРаздел Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.Раздел) Тогда
			Если ТекущиеДанные.ПолучитьЭлементы().Количество() > 0 Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить все настроенные даты запрета для раздела ""%1"" и его объектов?'; en = 'Disable all configured bans for section ""%1"" and its objects?'"), ТекущиеДанные.Раздел);
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить дату запрета для раздела ""%1""?'; en = 'Disable ban date for the section ""%1""?'"), ТекущиеДанные.Раздел);
			КонецЕсли;
		Иначе
			ТекстВопроса = НСтр("ru = 'Отключить общую дату запрета для всех разделов?'; en = 'Disable general date of ban for all sections?'");
		КонецЕсли;
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отключить дату запрета для объекта ""%1""?'; en = 'Disable date of ban for object ""%1""?'"), ТекущиеДанные.Объект);
	КонецЕсли;
...
Процедура ДатыЗапретаПолноеПредставлениеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Не ЗначениеНайдено Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '""%1"" уже есть в списке объектов'; en = '""%1"" is already in the object list'"), ВыбранноеЗначение));
		Возврат;
	КонецЕсли;
...
"Обновите данные формы (клавиша F5).'; en = '""%1"" is already in the object list."
"Refresh the form (F5 key).'"), ВыбранноеЗначение));
			Возврат;
		Иначе
			ОбновитьЗначенияПрочитанныхСвойств(
...
Процедура СпособУказанияДатыЗапретаОбработкаВыбораОбработчикОжидания()
	...
	Если ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ОбщаяДата" Тогда
		ТекстВопроса = НСтр("ru = 'Отключить даты запрета, установленные для разделов и объектов?'; en = 'Disable the ban date established for sections and objects?'");
	...
	ИначеЕсли ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ПоРазделам"
		...
		ТекстВопроса = НСтр("ru = 'Отключить даты запрета, установленные для объектов?'; en = 'Disable the ban dates, that set for objects?'");
	...
	ИначеЕсли ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ПоОбъектам"
		...
		ТекстВопроса = НСтр("ru = 'Отключить даты запрета, установленные для разделов?'; en = 'Disable the ban date that set for sections?'");
...
"Обновите данные формы (клавиша F5).'; en = '""%1"" is already in the user list."
"Refresh the form data (F5 key).'"), ВыбранноеЗначение));
			Возврат;
		КонецЕсли;
	...
	Иначе
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '""%1"" уже есть в списке пользователей.'; en = '""%1"" is already in user list.'"), ВыбранноеЗначение));
	КонецЕсли;
...
Процедура ПрочитатьДанныеПользователя(Форма, ТекстОшибки, ТекущийСпособУказания = Неопределено, Данные = Неопределено)
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			Форма.Элементы.ТекущийПользовательПредставление.Заголовок =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Настройка для ""%1"":'; en = 'Setup for ""%1"":'"), НайденныеСтроки[0].Представление);
		КонецЕсли;
	КонецЕсли;
	...
	Если Форма.ПоказыватьРазделыТекущегоПользователя Тогда
		Если Форма.ВсеРазделыБезОбъектов Тогда
			...
			Форма.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ru = 'Раздел'; en = 'Section'");
			Форма.Элементы.ДатыЗапрета.Отображение = ОтображениеТаблицы.Список;
			
		Иначе
			Форма.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ru = 'Раздел, объект'; en = 'Section, object'");
...
Процедура ВыбратьПодобратьПользователейЗавершение(ВыборИПодборВнешнихПользователей, Подобрать) Экспорт
	...
	Если ВыборИПодборВнешнихПользователей Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Недостаточно прав для выбора внешних пользователей.'; en = 'Insufficient rights to select external users.'"));
		КонецЕсли;
	Иначе
...
Процедура УстановитьВидимость()
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Если УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
			РасширеннаяПодсказка = НСтр("ru = 'Даты запрета загрузки данных из других программ действуют одинаково для всех пользователей.'; en = 'Date of ban downloading data from other programs are the same for all users.'");
		Иначе
			РасширеннаяПодсказка = НСтр("ru = 'Персональная настройка дат запрета загрузки данных прошлых периодов из других программ для выбранных пользователей.'; en = 'Personalize the dates of the ban download the historical data from other programs for selected users.'");
		КонецЕсли;
	Иначе
		Если УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
			РасширеннаяПодсказка = НСтр("ru = 'Даты запрета ввода и редактирования данных прошлых периодов действуют одинаково для всех пользователей.'; en = 'Ban dates of input and editing of past periods data are the same for all users.'");
		Иначе
			РасширеннаяПодсказка = НСтр("ru = 'Персональная настройка дат запрета ввода и редактирования данных прошлых периодов для выбранных пользователей.'; en = 'Personal setting of prohibits dates of input and editing of past period data for selected users.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура РедактироватьДатуЗапретаВФорме()
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выделенные строки не заполнены.'; en = 'The selected lines are not filled in.'"));
		Возврат;
	КонецЕсли;
	...
	Если ОбновитьВыделение Тогда
		...
		ПоказатьПредупреждение(
			...
			НСтр("ru = 'Снято выделение с незаполненных строк.'; en = 'Unfilled rows are unchecked.'"));
	Иначе
...
Процедура РедактироватьДатуЗапретаВФормеЗавершение(ВыделенныеСтроки) Экспорт
	...
	Иначе
		ПараметрыФормы.Объект = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выбранные строки (%1)'; en = 'Selected lines (%1)'"), ВыделенныеСтроки.Количество());
	КонецЕсли;
...
Функция НетДатыЗапретаПредставление(Данные)
	...
	Если Данные.ЭтоРаздел И Не Данные.Раздел.Пустая() Тогда
		Возврат НСтр("ru = 'Общая дата для всех разделов'; en = 'Common date for all sections'");
	...
	Если Форма.Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Для Каждого ЗначениеСписка Из Форма.СписокТиповПользователей Цикл
			Если ТипЗнч(ЗначениеСписка.Значение) = ТипЗнч(Пользователь) Тогда
				...
				Иначе
					Возврат ЗначениеСписка.Представление + ": " + НСтр("ru = '<Все информационные базы>'; en = '<All infobases>'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
"поэтому для всех пользователей будет действовать более простая настройка ""%1"".'; en = 'No ban date has been entered for any section,"
"so for all users, a simpler ""%1"" setting will work.'"),
				СпособУказанияВБазеДанных);
		Иначе
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ни для одного раздела не были введены даты запрета, "
"поэтому для ""%1"" будет действовать более простая настройка ""%2"".'; en = 'No ban date has been entered for any section,"
"so for ""%1"" a simpler ""%2"" setting will work.'"),
...
"поэтому для всех пользователей будет действовать более простая настройка ""%1"".'; en = 'No ban date has been entered for any object,"
"so for all users, a simpler ""%1"" setting will work.'"),
				СпособУказанияВБазеДанных);
		Иначе
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ни для одного объекта не были введены даты запрета, "
"поэтому для ""%1"" будет действовать более простая настройка ""%2"".'; en = 'No ban date has been entered for any object,"
"so for ""%1"" a simpler ""%2"" setting will work.'"),
				ТекущийПользователь, СпособУказанияВБазеДанных);
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстСообщенияВВыбранномРазделеДатыЗапретаДляОбъектовНеУстанавливаются(Раздел)
	...
	Возврат ?(Раздел <> РазделПустаяСсылка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В разделе ""%1"" не предусмотрена установка дат запрета для отдельных объектов.'; en = 'In section ""%1"" does not accommodate the dates of the ban for individual objects.'"), Раздел),
		НСтр("ru = 'Для установки дат запрета по отдельным объектам выберите один из разделов ниже и нажмите ""Подобрать"".'; en = 'To set date of ban on individual objects select one of sections below and click ""Select.""'"));
...
Функция ТекстЗаголовкаВыборТипаДанных()
	...
	Возврат НСтр("ru = 'Выбор типа данных'; en = 'Drivers'");
