...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого ТекущаяЗапись Из ЭтотОбъект Цикл
		...
		Если ВРег(ТекущаяЗапись.КатегорияНовостей.Код) = ВРег("ProductVersion")
			...
			// Версия ОТ.
			ТекстСообщения = НСтр("ru = 'Формат значения версии ОТ (%ВерсияОТ%) должен быть 99.99.999.9999 %ПодробныйТекстОшибки%.'; en = 'Format of FROM version value (%ВерсияОТ%) must be 99.99.999.9999 %ПодробныйТекстОшибки%.'");
			...
			// Версия ДО.
			ТекстСообщения = НСтр("ru = 'Формат значения версии ДО (%ВерсияДО%) должен быть 99.99.999.9999 %ПодробныйТекстОшибки%.'; en = 'Format of BEFORE version value (%ВерсияДО%) must be 99.99.999.9999 %ПодробныйТекстОшибки%.'");
...
Процедура ПроверитьВерсию(ВерсияСтрокой, ТекстСообщения, Отказ)
	...
	ВсеЦифры = ОбработкаНовостей.ВсеЦифры();
				...
				Если СтрДлина(МассивСловВерсии[СловоВерсии]) <> 2 Тогда
					...
					Сообщение.Текст = СтрЗаменить(
						...
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '(первое число версии %1 должно быть длиной 2 символа)'; en = '(first number of version %1 must be a length of 2 characters)'"),
							ВерсияСтрокой));
					...
					Для С=1 По СтрДлина(МассивСловВерсии[СловоВерсии]) Цикл
						...
						Если СтрНайти(ВсеЦифры, ТекущийСимвол) = 0 Тогда
							...
							Сообщение.Текст = СтрЗаменить(
								...
								"%ПодробныйТекстОшибки%",
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = '(первое число версии %1 должно содержать только цифры, а найден символ %2)'; en = '(first number of version %1 must contain only digits, and symbol %2 is found)'"),
									ВерсияСтрокой,
									ТекущийСимвол));
							Сообщение.УстановитьДанные(ЭтотОбъект);
							Сообщение.Сообщить();
			...
			ИначеЕсли СловоВерсии=1 Тогда // 2 цифры
				Если СтрДлина(МассивСловВерсии[СловоВерсии]) <> 2 Тогда
					...
					Сообщение.Текст = СтрЗаменить(
						...
						"%ПодробныйТекстОшибки%",
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '(второе число версии %1 должно быть длиной 2 символа)'; en = '(second number of version of %1 must be a length of 2 characters)'"),
							ВерсияСтрокой));
					Сообщение.УстановитьДанные(ЭтотОбъект);
					Сообщение.Сообщить();
					Отказ = Истина;
					...
					Для С=1 По СтрДлина(МассивСловВерсии[СловоВерсии]) Цикл
						...
						Если СтрНайти(ВсеЦифры, ТекущийСимвол) = 0 Тогда
							...
							Сообщение.Текст = СтрЗаменить(
								...
								"%ПодробныйТекстОшибки%",
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = '(второе число версии %1 должно содержать только цифры, а найден символ %2)'; en = '(second number of version %1 must contain only digits, and symbol %2 is found)'"),
									ВерсияСтрокой,
									ТекущийСимвол));
							Сообщение.УстановитьДанные(ЭтотОбъект);
							Сообщение.Сообщить();
			...
			ИначеЕсли СловоВерсии=2 Тогда // 3 цифры
				Если СтрДлина(МассивСловВерсии[СловоВерсии]) <> 3 Тогда
					...
					Сообщение.Текст = СтрЗаменить(
						...
						"%ПодробныйТекстОшибки%",
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '(третье число версии %1 должно быть длиной 3 символа)'; en = '(third number versions %1 must be of length 3 characters)'"),
							ВерсияСтрокой));
					Сообщение.УстановитьДанные(ЭтотОбъект);
					Сообщение.Сообщить();
					Отказ = Истина;
					...
					Для С=1 По СтрДлина(МассивСловВерсии[СловоВерсии]) Цикл
						...
						Если СтрНайти(ВсеЦифры, ТекущийСимвол) = 0 Тогда
							...
							Сообщение.Текст = СтрЗаменить(
								...
								"%ПодробныйТекстОшибки%",
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = '(третье число версии %1 должно содержать только цифры, а найден символ %2)'; en = '(third number of version %1 must contain only digits, and symbol %2 is found)'"),
									ВерсияСтрокой,
									ТекущийСимвол));
							Сообщение.УстановитьДанные(ЭтотОбъект);
							Сообщение.Сообщить();
			...
			ИначеЕсли СловоВерсии=3 Тогда // 4 цифры
				Если СтрДлина(МассивСловВерсии[СловоВерсии]) <> 4 Тогда
					...
					Сообщение.Текст = СтрЗаменить(
						...
						"%ПодробныйТекстОшибки%",
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '(четвертое число версии %1 должно быть длиной 4 символа)'; en = '(fourth number of version of %1 must be of length 4 characters)'"),
							ВерсияСтрокой));
					Сообщение.УстановитьДанные(ЭтотОбъект);
					Сообщение.Сообщить();
					Отказ = Истина;
					...
					Для С=1 По СтрДлина(МассивСловВерсии[СловоВерсии]) Цикл
						...
						Если СтрНайти(ВсеЦифры, ТекущийСимвол) = 0 Тогда
							...
							Сообщение.Текст = СтрЗаменить(
								...
								"%ПодробныйТекстОшибки%",
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = '(четвертое число версии %1 должно содержать только цифры, а найден символ %2)'; en = '(fourth version %1 must contain only numbers and not found symbol %2)'"),
									ВерсияСтрокой,
									ТекущийСимвол));
							Сообщение.УстановитьДанные(ЭтотОбъект);
							Сообщение.Сообщить();
	...
	Иначе
		...
		Сообщение.Текст = СтрЗаменить(
			...
			"%ПодробныйТекстОшибки%",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '(должно быть 4 числа версии, а не %1 как в %2)'; en = '(must be 4 numbers of version, and not %1 as in %2)'"),
