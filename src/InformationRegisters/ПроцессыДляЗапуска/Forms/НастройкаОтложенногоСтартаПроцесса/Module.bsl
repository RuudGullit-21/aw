...
Процедура ЗаписатьНастройкиНаКлиенте()
	...
	Если ОтложенныйСтартПроцесса <> ОтложенныйСтартПроцессаПриОткрытии Тогда 
		...
		ТекстОповещения = ?(ОтложенныйСтартПроцесса, НСтр("ru = 'Отложенный старт:'; en = 'Delayed start:'"), НСтр("ru = 'Отложенный старт отменен:'; en = 'Pending start cancelled:'"));
...
Процедура ОбновитьСписокВыбораВремени()
	...
	Для Инд = 1 По 48 Цикл
		Элементы.ДатаОтложенногоСтартаВремя.СписокВыбора.Добавить(ПустаяДата, Формат(ПустаяДата, НСтр("ru = 'ДФ=ЧЧ:мм'; en = 'ДФ=ЧЧ:мм'")));
...
Функция ТекстИнтервала(ДатаНачала, ДатаОкончания)
	...
	Если ДатаНачала > ДатаОкончания Тогда
		Возврат НСтр("ru = 'Дата запуска задания находится в прошлом.'; en = 'Start date of job is in the past.'");
	КонецЕсли;	
...
Функция ФормаЗаполненаКорректно()
	...
	Если ОтложенныйСтартПроцесса И ДатаОтложенногоСтарта < ТекущаяДатаСервера Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Дата и время отложенного старта должны быть больше текущей даты.'; en = 'Date and time of delayed start must be greater than current date.'"),,
			"ДатаОтложенногоСтарта");
		ЗаполненаКорректно = Ложь;
	КонецЕсли;
		
	Если ОтложенныйСтартПроцесса И ДатаОтложенногоСтарта > СрокИсполнения Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Дата и время отложенного старта должны быть меньше срока исполнения задания.'; en = 'Date and time of delayed start must be less than the execution time of the task.'"),,
			"ДатаОтложенногоСтарта");
		ЗаполненаКорректно = Ложь;
	КонецЕсли;
