...
Функция ДанныеКэшаВерсий(Знач Идентификатор, Знач ТипДанных, Знач ПараметрыПолучения, Знач ИспользоватьУстаревшиеДанные = Истина) Экспорт
	...
	Если ТребуетсяВыполнитьОбновление Тогда
		...
		Иначе
			...
			НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обновление кэша версий. Идентификатор записи %1. Тип данных %2.'; en = 'Updating version cache. Identifier of record %1. Data type %2.'"),
				Идентификатор,
				ТипДанных);
...
"Тип данных: %2'; en = 'An error occurred when updating the version cache."
"Data is not received. Record identifier: %1"
"Data type: %2'");
...
Функция ПодготовитьДанныеКэшаВерсий(Знач ТипДанных, Знач ПараметрыПолучения) Экспорт
	...
	Иначе
		ШаблонТекста = НСтр("ru = 'Неизвестный тип данных кэша версий: %1'; en = 'Unknown data type of versions cache: %1'");
...
Функция WSОпределения(Знач АдресWSDL, Знач ИмяПользователя, Знач Пароль, Знач Таймаут = 10, Знач ЗащищенноеСоединение = Неопределено)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Получение WSDL'; en = 'Receive WSDL'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ПолучитьWSDL(Знач Адрес, Знач ИмяПользователя, Знач Пароль, Знач Таймаут, Знач ЗащищенноеСоединение = Неопределено)
	...
	Иначе
		ВызватьИсключение 
			НСтр("ru = 'Подсистема ""Получение файлов из интернета"" не доступна.'; en = 'Subsystem ""Receive files from the Internet"" is not available.'");
	КонецЕсли;
	...
	Исключение
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1
			           ...
			           |Таймаут: %3'"),
			ТекстОшибки,
			Формат(ЗащищенноеСоединение, НСтр("ru = 'БЛ=Нет; БИ=Да'; en = 'BF=No; BT=Yes'")),
			Формат(Таймаут, "ЧГ=0"));
