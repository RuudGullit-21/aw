...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления % (60*24*7) = 0 Тогда // 1 раз в неделю
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ru = 'Список новостей обновляется еженедельно'; en = 'News list is updated weekly'");
	ИначеЕсли ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления % (60*24) = 0 Тогда // 1 раз в день
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ru = 'Список новостей обновляется ежедневно'; en = 'News list is updated daily'");
	ИначеЕсли ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления % 60 = 0 Тогда // 1 раз в час
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ru = 'Список новостей обновляется ежечасно'; en = 'News list is updated hourly'");
	Иначе
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Список новостей обновляется каждые %1 минут'; en = 'News list is updated every %1 minutes'"),
	...
	Если ЭтотОбъект.ТекущийПользователь <> Пользователи.ТекущийПользователь() Тогда
		ЭтотОбъект.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Настройки отображения новостей (%1)'; en = 'The display of information settings (%1)'"),
	...
	Если ТипЗнч(ПараметрыФормы) = ТипСтруктура Тогда
		...
		Иначе
			ПоказатьПредупреждение(
				...
				НСтр("ru = 'Вначале включите получение новостей по этой ленте.'; en = 'Enable news receipt from this new feed first.'"),
				0,
	...
	Для каждого ТекущаяСтрока Из ЭтотОбъект.ЛентыНовостей Цикл
		Если (ТекущаяСтрока.ЛентаНовостей = лкЛентаНовостей) ИЛИ (лкЛентаНовостей = Неопределено) Тогда
			...
			Если (ТекущаяСтрока.КоличествоВсехОтборов = 0) Тогда
				...
				ПредставлениеВсехОтборовПодробно = НСтр("ru = 'По этой ленте отборы не настраиваются'; en = 'Filters are not set for this feed'");
			Иначе
				ПредставлениеВсехОтборовКратко               = "";
				...
				ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно
					...
					+ ?(ПустаяСтрока(ПредставлениеПользовательскихОтборов),
							НСтр("ru = 'Нет отборов, разрешенных для настройки пользователем'; en = 'No selections allowed for the user settings'")
								+ Символы.ПС,
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Отборы, настроенные пользователем (можно изменить):"
"%1'; en = 'Selections that configured by user (can be changed):"
"%1'")
									+ Символы.ПС,
								ПредставлениеПользовательскихОтборов));
...
"%1'; en = 'Selections configured by administrator (cannot be changed):"
"%1'")
									+ Символы.ПС,
								ПредставлениеАдминистративныхОтборов));
...
"%1'; en = 'Automatically filled selections (do not change):"
"%1'")
									+ Символы.ПС,
								ПредставлениеАвтоматическиЗаполняемыхОтборов));

				Если ПустаяСтрока(ПредставлениеВсехОтборовПодробно) Тогда
					ПредставлениеВсехОтборовКратко   = НСтр("ru = 'Отборы не настроены'; en = 'Filters are not set'");
					ПредставлениеВсехОтборовПодробно = НСтр("ru = 'Отборы не настроены'; en = 'Filters are not set'");
				Иначе
					ПредставлениеВсехОтборовКратко   = НСтр("ru = 'Настроен детальный отбор'; en = 'Detailed filter is set'");
				КонецЕсли;
...
"%1'; en = 'For this feed, you can not disable receiving news (as configured by administrator)"
"%1'")
							+ Символы.ПС,
						ПредставлениеВсехОтборовПодробно);
			КонецЕсли;
