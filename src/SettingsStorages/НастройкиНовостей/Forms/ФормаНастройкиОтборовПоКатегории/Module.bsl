...
"Настройка отбора невозможна.'; en = 'The News feed value is not filled in."
...
"Настройка отбора невозможна.'; en = 'The News category value is required."
"Cannot set up filter.'");
	...
	Если ЭтотОбъект.ТекущийПользователь <> Пользователи.ТекущийПользователь() Тогда
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Настройка отборов для ленты новостей по категории (%1)'; en = 'Setting selections for news feeds by category (%1)'"),
