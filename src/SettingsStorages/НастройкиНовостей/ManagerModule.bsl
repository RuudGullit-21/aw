...
Процедура ОбработкаСохранения(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Исключение
				...
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru = 'БИП:Новости.Хранилище настроек новостей'; en = 'ISL:News.News setting storage'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
					НСтр("ru = 'Новости. Хранилище настроек новостей. ОбработкаСохранения. Ошибка'; en = 'News. Settingstorenews. ProcessingSaving. Error'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
	...
	ИначеЕсли КлючОбъекта = "ОтборыПоЛентамНовостейНастроенныеАдминистратором" Тогда
		...
		Иначе
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				...
				, // ОбъектМетаданных
				, // Данные
				НСтр("ru = 'Ошибка при проверке набора записей регистра ОтборыПоЛентамНовостейОбщие (ключ объекта ОтборыПоЛентамНовостейНастроенныеАдминистратором) - возможно, не заполнены какие-то поля.'; en = 'An error occurred while checking a record set of register ОтборыПоЛентамНовостейОбщие (object key ОтборыПоЛентамНовостейНастроенныеАдминистратором) - some fields may not be filled in.'"), // Комментарий
				ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		КонецЕсли;
		...
		Если ЗаписыватьДанные = Истина Тогда
			...
			Иначе
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					...
					, // ОбъектМетаданных
					, // Данные
					НСтр("ru = 'Ошибка при проверке набора записей регистра ОтборыПоЛентамНовостейДляОбластиДанных (ключ объекта ОтборыПоЛентамНовостейДляОбластиДанных) - возможно, не заполнены какие-то поля.'; en = 'Error in the validation of register record set ОтборыПоЛентамНовостейДляОбластиДанных (object key ОтборыПоЛентамНовостейДляОбластиданных) - may not be filled in any fields.'"), // Комментарий
					ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли КлючОбъекта = "ОтборыПоЛентамНовостейПользовательские" Тогда
		...
		Иначе
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				...
				, // ОбъектМетаданных
				, // Данные
				НСтр("ru = 'Ошибка при проверке набора записей регистра ОтборыПоЛентамНовостейПользовательские (ключ объекта ОтборыПоЛентамНовостейПользовательские) - возможно, не заполнены какие-то поля.'; en = 'An error occurred while checking a record set of register ОтборыПоЛентамНовостейПользовательские (object key ОтборыПоЛентамНовостейПользовательские) - some fields may not be filled in.'"), // Комментарий
				ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		КонецЕсли;
	...
	ИначеЕсли КлючОбъекта = "ОтключенныеЛентыНовостей" Тогда
		...
		Иначе
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				...
				, // ОбъектМетаданных
				, // Данные
				НСтр("ru = 'Ошибка при проверке набора записей регистра ОтключенныеЛентыНовостей (ключ объекта ОтключенныеЛентыНовостей) - возможно, не заполнены какие-то поля.'; en = 'An error occurred while checking a record set of register ОтключенныеЛентыНовостей (object key ОтключенныеЛентыНовостей) - some fields may not be filled in.'"), // Комментарий
				ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		КонецЕсли;
	...
	ИначеЕсли КлючОбъекта = "ДоступныеЛентыНовостей" Тогда
		...
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			, // ОбъектМетаданных
			, // Данные
			НСтр("ru = 'Ошибка при записи настроек новостей. Настройки для ключа ДоступныеЛентыНовостей не должны записываться. Записывать можно только ОтключенныеЛентыНовостей.'; en = 'An error occurred while writing news settings. Settings for key ДоступныеЛентыНовостей should not be written. You can write only ОтключенныеЛентыНовостей.'"), // Комментарий
			ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
