...
"но она не записана."
""
"Записать?'; en = 'Entered setting name (%1),"
			...
			Если ЭтотОбъект.ТекущийБуферОбмена.Количество() > 0 Тогда
				Если НЕ ПустаяСтрока(ЭтотОбъект.КлючНастройки) Тогда
					Если ЭтотОбъект.СписокКлючейНастроек.НайтиПоЗначению(ЭтотОбъект.КлючНастройки) = Неопределено Тогда
						...
						СписокКнопок.Добавить("ЗаписатьИЗакрыть", НСтр("ru = 'Записать и закрыть'; en = 'Write and close'"));
						СписокКнопок.Добавить("Закрыть", НСтр("ru = 'Закрыть без записи'; en = 'Close without recording'"));
						СписокКнопок.Добавить("Отмена", НСтр("ru = 'Не закрывать'; en = 'Keep open'"));
						ПоказатьВопрос(ОписаниеОповещенияОЗакрытии, ТекстВопроса, СписокКнопок, 0, "ЗаписатьИЗакрыть", НСтр("ru = 'Текущее значение буфера обмена не сохранено'; en = 'Current value of clipboard not saved'"));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура КомандаВыгрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = НСтр("ru = 'Выберите файл, куда записать список буферов обмена'; en = 'Select the file where to write the list of clipboards'");
...
Процедура КомандаЗагрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = НСтр("ru = 'Выберите файл с сохраненными ранее буферами обмена'; en = 'Select the file with the earlier saved clipboards'");
	Диалог.МножественныйВыбор = Ложь;
	Диалог.Расширение = "xml";
	Диалог.Фильтр = НСтр("ru = 'Файл xml (*.xml)|*.xml'; en = 'File xml (*.xml)|*.xml'");
...
"Вначале скопируйте в него строки данных.'; en = 'Empty the clipboard."
"Not allowed to save an empty clipboard."
"First copy the rows of data to it.'");
...
Функция ПолучитьОписаниеВажности(Важность)
	...
	Если Важность = 1 Тогда
		Возврат НСтр("ru = 'Очень важная'; en = 'Very important'");
	ИначеЕсли Важность = 2 Тогда
		Возврат НСтр("ru = 'Важная'; en = 'Important'");
	Иначе
	...
	ЗаписьХМЛ.ОткрытьФайл(
		...
		ЗаписьХМЛ.ЗаписатьКомментарий(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Выгрузка данных буферов обмена для %1, %2'; en = 'Uploading data of clipboards for %1, %2'"),
				ЭтотОбъект.КлючОбъекта,
...
Процедура ПослеВыгрузкиСпискаБуферовОбмена(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ТипЗнч(ВыбранныеФайлы) = ТипМассив)
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Данные выгружены.'; en = 'Data uploaded.'"),
...
Процедура ПослеЗагрузкиСпискаБуферовОбмена(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ТипЗнч(ВыбранныеФайлы) = ТипМассив)
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Данные загружены.'; en = 'Data loaded.'"),
			,
			,
