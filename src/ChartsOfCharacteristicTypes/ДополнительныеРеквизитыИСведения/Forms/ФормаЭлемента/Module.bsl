...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ПараметрыЗаписи.Свойство("ПодтвержденоИзменениеНаименования") Тогда
		Если ЗначениеЗаполнено(ТекущийЗаголовок) И ТекущийЗаголовок <> Объект.Заголовок Тогда
			...
			Кнопки.Добавить("ПродолжитьЗапись",            НСтр("ru = 'Переименовать'; en = 'Rename'"));
			Кнопки.Добавить("ВернутьНаименование", НСтр("ru = 'Отмена'; en = 'Cancel'"));
	...
	Если НЕ ПараметрыЗаписи.Свойство("КогдаНаименованиеУжеИспользуется") Тогда
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			...
			Кнопки.Добавить("ВернутьсяКВводуНаименования", НСтр("ru = 'Вернуться к вводу наименования'; en = 'Return to name input'"));
	...
	Если НЕ ПараметрыЗаписи.Свойство("КогдаИмяУжеИспользуется")
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("ПродолжитьЗапись",            НСтр("ru = 'Продолжить'; en = 'Continue'"));
			Кнопки.Добавить("ВернутьсяКВводуИмени", НСтр("ru = 'Отмена'; en = 'Cancel'"));
	...
	Если НЕ ПараметрыЗаписи.Свойство("КогдаИдентификаторДляФормулУжеИспользуется")
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("ПродолжитьЗапись",              НСтр("ru = 'Продолжить'; en = 'Continue'"));
			Кнопки.Добавить("ВернутьсяКВводуИдентификатора", НСтр("ru = 'Отмена'; en = 'Cancel'"));
...
""
"Данные будут записаны.'; en = 'Clear the entered weight coefficients?"
""
"Data will be written.'");
	...
	Иначе
		ТекстВопроса = НСтр("ru = 'Данные будут записаны.'; en = 'Data will be written.'");
...
#КонецЕсли
	...
	Если ИндексСтраницы = 0
		...
		ТекстПредупреждения = НСтр("ru = 'Элемент не выбран.'; en = 'Element is not selected.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура РедактироватьФорматЗначенияЗавершение(Текст, Контекст) Экспорт
	...
	Если Текст <> Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Следующие настройки формата автоматически не применяются в большинстве мест:'; en = 'Following format settings are not automatically applied in most places:'");
		...
		Для каждого Подстрока Из Массив Цикл
			Если СтрНайти(Подстрока, "ДП=") > 0 ИЛИ СтрНайти(Подстрока, "DE=") > 0 Тогда // @Non-NLS
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ " - " + НСтр("ru = 'представление пустой даты'; en = 'presentation of emty date'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ЧН=") > 0 ИЛИ СтрНайти(Подстрока, "NZ=") > 0 Тогда // @Non-NLS
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ " - " + НСтр("ru = 'представление пустого числа'; en = 'representation empty number'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДФ=") > 0 ИЛИ СтрНайти(Подстрока, "DF=") > 0 Тогда // @Non-NLS
				Если СтрНайти(Подстрока, "ддд") > 0 ИЛИ СтрНайти(Подстрока, "ddd") > 0 Тогда // @Non-NLS
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru = 'кратное название дня недели'; en = 'Multiple name of day of week'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "дддд") > 0 ИЛИ СтрНайти(Подстрока, "dddd") > 0 Тогда // @Non-NLS
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru = 'полное название дня недели'; en = 'full name of day of week'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "МММ") > 0 ИЛИ СтрНайти(Подстрока, "MMM") > 0 Тогда // @Non-NLS
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru = 'кратное название месяца'; en = 'fold month name'");
				КонецЕсли;
...
Процедура УстановитьНастройкиПомощника(ТекущаяСтраница = Неопределено)
	...
	ШаблонЗаголовкаСписка        = НСтр("ru = 'Выберите %1 для включения в набор ""%2""'; en = 'Select %1 to include in set ""%2""'");
	ШаблонЗаголовкаПереключателя = НСтр("ru = 'Выберите вариант добавления дополнительного %1 ""%2"" в набор ""%3""'; en = 'Select the option to add additional %1 ""%2"" into set ""%3""'");
	
	Если ТекущаяСтраница = Элементы.ВыборРеквизита Тогда
		
		Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
			Заголовок = НСтр("ru = 'Добавление дополнительного сведения'; en = 'Add additional information'");
		Иначе
		...
		Элементы.ДекорацияЗаголовок.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ru = 'дополнительное сведение'; en = 'additional information'"), НСтр("ru = 'дополнительный реквизит'; en = 'additional attribute'")),
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборДействия Тогда
		...
		Элементы.РежимДобавленияРеквизита.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ru = 'сведения'; en = 'information'"), НСтр("ru = 'реквизита'; en = 'attribute'")),
...
"""%2""."
""
"Второй тип удален.'; en = 'Cannot use value types"
"""%1"" and"
"""%2"" simultaneously."
""
...
"""%2""."
""
"Первый тип удален.'; en = 'Cannot use value types"
"""%1"" and"
"""%2"" simultaneously."
""
"The first type was deleted.'"),
	...
	Иначе
		...
		Иначе
			ШаблонУточнения = НСтр("ru = 'Список значений общий со сведением ""%1""'; en = 'List of values general with data ""%1""'");
		КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли СписокНаборов.Количество() < 2 Тогда
			Если Объект.ЭтоДополнительноеСведение Тогда
				ШаблонУточнения = НСтр("ru = 'Сведение входит в набор: %1'; en = 'Data is included in set: %1'");
			Иначе
				ШаблонУточнения = НСтр("ru = 'Реквизит входит в набор: %1'; en = 'The attribute belongs to the set: %1'");
			КонецЕсли;
		...
		Иначе
			...
			СтрокаНаборы = ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(СписокНаборов.Количество(),
				"", НСтр("ru = 'набор,набора,наборов,,,,,,0'; en = 'set, set, sets,,,,,,0'"));
...
""
"Будет создана копия сведения%1'; en = 'Back up data "
""
...
""
"Будет создана копия реквизита%1'; en = 'Back up attribute"
""
...
Процедура УстановитьЗаголовокФормы()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Если Объект.ЭтоДополнительноеСведение Тогда
			Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru = '(Дополнительное сведение)'; en = '(Additional info)'");
		Иначе
			Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru = '(Дополнительный реквизит)'; en = '(Additional attribute)'");
		КонецЕсли;
	Иначе
		Если Объект.ЭтоДополнительноеСведение Тогда
			Заголовок = НСтр("ru = 'Дополнительное сведение (создание)'; en = 'Additional information (creation)'");
		Иначе
			Заголовок = НСтр("ru = 'Дополнительный реквизит (создание)'; en = 'Additional attribute (creation)'");
		КонецЕсли;
	КонецЕсли;
...
"""%1"".'; en = 'Additional information exists with name"
"""%1"".'");
	Иначе
		ТекстВопроса = НСтр("ru = 'Существует дополнительный реквизит с наименованием"
"""%1"".'; en = 'An additional attribute with name"
"""%1"" already exists.'");
	КонецЕсли;
...
"иначе программа может работать некорректно.'; en = 'We recommend that you use another name;"
