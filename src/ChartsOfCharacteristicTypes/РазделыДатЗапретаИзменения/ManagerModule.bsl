...
Функция СвойстваРазделовДатЗапрета() Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		Если Не ЗначениеЗаполнено(Раздел.Имя) Тогда
			ВызватьИсключение ЗаголовокОшибки + НСтр("ru = 'Имя раздела дат запрета не заполнено.'; en = 'Name section deny date is not filled.'");
		КонецЕсли;
		...
		Если РазделыДатЗапрета.Получить(Раздел.Имя) <> Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Имя раздела дат запрета ""%1"" уже определено.'; en = 'Section name of the ban dates ""%1"" already defined.'"),
				Раздел.Имя);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Раздел.Идентификатор) И Раздел.Имя <> "ОбщаяДата" Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Идентификатор раздела дат запрета ""%1"" не заполнен.'; en = 'ID of the section of ban date ""%1"" is not filled.'"),
				Раздел.Имя);
		КонецЕсли;
...
"""%2"" уже используется для раздела ""%3"".'; en = 'ID ""%1"" section of ban date"
"""%2"" is already used for section ""%3"".'"),
...
"Однако это не тип ссылки.'; en = 'Type ""%1"" is specified as the object type for ""%2"" ban date section."
"However, this is not link type.'"),
...
"не указан в свойстве ""Тип"" плана видов характеристик ""Разделы дат запрета изменения"".'; en = 'Objects type ""%1"" of the ban date section ""%2"""
"is not specified in the ""Type"" property of the characteristics chart ""Change ban date sections"".'"),
					Строка(Тип), Раздел.Имя);
			КонецЕсли;
...
Функция СвойстваРаздела(Ссылка, Знач Имя = "", Знач Представление = "", Знач ТипыОбъектов = Неопределено) Экспорт
	...
	СвойстваРаздела.Вставить("Представление", ?(ПустаяСтрока(Представление), НСтр("ru = 'Общая дата'; en = 'Common date'"), Представление));
