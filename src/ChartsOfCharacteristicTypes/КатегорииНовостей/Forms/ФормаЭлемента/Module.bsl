		...
		Для каждого КлючЗначение Из СтруктураДействий Цикл
			...
			ИначеЕсли КлючЗначение.Ключ = "ОбновляетсяССервера" Тогда
				...
				Если Объект.ОбновляетсяССервера = Истина Тогда
					...
					Элементы.ЗначенияКатегорийНовостейСправочник.Заголовок = НСтр("ru = 'Список доступных значений (обновляется с сервера, редактирование запрещено)'; en = 'List of available values (updated from server, editing is prohibited)'");
				Иначе
					...
					Элементы.ЗначенияКатегорийНовостейСправочник.Заголовок = НСтр("ru = 'Список доступных значений'; en = 'Available value list'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция УправлениеФормойСервер(Знач СтруктураПараметров)
	...
	Если СтруктураПараметров.ЗаполняетсяАвтоматически = Истина Тогда
		...
		Если ПустаяСтрока(Значение) Тогда
			СтруктураДействий.Вставить("ФиксированноеЗначение", НСтр("ru = 'Невозможно определить значение для этой категории'; en = 'Cannot determine value for this category'"));
		...
		Иначе
			СтруктураДействий.Вставить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для этой конфигурации значение этой категории на текущий момент равно %1'; en = 'For this configuration, the value of this category is currently equal to %1'"),
					Значение));
		КонецЕсли;
	Иначе
		...
		Если Объект.ОбновляетсяССервера Тогда
			...
			Если Запись.Выбран() Тогда
				Если Запись.ТекущаяВерсияНаКлиенте >= Запись.ТекущаяВерсияНаСервере Тогда
					ТекстНадписи = НСтр("ru = 'Данные актуальны и соответствуют данным с сервера от %ТекущаяВерсияНаСервере%.'; en = 'Data is relevant and corresponds to the data from server from %ТекущаяВерсияНаСервере%.'");
...
"а на сервере уже версия от %ТекущаяВерсияНаСервере%.'; en = 'Last time the data was updated from the server was %ТекущаяВерсияНаКлиенте% and"
"on the server the version is from %ТекущаяВерсияНаСервере%.'");
					КонецЕсли;
		...
		Иначе
			Элементы.ДекорацияТребуетсяОбновлениеССервера.Заголовок  = НСтр("ru = 'Данные вводятся вручную и обновление с сервера не требуется.'; en = 'Data is entered manually and the update from server is not required.'");
			Элементы.ДекорацияТребуетсяОбновлениеССервера.ЦветТекста = ЦветаСтиля.ЦветМикротекста;
		КонецЕсли;
