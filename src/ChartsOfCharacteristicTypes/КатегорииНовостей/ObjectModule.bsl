...
"Разрешено использовать цифры, английские буквы, подчеркивание и минус.'; en = 'The identifier contains forbidden characters: %1."
"It is allowed to use numbers, English letters, underscore and minus.'"),
	...
	Если ТипЗначения.Типы().Количество() > 1 Тогда
		...
		Сообщение.ПутьКДанным = "Объект";
		Сообщение.Текст = НСтр("ru = 'Составные типы не поддерживаются. Выберите только один тип значения.'; en = 'Composite types are not supported. Select only one value type.'");
		Сообщение.Сообщить();
	КонецЕсли;
	...
	Если ТипЗначения.Типы().Количество() = 0 Тогда
		...
		Сообщение.Поле = "ТипЗначения";
		Сообщение.ПутьКДанным = "Объект";
		Сообщение.Текст = НСтр("ru = 'Пустые типы не поддерживаются. Выберите один любой тип значения.'; en = 'Empty types are not supported. Select a value type.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЗагруженоССервера = Истина Тогда
		...
		Если (ПометкаУдаления = Истина)
			...
			ТекстСообщения = НСтр("ru = 'Нельзя помечать на удаление категории, загруженные автоматически с сервера новостей.'; en = 'Cannot mark categories for deletion that were downloaded from the news server automatically.'");
			Сообщение = Новый СообщениеПользователю();
