...
Процедура ЗавершитьВывод(ДокументРезультат, ДанныеРасшифровки, НастройкиПрав)
	...
	ГруппаДоступаЗаголовок = НСтр("ru = 'Группа доступа'; en = 'Access group'");
	Если УправлениеДоступомСлужебный.УпрощенныйИнтерфейсНастройкиПравДоступа() Тогда
...
Функция ПраваПользователей()
	...
	ИначеЕсли ВариантСОграничениямиДоступа Тогда
		...
		Запрос.УстановитьПараметр("ТекстБезОграничения", "<" + НСтр("ru = 'Без ограничения'; en = 'Without restriction'") + ">");
		Запрос.УстановитьПараметр("ТекстВсеРазрешены", "<" + НСтр("ru = 'Все разрешены'; en = 'All allowed'") + ">");
		Запрос.УстановитьПараметр("ТекстВсеЗапрещены", "<" + НСтр("ru = 'Все запрещены'; en = 'All prohibited'") + ">");
...
Функция НастройкиПравПоОбъектам()
	...
	Запрос.УстановитьПараметр("ПредставлениеНеопределено", НСтр("ru = 'Неопределено'; en = 'Undefined'"));
	Запрос.УстановитьПараметр("ПредставлениеГруппыПользователей",
		" (" + НСтр("ru = 'Группа пользователей'; en = 'Users group'") + ")");
	Запрос.УстановитьПараметр("ПредставлениеГруппыВнешнихПользователей",
		" (" + НСтр("ru = 'Группа внешних пользователей'; en = 'External user group'") + ")");
...
Функция ОписаниеКолонкиДляПодпапок()
	...
	Результат.Вставить("Подсказка",
		НСтр("ru = 'Права не только для текущей папки, но и для ее нижестоящих папок'; en = 'Rights not only for the current folder, but also for its lower folders'"));
...
Функция НастройкиПравЛегенда(ЗаголовкиПрав, ЕстьИерархия)
	...
	Если ЕстьИерархия Тогда
		...
		НоваяСтрока.ПодсказкаПрава = НСтр("ru = 'Наследование прав от вышестоящих папок'; en = 'Inheriting rights from parent folders'");
	КонецЕсли;
