...
Функция ДатыЗапретаПодготовленные(ПараметрыДанных)
	...
	Для Каждого Строка Из Таблица Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(Строка.Раздел) Тогда
			Строка.ОбъектПредставление = НСтр("ru = 'Для всех объектов, кроме указанных'; en = 'For all objects except'");
		Иначе
			Строка.ОбъектПредставление = НСтр("ru = 'Для всех разделов и объектов, кроме указанных'; en = 'For all sections and objects, except specified'");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Строка.Раздел) Тогда
			Строка.РазделПредставление = Строка(Строка.Раздел);
		Иначе
			Строка.РазделПредставление = "<" + НСтр("ru = 'Общая дата'; en = 'Common date'") + ">";
		КонецЕсли;
		
		Если Строка.АдресатНастройки = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехИнформационныхБаз Тогда
			Строка.АдресатНастройкиПредставление = НСтр("ru = 'Для всех информационных баз, кроме указанных'; en = 'For all databases except specified'");
		Иначе
			Строка.АдресатНастройкиПредставление = Строка(ТипЗнч(Строка.АдресатНастройки)) + ":"
				+ ?(ЗначениеЗаполнено(Строка.АдресатНастройки),
					...
					НСтр("ru = 'Все информационные базы'; en = 'All information bases'"));
		КонецЕсли;
		
		Если Строка.ВладелецНастройки = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехИнформационныхБаз Тогда
			Строка.ВладелецНастройкиПредставление = НСтр("ru = 'Для всех информационных баз, кроме указанных'; en = 'For all databases except specified'");
		Иначе
