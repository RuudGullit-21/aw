...
Процедура СформироватьОтчет(Команда)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаПоиска) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Поле ""ИНН или наименование контрагента"" не заполнено'; en = 'The ""TIN or counterparty name"" field is not populated'"), , "СтрокаПоиска");
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьВСправочник(Команда)
	...
	Иначе
		СправочникиКонтрагентыСписок.ПоказатьВыборЭлемента(
			...
			НСтр("ru = 'Выбор справочника контрагентов'; en = 'Selection of counterpary catolog'"));
	КонецЕсли;
...
Функция СформироватьОтчетНаСервере(ЭтоПроверкаРезультата = Ложь)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Формирование отчета: Досье контрагента: %1'; en = 'Generate report: Counterparty profile: %1'"),
		ИННПоиска);
		...
		Если ДанныеПроверки.Свойство("РезультатПроверки") Тогда
			...
			Элементы.РезультатПроверки.ИспользуемоеИмяФайла = СокрЛП(
				СтроковыеФункции.СтрокаЛатиницей(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Досье %1 - проверки'; en = 'Profile %1 - check'"),
...
"Подключить Интернет-поддержку?'; en = 'To form a ""Counterparty''s file"""
...
"необходимо подключить Интернет-поддержку пользователей."
"Обратитесь к администратору.'; en = 'To form a ""Counterparty''s file"""
"it is necessary to connect users Internet support."
