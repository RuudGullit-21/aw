...
Процедура СформироватьРезультатГлавное(ИнформацияОСвязях, ИНН, ДанныеОтчета, ДанныеДосье, СтруктураГодов)
	...
	Если ИнформацияОСвязях.Статус.Код <> Неопределено Тогда
		Если ИнформацияОСвязях.Статус.Код = "001" Тогда
			ОбластиМакета.НаименованиеИАдрес.Параметры.ДействующаяОрганизация = НСтр("ru = 'Действующая организация (по данным ЕГРЮЛ)'; en = 'Valid company (by USRLE data)'");
		...
		Иначе
			ОбластиМакета.НаименованиеИАдрес.Параметры.ДействующаяОрганизация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 (по данным ЕГРЮЛ)'; en = '%1 (by USRLE data)'"),
		...
		Для Итератор = 0 По ВГраница Цикл
			...
			Если КоличествоОрганизацийГдеРуководитель > 0 Тогда
				ТекстРуководитель = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Руководитель еще в %1'; en = 'Manager also in %1'"),
					СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
			...
			Если КоличествоОрганизацийГдеУчредитель > 0 Тогда
				...
				Если ВыводитьЕще Тогда
					СтрокаПодстановки = ?(ПустаяСтрока(ТекстРуководитель), 
						НСтр("ru = 'Учредитель еще в %1'; en = 'Also founder in %1'"), 
						НСтр("ru = ', учредитель еще в %1'; en = ', also founder in %1'"));
				Иначе
					СтрокаПодстановки = ?(ПустаяСтрока(ТекстРуководитель), 
						НСтр("ru = 'Учредитель в %1'; en = 'Founder in %1'"), 
						НСтр("ru = ', учредитель в %1'; en = ', founder in %1'"));
				КонецЕсли;
			...
			Если Не ПустаяСтрока(ИнформацияОРуководителе) Тогда
				ИнформацияОРуководителе = ИнформацияОРуководителе
					+ ?(ТекущийРуководитель.ОрганизацииГдеРуководительПоФИО ИЛИ ТекущийРуководитель.ОрганизацииГдеУчредительПоФИО,
						" (" + НСтр("ru = 'поиск проведен по ФИО и может быть неточным'; en = 'search was performed by full name and may be inaccurate'") + ")",
						"");
			КонецЕсли;
	...
	ДанныеДосье.Вставить("ИмяФайлаГлавное",
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Досье %1 - главное'; en = 'Profile %1 - main'"),
...
Процедура ВывестиПоказателиВГлавное(ТабличныйДокумент, ИнформацияОСвязях, СтруктураГодов, ОбластиМакета)
	...
	Если ОткрытыеДанныеЗаГод <> Неопределено Тогда
		...
		ВывестиСтрокуПоказателя(
			...
			ВыведенаСтрока,
			Новый Структура("НазваниеПоказателя, ЗначениеПоказателя, ПредыдущееЗначениеПоказателя",
				НСтр("ru = 'Система налогообложения'; en = 'Taxation system'"),
	...
	Иначе
		...
		Иначе
			...
			Если ВсегоПроверок = 1 Тогда
				ТекстНарушения = НСтр("ru = 'выявлены нарушения'; en = 'violations are identified'");
			ИначеЕсли ВсегоПроверок = ПроверокСНарушениями Тогда
				ТекстНарушения = НСтр("ru = 'все проверки выявили нарушения'; en = 'all checks detected violations'");
			Иначе
				ШаблонТекста = "";
			...
			КонецЕсли;
			РезультатыПроверок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Известны результаты %1, %2'; en = 'Known results %1, %2'"), ТекстПроверки, ТекстНарушения);
		КонецЕсли;
...
Процедура СформироватьРезультатДанныеГосРеестров(ИнформацияОСвязях, ДанныеОтчета, ДанныеДосье)
	...
	ДанныеДосье.Вставить("ИмяФайлаДанныеГосРеестров",
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Досье %1 - данные реестра'; en = 'Profile %1 - data registry'"),
				ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
					ИнформацияОСвязях.НаименованиеСокращенное, ""))));
...
Процедура ВывестиРазделРегистрация(ТабличныйДокумент, ИнформацияОСвязях, ПараметрыВывода)
	...
	Если ИнформацияОСвязях.Статус.Код <> Неопределено Тогда
		...
		Если ИнформацияОСвязях.Статус.Код = "001" Тогда
			Если ЭтоЮридическоеЛицо Тогда
				ОбластиМакета.ДействующаяОрганизация.Параметры.ДействующаяОрганизация =
					НСтр("ru = 'Действующая организация (по данным ЕГРЮЛ)'; en = 'Valid company (by USRLE data)'");
			Иначе
				ОбластиМакета.ДействующаяОрганизация.Параметры.ДействующаяОрганизация =
					НСтр("ru = 'Действующий ИП (по данным ЕГРИП)'; en = 'Valid IE (by USRLE data)'");
			КонецЕсли;
		...
		Иначе
			...
			Иначе
				ОбластиМакета.НедействующаяОрганизация.Параметры.НедействующаяОрганизация =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1 (по данным ЕГРИП)'; en = '%1 (by USRIE data)'"),
						ИнформацияОСвязях.Статус.Наименование);
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИнформацияОСвязях.СостояниеПоДаннымЕГРН) Тогда
		...
		Если ИнформацияОСвязях.СостояниеПоДаннымЕГРН = 
			...
			Если ЭтоЮридическоеЛицо Тогда
				ОбластиМакета.ДействующаяОрганизация.Параметры.ДействующаяОрганизация =
					НСтр("ru = 'Организация есть в базе ЕГРН'; en = 'Company already exists in the USRT base'");
			Иначе
				ОбластиМакета.ДействующаяОрганизация.Параметры.ДействующаяОрганизация =
					НСтр("ru = 'ИП есть в базе ЕГРН'; en = 'This IE is already in the USRT base'");
			КонецЕсли;
			ТабличныйДокумент.Вывести(ОбластиМакета.ДействующаяОрганизация);
		Иначе
			Если НЕ ЭтоЮридическоеЛицо 
				...
				ОбластиМакета.НедействующаяОрганизация.Параметры.НедействующаяОрганизация = 
					НСтр("ru = 'Не действует (по данным ЕГРН)'; en = 'Not valid (according to USRT)'");
			Иначе
				ОбластиМакета.НедействующаяОрганизация.Параметры.НедействующаяОрганизация =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1 (по данным ЕГРН)'; en = '%1 (according to USRT data)'"),
...
Процедура ВывестиРазделРуководители(ТабличныйДокумент, ИнформацияОСвязях, ПараметрыВывода)
	...
	Для Итератор = 0 По ВГраница Цикл
		...
		Если КоличествоОрганизаций > 0 Тогда
			...
			СколькоЕщеОрганизаций = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				?(ЭтоЮридическоеЛицо, 
					НСтр("ru = 'Является руководителем еще в %1%2:'; en = 'This is a manager also in %1%2:'"), 
					НСтр("ru = 'Является руководителем в %1%2:'; en = 'This is a manager in %1%2:'")),
				СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
						НСтр("ru = '%1 организациях;%1 организации;%1 организациях;%1 организациях;%1 организациях;%1 организациях'"),
						КоличествоОрганизаций),
		...
		Если КоличествоОрганизаций > 0 Тогда
			...
			СколькоЕщеОрганизаций = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				?(ВыводитьЕще, 
					НСтр("ru = 'Является учредителем еще в %1%2:'; en = 'This is a founder also in %1%2:'"), 
					НСтр("ru = 'Является учредителем в %1%2:'; en = 'This is a founder in %1%2:'")),
				СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
...
Процедура СформироватьРезультатДанныеПрограммы(ИнформацияОСвязях, ДанныеОтчета, ДанныеДосье, ОписаниеДанныхПрограммы)
	...
	ДанныеДосье.Вставить("ИмяФайлаДанныеПрограммы",  
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Досье %1 - данные программы'; en = 'Profile %1 - program data'"),
				ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
					ИнформацияОСвязях.НаименованиеСокращенное, ""))));
...
Процедура СформироватьРезультатБухгалтерскаяОтчетность(ИнформацияОСвязях, ДанныеОтчета, ДанныеДосье, СтруктураГодов)
	...
	ДанныеДосье.Вставить("ИмяФайлаБухгалтерскаяОтчетность", 
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Досье %1 - бухгалтерская отчетность'; en = 'File %1 - accounting'"),
				ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
					ИнформацияОСвязях.НаименованиеСокращенное, ""))));
...
Функция ВалютаБухгалтерскойОтчетности(КодВалютыОтчетности)
	...
	Если КодВалютыОтчетности = "383" Тогда
		Возврат НСтр("ru = 'В рублях'; en = 'In rubles'");
	ИначеЕсли КодВалютыОтчетности = "384" Тогда
		Возврат НСтр("ru = 'В тысячах рублей'; en = 'In thousand of rubles'");
	Иначе // "385"
		Возврат НСтр("ru = 'В миллионах рублей'; en = 'In million of rubles'");
	КонецЕсли;
...
Процедура СформироватьРезультатАнализОтчетности(ИнформацияОСвязях, ДанныеОтчета, ДанныеДосье, СтруктураГодов)
	...
	ДанныеДосье.Вставить("ИмяФайлаАнализОтчетности", 
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Досье %1 - анализ отчетности'; en = 'Profile %1 - reporting analysis'"),
				ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
					ИнформацияОСвязях.НаименованиеСокращенное, ""))));
...
Процедура СформироватьРезультатФинансовыйАнализ(ИнформацияОСвязях, ДанныеОтчета, ДанныеДосье, СтруктураГодов)
	...
	ДанныеДосье.Вставить("ИмяФайлаФинансовыйАнализ", 
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Досье %1 - финансовый анализ'; en = 'Profile %1 - financial analysis'"),
				ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
					ИнформацияОСвязях.НаименованиеСокращенное, ""))));
...
Процедура ВывестиРазделРискБанкротства(ТабличныйДокумент, СтрокаДанных, ПараметрыВывода)
	...
	ТекстТочкиДиаграммы    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Индекс Альтмана Z = %1'; en = 'Altman Z-score = %1'"),
		ИндексАльтмана);
...
Процедура ВывестиРазделКредитоспособность(ТабличныйДокумент, СтрокаДанных, ПараметрыВывода)
	...
	ТекстТочкиДиаграммы    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Оценка кредитоспособности S = %1'; en = 'Creditworthiness assessment S = %1'"),
		Кредитоспособность);
	Диаграмма.УстановитьЗначение(0, 0, ЗначениеТочкиДиаграммы, , ТекстТочкиДиаграммы);
	// Комментарии к диаграмме
...
Процедура ВывестиРазделРентабельностьАктивов(ТабличныйДокумент, СтрокаДанных, ПараметрыВывода)
	...
	Если ЗначениеЗаполнено(Показатели.ВидДеятельности) Тогда
		ПояснениеРасчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Средняя рентабельность приведена по данным Росстата для вида деятельности ""%1""'; en = 'Average profitability is given according to Russian Federal State Statistics Service data for activity kind ""%1""'"),
...
Процедура ВывестиРазделРентабельностьПродаж(ТабличныйДокумент, СтрокаДанных, ПараметрыВывода)
	...
	ТекстТочкиДиаграммы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Рентабельность продаж %1'; en = 'Return on sales %1'"),
	...
	Для каждого ДанныеГода Из ДанныеПоГодам Цикл
		...
		Если НомерГода = "1" Тогда
			...
			ИначеЕсли ОценкаРискБанкротства = "СредняяОценка" Тогда
				Показатели.ПояснениеОценкиРискБанкротства = НСтр("ru = 'Вероятность банкротства невысока'; en = 'Bankruptcy possibility is low'");
			Иначе // "ХорошаяОценка"
	...
	Для каждого ДанныеГода Из ДанныеПоГодам Цикл
		...
		Если НомерГода = "1" Тогда
			...
			Если ОценкаКредитоспособность = "ПлохаяОценка" Тогда
				Показатели.ПояснениеОценкиКредитоспособность = НСтр("ru = 'Кредитование связано с повышенным риском'; en = 'Lending is a high risk'");
			ИначеЕсли ОценкаКредитоспособность = "СредняяОценка" Тогда
				Показатели.ПояснениеОценкиКредитоспособность = НСтр("ru = 'Кредитование требует взвешенного подхода'; en = 'Lending requires consideration'");
			Иначе // "ХорошаяОценка"
				Показатели.ПояснениеОценкиКредитоспособность = НСтр("ru = 'Кредитоспособность не вызывает сомнений'; en = 'Creditworthiness is proven'");
			КонецЕсли;
		КонецЕсли;
...
Функция ВывестиТаблицуНалогов(ТабличныйДокумент, ОбластиМакета, СтрокиОткрытыхДанных, ЕстьНД)
	...
	ПоказателиНалогов.Добавить("Уплачено"     , НСтр("ru = 'Оплачено'; en = 'Paid'"));
	ПоказателиНалогов.Добавить("Задолженность", НСтр("ru = 'Задолженность'"));
	ПоказателиНалогов.Добавить("Штраф"        , НСтр("ru = 'Штраф'; en = 'Fine'"));
		...
		Для каждого СтрокаПериода Из ТаблицаПериоды Цикл
			...
			Для каждого СтрокаПроверки Из ПроверкиПериода Цикл
				...
				Проверка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1, %2",
					...
					?(СтрокаПроверки.ПлановаяПроверка, НСтр("ru = 'плановая проверка'; en = 'scheduled inspection'"), НСтр("ru = 'внеплановая проверка'; en = 'unannounced inspection'")));
