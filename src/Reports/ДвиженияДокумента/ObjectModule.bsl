...
Функция НаборДанныхДляРегистраБухгалтерии(СвойстваРегистра, МетаданныеРегистра)
	...
	ПредставлениеДебет    = НСтр("ru = 'Дт'; en = 'Dr'");
	ПредставлениеКредит   = НСтр("ru = 'Кт'; en = 'Cr'");
	ПредставлениеСубконто = НСтр("ru = 'Субконто'; en = 'Extra dimension'");
	ПредставлениеСчет     = НСтр("ru = 'Счет'; en = 'Account'");
...
Процедура ПодготовитьГоризонтальныйВариантРегистровНакопленияИСведений(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	Если СтруктураСтандартныхРеквизитов.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("ru = 'Стандартные реквизиты'; en = 'Standard attributes'"));
	...
	Если СтруктураИзмерений.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("РасположениеВыбраннойГруппы",  Неопределено);
		ПараметрыГруппы.Вставить("ВыбраннаяГруппаПуста",         Ложь);
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("ru = 'Измерения'; en = 'Dimensions'"));
	...
	Если СтруктураРесурсов.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("РасположениеВыбраннойГруппы",  Неопределено);
		ПараметрыГруппы.Вставить("ВыбраннаяГруппаПуста",         Ложь);
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("ru = 'Ресурсы'; en = 'Resources'"));
	...
	Если СтруктураРеквизитов.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("РасположениеВыбраннойГруппы",  Неопределено);
		ПараметрыГруппы.Вставить("ВыбраннаяГруппаПуста",         Ложь);
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("ru = 'Реквизиты'; en = 'Attributes'"));
...
Процедура ПодготовитьГоризонтальныйВариантРегистровРасчета(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	ПользовательскийЗаголовок = НСтр("ru = 'Регистр расчета'; en = 'Calculation register'") + " " + ПредставлениеРегистра;
...
Процедура ПолучитьСвойстваРегистров(Контекст, ДокументВладелец = Неопределено)
	...
	Для Каждого Движение Из ДвиженияДокумента Цикл
		...
		Если ОбщегоНазначения.ЭтоРегистрНакопления(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("ru = 'Регистр накопления'; en = 'Accumulation register'");
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрСведений(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("ru = 'Регистр сведений'; en = 'Information register'");
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрБухгалтерии(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("ru = 'Регистр бухгалтерии'; en = 'Accounting register'");
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрРасчета(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("ru = 'Регистр расчета'; en = 'Calculation register'");
	...
	Для Каждого СвойстваРегистра Из СвойстваРегистров Цикл 
		...
		Для Индекс = 0 По ЭтажностьСтроки - 1 Цикл
			...
			Для Каждого ГруппаПолей Из ГруппыВыбранныхПолей Цикл
				...
				Заголовок = "";
				ЗаголовокСтандартныхРеквизитов = НСтр("ru = 'Стандартные реквизиты'; en = 'Standard attributes'");
				ЗаголовокИзмерений = НСтр("ru = 'Измерения'; en = 'Dimensions'");
				ЗаголовокРесурсов = НСтр("ru = 'Ресурсы'; en = 'Resources'");
				ЗаголовокРеквизитов = НСтр("ru = 'Реквизиты'; en = 'Attributes'");
				...
				ИначеЕсли ГруппаПолей.Заголовок = ЗаголовокИзмерений Тогда
					Заголовок = НСтр("ru = 'Измерение'; en = 'Измерение'");
				...
				ИначеЕсли ГруппаПолей.Заголовок = ЗаголовокРеквизитов Тогда
					Заголовок = НСтр("ru = 'Реквизит'; en = 'Property'");
