...
&НаКлиенте
Процедура ВерсияБухгалтерииПриИзменении(Элемент)
...
Процедура ВыполнитьВыгрузкуНаСервере()
	...
	Если ИспользуемыеПравила = 0 Тогда 
		...
		Если Объект.ВерсияБухгалтерии = "2.0" Тогда // правила для 1С:Бухгалтерия версии 2.0
			МакетПравилОбмена = ОбработкаОбъект.ПолучитьМакет("ПравилаОбменаТекст_БП_20");
		Иначе
...
Процедура ЗагрузитьСтандартныеПравилаОбмена()
	...
	Если Объект.ВерсияБухгалтерии = "2.0" Тогда // правила для 1С:Бухгалтерия версии 2.0
		ТекстПравил = ОбработкаОбъект.ПолучитьМакет("ПравилаОбменаТекст_БП_20");
	Иначе
