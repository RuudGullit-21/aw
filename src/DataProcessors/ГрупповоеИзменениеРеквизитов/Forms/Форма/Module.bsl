...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		КонецЕсли;
		Заголовок = НСтр("ru = 'Групповое изменение реквизитов'; en = 'Group attribute editing'");
...
Процедура Изменить(Команда)
	...
	Если НазначениеКнопки = "Изменить" Тогда
		...
		Иначе
			ТекстВопроса = НСтр("ru = 'Отбор не задан. Изменить все элементы?'; en = 'Filter is not set. Change all items?'");
...
Процедура ВыполнитьДействияПриКонтекстномОткрытии()
	...
	ШаблонЗаголовка = НСтр("ru = 'Изменение выделенных элементов ""%1"" (%2)'; en = 'Change selected items ""%1"" (%2)'");
	...
	Если ТипыОбъектов.Количество() > 1 Тогда
		ПредставлениеТипа = "";
		ШаблонЗаголовка = НСтр("ru = 'Изменение выделенных элементов (%2)'; en = 'Change selected items (%2)'");
	КонецЕсли;
