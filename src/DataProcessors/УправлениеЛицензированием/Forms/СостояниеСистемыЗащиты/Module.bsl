...
Процедура КомандаАктивироватьКлюч(Команда)
	...
	Иначе
		МассивСтрок.Добавить(НСтр("en='Using this form you can activate the software key for mobile apps.'
		|;ru='С помощью данной формы возможна активация программного ключа для мобильного приложения.'"));
	КонецЕсли;
...
Процедура КомандаУстановитьСерверЛицензирования(Команда)
	...
	#Если НЕ ВебКлиент Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаУстановитьСерверЛицензированияЗавершение", ЭтотОбъект), НСтр("ru = 'Сервер лицензирования конфигураций будет установлен на этом компьютере.
                          |Пользователь должен иметь права администратора компьютера. После нажатия на кнопку ""Да"" необходимо завершить все запущенные сеансы 1С:Предприятие, в том числе текущий, и дождаться сообщения о завершении процесса установки.
                          |Продолжить?'; en = 'The license server is installed on this computer.
