...
Процедура ПреобразоватьПеречисленияВПараметрыИПолучитьПредставление()
	...
	Если ИспользоватьImageMagickДляПреобразованияВPDF Тогда
		Если ФорматХраненияОдностраничный = Перечисления.ФорматыХраненияОдностраничныхФайлов.PDF Тогда
			...
			Представление = Представление + НСтр("ru = 'Формат сканирования:'; en = 'Scanning format:'") + " ";
	...
	Если ВРег(ФорматКартинки) = "TIF" Тогда
		Представление = Представление +  НСтр("ru = 'Сжатие:'; en = 'Deflation:'") + " " + Строка(СжатиеTIFF) + ". ";
	КонецЕсли;
	
	Представление = Представление + НСтр("ru = 'Формат хранения многостраничный:'; en = 'Multi-page storage format:'") + " ";
	...
	Представление = Представление + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Разрешение: %1 dpi. %2.'; en = 'Resolution: %1 dpi. %2.'") + " ",
	...
	Если ДвустороннееСканирование = Истина Тогда
		Представление = Представление +  НСтр("ru = 'Двустороннее сканирование'; en = 'Scan both sides'") + ". ";
	КонецЕсли;	
...
#Если НЕ ВебКлиент И НЕ МобильныйКлиент Тогда
	...
	Если Источник = "TWAIN" И Событие = "ImageAcquired" Тогда
		...
		СтрокаТаблицы.Представление = НСтр("ru = 'Изображение'; en = 'Image'") + Строка(НарастающийНомерИзображения);
...
"PDF в форме настроек сканирования.'; en = 'File ""%1"" is not found."
"Check that the ImageMagick application is installed,"
"and the correct path to the PDF conversion application"
"is specified in the form of scan settings.'"),
		ФайлРезультата);
