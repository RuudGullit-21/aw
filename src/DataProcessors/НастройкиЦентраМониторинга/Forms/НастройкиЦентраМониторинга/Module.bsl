...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Элементы.ОтправлятьИнформациюОбОшибкахРасширеннаяПодсказка.Заголовок = СтрЗаменить(СодержимоеПодсказки,"%ДопИнфо"," " + НСтр("ru = 'на сервере 1С'; en = 'on 1C server'"));
	КонецЕсли;
	
	// {УАТ}
	уатОбщегоНазначения.ИзменитьПолучателяСтатисики(ЭтаФорма, "ОтправлятьИнформациюОбОшибках", Истина);
	// {/УАТ}
