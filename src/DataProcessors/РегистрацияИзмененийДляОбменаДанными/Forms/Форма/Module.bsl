...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе
			ВызватьИсключение СтрЗаменить(
				НСтр("ru = 'Некорректные параметры объектов назначения открытия команды ""%1""'; en = 'Incorrect destination object parameters for running command ""%1""'"),
	...
	Если Не ПроконтролироватьНастройки() И ОткрытиеСУзломПараметром Тогда
		...
		ТекстСообщения = СтрЗаменить(
			НСтр("ru = 'Для ""%1"" редактирование регистрации объектов недоступно.'; en = 'Editing object registration is not available for ""%1"".'"),
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") 
		...
		Ошибка = НСтр("ru = 'Неожиданный результат выбора из консоли запросов'; en = 'Unexpected result when querying from console'");
	Иначе
...
"на узле ""%1""?'; en = 'Register the query result"
"on node ""%1""?'"); 
	Иначе
		Текст = НСтр("ru = 'Отменить регистрацию результата запроса"
"на узле ""%1""?'; en = 'Cancel registration of request result"
"on node ""%1""?'");
	КонецЕсли;
...
"на узле ""%1""?'; en = 'Register data for subordinate DIB node recovery"
"on node ""%1""?'"),
		"%1", УзелОбменаСсылка);
...
Процедура РедактироватьНомерСообщенияКонстанты()
	...
	Подсказка = НСтр("ru = 'Номер отправленного'; en = 'Number of the last sent message'"); 
...
Процедура УстановитьЗаголовокНомеровСообщений()
	...
	Текст = НСтр("ru = '№ отправленного %1, № принятого %2'; en = 'No. of sent %1, No. of received %2'");
...
Процедура СообщитьОРезультатахРегистрации(Результаты)
	...
	Если ТипЗнч(Команда) = Тип("Булево") Тогда
		Если Команда Тогда
			ЗаголовокПредупреждения = НСтр("ru = 'Регистрация изменений:'; en = 'Change registration:'");
			ТекстПредупреждения = НСтр("ru = 'Зарегистрировано %1 изменений из %2"
"на узле ""%0""'; en = '%1 of %2 changes are registered"
"on node ""%0""'");
		Иначе
			ЗаголовокПредупреждения = НСтр("ru = 'Отмена регистрации:'; en = 'Cancel registration:'");
			ТекстПредупреждения = НСтр("ru = 'Отменена регистрация %1 изменений "
"на узле ""%0"".'; en = 'Registration of %1 changes on"
"node ""%0"" was canceled.'");
		КонецЕсли;
	Иначе
		ЗаголовокПредупреждения = НСтр("ru = 'Изменение номера сообщения:'; en = 'Change message number:'");
		ТекстПредупреждения = НСтр("ru = 'Номер сообщения изменен на %3"
"у %1 объекта(ов)'; en = 'Message number for %1 object(s)"
"was changed to %3'");
	КонецЕсли;
...
"на узле ""%1""?'; en = 'Cancel registration of the selected constants"
"on node ""%1""?'"); 
	КонецЕсли;
...
""
"Изменение регистрации большого количества объектов может занять продолжительное время.'; en = 'Register the %1 to the upload on the node ""%2""?"
""
"Change of registration a large number of objects may take a long time.'");
...
""
"Изменение регистрации большого количества объектов может занять продолжительное время.'; en = 'Unregister the %1 to the upload on the node ""%2""?"
""
"Change of registration a large number of objects may take a long time.'");
...
Процедура ФоновоеЗаданиеЗапуститьНаКлиенте(ПараметрыФоновогоЗадания)
	...
	Если Результат = Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'При запуске фонового задания с целью изменения регистрации произошла ошибка.'; en = 'When a background job in order to change the registration was started an error has occurred.'");
...
Процедура СтраницаДлительнойОперации()
	...
	Если ВидДлительнойОперации Тогда
		СтатусОперации = НСтр("ru = 'Выполняется регистрация изменений. Пожалуйста, подождите.'; en = 'Changes are being registered. Please wait.'");
	Иначе
		СтатусОперации = НСтр("ru = 'Выполняется отмена регистрации изменений. Пожалуйста, подождите.'; en = 'The cancel of registration of changes is in progress. Please wait.'");
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПолучитьРезультатНаСервере()
	...
	Если ФоновоеЗаданиеРезультатВыполнения <> Неопределено Тогда
		...
		СообщениеОбОшибке = "";
		ТиповоеПредставлениеОшибки = НСтр("ru = 'При изменении регистрации произошла ошибка. Подробности см. в журнале регистрации'; en = 'An error occurred while changing the registration. See event log for details.'");
...
Процедура ОперацияСРезультатамиЗапроса(КомандаОперации)
	...
	Если ТекИмяФормы <> Неопределено Тогда
		...
		Если КомандаОперации Тогда
			Текст = НСтр("ru = 'Регистрация изменений результата запроса'; en = 'Registration of query result changes'");
		Иначе
			Текст = НСтр("ru = 'Отмена регистрации изменений результата запроса'; en = 'Cancel registration of request result changes'");
		КонецЕсли;
...
Функция КонтрольСсылокДляВыбораЗапросом(Адрес)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда 
		...
		Если Результат.Колонки.Найти("Ссылка") = Неопределено Тогда
			Возврат НСтр("ru = 'В последнем результате запроса отсутствует колонка ""Ссылка""'; en = 'Column ""Ref"" is missing in the last query result.'");
		КонецЕсли;
	Иначе		
		Возврат НСтр("ru = 'Ошибка получения данных результата запроса'; en = 'An error occurred when receiving the query result data'");
	КонецЕсли;
...
"Не выгружено объектов: %3'; en = 'Number of registered objects: %1"
"Unloaded objects: %2"
"Not unloaded objects: %3'");
...
Функция ПолучитьОписаниеВыбранныхМетаданных(БезУчетаАвторегистрации, МетаИмяГруппа = Неопределено, МетаИмяУзел = Неопределено)
	...
	Если МетаИмяГруппа = Неопределено И МетаИмяУзел = Неопределено Тогда
		// Не указано ничего
		Текст = НСтр("ru = 'все объекты %1 по выбранной иерархии вида'; en = 'all objects %1 by the selected kind hierarchy'");
	...
	Если БезУчетаАвторегистрации Тогда
		ТекстФлага = "";
	Иначе
		ТекстФлага = НСтр("ru = 'с признаком авторегистрации'; en = 'with automatic registration flag'");
	КонецЕсли;
...
Процедура ОбработатьЗапретИзмененияУзла()
	...
	Если ОперацииРазрешены Тогда
		...
		Заголовок = НСтр("ru = 'Регистрация изменений для обмена данными'; en = 'Register changes for data exchange'");
	Иначе
		Элементы.УзелОбменаСсылка.Видимость = Ложь;
		Заголовок = СтрЗаменить(НСтр("ru = 'Регистрация изменений для обмена с  ""%1""'; en = 'Changes registration for exchange with ""%1""'"), "%1", Строка(УзелОбменаСсылка));
	КонецЕсли;
...
"1С:Предприятие 8.3.5 с отключенным режимом совместимости или выше'; en = 'Data processor is designed to run on platform version"
"1C:Enterprise 8.3.5 or higher with disabled compatibility mode'");
