...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ШагПоиск.КнопкаОтмена.Заголовок = НСтр("ru = 'Прервать'; en = 'Abort'");
	ШагПоиск.КнопкаОтмена.Подсказка = НСтр("ru = 'Отказаться от объединения элементов'; en = 'Refuse to merge items'");
	...
	Шаг.КнопкаДалее.Заголовок = НСтр("ru = 'Объединить >'; en = 'Merge >'");
	Шаг.КнопкаДалее.Подсказка = НСтр("ru = 'Начать объединение элементов'; en = 'Start merging items'");
	Шаг.КнопкаОтмена.Заголовок = НСтр("ru = 'Отмена'; en = 'Cancel'");
	Шаг.КнопкаОтмена.Подсказка = НСтр("ru = 'Отказаться от объединения элементов'; en = 'Refuse to merge items'");
	...
	Шаг.КнопкаОтмена.Подсказка = НСтр("ru = 'Закрыть результаты объединения'; en = 'Close merging results'");
	
	// 5. Проблемы замены ссылок.
	Шаг = ДобавитьШагМастера(Элементы.ШагПовторОбъединения);
	Шаг.КнопкаНазад.Заголовок = НСтр("ru = '< В начало'; en = '< Go home'");
	Шаг.КнопкаНазад.Подсказка = НСтр("ru = 'Вернутся к выбору основного элемента'; en = 'Go back to the main item selection'");
	Шаг.КнопкаДалее.КнопкаПоУмолчанию = Истина;
	Шаг.КнопкаДалее.Заголовок = НСтр("ru = 'Повторить'; en = 'Retry'");
	Шаг.КнопкаДалее.Подсказка = НСтр("ru = 'Повторить объединение'; en = 'Repeat merging'");
	Шаг.КнопкаОтмена.Заголовок = НСтр("ru = 'Отмена'; en = 'Cancel'");
	Шаг.КнопкаОтмена.Подсказка = НСтр("ru = 'Закрыть результаты объединения'; en = 'Close merging results'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НастройкиМастера.ПоказатьДиалогПередЗакрытием
		...
		ТекстВопроса = НСтр("ru = 'Прервать объединение элементов и закрыть форму?'; en = 'Stop merging items and close the form?'");
...
Процедура МестаИспользованияПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Удалить из списка для объединения элемент ""%1""?'; en = 'Delete item ""%1"" from the list for merging?'"),
...
"Группы не могут быть объединены.'; en = 'One of the mergable items is a group."
...
"Такие элементы не могут быть объединены.'; en = 'Mergable items have different owners. "
"Such items cannot be merged.'");
	ИначеЕсли Контроль.КоличествоСсылок <> КоличествоСсылок Тогда
		Результат.Ошибка = НСтр("ru = 'Все объединяемые элементы должны быть одного типа.'; en = 'All mergable items must be of the same type.'");
	Иначе 
		// Все нормально
...
"использования на ""%2"" (отмечен стрелкой).'; en = 'Items (%1) will be marked for deletion and replaced in all usage"
...
Функция ОпределитьМестаИспользования()
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Поиск и удаление дублей: Определение мест использования'; en = 'Search and deletion of duplicates: Determine usage locations'");
...
Функция ЗаменитьСсылки()
	...
	НастройкиЗапуска = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Поиск и удаление дублей: Объединение элементов'; en = 'Search and deletion of duplicates: Merge items'");
...
Процедура ЗаполнитьМестаИспользования(Знач АдресРезультата)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Объединение элементов %1 в один'; en = 'Merge items of %1 into a single one'"), Представление);
КонецПроцедуры
