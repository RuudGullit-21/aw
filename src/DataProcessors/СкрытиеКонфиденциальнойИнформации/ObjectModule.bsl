...
Процедура ВыполнитьСкрытиеНаСервере(Параметры, АдресХранилища) Экспорт
	...
	// {УАТ}
	ПараметрыОтбора.Вставить("ДеревоОбрабатываемыхОбъектов", ДеревоОбрабатываемыхОбъектов);
	уатОбщегоНазначения.ОбработкаОбъектовУАТ(ПараметрыОтбора, ЭтотОбъект);
	// {/УАТ}
...
Процедура ОбработатьРегистры(ОбъектМетаданных, ПараметрыОтбора)
	...
	// {УАТ}
	Если ТипЗнч(Регистр) = Тип("РегистрСведенийМенеджер.ФИОФизическихЛиц") И Не уатОбщегоНазначения.ОбластьПерсональныхДанныхВключена("ИсторияФИО") Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(Регистр) = Тип("РегистрСведенийМенеджер.ДокументыФизическихЛиц") И Не уатОбщегоНазначения.ОбластьПерсональныхДанныхВключена("ПаспортныеДанные") Тогда
		Возврат;
	КонецЕсли;
	// {/УАТ}
...
Процедура ОбработатьОбъектМетаданных(ОбъектМетаданных, ПараметрыОтбора)
	...
	Если ЗначениеЗаполнено(Субъекты) Тогда
		...
		// {УАТ}
		// Результаты поиска связанных объектов дополняются субъектами для корректной очистки реквизитов субъекта
		//Если Результат.Количество() = 0 И Объект = Субъекты[0].Метаданные() Тогда
		Если Объект = Субъекты[0].Метаданные() Тогда
		// {/УАТ}
	...
	Пока Истина Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			// {УАТ}
			// Очистка данных регистров выполняеться отдельно от остальных.
			Если СтрНайти(ТипЗнч(СтрокаРезультата.Ссылка), "Регистр") > 0 Тогда 
				Продолжить;
			КонецЕсли;
			
			// Очистка данных сотрудников выполняеться отдельно от остальных.
			Если ТипЗнч(СтрокаРезультата.Ссылка) = Тип("СправочникСсылка.Сотрудники") Тогда 
				Продолжить;
			КонецЕсли;
			
			// Переопредление параметров обрабатываемого объекта метаданных.
			ОбъектОписания = Метаданные.НайтиПоТипу(ТипЗнч(СтрокаРезультата.Ссылка));
			ОбъектМетаданных = ПараметрыОтбора.ДеревоОбрабатываемыхОбъектов.Строки.Найти(ОбъектОписания.Имя, "Имя", Истина);
			// {/УАТ}
				...
				Для Каждого ОбрабатываемоеСвойство Из ГруппаСвойствОбъекта.Строки Цикл
					...
					Если ГруппаСвойствОбъекта.Тип = "ТабличнаяЧасть" Тогда
						...
						Для Каждого СтрокаТабличнойЧасти Из ОбрабатываемыеСвойства Цикл
							...
							// {УАТ}
							Если Не уатОбщегоНазначения.ОбластьПерсональныхДанныхВключена("КонтактнаяИнформация") Тогда
...
Процедура ДобавитьСвойства(ОбъектМетаданных, СтрокаДерева, Тип, ЕстьОбрабатываемыеСвойства)
	...
	Иначе
		...
		Если Тип = "ПризнакиУчета" Тогда
			Синоним = НСтр("ru = 'Признаки учета'; en = 'Accounting flags'");
		Иначе
			Синоним = Тип;
		КонецЕсли;
		Коллекция = ОбъектМетаданных[Тип];
