...
"только в форме объекта-владельца.'; en = 'You can see a list of attached files"
"only in object-owner form.'");
	...
	ЗаголовокОшибки = НСтр("ru = 'Ошибка при настройке динамического списка присоединенных файлов.'; en = 'An error occurred when configuring the dynamic list of attached files.'");
	ОкончаниеОшибки = НСтр("ru = 'В этом случае настройка динамического списка невозможна.'; en = 'In this case, dynamic list configuration is not supported.'");
...
Процедура УстановитьПометкуУдаления(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		ШаблонВопроса = ?(ТекущиеДанные.ПометкаУдаления,
			НСтр("ru = 'Снять с ""%1"" пометку на удаление?'; en = 'Clear mark for deletion for ""%1""?'"),
...
"Используйте команду ""Создать"" в списке файлов.'; en = 'File import is not supported in Web client."
...
"Используйте команду ""Создать"" в списке файлов.'; en = 'Folder import is unavailable in the web client."
"Use the command ""Create"" in the file list.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	#КонецЕсли
