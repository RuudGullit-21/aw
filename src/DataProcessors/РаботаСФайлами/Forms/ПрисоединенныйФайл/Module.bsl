...
"""%1""?'; en = 'Clear mark for deletion for file"
"""%1""?'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Пометить на удаление файл"
"""%1""?'; en = 'Mark for deletion file"
"""%1""?'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(СсылкаНаТекущийОбъект) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 (Присоединенный файл)'; en = '%1 (Attached file)'"), Строка(СсылкаНаТекущийОбъект));
	Иначе
		Заголовок = НСтр("ru = 'Присоединенный файл (Создание)'; en = 'Attached file (Creation)'")
	КонецЕсли;
...
Процедура УстановитьДоступностьКнопок(Форма, Элементы)
	...
	Если Не ДоступнаПечатьСоШтампом Тогда
		...
		Элементы.Печать.Заголовок = НСтр("ru = 'Печать'; en = 'Print'");
	Иначе
		Элементы.ПодменюПечать.Вид = ВидГруппыФормы.Подменю;
		Элементы.Печать.Заголовок = НСтр("ru = 'Сразу на принтер'; en = 'Directly to the printer'");
	КонецЕсли;
