...
Процедура СохранитьПапку()
	...
	Если ПустаяСтрока(ПапкаДляЭкспорта) Или ПапкаДляЭкспорта = ПолучитьРазделительПути() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Укажите папку.'; en = 'Specify the folder.'"));
		Возврат;
	КонецЕсли;
...
"Выберите другую папку.'; en = 'Folder ""%1"" not found."
"Select another folder.'"),
	...
	Если Не КаталогВыгрузки.Существует() Тогда
		...
		Попытка
			...
			Если Не КаталогВыгрузки.Существует() Тогда
				ВызватьИсключение НСтр("ru = 'После успешного создания подпапка не найдена.'; en = 'After successfully creating the subfolder is not found.'");
			КонецЕсли;
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось создать подпапку ""%1"" в папке ""%2"" по причине:"
"%3'; en = 'Unable to create subfolder ""%1"" in folder ""%2"" because:"
"%3'"),
				Строка(ЧтоСохраняем),
				ПапкаДляЭкспорта,
...
Процедура СохранитьПапкуЗавершение(Результат, ПараметрыВыполнения) Экспорт
	Если Результат.Успех = Истина Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Экспорт папки'; en = 'Export folder'"),,
		             СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		               НСтр("ru = 'Успешно завершен экспорт папки ""%1"""
"в папку на диске ""%2"".'; en = 'Exporting folder ""%1"" successfully completed"
"into folder on drive ""%2"".'"),
...
Процедура СоздатьПодкаталог(ПараметрыВыполнения)
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка создания папки ""%1"":'; en = 'An error occurred when creating folder ""%1"":'");
...
""
"Повторить экспорт этого файла?'; en = 'A folder with"
"the same name exists instead"
"of file ""%1"". "
""
...
""
"Заменить существующий файл файлом из хранилища файлов?'; en = 'The ""%1"" folder"
"contains the ""%2"" file "
...
"""%1""."
""
"%2.'; en = 'An error occurred when writing file"
"""%1""."
""
"%2.'"),
		ПараметрыВыполнения.ПолноеИмяФайла,
