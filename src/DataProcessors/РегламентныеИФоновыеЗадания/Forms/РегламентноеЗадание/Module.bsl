...
"is executed only by administrators.'");
	...
	Если СтрНайти(", Добавить, Скопировать, Изменить,", ", " + Действие + ",") = 0 Тогда
		...
		ВызватьИсключение НСтр("ru = 'Неверные параметры открытия формы ""Регламентное задание"".'; en = 'Incorrect parameters of opening form ""Scheduled job"".'");
	КонецЕсли;
	...
	Если Действие <> "Изменить" Тогда
		Идентификатор = НСтр("ru = '<будет создан при записи>'; en = '<will be created when writing>'");
		Использование = Ложь;
...
Процедура ВыборШаблонаНовогоРегламентногоЗадания()
	...
	ОписанияМетаданныхРегламентныхЗаданий.ПоказатьВыборЭлемента(
		...
		НСтр("ru = 'Выберите шаблон регламентного задания'; en = 'Select a scheduled job template'"));
...
Процедура ОбновитьЗаголовокФормы()
	...
	Иначе
		Заголовок = НСтр("ru = 'Регламентное задание (создание)'; en = 'Scheduled job (creation)'");
	КонецЕсли;
