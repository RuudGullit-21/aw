...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоУдалениеПомеченныхОбъектов Тогда
		...
		Элементы.ТекстСообщенияОбОшибке.Заголовок = НСтр("ru = 'Невозможно выполнить удаление помеченных объектов, т.к. в программе работают другие пользователи:'; en = 'Cannot delete selected objects as other users are using the application:'");
	КонецЕсли;
...
Процедура ОбновитьКоличествоАктивныхСеансов(Форма)
	...
	Если Форма.КоличествоАктивныхСеансов > 0 Тогда
		ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Активные пользователи (%1)'; en = 'Active users (%1)'"), 
			Форма.КоличествоАктивныхСеансов);
	Иначе
		ТекстГиперссылки = НСтр("ru = 'Активные пользователи'; en = 'Active users'");
	КонецЕсли;	
	...
	Иначе	
		...
		Иначе	
			ТекстСообщенияОбОшибке = НСтр("ru = 'Невозможно выполнить обновление версии программы, т.к. не удалось завершить работу пользователей:'; en = 'Cannot update application version as some user sessions are still active:'");
		КонецЕсли;
		Форма.Элементы.ТекстСообщенияОбОшибке.Заголовок = ТекстСообщенияОбОшибке;
		Форма.Элементы.ПояснениеПоОшибке.Заголовок = НСтр("ru = 'Для продолжения необходимо завершить их работу.'; en = 'Close them to continue.'");
	КонецЕсли;	
...
Процедура ПроверитьМонопольныйРежимНаСервере()
	...
	Если КоличествоАктивныхСеансов = 0 Тогда
		...
		Исключение
			ОшибкаУстановкиМонопольногоРежима = НСтр("ru = 'Техническая информация:'; en = 'Technical information:'") + " " 
...
Процедура УстановитьБлокировкуФайловойБазы()
	...
	Если ЭтоУдалениеПомеченныхОбъектов Тогда
		...
		Объект.СообщениеДляПользователей = НСтр("ru = 'Программа заблокирована для удаления помеченных объектов.'; en = 'The application is locked for deleting selected objects.'");
	Иначе
		Объект.НачалоДействияБлокировки = ТекущаяДатаСеанса() + 2*60;
