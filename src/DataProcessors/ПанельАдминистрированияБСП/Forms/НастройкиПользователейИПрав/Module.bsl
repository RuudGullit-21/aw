	...
	Если ОбщегоНазначения.РазделениеВключено()
	 ...
	 Или Не ПользователиСлужебный.ВнешниеПользователиВнедрены() Тогда
		...
		Элементы.ОписаниеРаздела.Заголовок =
			НСтр("ru = 'Администрирование пользователей, настройка групп доступа, управление пользовательскими настройками.'; en = 'User administration, setting up access groups, managing of user settings.'");
	КонецЕсли;
...
"от нескольких секунд до многих часов (в зависимости от объема данных).'; en = 'Enable access restriction on the record level?"
""
"Data population will be required which will be executed"
"by parts by scheduled job ""Data population for access"
"restriction"" (execution progress is in the event log)."
""
"Execution may slow down the application operation and can take"
"from several seconds to several hours (depending on the data size).'");
	Иначе
		ТекстВопроса = "";
	КонецЕсли;
...
" пользователей будет очищен и скрыт).'; en = 'Allow external user access?"
""
"After logon, user selection list will be empty"
"(attribute ""Show in the selection list"" in cards"
"of all users will be cleared and hidden).'");
...
"очищен в карточках всех внешних пользователей.'; en = 'Deny access to external users?"
""
"Attribute ""Signing in is available"""
"will be cleared in all external users'' cards.'");
