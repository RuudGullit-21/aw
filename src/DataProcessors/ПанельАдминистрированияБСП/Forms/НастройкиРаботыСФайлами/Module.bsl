...
Процедура МаксимальныйРазмерФайлаОбластиДанныхПриИзменении(Элемент)
	...
	Если МаксимальныйРазмерФайлаОбластиДанных = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Поле ""Максимальный размер файла"" не заполнено.'; en = '""Maximum file size"" field is not filled.'");
...
"Добавляемые файлы будут сохраняться в информационной базе до тех пор, пока не будет настроен хотя бы один том хранения файлов.'; en = 'Enabled storage of files in volumes on disk, but volumes are not configured yet."
"The added files will be saved in the information database until at least one volume of file storage is configured.'"));
		КонецЕсли;
