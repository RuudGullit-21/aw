...
Функция ЗапуститьИзменениеПрефиксаИБВФоновомЗадании()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Изменение префикса'; en = 'Changing the prefix'");
...
Процедура ПослеИзмененияПрефикса(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание <> Неопределено
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Префикс изменен.'; en = 'Prefix changed.'"));
...
Процедура УстановитьВидимость()
	...
	Если РазделениеВключено Тогда
		Элементы.ОписаниеРаздела.Заголовок = НСтр("ru = 'Синхронизация данных с моими приложениями.'; en = 'Sync data with my apps.'");
	КонецЕсли;
	...
	Если ПодсистемаСуществуетОбменДанными Тогда
		...
		Если РазделениеВключено Тогда
			...
			Элементы.ПрефиксУзлаРаспределеннойИнформационнойБазы.Заголовок = НСтр("ru = 'Префикс в этой программе'; en = 'Prefix in this program'");
		Иначе
...
"Не влияет на загрузку данных из автономных рабочих мест.'; en = 'Prohibit import of previous period data from other applications."
"It does not impact data import from offline work places.'");
		КонецЕсли;
	Иначе
