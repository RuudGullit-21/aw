...
Процедура УстановитьРежимВставкиИзБуфераОбмена(ШаблонСДанными, ИнформацияПоКолонкам, ОписаниеТипов) Экспорт
	...
	Для каждого Тип Из ОписаниеТипов.Типы() Цикл
		...
		ЗаголовокКолонки = НСтр("ru = 'Введенные данные'; en = 'Entered data'");
...
Процедура ИнформациюПоКолонкамИзРеквизитовСправочника(ПараметрыЗагрузки, ИнформацияПоКолонкам)
	...
	Для каждого Реквизит Из МетаданныеСправочника.Реквизиты Цикл
		...
		Если Реквизит.Тип.СодержитТип(Тип("Булево")) Тогда 
			ОписаниеТипаКолонки = НСтр("ru = 'Флаг, Да или 1 / Нет или 0'; en = 'Check box, Yes or 1 / No or 0'");
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("Число")) Тогда 
			ОписаниеТипаКолонки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Число, Длина: %1, Точность: %2'; en = 'Digit, Length: %1, Accuracy: %2'"),
		...
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("Строка")) Тогда
			Если Реквизит.Тип.КвалификаторыСтроки.Длина > 0 Тогда
				...
				ОписаниеТипаКолонки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Строка, макс. количество символов: %1'; en = 'String, maximum characters: %1'"), ДлинаСтроки);
			Иначе
				ОписаниеТипаКолонки = НСтр("ru = 'Строка неограниченной длины'; en = 'String of unlimited length'");
			КонецЕсли;
		...
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("УникальныйИдентификатор")) Тогда
			ОписаниеТипаКолонки = НСтр("ru = 'Уникальный идентификатор'; en = 'UUID'");
		КонецЕсли;
...
Процедура СоздатьКолонкуСтандартныеРеквизиты(ИнформацияПоКолонкам, МетаданныеСправочника, ИмяКолонки, Позиция)
	...
	Если ТипДанных = Тип("Строка") Тогда 
		ПредставлениеТипа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Строка (не более %1 символов)'; en = 'Line (no more than %1 characters)'"), ОписаниеТипа.КвалификаторыСтроки.Длина);
...
Процедура ОпределитьИнформацияПоКолонкамТабличнаяЧасть(Знач ИнформацияПоКолонкам, Макет, ОбластьЗаголовокТаблицы, Знач ПараметрыЗагрузки)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Не найден макет для загрузки данных из файла'; en = 'Template for data import from file is not found'");
	КонецЕсли;
...
Функция ПредопределенныеОбластиМакета()
	...
	ПредопределенныеОбластиМакета.Вставить("ДополнительныеРеквизиты", НСтр("ru = '<Дополнительные реквизиты>'; en = '<Additional attributes>'"));
	ПредопределенныеОбластиМакета.Вставить("КонтактнаяИнформация", НСтр("ru = '<Контактная информация>'; en = '<Contact information>'"));
		...
		Попытка
			Если ЗначениеЗаполнено(СтрокаТаблицы.ОбъектСопоставления) Тогда
				...
				Если ЭлементСправочника = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Номенклатура с артикулом %1 не существует.'; en = 'No products and services with SKU %1.'"),
...
"%2'; en = 'Failed to write catalog element %1 because:"
"%2'"), 
...
Процедура СформироватьМакетОтчета(ТаблицаОтчет, ШаблонСДанными)
	...
	ЗаполнитьЯчейкуЗаголовкаМакета(Ячейка, НСтр("ru = 'Результат'; en = 'Result'"), 12, НСтр("ru = 'Результат загрузки данных'; en = 'Data import result'"), Истина);
...
Функция СобытиеЖурналаРегистрации() 
	...
	Возврат НСтр("ru = 'Загрузка данных из файла'; en = 'Import data from file'", ОбщегоНазначения.КодОсновногоЯзыка());
