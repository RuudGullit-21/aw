...
"в справочнике найдены не уникальные записи.'; en = 'Non-unique entries were found in the catalog"
"before importing identifiers of metadata objects.'"));
			КонецЕсли;
...
"%1'; en = 'Priority data import failed."
...
"%1'"),
			...
			Если ЗначениеЗаполнено(ДублиПредопределенных) Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Предопределенные элементы.Найдены не уникальные записи.'; en = 'Predefined items.Non-unique entries found.'",
...
"%1'; en = 'When importing predefined items, non-unique records were found."
"%1'"),
						ДублиПредопределенных));
			КонецЕсли;
...
Процедура ДобавитьОписаниеДублейПредопределенного(ЗаписанныйОбъект, ДублиПредопределенных, Отказ, ОписаниеОтказа)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОшибкиНеУникальностиЗаписей(),
				НСтр("ru = 'При загрузке предопределенных элементов найдены не уникальные записи.'; en = 'When importing predefined items, non-unique records were found.'"));
		КонецЕсли;
...
"   - нажмите Выполнить."
"3. После этого запустите 1С:Предприятие и выполните повторную синхронизацию данных.'; en = 'Priority data import failed."
"%1"
...
Функция ОшибкаОткрытияФайлаСообщенияОбмена()
	...
	Возврат НСтр("ru = 'Ошибка открытия файла сообщения обмена'; en = 'An error occurred when opening the exchange message file'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаНачалаЧтенияФайлаСообщенияОбмена()
	
	Возврат НСтр("ru = 'Ошибка при начале чтения файла сообщения обмена'; en = 'An error occurred when starting reading the exchange message file'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаНачалаЗаписиФайлаСообщенияОбмена()
	
	Возврат НСтр("ru = 'Ошибка при начале записи файла сообщения обмена'; en = 'An error occurred when starting writing the exchange message file'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаЧтенияФайлаСообщенияОбмена()
	
	Возврат НСтр("ru = 'Ошибка чтения файла сообщения обмена'; en = 'An error occurred when reading an exchange message file'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаЗаписиФайлаСообщенияОбмена()
	
	Возврат НСтр("ru = 'Ошибка записи данных в файл сообщения обмена'; en = 'An error occurred when writing data to the exchange message file'");
	
КонецФункции

Функция ОшибкаВидаОбменаДанными()
	
	Возврат НСтр("ru = 'Обмен не по правилам конвертации не поддерживается'; en = 'Exchange not according to conversion rules is not supported'", ОбщегоНазначения.КодОсновногоЯзыка());
