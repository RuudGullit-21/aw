	...
	Если ПереключательОчищаемыеНастройки = "ОтдельныеНастройки" Тогда
		Если ПереключательКомуОчищатьНастройки <> "ВыбраннымПользователям"
		 Или КоличествоПользователей <> 1 Тогда
			...
			ПоказатьПредупреждение(,НСтр("ru = 'Очистка отдельных настроек доступна только при выборе одного пользователя.'; en = 'Clearing individual settings are available only when you select single user.'"));
		Иначе
...
"которым необходимо очистить настройки.'; en = 'Select the user or users"
"which you want to clear the settings.'"), , "Источник");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПереключательКомуОчищатьНастройки = "ВыбраннымПользователям" Тогда
		...
		Иначе
			ПояснениеУКогоОчищеныНастройки = НСтр("ru = '%1 пользователям'; en = 'For %1 users'");
	...
	Если ПереключательОчищаемыеНастройки = "ОтдельныеНастройки"
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Выберите настройки, которые необходимо очистить.'; en = 'Select settings that need to be cleaned.'"), , "ПереключательОчищаемыеНастройки");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПереключательОчищаемыеНастройки = "ОтдельныеНастройки" Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			...
			ТекстПояснения = НСтр("ru = '""%1"" очищена у %2'; en = '""%1"" cleaned %2'");
		...
		Иначе
			...
			ТекстПояснения = НСтр("ru = 'Очищено %1 у %2'; en = 'Cleaned %1 from %2'");
	...
	ИначеЕсли ПереключательОчищаемыеНастройки = "ОчиститьВсе" Тогда
		...
		ТекстПояснения = НСтр("ru = 'Очищены все настройки %1'; en = 'Cleared all settings %1'");
