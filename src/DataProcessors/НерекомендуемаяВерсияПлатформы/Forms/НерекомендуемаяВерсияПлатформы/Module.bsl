...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Текущая, Минимальная) < 0 Тогда
		ТекстУсловие                = НСтр("ru = 'необходимо'; en = 'required'");
		ПродолжениеРаботыНевозможно = Истина;
		НомерВерсии = Минимальная;
	Иначе
		ТекстУсловие = НСтр("ru = 'рекомендуется'; en = 'recommended'");
	КонецЕсли;
	...
	Если ПродолжениеРаботыНевозможно Тогда
		...
		Заголовок = НСтр("ru = 'Необходимо обновить версию платформы'; en = 'Update platform version'");
	КонецЕсли;
