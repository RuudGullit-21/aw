...
Процедура ЗагрузитьФайлыНовостейССервера(ЛентыНовостей, КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Загрузка файлов новостей'; en = 'Download news files'"));
...
#Область ЗагрузкаФайлов
		...
		ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			Для каждого ТекущееХТТПСоединение Из ТаблицаХТТПСоединений Цикл
				...
				ЛогШаговВыполненияВложенный.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Обработка: %1, %2'; en = 'Processing: %1, %2'"),
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка новостей'; en = 'ISL:News.Download news'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Загрузка новостей. Загрузка файлов новостей'; en = 'News. Download of news. Download news files'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
...
Процедура ПарсингХМЛФайловНовостей(ЛентыНовостей, КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Парсинг файлов новостей'; en = 'Parsing file news'"));
...
#Область ОсновныеОбработки
		...
		ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Сохранение даты последних загрузок'; en = 'Save date of last downloads'"));
			...
			// Прочитать файлы
			ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Обработка лент новостей:'; en = 'Newsfeeds processing:'"));
			Для каждого ТекущаяСтрока Из ЛентыНовостей Цикл

				ЛогШаговВыполненияВложенный.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Лента: %1, обязательная: %2, пометка: %3'; en = 'Tape: %1, required: %2 mark: %3'"),
				...
				Если (ТекущаяСтрока.Пометка) ИЛИ (ТекущаяСтрока.ОбязательныйКанал) Тогда
					...
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Обрабатывается...'; en = 'Processing...'"));
				...
				Иначе
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Не помечена для обработки'; en = 'Not marked for processing'"));
				КонецЕсли;
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка новостей'; en = 'ISL:News.Download news'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Загрузка новостей. Парсинг файлов новостей'; en = 'News. Download of news. Parsing news files'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
		...
		Для каждого ТекущаяСтрока Из ТаблицаОбновляемыхДанных Цикл
			...
			Иначе
				Если ТекущаяСтрока.Список = "Список лент новостей" Тогда // Идентификатор.
					ТекстСообщения = НСтр("ru = 'Загрузка списка лент новостей'; en = 'Import the news feed list'");
					ТекущаяСтрока.ПредставлениеДляЗагрузки = ТекстСообщения;
				ИначеЕсли ТекущаяСтрока.Список = "Список категорий новостей" Тогда // Идентификатор.
					ТекстСообщения = НСтр("ru = 'Загрузка списка категорий новостей'; en = 'Import the news category list'");
					ТекущаяСтрока.ПредставлениеДляЗагрузки = ТекстСообщения;
...
Функция ОбновитьСтандартныйСписокССервера(Список, ТекущаяВерсияНаКлиенте, Прокси = Неопределено, КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Обновление классификатора'; en = 'Update classifier'"));
		...
		Иначе
			БылиОшибки = Истина;
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Не удалось подключиться к веб-сервисам.'; en = 'Failed to connect to web-services.'"));
		КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка классификаторов'; en = 'ISL:News.Classifier import'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Загрузка классификаторов'; en = 'News. Download classifiers'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Функция ОбновитьВсеСтандартныеСпискиССервера(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
			...
			Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
				БылиОшибки = Истина;
				ОписаниеРезультата = НСтр("ru = 'Не удалось подключиться к прокси веб-сервисов обновления классификаторов'; en = 'Failed to connect to proxy web services of classifiers update'");
			Иначе
...
Процедура ОбновитьСписокСправочниковИВерсииССервера(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		ОбработкаНовостейКлиентСервер.ЗарегистрироватьРезультатВыполненияОперации(
			...
			НСтр("ru = 'Параметры выполнения'; en = 'Execution parameters'"),
			0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"%5'"),
					...
					?(ТипЗнч(ДатыОбновленияСписков) = ТипОбъектXDTO,
						...
						НСтр("ru = 'Получено значение с типом значения, отличным от ОбъектXDTO'; en = 'Value with a value type which is different from ObjectXDTO is received'"))),
				Неопределено);
		...
		Иначе
			ОписаниеРезультата = НСтр("ru = 'Не удалось подключиться к веб-сервису обновления классификаторов.'; en = 'Failed to connect to the web service of the classifiers update.'");
		КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка классификаторов'; en = 'ISL:News.Classifier import'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Загрузка классификаторов. Список обновлений классификаторов'; en = 'News. Download of the classifiers. Update list of classifiers'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПарсингХМЛФайлаНовостей(ОбъектХДТО, ФорматНовостей, ЛентаНовостей, Прокси, КонтекстВыполнения = Неопределено)
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Парсинг файла новостей'; en = 'Parsing news file'"));
...
"Определен формат ленты новостей: %2'; en = 'Parsing news file for news feed %1"
"Determined the format of news feed: %2'"),
					ЛентаНовостей,
					НРег(ФорматНовостей)));
...
"ДатаЗагрузкиПоследнихНовостейВФайле: %2"
"News in the program do not require updates.'"),
				...
				Иначе
					ЛогШаговВыполнения.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'В файле полученном для ленты новостей %1 отсутствует обязательное поле channel. Файл не был обработан.'; en = 'In the file received for news feed %1 missing required channel field. File was not processed.'"),
							ЛентаНовостей));
					НеобходимоОбработатьФайл = Ложь;
				КонецЕсли;
...
"News in the program do not require updates.'"),
			...
			Иначе // updated - обязательное поле, наверное формат некорректный
				...
				ЛогШаговВыполнения.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'В файле полученном для ленты новостей %1 отсутствует поле feed.updated. Все новости будут прочитаны и загружены, но это займет больше времени.'; en = 'In the file received for news feed %1 feed.updated field is missing. All the news will be read and downloaded, but it will take more time.'"),
						ЛентаНовостей));
			КонецЕсли;
...
"ДатаПоследнейПопыткиЗагрузкиНовостей: %2'"),
		...
		Иначе
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Новости не загружены, т.к. передан необрабатываемый формат ленты новостей: %1'; en = 'News not downloaded, because unsupported news feed format: %1'"),
					НРег(ФорматНовостей)));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка новостей'; en = 'ISL:News.Download news'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей'; en = 'News. News import. News file parsing. News file parsing'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ЗагрузитьНовость(
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Загрузка новости'; en = 'Download news'"));
		...
		Если НРег(ФорматНовостей) = "rss" Тогда
			...
			Если ЗаписыватьНовость = Истина Тогда
				...
				ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					Попытка
						...
						ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Начало попытки записи новости'; en = 'Begin of attempt to write news'"));
						...
						Иначе
							...
							ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Успешная попытка записи новости'; en = 'Successful attempt of news record'"));
			...
			Иначе
				...
				ЛогШаговВыполнения.Добавить(НСтр("ru = 'Новость пропущена'; en = 'News skipped'"));
			...
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ЗаписыватьНовость%", ?(ЗаписыватьНовость = Истина, НСтр("ru = 'Будет записана'; en = 'It will be written'"), НСтр("ru = 'Будет пропущена'; en = 'Will be skipped'")));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%СуществуетВБазе%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru = 'В базе еще не создана'; en = 'It has not been created in the base yet'"), НСтр("ru = 'Уже существует в базе'; en = 'Already exists in the infobase'")));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ДатаПубликацииИБ%", ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("ru = '(в базе не было)'; en = '(it was not in the base)'"), " (" + Формат(ДатаПубликацииИБ, "ДЛФ=DT") + ")"));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ЗначениеПометкаУдаленияИБ%", ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("ru = '(в базе не было)'; en = '(it was not in the base)'"), " (" + ЗначениеПометкаУдаленияИБ + ")"));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%УИН_ИБ%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru = '(еще не создана)'; en = '(not created yet)'"), СтруктураНовости.УИН_ИБ));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%НавигационнаяСсылка%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru = '(еще не создана)'; en = '(not created yet)'"), ПолучитьНавигационнуюСсылку(СтруктураНовости.Ссылка)));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка новостей'; en = 'ISL:News.Download news'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Загрузка новостей. Загрузка новости'; en = 'News. Download of news. Download news'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ТипЗнч(ДанныеХДТО) = ТипОбъектXDTO Тогда
		...
		Если ТипЗнч(СвойствоДвоичныеДанные) = ТипСвойствоXDTO Тогда
			...
			Исключение
				...
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru = 'БИП:Новости.Загрузка новостей'; en = 'ISL:News.Download news'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
					НСтр("ru = 'Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Чтение новости. Блок бинарных данных'; en = 'News. News import. News file parsing. News file parsing. News parsing. Read news. Binary data unit'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
...
Процедура ЗагрузитьДатуПоследнегоОбновленияСтандартногоСписка(Обновление, КонтекстВыполнения = Неопределено)
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Загрузка даты последнего обновления стандартного списка'; en = 'Date download of the latest standard list update'"));
...
"%1'; en = 'Error loading the string of lists update for the reason:"
"%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ЗаполнитьОбъектКатегорияНовостей(ОбъектХДТО, Режим = "")
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если НайденныеДубликаты.Количество() > 0 Тогда
			...
			КонецЦикла;
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru = 'БИП:Новости.Загрузка классификаторов'; en = 'ISL:News.Classifier import'", КодОсновногоЯзыка), // ИмяСобытия.
...
Функция ЗаполнитьОбъектЛентаНовостей(ОбъектХДТО, Режим = "", КонтекстВыполнения = Неопределено)
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Заполнение объекта Лента новостей'; en = 'Filling the News feed object'"));
				...
				Если ТипЗнч(ОбъектХДТО.categoriesRefList.categoryRef) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
					...
					Пока (ТекущийИндекс >= 0) Цикл
						...
						Если КатегорияЕстьНаСервере = Ложь Тогда
							ЛогШаговВыполнения.Добавить(
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Из ленты новостей %1 удалена категория %2.'; en = 'From the feed, %1 deleted category %2.'"),
					...
					Для каждого ТекущаяКатегория Из МассивДоступныхКатегорий Цикл
						...
						Если КатегорияЕстьНаКлиенте = Ложь Тогда
							ЛогШаговВыполнения.Добавить(
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'В ленте новостей %1 добавлена категория %2.'; en = 'News feed %1 added the category %2.'"),
									СокрЛП(лкОбъект.Наименование),
...
Функция ПодключитьсяКВебСервисуОбновленияСтандартныхСписков(КонтекстВыполнения = Неопределено)
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Подключение к веб-сервису обновления классификаторов'; en = 'Connecting to the web service of classifiers update'"));
...
"МестоположениеWSDL = %1'; en = 'Start download of standard lists"
"МестоположениеWSDL = %1'"),
		...
		Попытка
			...
			Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
				ЛогШаговВыполнения.Добавить(НСтр("ru = 'Неудачное подключение к веб-сервису обновления стандартных списков'; en = 'Unsuccessful connection to web service of standard list update'"));
			Иначе
				ЛогШаговВыполнения.Добавить(НСтр("ru = 'Успешное подключение к веб-сервису обновления стандартных списков'; en = 'Successfully connected to the web service of standard lists update'"));
			КонецЕсли;
...
"%1'; en = 'Failed connection to the web service of the standard lists update because:"
"%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка классификаторов'; en = 'ISL:News.Classifier import'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков'; en = 'News. Download of the classifiers. Connection with the web service of the standard lists update'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
