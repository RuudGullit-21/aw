...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ОтключеноОбновлениеЧерезИнтернет", НСтр("ru = '(отключено обновление через интернет)'; en = '(online update is disabled)'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_СоздаютсяНапрямуюВСправочнике", НСтр("ru = 'Создаются напрямую в справочнике'; en = 'Created directly in catalog'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ЗагружаютсяИзФайла", НСтр("ru = 'Загружаются из файла'; en = 'Imported from file'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ЗагружаютсяЧерезИнтернетПоПротоколу", НСтр("ru = 'Загружаются через интернет по протоколу'; en = 'Importing via the Internet by protocol'"));

	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_СписокЗначенийКлассификатора", НСтр("ru = 'Список значений классификатора'; en = 'Classifier value list'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ТребуетсяОбновление", НСтр("ru = 'Требуется обновление'; en = 'Update required'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ДанныеОтсутствуют", НСтр("ru = 'Данные не требуют обновления'; en = 'Data not required update'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ДанныеАктуальны", НСтр("ru = 'Данные актуальны'; en = 'Relevant data'"));
...
"или закройте окно вручную, до выхода из программы.'; en = 'Data is not saved."
"Click [save and close]"
"or close the window manually before exiting the program.'");
		КонецЕсли;
	Иначе
	КонецЕсли;
...
Процедура КомандаЗагрузитьНовости(Команда)
	...
	ФормаСостоянияОперации = ОткрытьФорму(
		...
		Новый Структура("Заголовок, СостояниеОперации", 
			НСтр("ru = 'Загрузка новостей'; en = 'Import news'"),
			НСтр("ru = 'Происходит загрузка новостей"
"Это может занять некоторое время...'; en = 'News is being imported"
...
Процедура КомандаЗагрузитьСтандартныеСписки(Команда)
	...
	ФормаСостоянияОперации = ОткрытьФорму(
		"Обработка.УправлениеНовостями.Форма.ФормаСостоянияОперации",
		Новый Структура("Заголовок, СостояниеОперации", 
			НСтр("ru = 'Обновление классификаторов с сервера'; en = 'Update classifiers from the server'"),
			НСтр("ru = 'Происходит обновление классификаторов с сервера"
"Это может занять некоторое время...'; en = 'Updating classifiers from the server"
"This might take a while...'")),
		ЭтотОбъект,
...
""
"Обновление дат актуальности классификаторов'; en = 'Updates of classifiers from the server"
"It may take some time..."
""
"Update relevance date of classifiers'"),
		ИдентификаторФормыСостоянияОперации);
...
"Это может занять некоторое время..."
""
"Обновление классификаторов'; en = 'Updating of classifiers from the server"
"It may take some time..."
""
"Updating of classifiers'"),
		ИдентификаторФормыСостоянияОперации);
...
""
"Данные получены."
"Обновление классификаторов на клиенте...'; en = 'Updates of classifiers from the server"
"It may take some time..."
""
"Data are obtained."
"Update classifiers on the client...'"),
		ИдентификаторФормыСостоянияОперации);
...
""
"Данные получены."
"Обновление лент новостей на клиенте...'; en = 'Updates of classifiers from the server"
"It may take some time..."
""
"Data are obtained."
"Update news feeds on the client...'"),
		ИдентификаторФормыСостоянияОперации);
	Элементы.ЛентыНовостей.Обновить();
...
Процедура КомандаПолучитьОтладочнуюИнформациюОНовости(Команда)
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.УИННовости) Тогда
		...
		#Если ВебКлиент Тогда
			...
			ТекстовыйДокумент.Показать(НСтр("ru = 'Отладочная информация о новости'; en = 'Debugging info about the news'"));
		...
		#Иначе
			...
			ТекстовыйДокумент.УстановитьТекст(ИнформацияОНовости);
			ТекстовыйДокумент.Показать(НСтр("ru = 'Отладочная информация о новости'; en = 'Debugging info about the news'"), ИмяВременногоФайла);
...
Процедура КомандаДиагностикаПолученияНовостей(Команда)
	...
	Для Каждого ТекущийШаг Из СписокШагов Цикл
		Состояние(
			НСтр("ru = 'Выполнение проверки'; en = 'Testing'"),
...
"%2'")
	...
	#Если ВебКлиент Тогда
		...
		ТекстовыйДокумент.Показать(НСтр("ru = 'Результат диагностики получения новостей'; en = 'Diagnosis result of receiving news'"));
	...
	#Иначе
		...
		ТекстовыйДокумент = Новый ТекстовыйДокумент; // АПК:561 форма не открывается в мобильном клиенте.
		ТекстовыйДокумент.УстановитьТекст(СтрСоединить(РезультатДиагностикиПолученияНовостей, Символы.ПС));
		ТекстовыйДокумент.Показать(НСтр("ru = 'Результат диагностики получения новостей'; en = 'Diagnosis result of receiving news'"), ИмяВременногоФайла);
...
Процедура УстановитьУсловноеОформление()
	...
	// 1. Частота обновления = 0 (обновлять вручную), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Обновлять только вручную'; en = 'Update only manually'"));
	...
	// 2. Частота обновления = 1 (Ежедневно), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Ежедневно'; en = 'Daily'"));
	...
	// 3. Частота обновления = 2 (Ежечасно), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Ежечасно'; en = 'Every hour'"));
	...
	// 4. Частота обновления = 3 (Каждые 15 минут), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Каждые 15 минут'; en = 'Every 15 minutes'"));
	...
	// 5. Частота обновления = 4 (Каждую минуту), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Каждую минуту'; en = 'Every minute'"));
	...
	// 6. Частота обновления = не требуется, т.к. это локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Не требуется'; en = 'Not required'"));
	...
	// 7. Дата последнего обновления = пустая, то вывести текст "никогда не обновлялся".
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'никогда не обновлялся'; en = 'never updated'"));
	...
	// 12. "Требуется обновление" - красным, "Данные актуальны", "Данные не требуют обновления" - серым;
		...
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = НСтр("ru = 'Данные актуальны'; en = 'Relevant data'");

		ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТребуетсяОбновление");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = НСтр("ru = 'Данные не требуют обновления'; en = 'Data not required update'");
	...
	// 13. "Требуется обновление" - красным, "Данные актуальны", "Данные не требуют обновления" - серым;
		...
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТребуетсяОбновление");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = НСтр("ru = 'Требуется обновление'; en = 'Update required'");
...
Функция ВыполнитьШагПроверкиНаСервере(Шаг, ПрочиеПараметры)
	...
	ИначеЕсли Шаг = 5 Тогда
		...
		Если ПустаяСтрока(Результат) Тогда
			Результат = Символы.Таб + НСтр("ru = 'Подсистема Новости внедрена правильно'; en = 'News subsystem is embedded correctly'")
				+ Символы.ПС;
		КонецЕсли;
...
Функция ВыполнитьШагПроверки_ЗаполненностьОбъектовМетаданных()
	...
	ЛогШаговВыполнения.Добавить("  " + НСтр("ru = 'Категории новостей (первые 20 значений):'; en = 'News categories (the first 20 values):'"));
	Если НЕ РезультатЗапроса[0].Пустой() Тогда
		...
		КонецЦикла;
	Иначе
		ЛогШаговВыполнения.Добавить("    " + НСтр("ru = 'Нет данных'; en = 'No data'"));
	КонецЕсли;
	ЛогШаговВыполнения.Добавить("");

	ЛогШаговВыполнения.Добавить("  " + НСтр("ru = 'Значения категорий новостей (первые 20 значений):'; en = 'Value of the news categories (the first 20 values):'"));
	...
	ЛогШаговВыполнения.Добавить("  " + НСтр("ru = 'Ленты новостей (первые 20 значений):'; en = 'News feed (the first 20 values):'"));
...
#Область АдминистративныеОтборы
	...
	Если НЕ РезультатЗапроса[0].Пустой() Тогда
		...
		КонецЦикла;
	Иначе
		Список.Добавить("    " + НСтр("ru = 'Нет данных'; en = 'No data'"));
	КонецЕсли;

	СписокСтрокой = ОбработкаНовостейКлиентСервер.ВставитьОтступВМногострочнуюСтроку(Список, Символы.Таб, Истина, Ложь);

	ЛогШаговВыполнения.Добавить("### " + НСтр("ru = 'Отборы, настроенные администратором (первые 40 значений):'; en = 'Selections configured by the administrator (first 40 values):'"));
...
#Область ОтборыДляОбластиДанных
	...
	ЛогШаговВыполнения.Добавить("### " + НСтр("ru = 'Отборы, настроенные для областей данных (первые 40 значений):'; en = 'Filters configured for the data areas (the first 40 values):'"));
	ЛогШаговВыполнения.Добавить(СписокСтрокой);
...
#Область ПользовательскиеОтборы
	...
	ЛогШаговВыполнения.Добавить("### " + НСтр("ru = 'Отборы, настроенные пользователями (первые 40 значений):'; en = 'Selections configured by users (first 40 values):'"));
	ЛогШаговВыполнения.Добавить(СписокСтрокой);
...
"Параметр запуска /C: %4'; en = 'Environment parameters of the news subsystem:"
"%1"
...
Функция СформироватьПолныйАдресЛентыНовостей(ЛокальнаяСсылка, ЛокальныйПротокол, ЛокальныйСайт, ЛокальноеИмяФайла)
	...
	Если СтрНайти(ВРег(ЛокальныйПротокол), ВРег("http")) = 1 Тогда // http, https
		Если ПустаяСтрока(ЛокальныйСайт) Тогда
			Результат = НСтр("ru = 'Не заполнено поле Сайт'; en = 'The Website field is required'");
		ИначеЕсли ПустаяСтрока(ЛокальноеИмяФайла) Тогда
			Результат = НСтр("ru = 'Не заполнено поле Имя файла'; en = 'The File name field is required'");
		...
		КонецЕсли;
	Иначе
		Результат = НСтр("ru = 'Адрес ленты новостей можно получить только для протоколов http и https'; en = 'News feed address can be received only for protocols http and https'");
	КонецЕсли;
...
#Область ОченьВажныеНовости
	...
	Если (ТипЗнч(ОченьВажныеНовостиСВключеннымиНапоминаниями) = ТипМассив)
		...
		КонецЦикла;
	Иначе
		Список.Добавить(НСтр("ru = 'Пустой.'; en = 'Empty.'"));
	КонецЕсли;

	СписокСтрокой = ОбработкаНовостейКлиентСервер.ВставитьОтступВМногострочнуюСтроку(Список, Символы.Таб, Истина, Ложь);

	ЛогШаговВыполнения.Добавить("### " + НСтр("ru = 'Список очень важных новостей:'; en = 'List of very important news:'"));
	ЛогШаговВыполнения.Добавить(СписокСтрокой);
...
#Область ВажныеНовости
	...
	ЛогШаговВыполнения.Добавить("### " + НСтр("ru = 'Список важных новостей:'; en = 'List of important news:'"));
	ЛогШаговВыполнения.Добавить(СписокСтрокой);
...
Функция ВыполнитьШагПроверки_ПроверкаHTTPСоединения()
	...
	ЛогШаговВыполнения.Добавить(НСтр("ru = 'Получение новостей по первым нескольким лентам новостей:'; en = 'Receive news on the first few news feeds:'"));
...
"%1Заголовки HTTP ответа:"
"%3"
"%1Имя временного файла: %4'; en = '%1Unfortune attempt to receive news file."
"%1HTTP Response Code: %2"
"%1Head of the HTTP response:"
"%3"
...
"%1'")
		...
		КонецЦикла;
	Иначе
		ЛогШаговВыполнения.Добавить("    " + НСтр("ru = 'Нет подходящих лент новостей'; en = 'No matching newsfeeds'"));
	КонецЕсли;
...
" на любом из серверов, поэтому проверять свободное место необходимо на всех серверах.'; en = 'Auto check is NOT implemented."
"required manually verify that there is enough places for storage files in the following path:"
"%1"
"Also is required to remember that in the case of use of a server cluster the news receive can be accessed"
"on any of the servers so check free location required on all servers.'"),
			ИмяВременногоФайла);
...
"%2'; en = 'Name of form: %1"
"Error receiving data of attribute News:"
"%2'")
						+ Символы.ПС,
					...
					Если ТекущаяИнформация.Значение.Новости.Количество() > 0 Тогда
						...
						Для Каждого ТекущаяНовость Из ТекущаяИнформация.Значение.Новости Цикл
							...
							СписокПервыхНовостей.Добавить(
								...
								+ ?(ТекущаяНовость.Прочтена, НСтр("ru = 'Прочтена'; en = 'Read'"), НСтр("ru = 'НЕ прочтена'; en = 'NOT read'")) + "; ");
						...
						КонецЦикла;
					Иначе
						СписокПервыхНовостей.Добавить(НСтр("ru = 'Пустой список контекстных новостей.'; en = 'An empty list of contextual news.'"));
					КонецЕсли;
				Иначе
					СписокПервыхНовостей.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'В реквизите Новости ключ массива контекстных новостей Новостей неправильного типа: %1.'; en = 'In news attribute, the key of contextual News is of wrong type: %1.'"),
							ТипЗнч(ТекущаяИнформация.Значение.Новости)));
				КонецЕсли;
			Иначе
				СписокПервыхНовостей.Добавить(НСтр("ru = 'В реквизите Новости отсутствует ключ массива контекстных новостей Новостей.'; en = 'In attribute News there is no key to the array of news context news.'"));
			КонецЕсли;
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Имя формы: %1"
...
"	Form: %3"
"	ЗаголовокФормыКонтекстныхНовостей: %4"
"	Total contextual news: %5"
"	A few random contextual news:"
"%6"
"	ПолучатьНовостиНаСервере: %7"
"	ХранитьМассивНовостейТолькоНаСервере: %8'")
		...
		Иначе // Неизвестные данные
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Имя формы: %1"
"Ошибка получения данных реквизита Новости. Неправильный тип данных: %2'; en = 'Name of form: %1"
"Error obtaining data of attribute News. Wrong data type: %2'")
						+ Символы.ПС,
					ТекущаяИнформация.Представление,
