...
Процедура ВнешнееСоединениеВыполнитьДействияПоНастройкеНовогоОбменаДанными(Отказ, 
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ru = 'При создании настройки синхронизации данных возникли ошибки.'; en = 'When creating data synchronization setting, errors occurred.'"));
		КонецЕсли;
...
"Возможно, информационная база не является подчиненным узлом в РИБ.'; en = 'Main node for the current infobase is not determined."
...
"В системе уже существует синхронизация данных для информационной базы (программы) с указанным префиксом.'; en = 'The value of the prefix of the first information base is not unique."
"The system already has data synchronization for the information database (program) with the specified prefix.'");
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуПараметровМастера(Отказ, СтрокаXML) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("ru = 'Синхронизация данных с этой программой в режиме сервиса не предусмотрена.'; en = 'Sync data with this program in service mode is not provided.'");
