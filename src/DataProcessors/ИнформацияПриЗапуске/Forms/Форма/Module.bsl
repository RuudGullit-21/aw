...
Функция ПодготовитьДанныеФормы()
	...
	Для Каждого ПакетСтраниц Из ПодготовленныеПакеты Цикл
		ПакетСтраниц.ЗаголовокФормы = НСтр("ru = 'Информация'; en = 'Information'");
		...
		Если СтрНачинаетсяС(ПакетСтраниц.Раздел, "_") Тогда
			...
			Иначе
				...
				Если ТекущееПодменю = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдена группа ""%1""'; en = 'Group ""%1"" is not found'"), ИмяПодменю);
				КонецЕсли;
		...
		ИначеЕсли НаименованиеТекущегоРаздела <> ПакетСтраниц.Раздел Тогда
			...
			ЭтоГлавная = (ПакетСтраниц.Раздел = НСтр("ru = 'Главная'; en = 'Main'"));
