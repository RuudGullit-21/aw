...
Процедура ГиперссылкаОтложенноеОбновлениеНажатие(Элемент)
	...
	Иначе
		ТекстПредупреждения = НСтр("ru = 'Обработка данных еще не выполнялась.'; en = 'Data has not been processed yet.'");
...
"т.к. процедуры отработки данных могут зависеть друг от друга.'; en = 'Stopping of additional processing procedures might prevent"
"your application from operating. You should do so only if an error"
"is detected in the data processing procedure and after"
"consulting with the technical support as these procedures"
...
"%2'");
	...
	ИначеЕсли СтрокаОбработчика.Статус = Перечисления.СтатусыОбработчиковОбновления.Выполняется Тогда
		...
		ПредставлениеСтатусаВыполнения = НСтр("ru = 'в данный момент выполняется'; en = 'is in progress now'");
		Если Прогресс <> Неопределено Тогда
			ШаблонСтатус = НСтр("ru = 'Выполняется (%1%)'; en = 'Executed (%1%)'");
			СтрокаСписка.СтатусПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтатус, Прогресс)
		Иначе
			СтрокаСписка.СтатусПредставление = НСтр("ru = 'Выполняется'; en = 'Выполняется'");
		КонецЕсли;
	ИначеЕсли СтрокаОбработчика.Статус = Перечисления.СтатусыОбработчиковОбновления.Ошибка Тогда
		
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ru = 'Процедура ""%1"" обработки данных завершилась с ошибкой:'; en = 'Data processing procedure ""%1"" is completed with error:'") + Символы.ПС + Символы.ПС;
	...
	ИначеЕсли СтрокаОбработчика.Статус = Перечисления.СтатусыОбработчиковОбновления.Приостановлен Тогда
		...
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ru = 'остановлена администратором'; en = 'stopped by administrator'");
		СтрокаСписка.СтатусПредставление = НСтр("ru = 'Остановлено'; en = 'Stopped'");
	Иначе
		
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ru = 'еще не выполнялась'; en = 'not executed yet'");
...
Функция ДлительностьВыполненияПроцедурыОбновления(ДлительностьВыполнения)
	...
	ШаблонСекунды = НСтр("ru = '%1 сек.'; en = '%1 sec.'");
	ШаблонМинуты = НСтр("ru = '%1 мин. %2 сек.'; en = '%1 min %2 sec'");
