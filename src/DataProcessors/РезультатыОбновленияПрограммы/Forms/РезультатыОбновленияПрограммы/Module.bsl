...
"""Информация"" на рабочем столе, команда ""Описание изменений программы"".'; en = 'You can also monitor the application version data progress in section"
"""Information"" on the desktop, command ""Application release notes"".'");
	...
	Иначе
		ЗаголовокОбновлениеЗавершено = НСтр("ru = 'Версия программы успешно обновлена на версию %1'; en = 'Program version was successfully updated to version %1'");
	...
	Иначе
		...
		ШаблонЗаголовка = НСтр("ru = 'Дополнительные процедуры обработки данных завершены %1 в %2'; en = 'Additional data processor procedures are completed %1 at %2'");
...
Процедура ПроблемныеСитуацииНажатие(Элемент)
	...
	ОтборЖурнала.Вставить("СобытиеЖурналаРегистрации", НСтр("ru = 'Обновление информационной базы'; en = 'Infobase update'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()));
...
Функция СообщениеОРезультатахОбновления()
	...
	Если Прогресс.ВсегоОбработчиков = Прогресс.ВыполненоОбработчиков Тогда
		...
		Если Прогресс.ВсегоОбработчиков = 0 Тогда
			...
			ТекстСообщения = "";
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Все процедуры обновления выполнены успешно (%1)'; en = 'All update procedures are completed successfully (%1)'"), Прогресс.ВыполненоОбработчиков);
		КонецЕсли;
		Элементы.КартинкаЗавершено.Картинка = БиблиотекаКартинок.Успешно32;
		ОбновлениеЗавершеноУспешно = Истина;
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не все процедуры удалось выполнить (выполнено %1 из %2)'; en = 'Not all procedures were executed (%1 is executed out of %2)'"), 
...
Процедура ИзменитьРасписаниеПослеУстановкиРасписания(НовоеРасписание, ДополнительныеПараметры) Экспорт
	...
	Если НовоеРасписание <> Неопределено Тогда
		Если НовоеРасписание.ПериодПовтораВТечениеДня = 0 Тогда
			...
			КнопкиВопроса.Добавить("НастроитьРасписание", НСтр("ru = 'Настроить расписание'; en = 'Configure schedule'"));
			КнопкиВопроса.Добавить("РекомендуемыеНастройки", НСтр("ru = 'Установить рекомендуемые настройки'; en = 'Set recommended settings'"));
...
"и заполнить поле ""Повторять через"".'; en = 'Additional data processor procedures are executed in small portions,"
