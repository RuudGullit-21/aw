...
Процедура ПриРаспознанииДокумента(ВидСообщения, ЭлементСхемы) Экспорт
	...
	Если ВидСообщения.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН
		Или ВидСообщения.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗН
		Или ВидСообщения.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСВ
		Или ВидСообщения.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗЗ
		Или ВидСообщения.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЛ
		Или ВидСообщения.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭДФ Тогда
				...
				Для Каждого РеквизитКиЗ Из ПоследняяВерсия Цикл
					Если СтрЗаканчиваетсяНа(РеквизитКиЗ.Ключ, "СсылкаСведенияОбОпасномГрузе")
						...
						Если ТипЗнч(РеквизитКиЗ.Значение) = Тип("СправочникСсылка.ХранимыеДанныеЭПД") Тогда
							...
							ОбменСГИСЭПДКлиентСервер.ЗаполнитьРеквизитыКрассификатораДОПОГ(РеквизитКиЗ.Значение, "", СтруктураДОПОГ);
...
// 
// Возвращаемое значение:
//  Соответствие - ключ ПеречислениеСсылка.ТипыЭлементовРегламентаЭДО, значение массив полей группировки
...
Процедура ПриСозданииНаСервереПодчиненнойФормы(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	ОписаниеРеквизитовФормы = ОписаниеРеквизитовФормы(Форма);
...
Функция ОписаниеРеквизитовФормы(Форма) Экспорт
	...
	НеСтроковыеТипы = Новый Структура;
		...
		Иначе
			...
			ИначеЕсли НЕ СтрНачинаетсяС(РеквизитФормы.Имя, "ОбщееПолеВвода") Тогда
				...
				Если НеСтроковыйТип <> "" Тогда
					НеСтроковыеТипы.Вставить(РеквизитФормы.Имя, НеСтроковыйТип);	
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Результат.Вставить("НеСтроковыеТипы", НеСтроковыеТипы);
...
Функция ПолучитьРеквизитыФизЛица(ФизЛицо) Экспорт
	...
	Если ЗначениеЗаполнено(ФизЛицо) Тогда
		...
		ЭлектронноеВзаимодействиеПереопределяемый.ПолучитьДанныеФизЛица(ФизЛицо, СведенияОбУчастнике);
...
Функция НайтиСоздатьЭПД(ДеревоДляСовместимостиСБЭД, ДеревоРазбора, ПараметрыОбработки, НайденныйОбъект)
	...
	Если КлючеваяИнформацияПоТитулу.Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул1 Тогда
		...
		Если ДокументОбъект.РольУчастника = 1 Тогда
			...
			Иначе
				...
				НастройкиОбменаСТретьимУчастником = НастройкиОтраженияВУчетеЭДО.ПолучитьНастройкиПолученияПоИдентификаторам(
		...
		Или КлючеваяИнформацияПоТитулу.Титул = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул7 Тогда
			...
			ИначеЕсли ДокументОбъект.РольУчастника = 3 Тогда
				...
				Иначе
					ОрганизацияПолучателяПоНастройкам = Неопределено;
					КонтрагентТретьегоУчастника = Неопределено;
					НастройкиОбменаСГрузополучателем = НастройкиОтраженияВУчетеЭДО.ПолучитьНастройкиПолученияПоИдентификаторам(
																											ИдентификаторГрузополучателя,
																											ПараметрыОбработки.ИдентификаторПолучателя, 
																											ОрганизацияПолучателяПоНастройкам, 
																											КонтрагентТретьегоУчастника, Истина);	
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Ссылка) Тогда
		...
		Если Выборка.Следующий() Тогда 
			Если ЗначениеЗаполнено(Выборка.НомерЭтойВерсии) И Выборка.НомерЭтойВерсии > -1 Тогда
...
Процедура ЗаполнитьПредставленияСсылочныхТипов(ДокументОбъект)
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЭлектронныйЗаказЗаявка") Тогда
		...
		ТаблицаУполномоченныхЛиц = ОбменСГИСЭПДКлиентСервер.МассивСтрокТаблицыИзСтруктурыРеквизитов(СтруктураРеквизитов, "ТитулГрузоотправителяУполномоченныеЛица");
		ИтераторЦикла = 1;
...
Функция ОписаниеСообщенияОтправителя(ОбъектУчетаИлиЭД, НастройкиОтправки = Неопределено) Экспорт
	...
	Если ТипЗнч(ОбъектУчетаИлиЭД) = Тип("ДокументСсылка.ЭлектронныйДокументВходящийЭДО")
		...
		КонецЕсли;
		ИдентификаторОтправителя = ОбъектУчетаИлиЭД.ИдентификаторОрганизации;
		ИдентификаторПолучателя = ОбъектУчетаИлиЭД.ИдентификаторКонтрагента;
		ВидДокумента = ОбъектУчетаИлиЭД.ВидДокумента;
...
Функция СформироватьДанныеПрикладногоДокумента(ОбъектУчета, Данные)
	...
	ИначеЕсли ТипЗнч(ОбъектУчета) = Тип("ДокументСсылка.ЭлектронныйЗаказНаряд") Тогда
		...
		ДанныеСообщения.Содержание.НомерДокумента = ?(ЗначениеЗаполнено(ОбъектУчета.ТитулФрахтователяПорядковыйНомерЗаказНаряда),
														ОбъектУчета.ТитулФрахтователяПорядковыйНомерЗаказНаряда,
														ОбъектУчета.Номер);
		ДанныеСообщения.Содержание.ДатаДокумента = ?(ЗначениеЗаполнено(ОбъектУчета.ТитулФрахтователяДатаСоставленияЗаказНаряда),
														ОбъектУчета.ТитулФрахтователяДатаСоставленияЗаказНаряда,
														ОбъектУчета.Дата);
	ИначеЕсли ТипЗнч(ОбъектУчета) = Тип("ДокументСсылка.ЭлектроннаяСопроводительнаяВедомость") Тогда
		...
		ДанныеСообщения.Содержание.НомерДокумента = ?(ЗначениеЗаполнено(ОбъектУчета.ТитулПеревозчикаПорядковыйНомерСопроводительнойВедомости),
														ОбъектУчета.ТитулПеревозчикаПорядковыйНомерСопроводительнойВедомости,
														ОбъектУчета.Номер);
		ДанныеСообщения.Содержание.ДатаДокумента = ?(ЗначениеЗаполнено(ОбъектУчета.ТитулПеревозчикаДатаСоставленияСопроводительнойВедомости),
														ОбъектУчета.ТитулПеревозчикаДатаСоставленияСопроводительнойВедомости,
														ОбъектУчета.Дата);
	ИначеЕсли ТипЗнч(ОбъектУчета) = Тип("ДокументСсылка.ЭлектронныйЗаказЗаявка") Тогда
		...
		ДанныеСообщения.Содержание.НомерДокумента = ?(ЗначениеЗаполнено(ОбъектУчета.ТитулГрузоотправителяЗаказНомер),
														ОбъектУчета.ТитулГрузоотправителяЗаказНомер,
														ОбъектУчета.Номер);
		ДанныеСообщения.Содержание.ДатаДокумента = ?(ЗначениеЗаполнено(ОбъектУчета.ТитулГрузоотправителяЗаказДата),
														ОбъектУчета.ТитулГрузоотправителяЗаказДата,
														ОбъектУчета.Дата);													
	...
	ИначеЕсли ТипЗнч(ОбъектУчета) = Тип("ДокументСсылка.ЭлектронныйДоговорФрахтования") Тогда
		...
		ДанныеСообщения.Содержание.НомерДокумента = ОбъектУчета.Номер;
		ДанныеСообщения.Содержание.ДатаДокумента = ОбъектУчета.Дата;
	КонецЕсли;
...
Функция ДвоичныеДанныеДокументаЭПД(Объект, Данные = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Если Объект.ВидДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН Тогда
		МенеджерОбъекта = Документы.ЭлектроннаяТранспортнаяНакладная;
	...
	ИначеЕсли Объект.ВидДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗН Тогда
		МенеджерОбъекта = Документы.ЭлектронныйЗаказНаряд;
	ИначеЕсли Объект.ВидДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСВ Тогда
		МенеджерОбъекта = Документы.ЭлектроннаяСопроводительнаяВедомость;
		
		МассивВозможныхПустыхЗначений.Добавить("//Файл/Документ/СодСВИнфПрв/СвКонтейн/ПрТаблПломб");
	ИначеЕсли Объект.ВидДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗЗ Тогда
		МенеджерОбъекта = Документы.ЭлектронныйЗаказЗаявка;
	...
	ИначеЕсли Объект.ВидДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЛ Тогда
		МенеджерОбъекта = Документы.ЭлектронныйПутевойЛист;
	ИначеЕсли Объект.ВидДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭДФ Тогда
		МенеджерОбъекта = Документы.ЭлектронныйДоговорФрахтования;
	КонецЕсли;
...
Процедура ПослеОтправкиОбъекта(Объект, ТранспортныйКонтейнер, ОшибкаПередачи, КонтекстДиагностики, Отказ, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("ДокументСсылка.СообщениеЭДО") Тогда
		Если Объект.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул2
			Или Объект.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул7
			Или Объект.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул8
			Или Объект.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул7
			Или Объект.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_ДопТитул8 Тогда
			...
			Если ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
				Запись = РегистрыСведений.ОтправкиQRКодовЭПД.СоздатьМенеджерЗаписи(); 
				Запись.Отправитель = ЭлектронныйДокумент.ИдентификаторОрганизации;
	...
	Если ТипЗнч(Объект) = Тип("ДокументСсылка.СообщениеЭДО") Тогда
		...
		Если ДанныеОбъекта.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул2
			...
			Если ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
				...
				НаборЗаписей.Отбор.Отправитель.Установить(ЭлектронныйДокумент.ИдентификаторОрганизации);
				НаборЗаписей.Отбор.ЭлектронныйДокумент.Установить(ЭлектронныйДокумент);
				НаборЗаписей.Прочитать();
...
Функция ДанныеСертификатаУчетнойЗаписиЭДО(ИдентификаторУчетнойЗаписи, ОтборПодписанта = Неопределено) Экспорт
	...
	|		ПО СертификатыУчетныхЗаписейЭДО.Доверенность = МашиночитаемыеДоверенностиОрганизаций.Ссылка
...
Функция ОсобоеОформлениеДляЗаказаЗаявки(ПутьКЭлементуФормы, ОбъектСДаннымиДокумента, СтруктураРеквизитов)
	...
	ИначеЕсли ПутьКЭлементуФормы = "ЗаполнитьТитулГрузоотправителяДоговор" Тогда
		...
		Заголовок = СтрШаблон("%1 %2 от %3", Наименование, Номер, Формат(Дата, "ДФ=dd.MM.yyyy"));
