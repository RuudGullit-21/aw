...
Функция ПроверитьПодписьПоМЧД(ФайлСообщения, НаправлениеСообщения, ОписаниеФайла, СвойстваПодписи,
	...
	Если Не ЗначениеЗаполнено(НомераМЧД) Тогда
		...
		Если ЗначениеЗаполнено(СтарыйРезультатПроверки) И ЗначениеЗаполнено(СтарыйРезультатПроверки.Доверенность) Тогда
			...
			НомерДоверенности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтарыйРезультатПроверки.Доверенность,
				"НомерДоверенности");
			...
			ИННДоверителя = СведенияОбУчастникеЭДО.ИНН;
			
			СоответствиеИННДоверителяСНомерамиМЧД.Вставить(НомерДоверенности, ИННДоверителя);
	...
	Если НаправлениеСообщения = Перечисления.НаправленияЭДО.Интеркампани Тогда
		...
		ХешПодписи = КриптографияБЭД.ХешПодписи(СвойстваПодписи.Подпись);
		ИННДоверителя = МашиночитаемыеДоверенности.ИННДоверителяЭлектроннойПодписиПоМЧД(ХешПодписи,
	...
	Если ЗначениеЗаполнено(ИННДоверителя)
		И Не ТребуетсяМашиночитаемаяДоверенностьПоДаннымДокумента(ИННДоверителя, СвойстваПодписи.Сертификат) Тогда
		РезультатПроверки = МашиночитаемыеДоверенности.НовыйРезультатПроверкиПодписи();
		РезультатПроверки.ПроверкаВыполнена = Истина;
		Возврат РезультатПроверки;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НомераМЧД) Тогда
		...
		Результат = МашиночитаемыеДоверенности.НомераДоверенностейПоУчастникам(
			ИННДоверителя, ИННПредставителя, ЭлектронныйДокумент);
...
Функция СформироватьОтвет(ЭлектронныйДокумент, КонтекстДиагностики, ДанныеОбъектаУчета = Неопределено)
	...
	Если ЭтоВходящийЭДО Тогда
		Если Не ФорматыЭДО.ВозможноФормированиеОтветногоТитулаБезУчетногоДокумента(
			ПараметрыДокумента.ВидДокументаТипДокумента) Тогда
			
			МассивЭД = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ЭлектронныйДокумент);
			ОбъектыУчетаЭД = ИнтеграцияЭДО.ОбъектыУчетаАктуальныхЭлектронныхДокументов(МассивЭД);
			
			Если ОбъектыУчетаЭД.Количество() = 0 Тогда
				Результат.Отказ = Истина;
