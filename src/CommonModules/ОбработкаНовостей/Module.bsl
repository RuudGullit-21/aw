...
Процедура УстановитьСостояниеНовостейДляПользователей(
	...
	Если (МассивПользователей.Количество() = 0) Тогда
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", НСтр("ru = 'По всем'; en = 'All'"));
	Иначе
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%2", НСтр("ru = 'Передано значения типа, отличного от Структуры'; en = 'Passed value type other than Structure'"));
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%3", НСтр("ru = 'Передано значения типа, отличного от Структуры'; en = 'Passed value type other than Structure'"));
	КонецЕсли;
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
		НСтр("ru = 'Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей'; en = 'News. Service and regulation. SetStateNewsForUsers'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
...
"Состояния новостей можно установить только для пользователей с ролью ЧтениеНовостей или ПолныеПрава'; en = 'Execution error: no users for whom it is required to set news states."
...
"Возможные имена ключей совпадают с наименованиями ресурсов регистра сведений СостоянияНовостей'; en = 'УстановитьСостояниеНовостейДляПользователей execution error: as values to fill is transferred value type that is different from the Structure or fill settings are not filled (not listed no one structure keys)."
"Possible key names match the descriptions of resources information register СостоянияНовостей'");
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей'; en = 'News. Service and regulation. SetStateNewsForUsers'", ОбщегоНазначения.КодОсновногоЯзыка()) , // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
Процедура ИзменитьПользовательскуюВидимостьЛентНовостей(ЛентыНовостей, Видимость = Ложь, Пользователь = Неопределено) Экспорт
	...
	Если ТипЗнч(ТекущийПользователь) <> ТипПользователь Тогда // Внешний пользователь, например.
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Передан параметр Пользователь неправильного типа: %1.'; en = 'Parameter passed to User of wrong type: %1.'"),
			ТипЗнч(Пользователь));
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Передан параметр ЛентыНовостей неправильного типа: %1.'; en = 'Parameter passed to ЛентыНовостей has wrong type: %1.'"),
...
Процедура ИзменитьВидимостьЛентНовостейДляОбластиДанных(
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если НЕ ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			...
			Иначе
				Если Видимость = Ложь Тогда
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'ИзменитьВидимостьЛентНовостейДляОбластиДанных: передан параметр МассивОбластейДанных нулевого размера или неправильного типа: %1.'; en = 'ИзменитьВидимостьЛентНовостейДляОбластиДанных: passed parameter МассивОбластейДанных of size zero or of the wrong type: %1.'"),
		...
		Иначе
			...
			Если (ТипЗнч(МассивОбластейДанных) = ТипМассив)
					И (МассивОбластейДанных.Количество() > 0) Тогда
				ТекстСообщения = НСтр("ru = 'ИзменитьВидимостьЛентНовостейДляОбластиДанных: для разделенного сеанса нельзя передавать параметр МассивОбластейДанных.'; en = 'ИзменитьВидимостьЛентНовостейДляОбластиДанных: for the split session, not allowed to pass a parameter МассивОбластейДанных.'");
				ВызватьИсключение ТекстСообщения;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Передан параметр МассивЛентНовостей неправильного типа: %1.'; en = 'parameter passed to МассивЛентНовостей of wrong type: %1.'"),
		...
		Если (Пользователи.ЭтоСеансВнешнегоПользователя()) Тогда
			...
			Если ПравоДоступа("Чтение", Метаданные.Справочники.Новости) Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					+ Символы.ПС;
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru = 'БИП:Новости.Разное'; en = 'ISL:News.Different'", КодОсновногоЯзыка), // ИмяСобытия.
					НСтр("ru = 'Новости. Разное. Ошибка работы с пользователями'; en = 'News. Different. Error of user management'", КодОсновногоЯзыка), // ИдентификаторШага.
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
Процедура ПриИзмененииЗначенийОтборовОбщих() Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			ТекстСообщения = НСтр("ru = 'Невозможно изменить общие отборы в разделенном сеансе.'; en = 'It is impossible to change the selections in the split session.'");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'При изменении значений общих отборов'; en = 'During change the values of  common selections'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. Общие'; en = 'Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. Общие'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
...
Процедура ПриИзмененииЗначенийОтборовДляОбластиДанных() Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'При изменении значений отборов для области данных'; en = 'While changing of selections values for data area'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. ДляОбластиДанных'; en = 'News. Services and regulations. ПриИзмененииЗначенийОтборов. ДляОбластиДанных'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПриИзмененииЗначенийОтборовПользовательских(Пользователь = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'При изменении значений отборов для пользователя'; en = 'While values changing of selections for user'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. Пользовательские'; en = 'News. Services and regulations. ПриИзмененииЗначенийОтборов. Пользовательские'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ru = 'Запускается при первом запуске, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).'; en = 'Started in common data on the first start. Set constants, import standard values, clear information registers (common data).'");
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Комментарий         = НСтр("ru = 'Запускается при первом запуске, для каждой области данных. Установка настроек пользователей (разделенных данных).'; en = 'Started for each data area on the first start. Configure user settings (separated data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.Комментарий         = НСтр("ru = 'Запускается при переходе на новую версию, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).'; en = 'Started in common data on migration to a new version. Set constants, import standard values, clear information registers (common data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Обработчик = Обработчики.Добавить();
			...
			Обработчик.НачальноеЗаполнение     = Ложь;
			Обработчик.Комментарий             = НСтр("ru = 'Запускается при переходе на любую новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).'; en = 'Started for each data area on migration to any new version. Configure user settings (separated data).'");
			Обработчик.РежимВыполнения         = "Оперативно";
	Иначе
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ru = 'Запускается при переходе на новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).'; en = 'Started for each data area on migration to a new version. Configure user settings (separated data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"news.webits.1c.ru",
		443,
		НСтр("ru = 'Веб-сервисы получения новостей'; en = 'Web services of news receipt'"));
...
"%2'; en = 'An error occurred when reading the file with name %1:"
"%2'"),
			ПолноеИмяФайла,
...
Функция СоединитьсяССерверомНовостей(ИмяСервера, Протокол, Логин, Пароль, HTTPСоединение, КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Соединение с сервером новостей'; en = 'Connection with news server'"));
		...
		Если ((ВРег(Протокол) = ВРег("http")))
			...
			Пока (СоединениеУстановлено <> Истина) И (ПопыткаСоединения < НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения) Цикл
					...
					Исключение
						...
						ЛогШаговВыполнения.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Попытка %1 / %2: Ошибка определения порта из имени сервера %3 (%4). Лента новостей будет пропущена.'; en = 'Attempt %1 / %2: Error of definition port of server name, %3 (%4). News feed will be ignored.'"),
								ПопыткаСоединения,
				...
				Попытка
					...
					ЛогШаговВыполнения.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Попытка %1 / %2: Протокол %3, Имя сервера %4, Порт %5'; en = 'Attempt %1 / %2: Protocol %3, Server name, %4, Port %5'"),
							ПопыткаСоединения,
							НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения,
							Протокол,
							ИмяСервера_Сервер,
...
"%5'; en = 'Attempt %1 / %2: failed to connect to server %3 (%4) for more news because:"
"%5'"),
							ПопыткаСоединения,
							НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения,
							ИмяСервера,
		...
		Иначе
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Протокол получения новостей %1 отличается от http и https. Соединение с сервером новостей не устанавливалось.'; en = 'Protocol of receive news %1 differs from http and https. Connection to news server is not installed.'"),
					Протокол));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка новостей'; en = 'ISL:News.Download news'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Загрузка новостей. Соединение с сервером новостей'; en = 'News. Download of news. Connection with the news server'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
""
"----------'; en = '# ОТЛАДОЧНАЯ ИНФОРМАЦИЯ О НОВОСТЯХ"
""
"Дата отчета: **%ТекущаяУниверсальнаяДата%**"
""
"Найдено элементов справочника Новости с UIN **%УИННовости%**: %КоличествоНайденныхНовостей%"
...
"%ПоставляемыеДанные%'; en = '### Новость: %НомерПоПорядку% / %КоличествоНайденныхНовостей%"
""
"#### Элемент справочника новости"
"%ЭлементСправочникаНовости%"
""
...
""
"%ТекстПоставляемыхДанных%'; en = '##### Supplied data identifier %Identificatiedoeleinden%"
""
"Text where UIN (+-200 characters):"
""
"%Textbestandteilen%'")
			+ Символы.ПС
			+ Символы.Таб
...
"%ЭлементСправочникаЛентыНовостей%'; en = '### News: %Nameboards% / %Kolichestvennoe%"
""
"#### element of the dictionary news"
"%Elementproperties%'")
			+ Символы.ПС
			+ Символы.ПС;
...
"%Константы%"
""
"### Other program environment settings"
"%ДругиеПараметрыПрограммногоОкружения%"
""
"### Values of predefined categories"
"%ЗначенияПредопределенныхКатегорий%"
""
"### Session options"
"%ПараметрыСеанса%"
""
"### Configuration information (from metadata) and operating system"
"%ДанныеОКонфигурацииИзМетаданных%"
""
"### Sheduled job schedule"
...
#Область ЗаполнениеШаблонов_Новости_КатегорииНовостейИнтервалыВерсий
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(
					РезультатТекущейНовости,
					"%РегистрСведений." + ИмяРегистраСведений + "%",
					Символы.Таб + НСтр("ru = 'Набор записей пустой.'; en = 'Recordset is empty.'"));
			КонецЕсли;
...
#Область ЗаполнениеШаблонов_Новости_РассчитанныеОтборыПоНовостям_Пользовательские
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(
					РезультатТекущейНовости,
					"%РегистрСведений." + ИмяРегистраСведений + "%",
					НСтр("ru = 'В модели сервиса для администратора системы нельзя получить пользовательские настройки или отборы.'; en = 'In the model of service for the system administrator cannot obtain user settings or selections.'"));
			КонецЕсли;
...
#Область ЗаполнениеШаблонов_Новости_ПоставляемыеДанные
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(РезультатТекущейНовости, "%ПоставляемыеДанные%", Символы.Таб + НСтр("ru = 'Не найдено в поставляемых данных.'; en = 'Not found in supplied data.'"));
			КонецЕсли;
...
#КонецОбласти
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОНовостях%", НСтр("ru = 'Не найдено ни одной новости с таким UIN'; en = 'Not found any news with this UIN'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ЛентыНовостей
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОЛентахНовостей%", НСтр("ru = 'Не найдено ни одной ленты новостей для новости с таким UIN'; en = 'Not found any feeds for news with same UIN'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_Константы
	...
	Иначе
		ПредставлениеНастройкиНовостей = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Настройки новостей необрабатываемого типа %1'; en = 'News settings an unhandled type %1'"),
...
"РазрешенаРаботаСНовостямиЧерезИнтернет: %3'; en = 'Constant.Astrononomical:"
"%1"
"Representationalists: %2"
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ДругиеПараметрыПрограммногоОкружения
	...
	ВременныйТекст = СтрЗаменить(ВременныйТекст, "%ОбластьДанных%",
		?(ПрочиеПараметры.ОбластьДанных = Неопределено,
			НСтр("ru = 'Неопределено'; en = 'Undefined'"),
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ПараметрыСеанса
	...
	Иначе
		ПредставлениеПараметрыОкруженияБИП_Новости = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Параметры окружения подсистемы новостей необрабатываемого типа %1'; en = 'Parameters of news subsystems environment of unhandled type %1'"),
...
"%1'; en = 'Environment parameters of the news subsystem:"
"%1'"),
...
"Version of the subsystem ISL: %3"
"Platform version: %4"
"Operating system: %5"
"Viewer: %6"
"Computer data : %7'"),
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_РасписаниеРегламентногоЗадания
	...
	Если ТекущееРегламентноеЗадание = Неопределено Тогда
		...
		ПредставлениеРегламентноеЗадание_ПоследнееЗадание = НСтр("ru = 'Никогда не выполнялось'; en = 'Never execute'");
	Иначе
		...
		Если ТипЗнч(РегламентноеЗадание_ПоследнееЗадание) = ТипФоновоеЗадание Тогда
			...
			Иначе
				РегламентноеЗадание_СтруктураПоследнегоЗадания.Вставить("ПодробноеПредставлениеОшибки", НСтр("ru = 'Выполнено без ошибок'; en = 'Completed without errors'"));
			КонецЕсли;
		...
		Иначе
			ПредставлениеРегламентноеЗадание_ПоследнееЗадание = НСтр("ru = 'Не выполнялось'; en = 'Not performed'");
		КонецЕсли;
	КонецЕсли;
		...
		Если НЕ ПустаяСтрока(ЛокальнаяСтрокаПоиска) Тогда
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru = 'БИП:Новости.Разное'; en = 'ISL:News.Different'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
					НСтр("ru = 'Новости. Разное. Ошибка полнотекстового поиска'; en = 'News. Different. Error full text search'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
Процедура ВсеОбновленияНовостей() Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Регламентное обновление подсистемы новостей'; en = 'Scheduled update of the news subsystem'"));
	...
	Если МонопольныйРежим() Тогда
		...
		ЕстьОшибки = Истина;
		ОписаниеРезультата = НСтр("ru = 'Принудительное завершение выполнения регламентного задания: информационная база работает в монопольном режиме.'; en = 'Forced completion of the performance of scheduled job: the information base operates in exclusive mode.'");
	КонецЕсли;
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() Тогда
			...
			ОписаниеРезультата = НСтр("ru = 'Разрешена работа с новостями. Выполнение шагов обновления.'; en = 'Allowed to work with the news. Execution of update steps.'");
...
#Область ВсеОбновленияНовостей_ЗагрузкаНовостей
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = '1. Получение и обработка новостей из лент новостей (с обязательным заполнением измерения ЛентаНовостей)'; en = '1. Receive and process news from news feeds (dimension ЛентаНовостей is required)'"));
				...
				Если МассивЛентНовостей.Количество() > 0 Тогда
					ЛогШаговВыполненияВложенный.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Массив лент новостей для получения новостей. Размер = %1. Состав: %2'; en = 'Array of news feeds to receive news. Size = %1. Content: %2'"),
				...
				Иначе
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Нет лент новостей для регламентного обновления'; en = 'No news feeds for scheduled update'"));
				КонецЕсли;
...
#Область ВсеОбновленияНовостей_ОбновлениеКлассификаторов
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				КонтекстВыполненияВложенный,
				"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ВидОбновления, // Идентификатор.
				НСтр("ru = '2. Обновление классификаторов'; en = '2. Update classifiers'"));
				...
				Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
					...
					Если ДатаСледующегоОбновления <= ТекущаяДатаДляРегистров Тогда
						ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Наступил срок выполнения.'; en = 'Came the due date.'"));
						ОбновитьСтандартныеСпискиДляНовостей_Ежедневно(КонтекстВыполненияВложенныйВложенный);
					Иначе
						ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Срок не наступил.'; en = 'Term has not come.'"));
					КонецЕсли;

				Иначе
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Обновление классификаторов через интернет отключено'; en = 'Classifier update via the Internet is disabled'"));
				КонецЕсли;
...
#Область ВсеОбновленияНовостей_АвтоудалениеНовостей
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				КонтекстВыполненияВложенный,
				"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ВидОбновления, // Идентификатор.
				НСтр("ru = '3. Автоудаление новостей'; en = '3. News auto deletion'"));
...
"ДатаСледующегоОбновления: %2'; en = '""TheCurrentDateforRegisters: %1"
"DateOfNextUpdate: %2""'"),
						ТекущаяДатаДляРегистров,
						ДатаСледующегоОбновления));
...
#Область ВсеОбновленияНовостей_ОбновлениеПериодическихСвойств
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				КонтекстВыполненияВложенный,
				"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ВидОбновления, // Идентификатор.
				НСтр("ru = '4. Обновление периодических свойств (важность, актуальность)'; en = '4. Periodic properties update (importance, relevance)'"));
...
#Область ВсеОбновленияНовостей_ОбновлениеПривязокКМетаданным
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				КонтекстВыполненияВложенный,
				"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ВидОбновления, // Идентификатор.
				НСтр("ru = '5. Обновление привязок к метаданным (+ важность)'; en = '5. Metadata link update (+ importance)'"));
...
#Область ВсеОбновленияНовостей_ПроверкаВерсииПлатформы
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				КонтекстВыполненияВложенный,
				"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ВидОбновления, // Идентификатор.
				НСтр("ru = '6. Проверка версии платформы'; en = '6. Platform version check'"));
...
#КонецОбласти
		...
		Иначе
			ОписаниеРезультата = НСтр("ru = 'Запрещена работа с новостями.'; en = 'Prohibited work with news.'");
		КонецЕсли;
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru = 'БИП:Новости.Все обновления новостей'; en = 'LIS:News.All updates of news'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
		НСтр("ru = 'Новости. Все обновления новостей'; en = 'News. All news updates'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
		?(КодРезультата > 0,
			УровеньЖурналаРегистрации.Ошибка,
			УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
		...
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		КомментарийДляЖурналаРегистрации(
			НСтр("ru = 'Все обновления новостей'; en = 'All news updates'"),
			ШагВыполнения,
			КонтекстВыполнения,
...
Процедура УдалитьНовости_Ежедневно(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Удаление новостей (ежедневно)'; en = 'Delete news (daily)'"));

		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() Тогда

			ОписаниеРезультата = НСтр("ru = 'Разрешена работа с новостями'; en = 'Work with news is allowed'");
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = 'Удаление новостей (ежедневно) / УдалениеНовостей_ПолучитьСписокДанныхДляУдаления'; en = 'Delete news (daily) / УдалениеНовостей_ПолучитьСписокДанныхдляудаления'"));
...
"КатегорииНовостейИнтервалыВерсийНеобработанные: %4'; en = 'Collected data for removal:"
"News (for delete): %1"
"News (for installation of deletion mark): %2"
"КатегорииНовостейПростыеНеобработанные: %3"
"КатегорииНовостейИнтервалыВерсийнеобработанные: %4'"),
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = 'Удаление новостей (ежедневно) / УдалениеНовостей_ПометитьНаУдалениеДанные'; en = 'Delete news (daily) / УдалениеНовостей_ПометитьНаУдалениеданные'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. УдалитьНовости_Ежедневно'; en = 'Новости. Сервис и регламент. УдалитьНовости_Ежедневно'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ОбновлениеПериодическихСвойствНовостей(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Обновление периодических свойств новостей'; en = 'Update of news periodic properties'"));
		...
		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() Тогда
			...
			Если НЕ Результат.Пустой() Тогда
				...
				ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Найдено %1 новостей для обновления периодических свойств'; en = 'Found %1 news for periodic properties update'"),
					ВсегоСтрок);
				Пока Выборка.Следующий() Цикл
				...
				КонецЦикла;
			Иначе
				ОписаниеРезультата = НСтр("ru = 'Не найдено данных для обновления'; en = 'Data for update not found'");
			КонецЕсли;
		...
		Иначе
			ОписаниеРезультата = НСтр("ru = 'Работа с новостями запрещена'; en = 'Work with news prohibited'");
		КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей'; en = 'Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
			КомментарийДляЖурналаРегистрации(
				НСтр("ru = 'Обработка периодических реквизитов'; en = 'Processing of periodic attributes'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ОбновлениеПривязокКМетаданным(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() Тогда
			...
			Если НЕ Результат.Пустой() Тогда
				...
				ВсегоСтрок = Выборка.Количество();
				ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Найдено %1 новостей для обновления привязок к метаданным'; en = 'Found %1 news for update of bindings to metadata'"),
					ВсегоСтрок);
				Пока Выборка.Следующий() Цикл
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным'; en = 'Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
			КомментарийДляЖурналаРегистрации(
				НСтр("ru = 'Обработка привязок к метаданным'; en = 'Binding processing to the metadata'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ПолучитьИОбработатьНовостиПоЛентамНовостей(МассивЛентНовостей, КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Получение и обработка новостей по лентам новостей'; en = 'Receive and processing news on newsfeeds'"));
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			// Зашли в конфигурацию под пользователем с разделением (с входом в область данных).
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ОписаниеРезультата = НСтр("ru = 'ПолучитьИОбработатьНовостиПоЛентамНовостей, при работе в модели сервиса запускать получение новостей вручную можно только в неразделенном сеансе.'; en = 'ПолучитьИОбработатьНовостиПоЛентамновостей, when working in the model of service start receiving news manually only in the undivided session.'");
				ЕстьОшибки = Истина;
			КонецЕсли;
		КонецЕсли;
...
"Разрешена работа через интернет: %2'; en = 'Array of feeds for processing: %1"
"Enabled through the Internet: %2'"),
					ОбработкаНовостейКлиентСервер.ПредставлениеЗначения(МассивЛентНовостей, ";"),
				...
				Если НЕ Результат.Пустой() Тогда
					...
					ОписаниеРезультата = ОписаниеРезультата
						+ Символы.ПС
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Запрос выполнен. Количество лент новостей, которые будут обработаны: %1'; en = 'request was fulfilled. number of feeds to be processed: %1'"),
...
"Query result: Empty"
"Perhaps, the list of news feeds is empty, or all news feeds have the ЛокальнаяЛентаНовостей sign (which means that it does not need to download news), or all news feeds have a deletion mark.'"),
							МассивЛентНовостей.Количество())
						+ Символы.ПС;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Все обновления новостей'; en = 'LIS:News.All updates of news'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей'; en = 'News. All news updates. Receiving and processing news by news feeds'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ОбновитьСтандартныеСпискиДляНовостей_Ежедневно(КонтекстВыполнения = Неопределено)
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Обновление классификаторов'; en = 'Update classifiers'"));
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ЛогШаговВыполнения.Добавить(
					НСтр("ru = 'ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, при работе в модели сервиса запускать получение классификаторов вручную можно только в неразделенном сеансе.'; en = 'ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, if work in a service model start obtaining classifiers manually is possible only in the undivided session.'"));
				ЕстьОшибки = Истина;
			КонецЕсли;
		КонецЕсли;
...
"РазрешенаРаботаСНовостямиЧерезИнтернет: %2'"),
			...
			Если РазрешенаРаботаСНовостями Тогда
				...
				Если РазрешенаРаботаСНовостямиЧерезИнтернет Тогда
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Обновление классификаторов запущено.'; en = 'Classifiers update is ran.'"));
					Результат = Обработки.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера(КонтекстВыполненияВложенный);
					ЛогШаговВыполнения.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Обновление классификаторов завершено. Результат: %1'; en = 'Classifiers update is completed. Result: %1'"),
							Результат));
				Иначе
					ЛогШаговВыполнения.Добавить(
...
"Их можно обновлять из метаданных (при обновлении конфигурации), либо вручную.'; en = 'ОбновитьСтандартныеСпискиДляНовостей_Ежедневно enabled mode [Work only with local news]"
"so any classifiers are not updated with the 1C servers."
"They can be updated from metadata when updating configuration or manually.'"));
				КонецЕсли;
...
"поэтому все механизмы новостного центра отключены.'; en = 'ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, disconnected mode [Enabled news operation],"
"therefore, all the mechanisms of news center are disabled.'"));
			КонецЕсли;
...
"ДатаСледующегоОбновления: %2'; en = 'Classifiers were updated without error."
"ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2'"),
						ТекущаяДатаДляРегистров,
...
"ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2'; en = 'There were errors classifiers updating. The next start time has been changed."
"ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2'"),
						ТекущаяДатаДляРегистров,
						Запись.ДатаСледующегоОбновления));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Все обновления новостей'; en = 'LIS:News.All updates of news'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Все обновления новостей. Обновление классификаторов'; en = 'News. All news updates. Classifiers update'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ПересчитатьНеобработанныеКатегорииНовостей(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Пересчет необработанных категорий новостей'; en = 'Recalculation of rough news categories'"));
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			// Зашли в конфигурацию под пользователем с разделением (с входом в область данных).
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ОписаниеРезультата = НСтр("ru = 'ПересчитатьНеобработанныеКатегорииНовостей, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях.'; en = 'ПересчитатьНеобработанныеКатегорииНовостей, when working in the service model, it is allowed to start updating the standard lists only if the delimiters are disabled.'");
				ЕстьОшибки = Истина;
			КонецЕсли;
		КонецЕсли;
...
#Область ОбновлениеКатегорийИЗначений
				...
				ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					НСтр("ru = 'Пересчет необработанных категорий новостей'; en = 'Recalculation of rough news categories'") + " / " + ИдентификаторШага1);
					...
					Если НЕ Результаты[0].Пустой() Тогда
						...
						ЛогШаговВыполненияВложенный.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Результаты[0] - непустой. ТаблицаКатегорийИЗначенийПростые.Количество() = %1'; en = 'Results[0] - not empty. ТаблицаКатегорийИЗначенийПростые.Количество() = %1'"),
								ТаблицаКатегорийИЗначенийПростые.Количество()));
					Иначе
						ТаблицаКатегорийИЗначенийПростые = Новый ТаблицаЗначений;
						ЛогШаговВыполненияВложенный.Добавить(
							НСтр("ru = 'Результаты[0] - пустой'; en = 'Results[0] - empty'"));
					КонецЕсли;
					Если НЕ Результаты[1].Пустой() Тогда
						...
						ЛогШаговВыполненияВложенный.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Результаты[1] - непустой. ТаблицаКатегорийИЗначенийИнтервалыВерсий.Количество() = %1'; en = 'Results[1] - not empty. ТаблицаКатегорийИЗначенийИнтервалыВерсий.Количество() = %1'"),
								ТаблицаКатегорийИЗначенийИнтервалыВерсий.Количество()));
					Иначе
						ТаблицаКатегорийИЗначенийИнтервалыВерсий = Новый ТаблицаЗначений;
						ЛогШаговВыполненияВложенный.Добавить(
							НСтр("ru = 'Результаты[1] - пустой'; en = 'results of[1] is empty'"));
					КонецЕсли;
					...
					Если ЕстьДанныеДляОбработки = Истина Тогда
						...
						ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Есть данные для обработки.'; en = 'There are data for data processors.'"));
...
#КонецОбласти
					...
					Иначе
						ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Нет данных для обработки.'; en = 'No data for processing.'"));
					КонецЕсли;
...
" - Условие отбора: %3"
" - ЛентаНовостей: %4"
" - УИННовости: %5'; en = 'Deleted record from КатегорииНовостейПростыеНеобработанные information register with keys:"
" - КатегорияНовостейСтрока: %1"
" - ЗначениеКатегорииНовостейСтрока: %2"
" - Условие отбора: %3"
" - ЛентаНовостей: %4"
...
" - ВерсияДО: %4"
" - ЛентаНовостей: %5"
" - УИННовости: %6'; en = 'The record from the КатегорииНовостейИнтервалыВерсийНеобработанные information register has been deleted with keys:"
"  - КатегорияНовостейСтрока: %1"
"  - Продукт: %2"
"  - ВерсияОТ: %3"
"  - ВерсияДО: %4"
				...
				Если ЕстьДанныеДляОбработки = Истина Тогда
					...
					ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
						...
						Если ТаблицаКатегорийИЗначенийПростые.Количество() > 0 Тогда
							...
							ЛогШаговВыполненияВложенный.Добавить(
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Перенесено %1 значений простых категорий'; en = 'Moved %1 the values of simple categories'"),
						...
						Если ТаблицаКатегорийИЗначенийИнтервалыВерсий.Количество() > 0 Тогда
							...
							ЛогШаговВыполненияВложенный.Добавить(
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Перенесено %1 значений категорий интервалов версий'; en = 'Moved %1 category values of versions intervals'"),
									ТаблицаЗаполненныхКатегорийИЗначений.Количество()));
						КонецЕсли;
...
#Область ОчисткаМусора
				...
				Если ЕстьДанныеДляОбработки = Истина Тогда
					...
					ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
						КонтекстВыполненияВложенный,
						"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ИдентификаторШага1, // Идентификатор.
						НСтр("ru = 'ПересчитатьНеобработанныеКатегорииНовостей, удаление записей, не привязанных к новостям.'; en = 'ПересчитатьНеобработанныеКатегорииновостей, deleting records not tied to the news.'") + " / " + ИдентификаторШага1);
						...
						Если НЕ Результат.Пустой() Тогда
							ЛогШаговВыполненияВложенный.Добавить(
								НСтр("ru = 'Есть необработанные простые категории, не привязанные к новостям:'; en = 'There are unprocessed simple categories that are not tied to the news:'"));
							Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
							Пока Выборка.Следующий() Цикл
								...
								ЛогШаговВыполненияВложенный.Добавить(
									СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'УИН: %1'; en = 'UIN: %1'"),
										Выборка.УИННовости));
							КонецЦикла;
						Иначе
							ЛогШаговВыполненияВложенный.Добавить(
								НСтр("ru = 'Нет необработанных простых категорий, не привязанных к новостям.'; en = 'There are no unprocessed simple categories that are not tied to news.'"));
						КонецЕсли;
						...
						Если НЕ Результат.Пустой() Тогда
							ЛогШаговВыполненияВложенный.Добавить(
								НСтр("ru = 'Есть необработанные категории интервалов версий, не привязанные к новостям:'; en = 'There are unprocessed categories of version intervals that are not tied to news:'"));
							Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
							Пока Выборка.Следующий() Цикл
						...
						Иначе
							ЛогШаговВыполненияВложенный.Добавить(
								НСтр("ru = 'Нет необработанных категорий интервалов версий, не привязанных к новостям.'; en = 'There are no untagged categories of version intervals that are not tied to news.'"));
						КонецЕсли;
						ТКонец = ТекущаяУниверсальнаяДатаВМиллисекундах();
						ЛогШаговВыполненияВложенный.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей'; en = 'Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ПересчитатьОтборыПоНовостям_РедкоМеняющиеся(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Пересчет отборов по новостям (редко меняющихся)'; en = 'Recalculation of selections on news (rarely changing)'"));
		...
		Если ВыполнениеРазрешено = Истина Тогда
			...
			Иначе
				ОписаниеРезультата = НСтр("ru = 'Отсутствует право на изменение для РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся.'; en = 'No right to change for РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся.'");
			КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. РедкоМеняющиеся'; en = 'News. Services and regulations. ПересчитатьОтборыПоНовостям. РедкоМеняющиеся'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПересчитатьОтборыПоНовостям_Общие(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Пересчет отборов по новостям (общих)'; en = 'Recalculation of filters on news (of general)'"));
...
#Область ЗаполнениеЗначенийАвтоматическихОтборов
		...
		ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru = 'Пересчет отборов по новостям / Заполнить значения автоматических отборов'; en = 'Recalculate selection by news / Fill in the automatic selection values'"));
...
#Область ПересчетОтборов_ВыполнениеЗапроса
		...
		ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru = 'Пересчет отборов по новостям / Выполнение запроса'; en = 'Recalculation of selections on news / Query execution'"));
		...
		ОбработкаНовостейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			...
			0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Результат запроса: %1'; en = 'The result of the query:% 1'"),
				?(РезультатЗапроса.Пустой(),
					НСтр("ru = 'пустой'; en = 'emty'"),
					НСтр("ru = 'есть данные'; en = 'There are data'"))),
			Неопределено);
...
#Область ПересчетОтборов_ЗаполнениеДанными
		...
		ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru = 'Пересчет отборов по новостям / Заполнение данными'; en = 'Recalculation of selections on news / Data filling'"));
		...
		ОбработкаНовостейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			...
			0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'НаборЗаписей.Количество(): %1'; en = 'НаборЗаписей.Количество(): %1'"),
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Общие'; en = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Общие'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
			КомментарийДляЖурналаРегистрации(
				НСтр("ru = 'Пересчет отборов по новостям (общие)'; en = 'Recalculation of filters on news (general)'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ПересчитатьОтборыПоНовостям_ДляОбластиДанных(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Пересчет отборов по новостям (общих для области данных)'; en = 'Recalculation of filters on the news (common for data area)'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. ДляОбластиДанных'; en = 'News. Services and regulations. ПересчитатьОтборыПоНовостям. ДляОбластиДанных'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
			КомментарийДляЖурналаРегистрации(
				НСтр("ru = 'Пересчет отборов по новостям (общие по области данных)'; en = 'Recalculation of filters for news (common in data area)'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ПересчитатьОтборыПоНовостям_Пользовательские(Пользователь = Неопределено, КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Пересчет отборов по новостям (пользовательских)'; en = 'Recalculation of selections on news (users)'"));
...
#Область ПолучениеСпискаПользователей
		...
		ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru = 'Пересчет отборов по новостям / Получение списка пользователей'; en = 'Recalculation of selections for news / Get list of users'"));
...
#Область ОчисткаПредыдущегоРезультата
		...
		ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru = 'Пересчет отборов по новостям / Очистка предыдущего результата'; en = 'Recalculation of selections for news / Clear the previous result'"));
...
#Область ФормированиеСпискаНовостейИсключений_ВыполнениеЗапроса
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = 'Пересчет отборов по новостям / Формирование списка новостей-исключений / Выполнение запроса'; en = 'Recounting the news selection / Creating a list of news-exceptions / Executing request'"));
...
#Область ФормированиеСпискаНовостейИсключений_РаботаСДаннымиЗапроса
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = 'Пересчет отборов по новостям / Формирование списка новостей-исключений / Работа с данными запроса'; en = 'Recalculate selection by news / Create list of news-exceptions / Work with query data'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Пользовательские'; en = 'News. Services and regulations. ПересчитатьОтборыПоНовостям. Пользовательские'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
			КомментарийДляЖурналаРегистрации(
				НСтр("ru = 'Пересчет отборов по новостям (пользовательские)'; en = 'Recalculation of selections for news (users)'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
"Подробности:"
"%3'; en = 'Error getting values of functional option %1 of the category %2."
"Details:"
"%3'"),
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
					НСтр("ru = 'Новости. Сервис и регламент. РассчитатьОтборПоКатегории. Общие'; en = 'Новости. Сервис и регламент. РассчитатьОтборПоКатегории. Общие'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
		...
		ИначеЕсли (ВРег(КодКатегорииНовостей) = ВРег("FnOptionDataAreaOn"))
			...
			Если ИспользоватьНомерОбластиДанных = Истина Тогда
				...
				Исключение
					...
					ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
						НСтр("ru = 'Новости. Сервис и регламент. РассчитатьОтборПоКатегории. ДляОбластиДанных'; en = 'News. Services and regulations. РассчитатьОтборПоКатегории. ДляОбластиДанных'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
						УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
						Метаданные.Справочники.Новости, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
...
Процедура ПроверитьИзменениеВерсииПлатформы(КонтекстВыполнения = Неопределено)
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Проверка изменения версии платформы'; en = 'Checking the version of the platform'"));
	...
	Если МонопольныйРежим() Тогда
		...
		//  то будут ошибки при всех попытках записи данных в базу.
		// Поэтому в монопольном режиме не следует выполнять никаких обновлений в подсистеме новостей.
		ОписаниеРезультата = НСтр("ru = 'В монопольном режиме проверка версии платформы не осуществляется'; en = 'Platform version is not checked in the exclusive mode'");
		ЕстьОшибки = Истина;
	КонецЕсли;
...
""
"Версия конфигурации в этой обработке не проверяется, а пересчитывается принудительно в модуле обновления конфигурации, а также после каждого получения новостей.'; en = 'Saved platrorm version: %СохраненнаяВерсияПлатформы%"
"The current platform version: %ТекущаяВерсияПлатформы%"
"%ТребуетсяОбновлениеРедкоменяющихсяОтборов%"
""
"Configuration version in this data processor is not checked, but forcefully recalculated in the configuration update module as well as after every news receipt.'");
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если (ТипЗнч(ЛокальнаяПоследняяВерсия) <> ТипСтрока) // Первый запуск
				...
				Если МенеджерЗаданийАктивен = Истина Тогда
					Если МассивВыполняемыхФоновыхЗаданий.Количество() = 0 Тогда
						...
						МассивФоновыхЗаданий.Добавить(
							ФоновыеЗадания.Выполнить(
								...
								"", // Ключ
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с %1 на %2'; en = 'ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся in connection with a change of platform version from %1 to %2'"),
						...
						ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ru = 'Редко меняющиеся отборы будут пересчитаны (запущено фоновое задание)'; en = 'Rarely changed filters will be recalculated (background job is started)'"));
					Иначе
					...
					КонецЕсли;
				Иначе
					ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ru = 'Редко меняющиеся отборы будут пересчитаны (явный запуск ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())'; en = 'Rarely changing selections will be converted (explicitly start ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())'"));
			...
			Иначе
				ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%СохраненнаяВерсияПлатформы%", ЛокальнаяПоследняяВерсия);
				ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ТекущаяВерсияПлатформы%", ЛокальнаяТекущаяВерсия);
				ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ru = 'Редко меняющиеся отборы останутся как есть'; en = 'Rarely changed filters will remain as is'"));
			КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Все обновления новостей'; en = 'LIS:News.All updates of news'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Все обновления новостей. Проверка версии платформы'; en = 'News. All news updates. Test of the platform version'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область ИнициализацияМетода
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Оптимизация отборов по новостям'; en = 'News filter optimization'"));
...
#Область ВыполнениеЗапроса
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / Выполнение запроса'; en = 'Optimization of selections for news / Query execution'"));
	...
	РезультатЗапроса1 = ?(РезультатыЗапроса[1].Пустой(), НСтр("ru = 'пустой'; en = 'emty'"), НСтр("ru = 'есть данные'; en = 'There are data'"));
	РезультатЗапроса2 = ?(РезультатыЗапроса[2].Пустой(), НСтр("ru = 'пустой'; en = 'emty'"), НСтр("ru = 'есть данные'; en = 'There are data'"));
	Если ОптимизироватьОтборыПоНовостямОбщие = Истина Тогда
		РезультатЗапроса3 = ?(РезультатыЗапроса[3].Пустой(), НСтр("ru = 'пустой'; en = 'emty'"), НСтр("ru = 'есть данные'; en = 'There are data'"));
	Иначе
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_ДляОбластиДанных
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / Удаление отборов по несуществующим категориям / Для области данных'; en = 'Filters optimization for news / Remove filters for non-existent categories / For a data area'"));
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_Пользовательские
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / УдалениеОтборовПоНесуществующимКатегориям / Пользовательские'; en = 'Optimization selections for news / УдалениеОтборовПоНесуществующимКатегориям / User'"));
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_Общие
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / УдалениеОтборовПоНесуществующимКатегориям / Общие'; en = 'Optimization of selection of news / УдалениеОтборовПоНесуществующимКатегориям / General'"));
...
#Область Оптимизация_УдалениеЗначенийОтборов_ДляОбластиДанных
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / Удаление значений отборов / Общие для области данных'; en = 'Filters optimization for news / Removing values of filters / Common for data area'"));
...
#Область Оптимизация_УдалениеЗначенийОтборов_Пользовательские
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / УдалениеЗначенийОтборов / Пользовательские'; en = 'Optimization of selection by news / DeleteSelectionValues / User'"));
...
#Область Оптимизация_ЛишниеКатегорииВНовостях
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / Удаление лишних категорий в новостях'; en = 'Optimization of selections by news / Removal of unnecessary categories in news'"));
...
#Область Завершение
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ОптимизацияОтборовПоНовостям'; en = 'Новости. Сервис и регламент. ОптимизацияОтборовПоНовостям'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура НайтиИОбработатьНеактивныеЛентыНовостей(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Поиск неактивных лент новостей'; en = 'Search of unactive newsfeeds'"));
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если НЕ Результат.Пустой() Тогда
				...
				ОписаниеРезультата = НСтр("ru = 'Запрос выполнен.'; en = 'Request completed.'") + Символы.ПС;
				...
				Пока Выборка.Следующий() Цикл
					...
					Если Выборка.ЛентаНовостейЧастотаОбновления = 0 Тогда
						ЛокальнаяЧастотаОбновления = НСтр("ru = 'вручную (0)'; en = 'manual (0)'");
					ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 1 Тогда
						ЛокальнаяЧастотаОбновления = НСтр("ru = 'один раз в день (1)'; en = 'once a day (1)'");
					ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 2 Тогда
						ЛокальнаяЧастотаОбновления = НСтр("ru = 'каждый час (2)'; en = 'every hour (2)'");
					ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 3 Тогда
						ЛокальнаяЧастотаОбновления = НСтр("ru = 'каждые 15 минут (3)'; en = 'every 15 minutes (3)'");
					ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 4 Тогда
						ЛокальнаяЧастотаОбновления = НСтр("ru = 'каждую минуту (4)'; en = 'every minute (4)'");
					Иначе
					...
					Если Выборка.ЛентаНовостейАктивна = Истина Тогда
						...
						Если СтруктураНовости.Ссылка.Пустая() Тогда
							...
							РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru = 'Лента новостей активна, новости о неактивности нет'; en = 'News feed is active, no news about inactivity'"));
						ИначеЕсли СтруктураНовости.ПометкаУдаления = Истина Тогда
							...
							РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru = 'Лента новостей активна, новость о неактивности уже была помечена на удаление'; en = 'News feed is active, news about inactivity was marked for deletion'"));
						Иначе
							// Пометить найденную новость на удаление.
							НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
							НовостьОбъект.УстановитьПометкуУдаления(Истина);
							РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru = 'Лента новостей активна, новость о неактивности будет помечена на удаление'; en = 'News feed is active, news about inactivity will be marked for deletion'"));
						КонецЕсли;
					Иначе
						// Найти новость и обновить. Если ее нет - создать и заполнить.
						Если СтруктураНовости.Ссылка.Пустая() Тогда
							НовостьОбъект = Справочники.Новости.СоздатьЭлемент();
							РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru = 'Лента новостей НЕактивна, новость о неактивности будет создана'; en = 'News feed in NOT active, news about inactivity will be created'"));
						Иначе
							НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
							РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru = 'Лента новостей НЕактивна, новость о неактивности будет обновлена'; en = 'News feed in NOT active, news about inactivity will be updated'"));
						КонецЕсли;
						...
						НовостьОбъект.Наименование                       = НСтр("ru = 'Лента новостей неактивна'; en = 'News feed is not active'");
						НовостьОбъект.Подзаголовок                       = НСтр("ru = 'Лента новостей неактивна. Рекомендуется исключить ее из автообновления'; en = 'News feed is not active. It is recommended to remove it from auto update'");
...
"Подробно:"
"%ИнформацияОбОшибке%'; en = 'An error occurred while creating / updating the news about the inactivity of news feed %ЛентаНовостей%"
"Details:"
"%ИнформацияОбОшибке%'")
									+ Символы.ПС;
							...
							КонецПопытки;
						Иначе
							РезультатВыполнения_ЛентаНовостей = РезультатВыполнения_ЛентаНовостей + Символы.ПС + НСтр("ru = 'Отменено создание / изменение новости про отсутствие активности для ленты новостей'; en = 'Generation / change of the news about lack of activity for the news feed is canceled'");
						КонецЕсли;
					КонецЕсли;
			...
			Иначе
				ОписаниеРезультата = НСтр("ru = 'Запрос пустой.'; en = 'query is empty.'") + Символы.ПС;
			КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей'; en = 'Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ИзменитьИспользованиеРегламентныхЗаданий(Использовать) Экспорт
	...
	Если ТекущееРегламентноеЗадание = Неопределено Тогда
		ЕстьОшибка = Истина;
		ТекстСообщения = НСтр("ru = 'Не найдено предопределенное регламентное задание ВсеОбновленияНовостей.'; en = 'Predefined scheduled job ВсеОбновленияНовостей was not found.'");
	Иначе
		Записывать = Истина;
...
"в параметре Использовать передано значение неправильного типа: %1'; en = 'ОбработкаНовостей.ИзменитьИспользованиеРегламентныхЗаданий:"
"The value of the wrong type was passed to the Use parameter: %1'"),
				ТипЗнч(Использовать));
		КонецЕсли;
		Если Записывать = Истина Тогда
			ТекущееРегламентноеЗадание.Наименование = НСтр("ru = 'Все обновления новостей'; en = 'All news updates'");
	...
	Если ЕстьОшибка = Истина Тогда
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий'; en = 'Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
Процедура УдалениеНовостей_ПометитьНаУдалениеДанные(ДанныеДляУдаления, КонтекстВыполнения = Неопределено)
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Получение списка новостей для удаления'; en = 'Getting a news list to delete'"));
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если (ДанныеДляУдаления.Свойство("НовостиДляУдаления"))
				...
				ТекстСообщения = НСтр("ru = 'Есть помеченные на удаление новости (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новостей). Их надо удалить вручную через Все функции / Стандартные / Удаление помеченных объектов.'; en = 'Some news are marked for deletion (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% of news). You should delete them manually in All functions / Standard / Deletion of marked objects.'");
			...
			Иначе
				ТекстСообщения = НСтр("ru = 'Нет помеченных на удаление новостей'; en = 'No news marked for deletion'");
			КонецЕсли;
			...
			Если (ДанныеДляУдаления.Свойство("НовостиДляУстановкиПометкиУдаления"))
				...
				Для Каждого ТекущаяНовость Из ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления Цикл
					...
					КонецЕсли;
				КонецЦикла;
				ТекстСообщения = НСтр("ru = 'Все устаревшие новости успешно помечены на удаление (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новостей).'; en = 'All obsolete news are successfully marked for deletion (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% news).'");
			...
			Иначе
				ТекстСообщения = НСтр("ru = 'Нет устаревших новостей, которые надо пометить на удаление.'; en = 'No old news to mark for deletion.'");
			КонецЕсли;
			...
			Если (ДанныеДляУдаления.Свойство("КатегорииНовостейПростыеНеобработанные"))
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					(Т2-Т1));
			Иначе
				ТекстСообщения = НСтр("ru = 'Простых необработанных категорий не было найдено.'; en = 'No simple unprocessed categories were found.'");
			КонецЕсли;
			...
			Если (ДанныеДляУдаления.Свойство("КатегорииНовостейИнтервалыВерсийНеобработанные"))
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					(Т2-Т1));
			Иначе
				ТекстСообщения = НСтр("ru = 'Необработанных категорий интервалов версий не было найдено.'; en = 'No unprocessed version interval categories.'");
			КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. УдалитьНовости_Ежедневно. После сбора данных для удаления'; en = 'News. Services and regulations. УдалитьНовости_Ежедневно. After collecting the data to remove'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ЗагрузитьВсеСтандартныеЗначения(КонтекстВыполнения = Неопределено)
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("ru = 'Загрузка стандартных значений'; en = 'Download standard values'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. Загрузка стандартных значений. Все стандартные значения'; en = 'News. Services and regulations. Loading the standard values. All the standard values'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область ОсновныеДействия
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = 'Основные действия'; en = 'Main action'"));

				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Проверка правильности внедрения подсистемы. Начало.'; en = 'Verification of the correct implementation of the subsystem. Start.'"));
...
"%Результат%'; en = 'Errors of the ИнтернетПоддержкаПользователей.Новости subsystem implementation are found."
"Found implementation errors:"
"%Результат%'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Результат%", Результат);
					ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru = 'БИП:Новости.Обновление ИБ'; en = 'LIS:News.Update of IL'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
						НСтр("ru = 'Новости. Обновление ИБ. Проверка внедрения'; en = 'News. Update of IB. Implementation check'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
						УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
						ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
				КонецЕсли;
				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Проверка правильности внедрения подсистемы. Конец.'; en = 'Verification of correct implementation of subsystem. End.'"));

				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Установка основных констант. Начало.'; en = 'Basic constants setting. Begin.'"));
				...
				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Установка основных констант. Конец.'; en = 'Basic constants setting. End.'"));
				...
				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Очистка регистра ДатыЗагрузкиПоследнихНовостей. Начало.'; en = 'Clearing ДатыЗагрузкиПоследнихНовостей register. The start.'"));
				НаборЗаписей = РегистрыСведений.ДатыЗагрузкиПоследнихНовостей.СоздатьНаборЗаписей();
				НаборЗаписей.Записать(Истина);
				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Очистка регистра ДатыЗагрузкиПоследнихНовостей. Конец.'; en = 'Clearing ДатыЗагрузкиПоследнихНовостей register. The end.'"));
				...
				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Загрузка стандартных значений. Начало.'; en = 'Loading the standard values. Beginning.'"));
				ЗагрузитьВсеСтандартныеЗначения(КонтекстВыполненияВложенныйВложенный);
				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Загрузка стандартных значений. Конец.'; en = 'The standard values loading. End.'"));
				...
				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Установка даты загрузки новостей на -3 месяца. Начало.'; en = 'Setting the date of news download at -3 months. Beginning.'"));
				Запрос = Новый Запрос;
				Запрос.Текст = "
					|ВЫБРАТЬ
					|	Спр.Ссылка КАК ЛентаНовостей
				...
				Если НЕ РезультатЗапроса.Пустой() Тогда
					...
					КонецЦикла;
				КонецЕсли;
				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Установка даты загрузки новостей на -3 месяца. Конец.'; en = 'Setting the date of news download at -3 months. End.'"));

				// Установить / переустановить значение расписания для регламентного задания ВсеОбновленияНовостей.
				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Установка расписания регламентного задания. Начало.'; en = 'Setting the schedule of scheduled job. Start.'"));
				...
				Если ТекущееРегламентноеЗадание <> Неопределено Тогда
					...
					ТекущееРегламентноеЗадание.Записать();
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Установка расписания регламентного задания. Конец.'; en = 'Setting schedule of scheduled job. End.'"));
				Иначе
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Регламентное задание ВсеОбновленияНовостей не найдено.'; en = 'Schedule job ВсеОбновленияНовостей not found.'"));
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Установка расписания регламентного задания. Конец.'; en = 'Setting schedule of scheduled job. End.'"));
				КонецЕсли;
...
#Область ПереопределяемыеДействия
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = 'Переопределяемые действия'; en = 'Override actions'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Обновление ИБ'; en = 'LIS:News.Update of IL'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Обновление ИБ. ОбщиеДанные. ПервыйЗапуск'; en = 'News. Update of IB. ОбщиеДанные. ПервыйЗапуск'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск() Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
			...
			ТекстСообщения = НСтр("ru = 'Работа с новостями не поддерживается для внешнего пользователя. Обработчик обновления пропущен.'; en = 'Work with news is not supported for an external user. Update handler is skipped.'");
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru = 'БИП:Новости.Обновление ИБ'; en = 'LIS:News.Update of IL'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
				НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск. Конец'; en = 'Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск. Конец'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
#Область ОсновныеДействия
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				Иначе
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Работа с новостями отключена. Обработчик пропущен.'; en = 'Work with news turned off. Handler is skipped.'"));
				КонецЕсли;
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Обновление ИБ'; en = 'LIS:News.Update of IL'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск'; en = 'News. Update of IB. ОбластьДанных. ПервыйЗапуск'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию() Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Переход на версию %1 (общие данные)'; en = 'Upgrade to version% 1 (General information)'"),
			ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()));
...
"%Результат%'");
				...
				Иначе
					ОписаниеРезультата = НСтр("ru = 'Работа с новостями отключена.'; en = 'Work with the news is disabled.'") + " ";
					// Записать об отключении подсистемы в журнал регистрации.
					ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru = 'БИП:Новости.Обновление ИБ'; en = 'LIS:News.Update of IL'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
						НСтр("ru = 'Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию'; en = 'Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
						УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						0, // Данные
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Обновление ИБ'; en = 'LIS:News.Update of IL'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ'; en = 'News. Update of IB. ОбщиеДанные. ПерейтиНаВерсию_ХХХ'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию() Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Переход на версию %1 (разделенные данные)'; en = 'Upgrade to version% 1 (separated data)'"),
			ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()));

		ЕстьОшибки = Ложь;

		Если Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
			ОписаниеРезультата = НСтр("ru = 'Работа с новостями не поддерживается для внешнего пользователя. Обработчик обновления пропущен.'; en = 'Work with news is not supported for an external user. Update handler is skipped.'");
			ЕстьОшибки = Истина;
		КонецЕсли;
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Обновление ИБ'; en = 'LIS:News.Update of IL'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию'; en = 'Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Обновление ИБ'; en = 'LIS:News.Update of IL'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ'; en = 'Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
"%1'; en = 'Error configuring shared props ОбластьДанныхОсновныеДанные:"
"%1'")
						+ Символы.ПС
						+ Символы.ПС,
					РезультатЛокальный);
...
"%1'; en = 'Error configuring shared attribute ОбластьДанныхВспомогательныеДанные:"
"%1'")
						+ Символы.ПС
						+ Символы.ПС,
					РезультатЛокальный);
	...
	Если ВариантПроверки = "Все" Тогда
		...
		Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
			Результат = Результат
				+ НСтр("ru = 'Ошибки настройки прав для роли АдминистраторСистемы:'; en = 'Error of setting rights for roles AdministratorSystem:'")
		...
		Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
			Результат = Результат
				+ НСтр("ru = 'Ошибки настройки прав для роли ПолныеПрава:'; en = 'Error rights setting for the role ПолныеПрава:'")
				+ Символы.ПС + РезультатЛокальный
				+ Символы.ПС + Символы.ПС;
		КонецЕсли;
	...
	Если (ВариантПроверки = "Все") ИЛИ (ВариантПроверки = "ТолькоПереопределения") Тогда
		...
		Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
			Результат = Результат
				+ НСтр("ru = 'Ошибки настройки кода конфигурации:'; en = 'Errors of configuration code setting:'")
				+ Символы.ПС + РезультатЛокальный
				+ Символы.ПС + Символы.ПС;
		КонецЕсли;
	...
	Если (ВариантПроверки = "Все") ИЛИ (ВариантПроверки = "ТолькоПереопределения") Тогда
		...
		Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
			Результат = Результат
				+ НСтр("ru = 'Ошибки настройки версии конфигурации:'; en = 'Errors of configuration version setting:'")
				+ Символы.ПС + РезультатЛокальный
				+ Символы.ПС + Символы.ПС;
		КонецЕсли;
...
Процедура Проверка_ОбщиеРеквизиты(
	...
	Если НайденныйЭлемент <> Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		ТекущийРезультат = НСтр("ru = 'Не найден требуемый объект метаданных: %ПолноеИмяМетаданных%'; en = 'Required metadata object was not found: %ПолноеИмяМетаданных%'");
...
Процедура Проверка_Права(
	...
	Если ТекущееЗначениеПрава <> ПравильноеЗначениеПрава Тогда
		ТекущийРезультат = НСтр("ru = 'Неправильно установлено право %Право% в роли %Роль% для %ПолноеИмяМетаданных%. Установлено: %ТекущееЗначениеПрава%, ожидается: %ПравильноеЗначениеПрава%'; en = 'Right %Право% is specified incorrectly for role %Роль% for %ПолноеИмяМетаданных%. Set: %ТекущееЗначениеПрава%, expected: %ПравильноеЗначениеПрава%'");
		ТекущийРезультат = СтрЗаменить(
			ТекущийРезультат,
			"%ПолноеИмяМетаданных%",
			ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(ЛокальныйОбъектМетаданных) + "." + ЛокальныйОбъектМетаданных.Имя);
				...
				Если ТипЗнч(ПараметрыФайлаВыгрузки) = ТипСтруктура Тогда
					Если (ПараметрыФайлаВыгрузки.Свойство("Архивировать"))
						...
						Архив = Новый ЗаписьZipФайла(
							...
							, // Без пароля
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Выгрузка журнала регистраций библиотеки БИП за период %1...%2'; en = 'Export of the ISL library event log for the period %1...%2'"),
...
"Возможно, отсутствуют события за указанный период (%1...%2).'; en = 'File with export of event log events does not exist."
...
"%1'; en = 'Cannot export events of the Event log to file due to:"
"%1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент'; en = 'LIS: News. Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
...
"%1'; en = 'An error occurred while receiving xml presentation of XDTO object:"
"%1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru = 'БИП:БазовыйФункционал.Отладка'; en = 'ISL:BasicFunctionality.Debug'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
				НСтр("ru = 'БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО'; en = 'БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
