...
Функция ЗаполнитьДанныеДоверенностейДокумента(ОписаниеФайла, Организация, Доверенности, СодержаниеДокумента) Экспорт
	...
	Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда
		...
		Если Найти(ДанныеФайлаЭД.ИдФайл, "ON_NSCHFDOPPR") > 0 Тогда
			ЗаполнитьДанныеДоверенностейЭД_УПД2019_ИнформацияПродавца(ДанныеФайлаЭД, Доверенности, ПространствоИмен)	
			
		ИначеЕсли Найти(ДанныеФайлаЭД.ИдФайл, "DP_IZVPOL") > 0 Или Найти(ДанныеФайлаЭД.ИдФайл, "DP_UVUTOCH") > 0 Тогда
...
Функция ИННДоверителяМашиночитаемойДоверенности(ОписаниеФайла, Формат) Экспорт
	...
	Если ЗаполнениеДанныхДоверенностиДоступноПоФормату(Формат) Тогда
		Результат = ФорматыЭДО_ФНС.ИННДоверителяИзДополнительныхДанных_УПД2019_ИнформацияПродавца(ОписаниеФайла, Формат);
	КонецЕсли;
...
// Заполняет данные МЧД подписанта в файле электронного документа УПД2019.
// 
// Параметры:
//  ДанныеФайлаЭД - см. ДанныеФайлаЭД() - данные электронного документа.
//  Доверенности - Массив из СправочникСсылка.МашиночитаемыеДоверенностиОрганизаций
//  ПространствоИмен - Строка - пространство имен схемы документа УПД2019.
//
Процедура ЗаполнитьДанныеДоверенностейЭД_УПД2019_ИнформацияПродавца(ДанныеФайлаЭД, Доверенности, ПространствоИмен) 
	...
	Для Каждого Доверенность Из Доверенности Цикл
		...
		ДанныеМЧД = Справочники.МашиночитаемыеДоверенностиОрганизаций.ПолучитьДанныеМЧД(Доверенность);
		
		Если МассивМЧД.Найти(ДанныеМЧД.НомерДоверенности) = Неопределено Тогда
			...
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекстИнфТип, "Значен", ДанныеМЧД.НомерДоверенности,  Истина, Ошибки);
			ТекстИнф.Добавить(ТекстИнфТип);
			
			ТекстИнфТип = ПолучитьОбъектТипаCML("ТекстИнфТип", ПространствоИмен);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекстИнфТип, "Идентиф", "ИННДоверителя_" + ДанныеМЧД.НомерДоверенности, Истина, Ошибки);
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ТекстИнфТип, "Значен", ДанныеМЧД.ИННДоверителя,  Истина, Ошибки);
			ТекстИнф.Добавить(ТекстИнфТип);
...
//  ДанныеФайлаЭД - см. ДанныеФайлаЭД() - данные электронного документа.
//  Доверенности - Массив из СправочникСсылка.МашиночитаемыеДоверенностиОрганизаций
//  ПространствоИменСхемы - Строка - пространство имен схемы документа УОУ, ИОП.
//
Процедура ЗаполнитьДанныеДоверенностейЭД_УОУ_ИОП(ДанныеФайлаЭД, Доверенности, ПространствоИмен) 
	...
	Если ЗначениеЗаполнено(Доверенность) Тогда
		...
		НомерДоверенности = Справочники.МашиночитаемыеДоверенностиОрганизаций.ПолучитьИдентификаторМЧД(Доверенность);
		...
		Иначе
			...
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СвДоверЭл, "ИдСистХран", ДанныеДоверенности.СсылкаНаРеестр, Истина);
		КонецЕсли;
