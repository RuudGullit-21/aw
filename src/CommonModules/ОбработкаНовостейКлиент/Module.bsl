		...
		Если НайденноеДействие <> Неопределено Тогда
			...
			ИначеЕсли НайденноеДействие.Значение = "Предупреждение" Тогда
				...
				Если ТипЗнч(Текст) = Тип("ЭлементСпискаЗначений") Тогда
					...
					Иначе
						Заголовок = НСтр("ru = 'Информация'; en = 'Information'");
					КонецЕсли;
...
Процедура ИнтернетПоддержкаИСервисы_ВключитьРаботуСНовостямиПриИзменении(Форма, Элемент) Экспорт
	...
	Если НЕ Форма.БИПВключитьРаботуСНовостями Тогда
		...
		СписокКнопок.Добавить("Отключить", НСтр("ru = 'Отключить'; en = 'Disable'"));
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Предупреждение'; en = 'Warning'"));
	Иначе
...
"(the value of %1 transferred in the parameter was increased to 2)'"),
								ИнтервалСекундЧислом);
							ПоказатьОповещениеПользователя(
								НСтр("ru = 'Изменено время первого показа новостей'; en = 'Changed the first time showing news'"),
								,
								ТекстСообщения);
...
"(переданное в параметре значение %1 было уменьшено до 999)'; en = 'The show of important and very important news will be postponed for 999 seconds"
"(the value of %1 transferred in the parameter was reduced to 999)'"),
								ИнтервалСекундЧислом);
							ПоказатьОповещениеПользователя(
								НСтр("ru = 'Изменено время первого показа новостей'; en = 'Changed the first time showing news'"),
								,
						...
						Иначе
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Показ важных и очень важных новостей будет отложен на %1 секунд'; en = 'Showing of important and very important news will be delayed for %1 seconds'"),
								ИнтервалСекундЧислом);
							ПоказатьОповещениеПользователя(
								НСтр("ru = 'Изменено время первого показа новостей'; en = 'Changed the first time showing news'"),
								,
...
"Строка параметра: %2."
"Необходимо указать время в секундах, от 2 до 999'; en = 'Failed to convert startup parameter (news feed shift): %1."
"Parameter string: %2."
"It is necessary to specify a time in seconds, from 2 to 999'"),
							ИнтервалСекундСтрокой,
							Сред(ПараметрЗапуска, ГдеНачало, ГдеКонец - ГдеНачало + 1));
						ПоказатьОповещениеПользователя(
							НСтр("ru = 'Ошибка изменения времени первого показа новостей'; en = 'Error of changing time of first news showing'"),
							,
							ТекстСообщения);
					КонецПопытки;
				Иначе
...
"in the start /C parameter, necessary to pass"
"ОтложитьПоказНовостей=XX;"
...
"%3'; en = 'Error occurred while processing action %1, parameters %2."
...
"%3'"),
					...
					ОбработкаНовостейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru = 'БИП:Новости.Клиент'; en = 'ISL:News.Client'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИмяСобытия.
						НСтр("ru = 'Новости. Клиент. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки'; en = 'News. Client. ProcessingOfClientNews. ProcessingNavigationLink'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИдентификаторШага.
...
"%3'"),
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Действие %1 с параметрами %2 не обрабатывается.'; en = 'Action %1 with parameters %2 is not processed.'"),
			Действие,
			СписокПараметров);
		ОбработкаНовостейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Клиент'; en = 'ISL:News.Client'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИмяСобытия.
...
#Область ОбработкаОшибок
	...
	Если ОшибкиПереходаПоСсылке.Количество() > 0 Тогда
		...
		ОбработкаНовостейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Клиент'; en = 'ISL:News.Client'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Клиент. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке'; en = 'Новости. Клиент. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИдентификаторШага.
			"Ошибка", // УровеньЖурналаРегистрации строкой
			"", // ОбъектМетаданных строкой
...
Процедура ПерейтиПоИнтернетСсылке(ИнтернетСсылка) Экспорт
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Произошла ошибка при переходе по ссылке %1."
"Подробная информация об ошибке:"
"%2'; en = 'Error occurred while linking %1."
"Detailed information about error:"
"%2'"),
			ИнтернетСсылка,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
