Функция ВерсияФормата(НаименованиеДокумента = Неопределено) Экспорт 
	...
	Если НаименованиеДокумента = "ЭлектроннаяТранспортнаяНакладная" Тогда
		Возврат "5.02";	
	Иначе
		Возврат "5.01";
	КонецЕсли;
...
Процедура УстановитьТекущийДоступныйТитулЭПЛ(Объект, Форма = Неопределено, ТитулыПоДокументу = Неопределено, ТребуетсяПовторно = Ложь, ВводимДопТитул = Ложь) Экспорт
	...
	ИначеЕсли ВхождениеРоли(Объект.РольУчастника, 8)
		И (ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул3")) <> Неопределено
			Или Объект.ТитулОформлениеПризнакНачалаРейса = "2"
				//И (Объект.ТекущийПолученныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул2")
	...
	ИначеЕсли ВхождениеРоли(Объект.РольУчастника, 4)
		...
		//И (Объект.ТекущийПолученныйТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул2")
		//	Или ВхождениеРоли(Объект.РольУчастника, 2) 
		//		И ТитулыПоДокументу.Получить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул2")) <> Неопределено)
		И (МедосмотрыНеПройденыПолностью = Ложь
...
Процедура ИзменитьОформлениеФормы(Форма, ИмяЭлемента = Неопределено) Экспорт
	...
	ИначеЕсли Форма.ИмяФормы = "Документ.ЭлектронныйЗаказЗаявка.Форма.ОсновнаяФорма" Тогда
		...
		Если ЗначениеЗаполнено(ПолучитьЗначениеРеквизитаИзСтруктурыФормы(Форма, "ТитулГрузоотправителяСодержаниеОперации")) = "2" Тогда
...
Процедура ЗаполнитьРеквизитыУчастника(Участник, ПрефиксРеквизитов, КонтейнерРеквизитов, Форма, ПрефиксТитула = Неопределено) Экспорт
	...
	//		ПрефиксТитула = ПолучитьПрефиксТитулаПоСтраницеОсновнойФормы(Форма.Элементы.Страницы.ТекущаяСтраница.Имя, ТипДокументаПоИмениФормы(Форма.ИмяФормы));
	//	Иначе
	//		ПрефиксТитула = Форма.ОписаниеРеквизитовФормы.ПрефиксТитула;
	//	КонецЕсли;
	//КонецЕсли;
...
Процедура ЗаполнитьРеквизитыФизическогоЛица(ФизическоеЛицо, ПрефиксРеквизитов, КонтейнерРеквизитов) Экспорт
	...
	РеквизитыФизЛица = МодульОбменСГИСЭПД.ПолучитьРеквизитыФизЛица(ФизическоеЛицо);
	
	//Если ТипЗнч(КонтейнерРеквизитов) = Тип("Структура") Тогда
	//	КонтейнерРеквизитов.Вставить(ПрефиксРеквизитов + "Фамилия", РеквизитыФизЛица.Фамилия);
	//	КонтейнерРеквизитов.Вставить(ПрефиксРеквизитов + "Имя", РеквизитыФизЛица.Имя);
	//	КонтейнерРеквизитов.Вставить(ПрефиксРеквизитов + "Отчество", РеквизитыФизЛица.Отчество);	
	//Иначе
	//	КонтейнерРеквизитов[ПрефиксРеквизитов + "Фамилия"] = РеквизитыФизЛица.Фамилия;
	//	КонтейнерРеквизитов[ПрефиксРеквизитов + "Имя"] = РеквизитыФизЛица.Имя;
	//	КонтейнерРеквизитов[ПрефиксРеквизитов + "Отчество"] = РеквизитыФизЛица.Отчество;
	//КонецЕсли;
...
Процедура ПоказатьОшибкиПоАдресу(Форма, КонтейнерРеквизитов, Префикс, ИмяЭлемента)
	...
	Если ПустаяСтрока(Форма[ИмяЭлемента])
