...
Процедура ОбновлениеИндексаППД() Экспорт
	...
	ОбновитьИндекс(НСтр("ru = 'Обновление индекса ППД'; en = 'Update full-text search index'"), Ложь, Истина);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Иначе 
		Интервал = НСтр("ru = 'Никогда'; en = 'Never'");
		ЕстьДела = Ложь;
	КонецЕсли;
	...
	Дело.Представление = НСтр("ru = 'Индекс полнотекстового поиска устарел'; en = 'Full-text search index is out of date'");
	Дело.Форма = "Обработка.ПолнотекстовыйПоискВДанных.Форма.УправлениеПолнотекстовымПоискомИИзвлечениемТекстов";
...
Функция ВыполнитьПолнотекстовыйПоиск(ПараметрыПоиска) Экспорт 
	...
	Если ПолноеКоличество = 0 Тогда
		ОписаниеОшибки = НСтр("ru = 'По запросу ничего не найдено'; en = 'Nothing found'");
...
Процедура ОбновитьИндекс(ПредставлениеПроцедуры, РазрешитьСлияние = Ложь, Порциями = Ложь)
	...
	ЗаписьЖурнала(
		Неопределено, 
		НСтр("ru = 'Запуск процедуры ""%1"".'; en = 'Start the ""%1"" procedure.'"),,
	...
	Попытка
		...
		ЗаписьЖурнала(
			Неопределено, 
			НСтр("ru = 'Успешное завершение процедуры ""%1"".'; en = 'Procedure ""%1"" is completed successfully.'"),, 
			ПредставлениеПроцедуры);
	Исключение
		ЗаписьЖурнала(
			УровеньЖурналаРегистрации.Предупреждение, 
...
Процедура ЗаписьЖурнала(
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Полнотекстовое индексирование'; en = 'Full-text indexing'", ОбщегоНазначения.КодОсновногоЯзыка()), 
		УровеньЖурнала, , , 
		ТекстДляЖурнала);
