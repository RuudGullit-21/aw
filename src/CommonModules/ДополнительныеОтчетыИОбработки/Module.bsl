...
Процедура ПриПолученииПрочихНастроек(СведенияОПользователе, Настройки) Экспорт
	...
	НазваниеНастройки = НСтр("ru = 'Настройки быстрого доступа к дополнительным отчетам и обработкам'; en = 'Settings of quick access to additional reports and data processors'");
...
Процедура ПриОпределенииВидовПодключаемыхКоманд(ВидыПодключаемыхКоманд) Экспорт
	Если ВидыПодключаемыхКоманд.Найти("ЗаполнениеОбъектов", "Имя") = Неопределено Тогда
		...
		Вид.Заголовок   = НСтр("ru = 'Заполнить'; en = 'Fill'");
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Если ТипыЗаполнениеОбъекта.Количество() > 0 Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
			...
			Команда.Представление = НСтр("ru = 'Дополнительные обработки заполнения...'; en = 'Additional filling processings...'");
			Команда.Важность      = "СмТакже";
		Иначе
			Команда.Вид           = "КоманднаяПанель";
			Команда.Представление = НСтр("ru = 'Заполнение...'; en = 'Filling...'");
		КонецЕсли;
	...
	Если ТипыОтчетов.Количество() > 0 Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
			Команда.Вид           = "Отчеты";
			Команда.Важность      = "СмТакже";
			Команда.Представление = НСтр("ru = 'Дополнительные отчеты...'; en = 'Additional reports...'");
		Иначе
			Команда.Вид           = "КоманднаяПанель";
			Команда.Представление = НСтр("ru = 'Отчеты...'; en = 'Reports...'");
		КонецЕсли;
		Команда.Порядок            = 50;
		Команда.Обработчик         = "ДополнительныеОтчетыИОбработкиКлиент.ОткрытьСписокКоманд";
		Команда.РежимЗаписи        = "Записывать";
	...
	Если ТипыСозданияСвязанныхОбъектов.Количество() > 0 Тогда
		...
		Иначе
			...
			Команда.Представление      = НСтр("ru = 'Создание связанных объектов...'; en = 'Creating linked objects...'");
...
Процедура ПриПодключенииДопОтчета(Ссылка, ПараметрыОтчета, Результат, ПолучатьМетаданные) Экспорт
	...
	Если Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет
		...
		Исключение
			ПараметрыОтчета.ТекстОшибки = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'При подключении дополнительного отчета ""%1"" возникла ошибка:'; en = 'An error occurred while connecting additional report ""%1"":'"), Строка(Ссылка))
	...
	Иначе
		...
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Элемент %1 не является дополнительным отчетом'; en = 'Item %1 is not an additional report'"),
			"'"+ Строка(Ссылка) +"'");
...
Процедура ПриПодключенииОтчета(Контекст) Экспорт
	...
	Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуру ""%1"" не передан отчет'; en = 'Procedure ""%1"" did not receive a report'"),
...
Процедура ВыполнитьОбработкуПоРегламентномуЗаданию(ВнешняяОбработка, ИдентификаторКоманды) Экспорт
	...
	ЗаписатьИнформацию(ВнешняяОбработка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Команда %1: Запуск.'; en = 'Command %1: Start.'"), ИдентификаторКоманды));
	...
	// Запись журнала регистрации
	ЗаписатьИнформацию(ВнешняяОбработка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Команда %1: Завершение.'; en = 'Command %1: End.'"), ИдентификаторКоманды));
...
Функция НаименованиеПодсистемы()
	Возврат НСтр("ru = 'Дополнительные отчеты и обработки'; en = 'Additional reports and data processors'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
...
Функция ВыполнитьКомандуВнешнегоОбъекта(ВнешнийОбъект, ИдентификаторКоманды, ПараметрыКоманды, АдресРезультата)
	...
	Если ОписаниеКоманды = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Команда %1 не обнаружена.'; en = 'Command %1 not found.'"), ИдентификаторКоманды);
	КонецЕсли;
...
Процедура ВыгрузитьОтчетыИОбработкиВФайлы(ОтчетыИОбработки)
	...
	Если Не Выгрузка.Успех Тогда
		ТекстОшибки = СокрЛП(
			НСтр("ru = 'Не удалось выгрузить отчеты и обработки конфигурации во внешние файлы:'; en = 'Failed to upload reports and data processors of configuration to an external files:'")
	...
	Пока ОбратныйИндекс > 0 Цикл
		...
		Иначе
			ЗаписатьОшибку(СтрокаТаблицы.Ссылка, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Неподдерживаемый вид объектов метаданных: ""1""'; en = 'Unsupported object type of metadata: ""1""'"), СтрокаТаблицы.Вид));
			ОтчетыИОбработки.Удалить(ОбратныйИндекс);
			Продолжить;
		КонецЕсли;
		...
		Если ТекстСхемы = Неопределено Тогда
			ЗаписатьОшибку(СтрокаТаблицы.Ссылка, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не обнаружен файл ""%1"".'; en = 'Not found file ""%1"".'"), 
...
Функция ПакетныйЗапускКонфигуратора(Параметры, ПереданныеКомандыЗапуска)
	...
	Результат.Кратко = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось выгрузить конфигурацию в XML (код ошибки ""%1"")'; en = 'Unable to unload the configuration in XML (error code ""%1"")'"),
		КодВозврата);
		...
		Если Не Результат.Успех Тогда
			Если Результат.ИмяОбъектаЗанято Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Имя ""%1"" занято объектами ""%2""'; en = 'Name ""%1"" busy by objects ""%2""'"),
					ИмяОбъекта,
		...
		Для Каждого КлючИЗначение Из ПоискЗаданий Цикл
			...
			Исключение
				ЗаписьЖурналаРегистрации(
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка при удалении задания ""%1"":%2'; en = 'Error when deleting a task ""%1"":%2'"),
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Если РегистрационныеДанные.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма
		...
		Результат.ТекстОшибки = НСтр("ru = 'Работа с печатными формами не поддерживается.'; en = 'Work printed forms is not supported.'");
		Возврат Результат;
	КонецЕсли;
...
"Для загрузки нового объекта нажмите ""Создать"".'; en = 'Kind of imported object (%1) does not correspond to the current one (%2)."
"Click ""Create"" to import a new object.'"),
	...
	ИначеЕсли ПараметрыРегистрации.ЭтоОтчет <> ЭтоВнешнийОтчет Тогда
		Результат.ТекстОшибки = НСтр("ru = 'Вид обработки, указанный в сведениях о внешней обработке, не соответствует ее расширению.'; en = 'Data processor kind from external data processor data does not correspond to its extension.'");
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого КомандаОписание Из РегистрационныеДанные.Команды Цикл
		...
		Если НЕ ЗначениеЗаполнено(КомандаОписание.ВариантЗапуска) Тогда
			ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для команды ""%1"" не определен способ запуска.'; en = 'The launch method is not defined for the ""%1"" command.'"), КомандаОписание.Представление));
		КонецЕсли;
...
"%2'; en = 'Error obtaining registration data:"
"Error while deleting temporary file ""%1"":"
"%2'"),
				РезультатРегистрации.ИмяОбъекта,
...
"Возможно, он не подходит для этой версии программы.'; en = 'Cannot enable additional report from file."
...
"Возможно, она не подходит для этой версии программы.'; en = 'Cannot enable additional data processor from file."
"It might be not suitable for this application version.'");
		КонецЕсли;
