...
Процедура УстановитьУсловноеОформлениеПоляДата(Форма, 
	...
	ЭлементОформления = УсловноеОформление.Элементы.Добавить();
	ЭлементОформления.Использование = Истина;
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("ru = 'ДФ=ЧЧ:мм'; en = 'ДФ=ЧЧ:мм'"));
...
Процедура ПотребоватьПерезапускСеансаПоПричинеДинамическогоОбновленияВерсииПрограммы() Экспорт
	...
	ТекстОшибки = НСтр("ru = 'Версия программы обновлена, требуется перезапустить сеанс.'; en = 'Program version is updated, restart the session is required.'");
...
Процедура ЗарегистрироватьОбъектНаВсехУзлах(Знач Объект, Знач ИмяПланаОбмена, Знач ВключаяГлавныйУзел = Истина) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ЭтоРазделенныйОбъектМетаданных Тогда
				ВызватьИсключение НСтр("ru = 'Регистрация изменений для разделенных объектов не поддерживается.'; en = 'Changes registration for divided objects is not supported.'");
		КонецЕсли;
...
Функция ПредставлениеНачальнойСтраницы() Экспорт 
	...
	Возврат НСтр("ru = 'Главное'; en = 'Main'");
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешения.Добавить(МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаВременныхФайлов(Истина, Истина,
		НСтр("ru = 'Для возможности работы программы.'; en = 'For the application operation.'")));
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Важное        = Истина;
	Дело.Представление = НСтр("ru = 'Скорость работы снижена'; en = 'Speed is reduced'");
...
"""%1"".'; en = 'Cannot mark for deletion the predefined item:"
"""%1"".'"),
...
"""%1"".'; en = 'An item cannot be bound to the name of the item marked for deletion:"
"""%1"".'"),
			Строка(Источник.Ссылка));
	КонецЕсли;
...
"""%1"".'; en = 'Cannot delete the predefined item "
"""%1"".'"),
		Строка(Источник.Ссылка));
	...
	Для Каждого Строка Из ОбразецСчета.ВидыСубконто Цикл
		...
		Если Счет.ВидыСубконто.Количество() > Индекс Тогда
			Если Счет.ВидыСубконто[Индекс].ВидСубконто <> Строка.ВидСубконто Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Обмен данными.Отключение связи с главным узлом'; en = 'Exchange of data.Disable communication with main node'",
						ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Счет.Метаданные(),
					Счет,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'У счета ""%1"" субконто №%2 ""%3"" не совпадает с предопределенным субконто ""%4"".'; en = 'Account ""%1"" subkonto №%2 ""%3"" does not match any of the predefined subkonto ""%4"".'"),
						Строка(Счет),
						Индекс + 1,
...
Процедура ПередЗапускомПрограммы()
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, МинимальнаяВерсияПлатформы) < 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для запуска необходима версия платформы 1С:Предприятие %1 или выше.'; en = 'For launch 1C:Enterprise platform version %1 or higher is required.'"), МинимальнаяВерсияПлатформы);
	КонецЕсли;
	...
	Если ПустаяСтрока(Метаданные.Версия) Тогда
		ВызватьИсключение НСтр("ru = 'Не заполнено свойство конфигурации Версия.'; en = 'Property of configuration Version is not entered.'");
	Иначе
		Попытка
...
"Правильный формат, например: ""1.2.3.45"".'; en = 'Property of configuration version ""%1"" is entered incorrectly."
"Correct format is, for example, ""1.2.3.45"".'"),
		...
		Если НулеваяВерсия Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не правильно заполнено свойство конфигурации Версия: ""%1""."
"Версия не может быть нулевой.'; en = 'Property of configuration version ""%1"" is entered incorrectly."
