...
Функция ПереместитьЭлемент(ЭлементСписок, ТекущийЭлементСсылка, Направление) Экспорт
	...
	Если Не ПараметрыДоступа.Доступность Тогда
		Возврат НСтр("ru = 'Недостаточно прав для изменения порядка элементов.'; en = 'Insufficient rights to change item order.'");
	КонецЕсли;
	...
	Если Информация.ЕстьГруппы Тогда
		...
		Если ЭтоГруппа И Не Информация.ДляГрупп Или Не ЭтоГруппа И Не Информация.ДляЭлементов Тогда
			Возврат НСтр("ru = 'Выбранный элемент нельзя перемещать.'; en = 'The selected item cannot be transferred.'");
		КонецЕсли;
	КонецЕсли;
...
"(Меню Еще - Установить стандартные настройки).'; en = 'To change the order of the items, it is necessary to reset the list settings"
"(More details menu - Set default settings).'");
	КонецЕсли;
