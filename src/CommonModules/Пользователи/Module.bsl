...
Функция СоздатьАдминистратора(ПользовательИБ = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ТекстОшибки = НСтр("ru = 'Справочник Пользователи недоступен в неразделенном режиме.'; en = 'The ""Users"" catalog is not available in undivided mode.'");
	...
	Если ПользовательИБ = Неопределено Тогда
		...
		Если ПользователиИБ.Количество() = 0 Тогда
			Если ОбщегоНазначения.РазделениеВключено() Тогда
				ТекстОшибки =
					НСтр("ru = 'Невозможно автоматически создать первого администратора области данных.'; en = 'Cannot automatically create the first administrator of the data area.'");
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
...
"назначить ему роли Полные права и Администратор системы.'; en = 'Cannot create user in the catalog for a user"
"of infobase ""%1"" as the user"
"is not assigned a Full rights role and System administrator."
""
"The user might have been created in Designer."
"To create a user in the catalog automatically, you should assign"
"a Full rights role and System administrator role to this user.'"),
	...
	Попытка
		...
		Пользователь.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
			?(ПользовательИБ = Неопределено,
			  НСтр("ru = 'Выполнено создание первого администратора.'; en = 'The first administrator is created.'"),
			  ?(ПользовательСоздан,
			    НСтр("ru = 'Администратор сопоставлен с новым пользователем справочника.'; en = 'Administrator is mapped with a new catalog user.'"),
			    НСтр("ru = 'Администратор сопоставлен с существующим пользователем справочника.'; en = 'Administrator is mapped with the existing catalog user.'")) ) );
...
Процедура УстановитьСвойстваПользователяИБ(Знач ИмяИлиИдентификатор, Знач ОбновляемыеСвойства,
	...
	Если Не ПользовательСуществует Тогда
		Если Не СоздатьНового Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Пользователь информационной базы ""%1"" не существует.'; en = 'The user information database ""% 1"" does not exist.'"),
				ИмяИлиИдентификатор);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		ПользовательИБ = ПользователиИнформационнойБазы.СоздатьПользователя();
...
"Имя (для входа): ""%1"""
"превышает длину 64 символа.'; en = 'An error occurred when writing the infobase user."
"Login: ""%1"""
"is longer than 64 characters.'"),
						...
						ИначеЕсли СтрНайти(ЗначениеСвойства, ":") > 0 Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Ошибка записи пользователя информационной базы"
"Имя (для входа): ""%1"""
"содержит запрещенный символ "":"".'; en = 'An error occurred when writing the infobase user"
"Login ""%1"""
...
Процедура УстановитьПравоОткрытияВнешнихОтчетовИОбработок(ОткрытиеРазрешено) Экспорт
	...
	Для Каждого ПользовательИБ Из ПользователиИБ Цикл
		...
		ОписаниеНастроек.Представление = НСтр("ru = 'Предупреждение безопасности'; en = 'Security warning'");
