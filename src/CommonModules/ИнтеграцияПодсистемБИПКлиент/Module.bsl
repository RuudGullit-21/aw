...
"Отключить Интернет-поддержку?'; en = 'The login and password for connecting to the users Internet support services will be removed from the program."
"Disable Internet support?'"),
			РежимДиалогаВопрос.ДаНет,
			,
			КодВозвратаДиалога.Нет,
			НСтр("ru = 'Выход из Интернет-поддержки пользователей'; en = 'Exit from users Internet support'"));
	КонецЕсли;
...
Процедура ИнтернетПоддержкаИСервисы_СообщениеВСлужбуТехническойПоддержки(Форма, Команда) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СообщенияВСлужбуТехническойПоддержки") Тогда
		...
		ДанныеСообщения.Вставить("Тема",       НСтр("ru = 'Интернет-поддержка пользователей'; en = 'Online user support'"));
		ДанныеСообщения.Вставить("Сообщение",  НСтр("ru = '<Заполните текст сообщения>'; en = '<Complete text of message>'"));
	...
	Иначе
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru = 'Интернет-поддержка пользователей'; en = 'Online user support'"),
			НСтр("ru = '<Заполните текст сообщения>'; en = '<Complete text of message>'"));
