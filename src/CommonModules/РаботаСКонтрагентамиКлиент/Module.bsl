...
Процедура ОбработатьОшибку(
	...
	ИначеЕсли ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Или ОписаниеОшибки = "НеУказанПароль" Тогда
		...
		Если Не ЗначениеЗаполнено(ПредставлениеДействия) Тогда
			ТекстСообщения = НСтр("ru = 'Для продолжения необходимо подключить Интернет-поддержку пользователей.'; en = 'To continue you must connect to the Internet-support of users.'");
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 станет возможным после подключения Интернет-поддержки пользователей.'; en = '%1 will be possible after connecting to Internet user support.'"),
		...
		Если ИнтернетПоддержкаПользователейКлиент.ДоступноПодключениеИнтернетПоддержки() Тогда
			...
			ПоказатьВопрос(
				...
				ТекстСообщения + Символы.ПС + НСтр("ru = 'Подключить Интернет-поддержку?'; en = 'Connect Internet support?'"),
		...
		Иначе
			...
			ОбработчикЗавершения = Новый ОписаниеОповещения(
				...
				ЭтотОбъект,
				ДополнительныеПараметрыОбработчиковЗавершения);
			ПоказатьПредупреждение(ОбработчикЗавершения, ТекстСообщения + Символы.ПС + НСтр("ru = 'Обратитесь к администратору.'; en = 'Contact administrator.'"));
