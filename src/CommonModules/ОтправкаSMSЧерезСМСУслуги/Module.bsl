...
Функция СтатусДоставки(Знач ИдентификаторСообщения, НастройкиОтправкиSMS) Экспорт
	...
	Если КодРезультата = "1" Тогда
		...
		КонецЦикла;
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось получить статус доставки SMS (smsid: ""%3""):"
"%1 (код ошибки: %2)'; en = 'Failed to obtain the delivery status of SMS (smsid: ""%3""):"
...
Функция Разрешения() Экспорт
	...
	Порт = Неопределено;
	Описание = НСтр("ru = 'Отправка SMS через ""СМС-Услуги"".'; en = 'Sending SMS through ""SMS-Services"".'");
