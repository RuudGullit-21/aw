...
Процедура ВыбратьПользователейПослеВыбораТипа(ВыбранныйЭлемент, Контекст) Экспорт
	...
	Если ВыбранныйЭлемент.Значение = Тип("СправочникСсылка.Пользователи") Тогда
		...
		Иначе
			ЗаголовокФормыПодбора = НСтр("ru = 'Подбор пользователей'; en = 'Pick users'");
		КонецЕсли;
	Иначе
		...
		Иначе
			ЗаголовокФормыПодбора = НСтр("ru = 'Подбор внешних пользователей'; en = 'Selection of external users'");
		КонецЕсли;
	КонецЕсли;
