...
Процедура ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель) Экспорт
	...
	Попытка
		...
		Если НомерВерсииСКоллизией = Неопределено Тогда
			...
			Запись.ПредупреждениеСинхронизации = НСтр("ru = 'Отклоненная версия (автоматическое разрешение конфликта).'; en = 'Rejected version (automatic conflict resolution).'", ОбщегоНазначения.КодОсновногоЯзыка());
			НаборЗаписей.Записать();
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Подсказка = НСтр("ru = 'Устаревших версий: %1 (%2)'; en = 'Obsolete versions: %1 (%2)'");
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ru = 'Устаревшие версии объектов'; en = 'Obsolete object versions'");
		...
		Если Не Объект.ЭтоНовый() И (ИзмененаПроведенность И СведенияОВерсииОбъекта.НомерВерсии > 1 Или ВерсияОтличаетсяОтРанееЗаписанной(Объект)) Тогда
			...
			Если СведенияОВерсииОбъекта.НомерВерсии = 1 Тогда
				Если ОбъектВерсионируется(Объект.Ссылка) Тогда
					...
					ПараметрыВерсии.Вставить("Комментарий", НСтр("ru = 'Версия создана по уже имеющемуся объекту'; en = 'Version was created from the existing object'"));
...
Процедура ПроверитьНаличиеПравНаИзменениеОбъекта(ОбъектМетаданных)
	...
	Если Не ПривилегированныйРежим() И Не ПравоДоступа("Изменение", ОбъектМетаданных)Тогда
		ТекстСообщения = НСтр("ru = 'Недостаточно прав на изменение ""%1"".'; en = 'Insufficient rights to change ""%1"".'");
...
Функция ВосстановитьОбъектПоXML(ДанныеОбъекта, ТекстСообщенияОбОшибке = "")
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Версионирование'; en = 'Versioning'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
"Техническая информация об ошибке: %1'; en = 'Cannot transfer to the selected version."
...
Функция СведенияОВерсииОбъекта(Знач Ссылка, Знач НомерВерсии) Экспорт
	СообщениеНеУдалосьПолучитьВерсию = НСтр("ru = 'Не удалось получить предыдущую версию объекта.'; en = 'Cannot receive a previous version of the object.'");
	...
	Если Результат.ВерсияОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Выбранная версия объекта отсутствует в программе.'; en = 'The selected object version is missing in the application.'");
	КонецЕсли;
...
Функция РазмерДанныхСтрокой(Знач РазмерДанных)
	...
	ЕдиницаИзмерения = НСтр("ru = 'байт'; en = 'bytes'");
...
Функция ПерейтиНаВерсиюСервер(Ссылка, НомерВерсии, ТекстСообщенияОбОшибке, ОтменаПроведения = Ложь) Экспорт
	...
	Попытка
		...
		Объект.ДополнительныеСвойства.Вставить("ВерсионированиеОбъектовКомментарийКВерсии",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выполнен переход к версии №%1 от %2'; en = 'Transfer to version No. %1 from %2 is performed'"),
...
Процедура СоздатьВерсиюВладельцаНепринятых(НомерПоследнейОтклоненнойВерсии, НомерВерсииСКоллизией, Объект, АвторВерсии)
	...
	Запись.НомерВерсии = НомерВерсииСКоллизией;
	Запись.ДатаВерсии = ТекущаяДатаСеанса();
	Запись.ПредупреждениеСинхронизации = НСтр("ru = 'Отклоненная версия (автоматическое разрешение конфликта).'; en = 'Rejected version (automatic conflict resolution).'");
	Запись.АвторВерсии = АвторВерсии;
