...
Процедура ЗаполнитьОбластиПерсональныхДанных(ОбластиПерсональныхДанных) Экспорт
	...
	уатОбщегоНазначения.ЗаполнитьОбластиПерсональныхДанных(ОбластиПерсональныхДанных);
	// {/УАТ}
	...
	Если уатОбщегоНазначения.ОбластьПерсональныхДанныхВключена("ЛичныеДанные") Тогда
		Для Каждого Субъект Из Субъекты Цикл
			Если Субъект.ЭтоГруппа Тогда
				...
				СтрокаИсключения.ПричинаОтменыСкрытия = НСтр("ru='Группы субъектов не подлежат скрытию'");
			КонецЕсли;
		КонецЦикла;
