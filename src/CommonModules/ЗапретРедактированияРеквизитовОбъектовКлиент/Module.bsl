...
Процедура ПроверитьСсылкиНаОбъект(Знач ОбработкаПродолжения, Знач МассивСсылок, Знач СинонимыРеквизитов) Экспорт
	...
	ЗаголовокДиалога = НСтр("ru = 'Разрешение редактирования реквизитов'; en = 'Allow editing attributes'");
...
"Поиск мест использования может занять длительное время.'; en = 'To prevent data inconsistency in the application,"
...
""
"It is recommended to review the effects before allowing its editing"
...
"attribute %1 cannot be edited."
""
"It is recommended to review the effects in the application before allowing its editing"
...
"Usage location search may take a long time.'"),
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Проверить и разрешить'; en = 'Check and allow'"));
...
Процедура ПоказатьПредупреждениеВсеВидимыеРеквизитыРазблокированы(ОбработкаПродолжения = Неопределено) Экспорт
	...
	ПоказатьПредупреждение(ОбработкаПродолжения,
		НСтр("ru = 'Редактирование всех видимых реквизитов объекта уже разрешено.'; en = 'Editing all visible object attributes allowed.'"));
