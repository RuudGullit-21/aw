...
Функция ТикетАутентификацииНаПорталеПоддержки(ВладелецТикета) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецТикета) Тогда
		ВызватьИсключение НСтр("ru = 'Не заполнено значение параметра ""ВладелецТикета""'; en = 'The ""ВладелецТикета"" parameter is not filled in'");
	КонецЕсли;
...
Функция ПроверитьЛогинИПароль(Логин, Пароль) Экспорт
	...
	ИначеЕсли РезультатПолученияФайла.КодСостояния = 403 Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("ru = 'Неверный логин или пароль.'; en = 'Incorrect username or password.'");
	...
	Иначе
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось проверить логин и пароль в сервисе %1. %2'; en = 'Cannot check username and password on service %1. %2'"),
				URLСервиса,
	...
	Иначе
		...
		Если ТребуетсяРазделение Тогда
			Если ЗначениеРазделителя = Неопределено Тогда
				ВызватьИсключение НСтр("ru = 'Не заполнено значение параметра ""ЗначениеРазделителя"".'; en = 'Not filled the value of the parameter ""ЗначениеРазделителя"".'");
			КонецЕсли;
			МодульРаботаВМоделиСервисаБИП = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервисаБИП");
	...
	Пока КоличествоПеренаправлений < МаксКолвоПеренаправлений Цикл
		...
		Исключение
			...
			ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось загрузить содержимое (%1). %2'; en = 'Cannot download content (%1). %2'"),
				URL,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Если Ответ.КодСостояния = 301 // 301 Moved Permanently
			...
			Если КоличествоПеренаправлений > МаксКолвоПеренаправлений Тогда
				УстановитьОписаниеОшибки(
					Результат,
					"ServerError",
					НСтр("ru = 'Превышено количество перенаправлений.'; en = 'You have exceeded the number of redirections.'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка сервера при получении файла (%1). Превышено количество перенаправлений (%2).'; en = 'Server error occurred while receiving file (%1). Number of redirect actions exceeded (%2).'"),
						URL,
			...
			Иначе
				...
				Если Location = Неопределено Тогда
					УстановитьОписаниеОшибки(
						...
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Ошибка сервера (%1) при получении файла (%2). Некорректное перенаправление, отсутствует HTTP-заголовок ответа ""Location"".'; en = 'Server (%1) error occurred while receiving file (%2). Incorrect redirect. No HTTP header of the ""Location"" response.'"),
							Ответ.КодСостояния,
							URL),
						Перенаправления);
					Возврат Результат;
				Иначе
					...
					Если ПустаяСтрока(Location) Тогда
						УстановитьОписаниеОшибки(
							...
							НСтр("ru = 'Некорректное перенаправление.'; en = 'Incorrect redirection.'"),
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Ошибка сервера (%1) при получении файла (%2). Некорректное перенаправление, пустой HTTP-заголовок ответа ""Location"".'; en = 'Server (%1) error occurred while receiving file (%2). Incorrect redirect. Empty HTTP header of the ""Location"" response.'"),
								Ответ.КодСостояния,
								URL),
							Перенаправления);
						Возврат Результат;
					...
					Если ВыполненныеПеренаправления.Получить(Location) <> Неопределено Тогда
						УстановитьОписаниеОшибки(
							Результат,
							"ServerError",
							НСтр("ru = 'Циклическое перенаправление.'; en = 'Circular redirection.'"),
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Ошибка сервера (%1) при получении файла (%2). Циклическое перенаправление (%3).'; en = 'Server (%1) error occurred while receiving file (%2). Circular redirect (%3).'"),
								Ответ.КодСостояния,
								URL,
								Location),
							Перенаправления);
	...
	Если Ответ.КодСостояния < 200 Или Ответ.КодСостояния >= 300 Тогда
		...
		Если Ответ.КодСостояния = 407 Тогда
			...
			УстановитьОписаниеОшибки(
				Результат,
				"ConnectError",
				НСтр("ru = 'Ошибка аутентификации на прокси-сервере.'; en = 'An error occurred while	authenticating on the proxy server.'"),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка клиента (%1) при выполнении запроса к ресурсу (%2)."
"Тело ответа: %3'; en = 'Client error (%1) occurred while executing a resource query (%2)."
"Response body: %3'"),
					Ответ.КодСостояния,
					URL,
...
"Тело ответа: %3'; en = 'Server error occurred while receiving file (%1). Incorrect (unsupported) response (%2)."
"Response body: %3'"),
					URL,
					Ответ.КодСостояния,
					Лев(Ответ.ПолучитьТелоКакСтроку(), 5120)),
		...
		ИначеЕсли Ответ.КодСостояния >= 400 И Ответ.КодСостояния < 500 Тогда
			...
			УстановитьОписаниеОшибки(
				...
				"ClientError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка (%1) при выполнении запроса к ресурсу.'; en = 'An error (%1) occurred while executing a resource query.'"),
...
"Тело ответа: %3'; en = 'Server (%1) error occurred while processing a resource request (%2)."
"Response body: %3'"),
					Ответ.КодСостояния,
					URL,
					Лев(Ответ.ПолучитьТелоКакСтроку(), 5120)),
...
"%2"
"Код состояния: %3;"
"Тело ответа: %4'; en = 'Failed to retrieve authentication ticket in service %1."
...
"Response body: %4'"),
			...
			Результат.СообщениеОбОшибке =
				НСтр("ru = 'Ошибка аутентификации. Некорректный ответ сервиса.'; en = 'Authentication error. Incorrect response from service.'");
			Результат.СообщениеОбОшибке =
...
"Повторите попытку позже.'; en = 'You have exceeded the number of authentication attempts."
...
"Код состояния: %2;"
"Тело ответа: %3'; en = 'Failed to retrieve authentication ticket in service %1."
...
"For details, see the event log.'");
	...
	ИначеЕсли РезультатОперации.КодСостояния = 0 Тогда
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось получить тикет аутентификации в сервисе %1."
"%2'; en = 'Failed to get ticket authentication in service %1."
"%2'"),
			URLСервиса,
...
Процедура УстановитьОписаниеОшибки(
	...
	Если КодОшибки = "ConnectError" Тогда
		Сообщение = НСтр("ru = 'Ошибка при подключении к серверу.'; en = 'An error occurred while connecting to the server.'");
	ИначеЕсли КодОшибки = "ServerError" Тогда
		Сообщение = НСтр("ru = 'На сервере возникла внутренняя ошибка при обработке запроса.'; en = 'An internal error occurred on the server while processing the request.'");
	ИначеЕсли КодОшибки = "ClientError" Тогда
		Сообщение = НСтр("ru = 'Некорректный запрос.'; en = 'Incorrect request.'");
	...
	ИначеЕсли КодОшибки = "LoginError" Тогда
		Сообщение = НСтр("ru = 'Ошибка аутентификации на сервере.'; en = 'An error occurred while	authenticating on the server.'");
	КонецЕсли;
	...
	Если Перенаправления.Количество() > 0 Тогда
		Результат.ИнформацияОбОшибке = Результат.ИнформацияОбОшибке + Символы.ПС
			+ НСтр("ru = 'Перенаправления:'; en = 'Redirection:'") + Символы.ПС
...
Процедура СлужебнаяПроверитьURLДоступен(
	...
	Исключение
		ИмяОшибки = "Unknown";
		СообщениеОбОшибке = НСтр("ru = 'Неизвестная ошибка. Подробнее см. в журнале регистрации.'; en = 'Unknown error. For more information, view the Event log.'");
...
"%1'; en = 'Unknown error occurred during check of availability URL."
"%1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Интернет-поддержка пользователей'; en = 'Online user support'",
...
"%1'; en = 'Failed to send data messages to technical support."
"Failed to verify the availability of the service to send messages."
"%1'"),
				РезультатПроверки.ИнформацияОбОшибке));
...
"%1'; en = 'Failed to given send message to technical support."
"Failed to verify the availability of the service."
"%1'"),
				РезультатПроверки.СообщениеОбОшибке);
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось отправить данные сообщения в службу технической поддержки."
"%1'; en = 'Cannot send message data to technical support service."
"%1'"),
...
"%1"
"Тело ответа: %2'; en = 'Cannot send message data to technical support service."
...
"Response body: %2'"),
		...
		Результат.СообщениеОбОшибке =
			НСтр("ru = 'Не удалось отправить данные сообщения в службу технической поддержки."
"Некорректный ответ сервиса отправки сообщений.'; en = 'Cannot send message data to technical support service."
...
"Ответ сервиса: %1'; en = 'Cannot send message data to technical support service."
...
"Service response: %1'"),
		...
		Результат.СообщениеОбОшибке =
			НСтр("ru = 'Не удалось отправить данные сообщения в службу технической поддержки."
"Ошибка при передаче сообщения в тех. поддержку."
"Подробнее см. в журнале регистрации.'; en = 'Cannot send message data to technical support service."
"An error occurred while transmitting message to technical support service."
