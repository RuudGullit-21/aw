...
Функция ПКОПоИмени(КомпонентыОбмена, Имя) Экспорт
	...
	Если ПравилоКонвертации = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПКО с именем %1'; en = 'Credit slip with name %1 is not found'"), Имя);
	...
	Для Каждого Свойство Из СвойстваИсточника Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неизвестный тип свойства <%1>. Тип объекта: %2'; en = 'Unknown property type <%1>. Object type: %2'"),
				Свойство.Имя,
				Строка(ТипXDTO));
		КонецЕсли;
...
"Тип источника: %1"
"Тип приемника: %2'; en = 'XML scheme error. One property shall be set for the target."
"Source type: %1"
"Target type: %2'"),
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Если ТранзакцияАктивна() Тогда
			ВызватьИсключение НСтр("ru = 'Блокировка на обмен данными не может быть установлена в активной транзакции.'; en = 'Exchange data lock cannot be set in an active transaction.'");
		КонецЕсли;
...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока Не ПрерватьЦикл Цикл
		...
		Исключение
			СтрокаСообщенияОбОшибке = НСтр("ru = 'Ошибка при загрузке данных: %1'; en = 'An error occurred when importing data: %1'");
...
"обновление настройки синхронизации данных. Необходимо:'; en = 'Receiving data from the source without update"
"of data synchronization settings. Required:'")
					+ Символы.ПС + НСтр("ru = '1) Выполнить повторную синхронизацию данных через некоторое время.'; en = '1) Repeat data synchronization later.'")
					+ Символы.ПС + НСтр("ru = '2) Выполнить синхронизацию данных на стороне источника, после этого "
"повторно выполнить синхронизацию данных в этой информационной базе.'; en = '2) Synchronize data on the source side,"
"then repeat data synchroniozation in the infobase.'")
					+ Символы.ПС + НСтр("ru = '(1 - для вида транспорта Через интернет, 2 - для вида транспорта Другое)'; en = '(1 - for transport kinds Via internet, 2- for transport kind Another)'");
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СтрокаСообщенияОбОшибке);
			Иначе
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если Не РазрешенаЗаписьОбъекта(Объект, КомпонентыОбмена) Тогда
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Попытка изменения неразделенных данных (%1: %2) в разделенном режиме.'; en = 'Attempt to modify undivided data (%1: %2) in split mode.'"),
...
Функция МенеджерОбменаВерсииФормата(Знач ВерсияФормата, Знач УзелИнформационнойБазы = Неопределено) Экспорт
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не определен Менеджер конвертации для версии формата обмена <%1>.'; en = 'Conversion Manager for exchange format version <%1> is not defined.'"),
			ВерсияФормата);
	КонецЕсли;
...
"Задано более одного правила конвертации для типа источника <%1>.'; en = 'An error occurred when converting the predefined data."
"More than one conversion rule is specified for source type <%1>.'"),
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка правил конвертации предопределенных данных."
"Правило конвертации не определено для типа источника <%1>.'; en = 'An error occurred when converting the predefined data."
"The conversion rule is not specified for source type <%1>.'"),
	...
	Иначе
		...
		ИначеЕсли НайденныеПравила.Количество() > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка правил конвертации предопределенных данных."
"Задано более одного правила конвертации для типа источника <%1> и типа приемника <%2>.'; en = 'An error occurred when converting the predefined data."
"More than one conversion rule is specified for source type <%1> and target type <%2>.'"),
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка правил конвертации предопределенных данных."
"Правило конвертации не определено для типа источника <%1> и типа приемника <%2>.'; en = 'An error occurred when converting the predefined data."
...
"Значение источника: <%2>'; en = 'Conversion rule for the predefined data value is not found."
"Source value type: <%1>"
"Source value: <%2>'"),
			ТипЗнч(Значение),
			Строка(Значение));
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(Источник, Свойство, Приемник,
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка чтения объекта XDTO, имя свойства: <%1>.'; en = 'An error occurred when reading XDTO object, property name: <%1>.'"), ИмяСвойства)
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если ЭлементыФормата.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неканоническое имя формата обмена <%1>'; en = 'Non-canonical name of the exchange format <%1>'"), ФорматОбмена);
	КонецЕсли;
...
"Тип ссылки XDTO: %1"
"Описание ошибки: %2'; en = 'POD for a type of the XDTO reference is not found."
"XDTO ref type: %1"
"Error details: %2'"),
...
"Тип объекта XDTO: %1"
"Описание ошибки: %2'; en = 'DPR for a type of the XDTO object is not found."
"XDTO object type: %1"
"Error details: %2'"),
...
"Объект метаданных: %3.'; en = 'DPR for metadata object is not found."
"Metadata object: %3.'"),
...
Функция ПОДПоИмени(КомпонентыОбмена, Имя)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПОД с именем %1'; en = 'Not found under %1'"), Имя);
...
Процедура ПроверитьВерсию(Знач Версия)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неканоническое представление версии формата обмена: <%1>.'; en = 'Non-canonical presentation of the exchange format version: <%1>.'"), Версия);
	КонецЕсли;
