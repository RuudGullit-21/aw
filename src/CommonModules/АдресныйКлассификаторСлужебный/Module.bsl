...
Функция ОписаниеПоследнейЗагрузки(ИдентификаторРегиона = Неопределено) Экспорт
	...
	Иначе
		...
		Результат.Представление        = НСтр("ru = 'Адресный классификатор еще не загружался.'; en = 'Address classifier has not been imported yet.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ДнейРазницы = 0 Тогда
		Представление = НСтр("ru = 'Адресный классификатор был загружен сегодня.'; en = 'Address classifier has been imported today.'");
...
Функция АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ТипАдреса, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Уровень = 2 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите округ'; en = 'Select okrug'");
	...
	ИначеЕсли Уровень = 5 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите внутригородской район'; en = 'Select urban area'");
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Описание = НСтр("ru = 'Загрузка обновлений адресного классификатора.'; en = 'Import address classifier updates.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Количество     = 0;
		Дело.Подсказка      = НСтр("ru = 'Автоподбор и проверка корректности адресов временно недоступны.'; en = 'Auto pick and check of address validity are temporarily unavailable.'");
	...
	Подсказка = Подсказка + Символы.ПС + НСтр("ru = 'Необходимо проверить наличие обновлений.'; en = 'Check for updates.'");
	Для Каждого Раздел Из Разделы Цикл
		Дело = ТекущиеДела.Добавить();
...
Процедура ОповеститьОПрогрессе(Знач НаименованиеРегиона, Знач СубъектРФ, Знач ОсталосьДляЗагрузки)
	...
	Иначе
		ШаблонСообщения = НСтр("ru = 'Загрузка региона ""%1 - %2""'; en = 'Download region ""%1 - %2""'") + "...";
...
Процедура ПроверитьКорректностьАдресов(СписокНекорректныхАдресов, Знач КакиеУровниПроверять, Адрес, Знач НайденныеАдреса, ОшибкиПроверки)
	...
	Для каждого АдресСтрока Из НайденныеАдреса Цикл
			...
			Если НЕ ДанныеУровняКорректные("Area", АдресСтрока, КакиеУровниПроверять) Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Субъект РФ'; en = 'RF territorial entity'"),
			...
			Если НЕ ДанныеУровняКорректные("Distict", АдресСтрока, КакиеУровниПроверять) Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Район'; en = 'District'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Район ""%1"" отсутствует в адресном классификаторе'"),
					СоединитьНаименованиеИТипАдресногоОбъекта(Адрес.District, Адрес.DistrictType)));
				Продолжить;
			...
			Если НЕ ДанныеУровняКорректные("City", АдресСтрока, КакиеУровниПроверять) Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Город'; en = 'City'"),
			...
			Если НЕ ДанныеУровняКорректные("Territory",  АдресСтрока, КакиеУровниПроверять) Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Территория'; en = 'Area'"),
			...
			Если НЕ ДанныеУровняКорректные("Locality", АдресСтрока, КакиеУровниПроверять) Тогда
					...
					ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Населенный пункт'; en = 'Locality'"),
			...
			Если НЕ ДанныеУровняКорректные("Street",  АдресСтрока, КакиеУровниПроверять) Тогда
				...
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, НСтр("ru = 'Улица'; en = 'Street'"),
...
Процедура ОпределитьИдентификаторыАдресаСервис1С(Результат, АдресДляПроверки)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Сведения о контактной информации были повреждены или некорректно заполнены'; en = 'Contact information data was damaged or populated incorrectly'");
	КонецЕсли;
...
Функция ПараметрыАутентификацииНаСайте() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Сервис интернет-поддержки пользователей не подключен.'; en = 'User support is not connected.'");
	КонецЕсли;
...
Функция СтруктураОписанияОшибкиПоставщика(Описание = Неопределено, ИнформацияОбОшибке = Неопределено)
	...
	Если ИнформацияОбОшибке = Неопределено Тогда
		Описание.ПодробноеПредставлениеОшибки = НСтр("ru = 'Информация об ошибке отсутствует'; en = 'Error info is missing'");
		Возврат Описание;
	КонецЕсли;
...
Процедура ОчиститьКлассификаторАдресов(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Очистка региона ""%1 - %2"" (осталось %3) ...'; en = 'Clearing region ""%1 - %2"" (%3 is left) ...'"),
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	Если ТипXDTOЗаписи = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В файле ""%1"" не найден список записей в типе  %2'; en = 'Record list in type %2 was not found in file ""%1""'"), Результат.ПолноеИмя, Результат.ТипXDTO);
	КонецЕсли;
	...
	Если НЕ КореньНайден Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В файле ""%1"" не найден корневой узел %2'; en = 'Root node %2 is not found in file ""%1""'"), Результат.ПолноеИмя, ИмяОписывающегоУзла);
	КонецЕсли;
