...
Функция СведенияОЮридическомЛицеПоИНН(Знач ИНН) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИНН) Тогда
		Результат.ОписаниеОшибки = НСтр("ru = 'Не указан ИНН юридического лица.'; en = 'Not specified TIN of legal person.'");
		ЗаписатьИнформациюВЖурналРегистрации(
			Результат.ОписаниеОшибки,
			ИдентификаторСервисаКонтрагент(),
			НСтр("ru = 'Получение данных'; en = 'Data receiving'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат Результат;
	КонецЕсли;
	...
	РезультатВызова = ПроверитьУслугуИВызватьОперациюRESTСервиса(
		...
		URLОперации,
		ИдентификаторСервисаКонтрагент(),
		НСтр("ru = 'ИНН юридического лица должен состоять из 10 цифр.'; en = 'TIN of a legal entity must contain 10 digits.'"));
...
Функция РеквизитыПредпринимателяПоИНН(Знач ИНН) Экспорт
	...
	Результат.ИНН = ИНН;
	Если Не ЗначениеЗаполнено(ИНН) Тогда
		Результат.ОписаниеОшибки = НСтр("ru = 'Не указан ИНН предпринимателя.'; en = 'Not specified TIN of entrepreneur.'");
		ЗаписатьИнформациюВЖурналРегистрации(
			Результат.ОписаниеОшибки,
			ИдентификаторСервисаКонтрагент(),
			НСтр("ru = 'Получение данных'; en = 'Data receiving'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РезультатВызова = ПроверитьУслугуИВызватьОперациюRESTСервиса(
		...
		URLОперации,
		ИдентификаторСервисаКонтрагент(),
		НСтр("ru = 'ИНН предпринимателя должен состоять из 12 цифр.'; en = 'TIN of entrepreneur must consist of 12 digits.'"));
	ЗаполнитьЗначенияСвойств(Результат, РезультатВызова, "ОписаниеОшибки");
	Если ЗначениеЗаполнено(Результат.ОписаниеОшибки) Тогда
		Возврат Результат;
	КонецЕсли;
...
Функция ЮридическиеЛицаПоНаименованию(Знач Наименование, Знач КодРегиона = "", Знач Адрес = "") Экспорт
	...
	Если Не ЗначениеЗаполнено(Наименование) Тогда
		Результат.ОписаниеОшибки = НСтр("ru = 'Не указано наименование юридического лица.'; en = 'Not specified name of the legal entity.'");
		ЗаписатьИнформациюВЖурналРегистрации(
			Результат.ОписаниеОшибки,
			ИдентификаторСервисаКонтрагент(),
			НСтр("ru = 'Получение данных'; en = 'Data receiving'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		...
		Исключение
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Код налогового органа %1:'; en = 'Code of tax authority %1:'"), Результат.Код)
...
Функция РеквизитыОтделенияФССПоКоду(Знач КодОтделенияФСС) Экспорт
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Код отделения ФСС %1:'; en = 'Branch code SIF %1:'"), Результат.Код)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
...
Функция РеквизитыОтделенияПФРПоКоду(Знач КодОтделенияПФР) Экспорт
	...
	Если ПустаяСтрока(ОписаниеОшибки) Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Код отделения ПФР %1:'; en = 'Branch code PFR %1:'"), Результат.Код)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
...
"До завершения выполнения существование контрагента может быть определено не для всех контрагентов.'; en = 'It checks the existence of counterparties in the FTS base."
"Until the completion of the existence of the counterparty can be determined not for all counterparties.'");
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("ru = 'Сервис 1С:Контрагент'; en = '1C:Counterparty service'"));
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		80,
		НСтр("ru = 'Сервис ФНС для проверки контрагентов'; en = 'Service FTS for verification of counterparties'"));
	НовыеРазрешения.Добавить(Разрешение);
...
Процедура ПриФормированииСпискаУслуг(Услуги) Экспорт
	...
	НоваяУслуга.Вставить("Наименование" , НСтр("ru = 'Автозаполнение реквизитов контрагентов'; en = 'Autofill of counterparties details'"));
	...
	НоваяУслуга.Вставить("Наименование" , НСтр("ru = 'Автозаполнение и проверка реквизитов ФНС, ФСС, ПФР'; en = 'AutoFill and checking the attributes of FTS, FSS, FIU'"));
	НоваяУслуга.Вставить("ТипУслуги"    , ТипУслугиБезлимитная);
	Услуги.Добавить(НоваяУслуга);
	
	НоваяУслуга = Новый Структура;
	НоваяУслуга.Вставить("Идентификатор", ИдентификаторУслугиИнформацияОСвязяхКонтрагентов());
	НоваяУслуга.Вставить("Наименование" , НСтр("ru = 'Досье контрагента'; en = 'Counterparty profile'"));
	НоваяУслуга.Вставить("ТипУслуги"    , ТипУслугиБезлимитная);
	Услуги.Добавить(НоваяУслуга);
	
	НоваяУслуга = Новый Структура;
	НоваяУслуга.Вставить("Идентификатор", ИдентификаторУслугиИнформацияОПроверкахКонтролирующимиОрганами());
	НоваяУслуга.Вставить("Наименование" , НСтр("ru = 'Информация о плановых и внеплановых проверках организации'; en = 'Information about planned and unscheduled inspections of company'"));
	НоваяУслуга.Вставить("ТипУслуги"    , ТипУслугиБезлимитная);
	Услуги.Добавить(НоваяУслуга);
...
//  ОписательСервиса.Наименование    = НСтр("ru = '1С:Контрагент'; en = '1C:Counterparty'");
...
"общего модуля РаботаСКонтрагентамиПереопределяемый.'; en = 'Could not determine the properties of the catalog Counterparties."
"The implementation of the method ПриОпределенииСвойствСправочниковКонтрагентов()"
"common module РаботаСКонтрагентамиПереопределяемый is not filled.'");
	КонецЕсли;
...
Функция ИмяСобытияЖурналаРегистрации(ИдентификаторСервиса)
	...
	Если ИдентификаторСервиса = "Контрагент" Тогда
		Возврат НСтр("ru = 'Сервис данных единых гос_реестров'; en = 'Data service single Gosreestr'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	ИначеЕсли ИдентификаторСервиса = "ГосОрганы" Тогда
		Возврат НСтр("ru = 'Сервис данных гос_органов'; en = 'Data service of governmental authorities'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли ИдентификаторСервиса = "ПроверкаКонтрагентов" Тогда
		Возврат НСтр("ru = 'Проверка контрагентов'; en = 'Counterparty check'", ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		Возврат НСтр("ru = 'Работа с контрагентами'; en = 'Work with counterparties'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
"общего модуля ИнтернетПоддержкаПользователейПереопределяемый.'; en = 'There is no description of the catalog with the name ""%1"" in the method ПриОпределенииСвойствСправочниковКонтрагентов()"
"of common module ИнтернетПоддержкаПользователейПереопределяемый.'"),
...
Процедура ОбработатьОшибкуСервиса(ОбъектXDTO, ОписаниеОшибки, Сервис, ИмяМетода, ИдентификаторУслуги, Результат)
	...
	Если ОписаниеОшибкиВРег = ВРег("НеУказаныПараметрыАутентификации")
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Интернет-поддержка пользователей не подключена.'; en = 'Online user support is not connected.'"),
			Сервис,
			НСтр("ru = 'Аутентификация'; en = 'Authentication'", КодОсновногоЯзыка));
		
	ИначеЕсли СтрНайти(ОписаниеОшибкиВРег, """STATUS"":401") > 0
		Или СтрНайти(ОписаниеОшибкиВРег, "SERVER-9:") > 0 Тогда
		
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'Ошибка аутентификации в сервисе. Обратитесь к администратору.'; en = 'Authentication error in service. Please contact your administrator.'");
...
"%1'; en = 'Online user support is not connected."
"%1'"),
						ОписаниеОшибки),
				Сервис,
				НСтр("ru = 'Аутентификация'; en = 'Authentication'", КодОсновногоЯзыка));
	...
	ИначеЕсли ОписаниеОшибкиВРег = ВРег("УслугаНеПодключена") Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Услуга с идентификатором ""%1"" не подключена.'; en = 'Service with ID ""%1"" is not connected.'"),
				ИдентификаторУслуги),
			Сервис,
			НСтр("ru = 'Доступ'; en = 'Access'", КодОсновногоЯзыка));
...
"%1'; en = 'Service 1C:Counterparty not connected."
"%1'"),
					ОписаниеОшибки),
			Сервис,
			НСтр("ru = 'Доступ'; en = 'Access'", КодОсновногоЯзыка));
	...
	ИначеЕсли СтрНайти(ОписаниеОшибкиВРег, "SERVER-1:") > 0 Тогда
		...
		ИначеЕсли ИмяМетода = "findInspectionsByInnList" Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'Не указан ИНН проверяемого лица.'; en = 'Not specified TIN of controlled person.'");
		ИначеЕсли ИмяМетода = "findPfrByCode"
			Или ИмяМетода = "findFssByCode"
			Или ИмяМетода = "findIfnsByCode" Тогда
			Результат.ОписаниеОшибки  = НСтр("ru = 'Не указан код государственного органа.'; en = 'ID of public authority is not specified.'");
		Иначе
...
"%2'"),
					...
					ОписаниеОшибки),
			Сервис,
			НСтр("ru = 'Получение данных'; en = 'Data receiving'", КодОсновногоЯзыка));
		
	ИначеЕсли СтрНайти(ОписаниеОшибкиВРег, "SERVER-2:") > 0 Тогда
		
		Если ИмяМетода = "findCorporationByInn" 
			ИЛИ ИмяМетода = "findCorpTrustability" Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'ИНН юридического лица должен состоять из 10 цифр.'; en = 'TIN of a legal entity must contain 10 digits.'");
		ИначеЕсли ИмяМетода = "findEntrepreneurByInn" 
			ИЛИ ИмяМетода = "findEntrTrustabilityByInn" Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'ИНН предпринимателя должен состоять из 12 цифр.'; en = 'TIN of entrepreneur must consist of 12 digits.'");
		ИначеЕсли ИмяМетода = "findInspectionsByInnList" Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'ИНН проверяемого лица должен состоять из 10 или 12 цифр.'; en = 'TIN of check person should consist of 10 or 12 digits.'");
		Иначе
			Результат.ОписаниеОшибки = ОписаниеОшибки;
		КонецЕсли;
...
"%2'"),
	...
	ИначеЕсли СтрНайти(ОписаниеОшибкиВРег, "SERVER-3:") > 0 Тогда
		...
		Если ИмяМетода = "findCorporationByInn" 
			ИЛИ ИмяМетода = "findCorpTrustability" Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'ИНН юридического лица должен содержать только цифры.'; en = 'TIN of legal entity must contain only digits.'");
		ИначеЕсли ИмяМетода = "findEntrepreneurByInn" 
			ИЛИ ИмяМетода = "findEntrTrustabilityByInn" Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'ИНН предпринимателя должен содержать только цифры.'; en = 'TIN of entrepreneur must contain only numbers.'");
		ИначеЕсли ИмяМетода = "findInspectionsByInnList" Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'ИНН проверяемого лица должен содержать только цифры.'; en = 'TIN of checked person should contain only numbers.'");
		ИначеЕсли ИмяМетода = "findPfrByCode"
			Или ИмяМетода = "findFssByCode"
			Или ИмяМетода = "findIfnsByCode" Тогда
			Результат.ОписаниеОшибки  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Данные о государственном органе с кодом %1 не найдены.'; en = 'Data on public authority with code % 1 was not found.'"),
				Результат.Код);
		Иначе
			Результат.ОписаниеОшибки = ОписаниеОшибки;
		КонецЕсли;
...
"%2'"),
	...
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-7:") > 0 Тогда
		...
		Результат.ОписаниеОшибки = НСтр("ru = 'Превышен лимит количества вызовов сервиса за один день'; en = 'The maximum number of service calls per day has been exceeded'");
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1"
"%2'; en = '%1"
"%2'"),
	...
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-8:") > 0 Тогда
		...
		Результат.ОписаниеОшибки = НСтр("ru = 'Отсутствует действующий договор ИТС'; en = 'No valid ITS contract'");
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1"
"%2'; en = '%1"
"%2'"),
	...
	ИначеЕсли Не ЗначениеЗаполнено(ОписаниеОшибки) И ОбъектXDTO = Неопределено Тогда
		...
		Если ИмяМетода = "findCorporationByInn"
			...
			Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось найти данные для заполнения реквизитов по ИНН %1.'; en = 'Failed to find data to fill in details on TIN %1.'"),
				Результат.ИНН);
		ИначеЕсли ИмяМетода = "findCorporationsByName" Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'Не удалось выполнить поиск по наименованию.'; en = 'Failed to perform search by title.'");
		ИначеЕсли ИмяМетода = "findInspectionsByInnList" Тогда
			Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось найти данные о проверках по ИНН %1.'; en = 'Could not find information about checks on TIN %1.'"),
				Результат.ИНН);
		ИначеЕсли ИмяМетода = "findCorpTrustability" Тогда
			Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось найти данные о юридическом лице с ИНН %1.'; en = 'Failed to find data on a legal entity with TIN %1.'"),
				Результат.ИНН);
		ИначеЕсли ИмяМетода = "findEntrTrustabilityByInn" Тогда
			Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось найти данные о предпринимателе с ИНН %1.'; en = 'Failed to find data about entrepreneur with TIN %1.'"),
				Результат.ИНН);
		ИначеЕсли ИмяМетода = "findPfrByCode"
			Или ИмяМетода = "findFssByCode"
			Или ИмяМетода = "findIfnsByCode" Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1"
"Получен пустой ответ сервиса.'; en = '%1"
"Received empty response service.'"),
				Результат.ОписаниеОшибки,
				ОписаниеОшибки),
			Сервис,
			НСтр("ru = 'Получение данных'; en = 'Data receiving'", КодОсновногоЯзыка));
	...
	Иначе
		...
		Результат.ОписаниеОшибки = НСтр("ru = 'Ошибка при работе с сервисом (подробнее см. Журнал регистрации)'; en = 'An error occurred when operating with the service (for more information, see the event log)'");
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при работе с сервисом."
"%1.'; en = 'Error when working with the service."
"%1.'"),
				ОписаниеОшибки),
			Сервис,
			НСтр("ru = 'Ошибка'; en = 'Error'", КодОсновногоЯзыка));
...
Функция ДополнительноеСобытиеПолучениеДанных()
	...
	Возврат НСтр("ru = 'Получение данных'; en = 'Data receiving'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если ТипЗнч(ЗначениеCertificateInfos) = Тип("Массив") Тогда
			Для Каждого ТекущееCertificateInfos Из ЗначениеCertificateInfos Цикл
				НоваяСтрока.ОписаниеИзменения = НоваяСтрока.ОписаниеИзменения + ?(ПустаяСтрока(НоваяСтрока.ОписаниеИзменения), "", Символы.ПС)
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Выдано свидетельство %1 %2'; en = 'Certificate %1 %2 is issued'"), 
...
Процедура ЗаполнитьРеквизитыПенсионногоФонда(ТелоОтветаОбъект, Реквизиты)
	...
	Если ЗначениеPensionFundRegistrationInfo <> Неопределено Тогда
		...
		Если РегистрацияВПФР.РегистрационныйНомерПФР <> Неопределено Тогда
			РегистрацияВПФР.РегистрационныйНомерПФР = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1-%2-%3'; en = '%1-%2-%3'"), 
		...
		Если РегистрацияВПФР.КодОрганаПФР <> Неопределено Тогда
			РегистрацияВПФР.КодОрганаПФР = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1-%2'; en = '%1-%2'"), 
				...
				Для каждого СтрокаПроверки Из ПроверкиЗаМесяц.InspectionUnregular294 Цикл
					...
					Если Завершена Тогда
						...
						Если КоличествоНарушений = 0 Тогда
							НоваяСтрока.Результат = НСтр("ru = 'Нарушений не выявлено'; en = 'Violations are not found'");
						Иначе
...
Функция КодГосударственногоОрганаДляЗапросаРеквизитов(ВидГосударственногоОргана, КодГосударственногоОргана)
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректное значение параметра ""ВидГосударственногоОргана"" (%1).'; en = 'Incorrect parameter value ""ВидГосударственногоОргана"" (%1).'"),
