...
"не предусмотрены ни для одного плана обмена.'; en = 'Data import prohibition dates of previous periods from"
"other applications are not available for any exchange plan.'");
			...
			Если ИменаПредопределенных.Найти(Выборка.ИмяПредопределенныхДанных) <> Неопределено Тогда
				...
				Иначе
					...
					ИначеЕсли Выборка.Ссылка <> СвойстваРаздела.Ссылка Тогда
						...
						Объект.Наименование = "(" + НСтр("ru = 'не используется'; en = 'not used'") + ") " + СвойстваРаздела.Представление;
					КонецЕсли;
				КонецЕсли;
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		Если ПоискЗапретовЗагрузки Тогда
			Если НоваяВерсия Тогда
				Шаблон = НСтр("ru = '%1 невозможно загрузить в запрещенный период.'; en = '%1 cannot be imported to the prohibited period'");
			Иначе
				Шаблон = НСтр("ru = '%1 в запрещенном периоде невозможно заменить загружаемыми данными.'; en = '%1 cannot be replaced with the imported data in the prohibited period.'");
			КонецЕсли;
		Иначе
			Если НоваяВерсия Тогда
				Шаблон = НСтр("ru = '%1 невозможно поместить в запрещенный период.'; en = '%1 cannot be within the prohibited period.'");
	...
	Если Не СвойстваРазделов.БезРазделовИОбъектов Тогда
		Если ЗначениеЗаполнено(Запрет.Раздел) Тогда
			Если Запрет.Объект = Запрет.Раздел Тогда
				Текст = Текст + " " + НСтр("ru = '(запрет установлен на раздел ""%2"")'; en = '(section ""%2"" is prohibited)'");
			ИначеЕсли ЗначениеЗаполнено(СвойстваРазделов.ЕдинственныйРаздел) Тогда
				Текст = Текст + " " + НСтр("ru = '(запрет установлен на объект ""%3"")'; en = '(object ""%3""  is prohibited)'");
			Иначе
				Текст = Текст + " " + НСтр("ru = '(запрет установлен на объект ""%3"" раздела ""%2"")'; en = '(object ""%3"" of section ""%2"" is prohibited)'");
			КонецЕсли;
		Иначе
			Текст = Текст + " " + НСтр("ru = '(установлена общая дата запрета)'; en = '(common prohibition date is set)'");
		КонецЕсли;
	КонецЕсли;
...
"""%1"".'; en = 'To check the prohibition of changes in data source, the table is not specified correctly"
"""%1"".'"),
				Строка.Таблица);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
"не задано поле даты.'; en = 'The date field is not specified in data"
"source for the ""%1"" table for checking"
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Поля[0]) Тогда
		Если ВидПоля = "ПолеДаты" Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
"неверно задано поле даты: ""%2""'; en = 'To check the change prohibition"
...
"the date field is incorrect: ""%2""'"),
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
"неверно задано поле объекта: ""%2""'; en = 'To check the change prohibition"
"in the data source for the ""%1"" table,"
...
"не задано поле заданной табличной части ""%2"".'; en = 'To check the change prohibition"
"in the data source for the ""%1"" table,"
...
"в источнике данных для таблицы ""%1"""
"неверно задано поле даты:"
"неверно задано поле заданной табличной части ""%2"".'; en = 'To check the change prohibition"
"in the data source for the ""%1"" table,"
"the date field is incorrect:"
...
"допускается только в разрезе измерения Пользователь.'; en = 'Uploading or downloading of the records of the information register ""Changes ban dates"""
"allowed only in the context of User dimension.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
