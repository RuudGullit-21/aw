...
Процедура ПерейтиНаНовуюРедакцию(НомерРедакции, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(НомерРедакции) Тогда
		ВызватьИсключение НСтр("ru = 'Не передан номер редакции.'; en = 'Edition number not given.'");
	КонецЕсли;
...
Процедура ПерейтиНаДругуюПрограмму(
	...
	Если Не ЗначениеЗаполнено(ИмяНовойПрограммы) Тогда
		ВызватьИсключение НСтр("ru = 'Не передано имя новой программы.'; en = 'The new program name has not been transferred.'");
	КонецЕсли;
	...
	Исключение
		...
		ПолучениеОбновленийПрограммыВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка проверки версии платформы при запуске. %1'; en = 'An error occurred while checking the platform on start. %1'"),
...
Процедура ОбработатьОповещениеОДоступномОбновлении(ПараметрыОповещения)
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru = 'Обновление программы'; en = 'Program update'"),
		Новый ОписаниеОповещения(
...
Процедура ПриИзмененииРасписания(Расписание, Форма) Экспорт
	...
	Если Форма.БИПАвтоматическаяПроверкаОбновлений = 2
		...
		ПоказатьПредупреждение(,
			НСтр("ru = 'Интервал проверки не может быть задан чаще, чем один раз 5 минут.'; en = 'Check interval cannot be set to more often than once every 5 minutes.'"));
		Возврат;
	КонецЕсли;
