...
Процедура ДополнитьПолучателей(Объект, Узлы) Экспорт
	...
	Для Каждого Элемент Из Узлы Цикл
		...
		Исключение
			...
			СтрокаСообщения  = НСтр("ru = 'Для состава плана обмена [ИмяПланаОбмена] не указана регистрация объекта [ПолноеИмя]'; en = 'Registration of object [ПолноеИмя] is not specified for exchange plan content [ИмяПланаОбмена]'");
	...
	Иначе
		...
		Если РежимВыгрузкиОбъекта = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости
			...
			Если ЭтоНовыйОбъект Тогда
				ВызватьИсключение НСтр("ru = 'Регистрация незаписанных объектов, выгружаемых по ссылке, не поддерживается.'; en = 'Registration of unwritten objects that can be exported by reference is not supported.'");
			КонецЕсли;
...
Процедура ПроверитьВозможностьИзмененияНастроекОбменаДанными(Источник, Отказ) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		ВызватьИсключение НСтр("ru = 'Изменение наименования и кода синхронизации данных недопустимо.'; en = 'Cannot change name and code of data synchronization.'");
...
Процедура ЗарегистрироватьИзменениеОбъекта(ИмяПланаОбмена, Объект, Отказ, ДополнительныеПараметры = Неопределено)
	...
	Попытка
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если Не РазделенныйПланОбмена(ИмяПланаОбмена) Тогда
				ВызватьИсключение НСтр("ru = 'Регистрация изменений для неразделенных планов обмена не поддерживается.'; en = 'Changes registration for undivided exchange plans is not supported.'");
			КонецЕсли;
			...
			Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
				...
				Если Не ЭтоРазделенныеДанные И Не ЭтоСовместноРазделенныеДанные Тогда
					ВызватьИсключение НСтр("ru = 'Регистрация изменений неразделенных данных в разделенном режиме.'; en = 'Changes registration of shared data in the split mode.'");
				КонецЕсли;
				
			Иначе
				
				Если ЭтоРазделенныеДанные Тогда
					ВызватьИсключение НСтр("ru = 'Регистрация изменений разделенных данных в неразделенном режиме.'; en = 'Change registration of separated data in the split mode.'");
				КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен данными.Правила регистрации объектов'; en = 'Data exchange.Objects registration rules'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ОбъектМодифицированДляПланаОбмена(Источник, ОбъектМетаданных, ИмяПланаОбмена, РежимЗаписи, ЗарегистрироватьОбъектКВыгрузке)
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка определения модифицированности объекта: %1'; en = 'An error occurred when determining modification of object: %1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
"Описание ошибки:"
"%2'; en = 'An error occurred when executing the object registration rules for exchange plan %1."
"Error details:"
"%2'"),
			ИмяПланаОбмена,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ЗначенияСвойствДляСсылки(Ссылка, СвойстваОбъекта, Знач СвойстваОбъектаСтрокой, Знач ОбъектМетаданныхИмя)
	...
	Исключение
		СтрокаСообщения = НСтр("ru = 'Ошибка при получении свойств ссылки. Ошибка выполнения запроса: [ОписаниеОшибки]'; en = 'An error occurred when receiving the reference properties. Query execution failed: [ОписаниеОшибки]'");
...
Функция МассивУзловПоЗначениямСвойств(ЗначенияСвойств, Знач ТекстЗапроса, Знач ИмяПланаОбмена, Знач ИмяРеквизитаФлага, Знач Выгрузка = Ложь) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("ru = 'Ошибка при получении списка узлов получателей. Ошибка выполнения запроса: [ОписаниеОшибки]'; en = 'An error occurred when receiving the list of target nodes. Failed to execute the query: [ОписаниеОшибки]'");
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОписаниеОшибки]", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СтрокаСообщения;
	КонецПопытки;
...
Функция НаборЗаписейПоТипу(ОбъектМетаданных)
	...
	Иначе
		...
		СтрокаСообщения = НСтр("ru = 'Для объекта метаданных %1 не предусмотрено набора записей.'; en = 'The record set is not provided for metadata object %1.'");
...
"// {Начало алгоритма}"
"[ЗначениеКонстанты]"
"// {Окончание алгоритма}'; en = 'Error of the algorithm for calculating the value of a constant:"
"Plan of exchange: [Name of Plans Exchange]"
"Metadata object: [Metadata objectName]"
...
Функция ДанныеРазличаются(Данные1, Данные2, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru = 'Задано недопустимое значение параметра [1] метода ОбщегоНазначения.ЗначенияСвойствИзменены.'; en = 'Invalid value of parameter [1] of method ОбщегоНазначения.ЗначенияСвойствИзменены is set.'");
...
Процедура ВыполнитьОбработчикВПривилегированномРежиме(Значение, Знач СтрокаОбработчика)
	...
	Если ТекущийРежимЗапуска() = РежимЗапускаКлиентскогоПриложения.УправляемоеПриложение Тогда
		ВызватьИсключение НСтр("ru = 'Метод не поддерживается в режиме управляемого приложения.'; en = 'Method is not supported in the managed application mode.'");
	КонецЕсли;
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ru = 'Метод не поддерживается при работе в модели сервиса.'; en = 'Method is not supported in SaaS operations.'");
	КонецЕсли;
		...
		Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель) Тогда
			Если СсылкаСуществует Тогда
				...
				Если ЗаписатьОбъект Тогда
					ПредупреждениеСинхронизации = НСтр("ru = 'Предыдущая версия (автоматическое разрешение конфликта).'; en = 'Previous version (automatic conflict resolution).'");
				Иначе
					ПредупреждениеСинхронизации = НСтр("ru = 'Текущая версия (автоматическое разрешение конфликта).'; en = 'Current version (automatic conflict resolution).'");
				КонецЕсли;
			...
			Если ЗаписатьОбъект Тогда
				...
				СведенияОВерсииОбъекта.Вставить("ПредупреждениеСинхронизации", НСтр("ru = 'Текущая версия (автоматическое разрешение конфликта).'; en = 'Current version (automatic conflict resolution).'"));
			Иначе
				СведенияОВерсииОбъекта.Вставить("АвторВерсии", Отправитель);
				СведенияОВерсииОбъекта.Вставить("ТипВерсииОбъекта", "НепринятыеДанныеПоКоллизии");
				СведенияОВерсииОбъекта.Вставить("ПредупреждениеСинхронизации", НСтр("ru = 'Отклоненная версия (автоматическое разрешение конфликта).'; en = 'Rejected version (automatic conflict resolution).'"));
			КонецЕсли;
...
Процедура ЗарегистрироватьЗапретЗагрузкиДанныхПоДате(ЭлементДанных, Отправитель, СообщениеОбОшибке)
	...
	Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель.Ссылка) И ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ЭлементДанных.Метаданные()) Тогда
		...
		Если СсылкаСуществует Тогда
			...
			ПредупреждениеСинхронизации = НСтр("ru = 'Версия создана при синхронизации данных.'; en = 'Version was created during data synchronization.'");
		...
		Иначе
			...
			СтрокаСообщенияОбОшибке = НСтр("ru = '%1 запрещено загружать в запрещенный период.%2%2%3'; en = '%1 cannot be imported to the prohibited period.%2%2%3'");
...
"Объект этой информационной базы был заменен версией объекта из второй информационной базы.'; en = 'An object change conflict occurred."
"The object from this infobase is replaced with the object version from the second infobase.'");
		
	Иначе
		
		ТекстПредупрежденияЖР = НСтр("ru = 'Возник конфликт изменений объектов."
"Объект из второй информационной базы не принят. Объект этой информационной базы не изменен.'; en = 'An object change conflict occurred."
...
Функция ДанныеОбъектаСтрокойДоИзменения(Объект, МетаданныеОбъекта, ЭтоСсылочныйТип, СсылкаСуществует, ОбъектВБазе)
	...
	Если ЭтоСсылочныйТип Тогда
		...
		Иначе
			...
			СтрокаОбъекта = НСтр("ru = 'Объект удален'; en = 'Object is removed'");
