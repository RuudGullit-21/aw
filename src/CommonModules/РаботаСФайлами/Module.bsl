...
Функция ДанныеФайла(Знач ПрисоединенныйФайл, Знач ДополнительныеПараметры = Неопределено,
	...
	Если ВызыватьИсключение Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(ФайлОбъект <> Неопределено, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найден присоединенный файл ""%1"" (%2)'; en = 'Attached file ""%1"" is not found (%2)'"),
...
Функция НоваяСсылкаНаФайл(ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru = 'Ошибка при получении новой ссылки на присоединенный файл.'; en = 'An error occurred when receiving a new reference to the attached file.'");
...
Функция ИмяФормыОбъектаФайловПоВладельцу(Знач ВладелецФайлов) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru = 'Ошибка при получении имени формы присоединенного файла.'; en = 'An error occurred when receiving a form name of the attached file.'");
	ОкончаниеОшибки = НСтр("ru = 'В этом случае получение формы невозможно.'; en = 'In this case, the form cannot be received.'");
...
Функция ДобавитьФайл(ПараметрыФайла,
	...
	ЗаголовокОшибки = НСтр("ru = 'Ошибка при добавлении присоединенного файла.'; en = 'An error occurred when adding the attached file.'");
...
"Ссылка на новый файл не заполнена.'; en = 'An error occurred when adding the attached file."
...
"%2'; en = 'An error occurred when adding attached file ""%1"":"
"%2'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Файлы.Добавление присоединенного файла'; en = 'File.Adding an attached file'",
...
Функция ГиперссылкаФайлов() Экспорт
	...
	ПараметрыГиперссылки.Вставить("Заголовок",                 НСтр("ru = 'Файлы'; en = 'Files'"));
...
Функция ПолеФайла() Экспорт
	...
	ПараметрыПоля.Вставить("ФильтрДиалогаВыбора",       НСтр("ru = 'Все файлы (*.*)|*.*'; en = 'All files (*.*)|*.*'"));
...
Процедура ПометитьНаУдалениеПрисоединенныеФайлы(Знач Источник, ИмяСправочника = Неопределено)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'Ошибка при пометке на удаление присоединенных файлов.'; en = 'An error occurred when marking the attached files for deletion.'")
			+ Символы.ПС
			+ ПредставлениеОшибки;
	КонецПопытки;
...
"занятый для редактирования.'; en = '""%1"" cannot be deleted"
...
Процедура ВыполнитьДействияПередЗаписьюПрисоединенногоФайла(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Тогда
		...
		Если НЕ РаботаСФайламиСлужебный.ЕстьПраво("ДобавлениеФайлов", Источник.ВладелецФайла) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недостаточно прав для добавления файлов в папку ""%1"".'; en = 'Insufficient rights to add files to the folder ""%1"".'"),
	...
	Иначе
		...
		Если ИзмененаПометкаУдаления Тогда
			...
			Если НЕ РаботаСФайламиСлужебный.ЕстьПраво("ПометкаУдаленияФайлов", Источник.ВладелецФайла) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недостаточно прав для пометки файлов на удаление в папке ""%1"".'; en = 'Insufficient permissions to mark files for deletion in the folder ""%1"".'"),
					Строка(Источник.ВладелецФайла));
			КонецЕсли;
		КонецЕсли;
		...
		Если ИзмененаПометкаУдаления И ЗначениеЗаполнено(Источник.Редактирует) Тогда
			...
			Если Источник.Редактирует = Пользователи.АвторизованныйПользователь() Тогда
				ТекстОшибки = НСтр("ru = 'Действие недоступно, так как файл ""%1"" занят для редактирования.'; en = 'Action is not available as the ""%1"" file is locked for editing.'");
...
"пользователем %2.'; en = 'Action is not available as user %2"
"is editing the ""%1"" file.'");
		...
		Если ЗаписьПодписанногоОбъекта <> Истина Тогда
			...
			Если Не Источник.ЭтоГруппа И Источник.ПодписанЭП И СсылкаПодписан И Занят И Не СсылкаЗанят Тогда
				ВызватьИсключение НСтр("ru = 'Подписанный файл нельзя редактировать.'; en = 'A signed file cannot be edited.'");
			КонецЕсли;
...
"""%1"".'; en = 'Owner in file"
"""%1"" is not entered.'"), Источник.Наименование);
		
		Если ОбновлениеИнформационнойБазы.ВыполняетсяОбновлениеИнформационнойБазы() Тогда
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Ошибка записи файла при обновлении ИБ'; en = 'Files. An error occurred when writing the file while updating IB'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура СоздатьГиперссылкуФайлов(Форма, ДобавляемыйЭлемент, ВладелецПрисоединенныхФайлов, ПараметрыГиперссылки)
	...
	Если ДобавляемыйЭлемент.ДобавлятьФайлы
		...
		ЗагрузитьФайл.Заголовок = НСтр("ru = 'Загрузить...'; en = 'Import...'");
...
Процедура СоздатьПолеФайла(Форма, ДобавляемыйЭлемент, ВладелецПрисоединенныхФайлов, ПараметрыПоляФайла)
	...
	СвойстваКнопкиЗагрузка.Вставить("Заголовок",            НСтр("ru = 'Загрузить...'; en = 'Import...'"));
	...
	Если ДобавляемыйЭлемент.ВыводитьЗаголовокФайла Тогда
		...
		ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗнч(РеквизитРазмещения)) Тогда
			ЗаголовокФайла.Подсказка       = НСтр("ru = 'Открыть файл'; en = 'Open file'");
	...
	Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
		...
		ПодменюДобавления.Заголовок   = НСтр("ru = 'Заменить'; en = 'Replace'");
		ПодменюДобавления.Отображение = ОтображениеКнопки.Картинка;
	...
	Если ДобавляемыйЭлемент.ПросматриватьФайл Тогда
		...
		ПросмотретьФайл.Заголовок = НСтр("ru = 'Просмотреть'; en = 'View'");
	...
	Если ДобавляемыйЭлемент.ОчищатьФайл
		...
		ОчиститьФайл.Заголовок = НСтр("ru = 'Очистить'; en = 'Clear'");
	...
	Если ДобавляемыйЭлемент.РедактироватьФайл = "ВФорме" Тогда
		...
		РедактироватьФайл.Заголовок = НСтр("ru = 'Открыть карточку'; en = 'Open card'");
	...
	ИначеЕсли ДобавляемыйЭлемент.РедактироватьФайл = "Непосредственно"
		...
		РедактироватьФайл.Заголовок = НСтр("ru = 'Редактировать'; en = 'Edit'");
		...
		ПоместитьФайл.Заголовок = НСтр("ru = 'Закончить редактирование'; en = 'End editing'");
		ПоместитьФайл.Подсказка = НСтр("ru = 'Сохранить и освободить файл в информационной базе'; en = 'Save and unlock file in infobase'");
		...
		ОтменитьРедактирование.Заголовок = НСтр("ru = 'Отменить редактирование'; en = 'Cancel editing'");
		ОтменитьРедактирование.Подсказка = НСтр("ru = 'Освободить занятый файл'; en = 'Unlock locked file'");
