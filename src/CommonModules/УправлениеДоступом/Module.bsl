...
Процедура ПриСозданииФормыЗначенияДоступа(Форма, ДополнительныеПараметры = Неопределено,
	...
	Если ГруппыЗначенийДляИзменения.Количество() = 0
	   И СозданиеНового Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для добавления требуются разрешенные ""%1"".'; en = 'Permitted ""%1"" are required for adding.'"),
...
//	ОписаниеПрофиля.Наименование  = НСтр("ru = 'Пользователь'; en = 'User'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УточнитьНаборыЗначенийДоступа(СсылкаНаОбъект, Таблица)
	...
	Для каждого Строка Из Таблица Цикл
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Объект ""%1"" сформировал набор значений доступа,"
"содержащий неизвестный вид доступа ""%2"".'; en = 'Object ""%1"" has generated a set of access values"
"that contains an unknown access kind ""%2"".'"),
				ТипЗнч(СсылкаНаОбъект),
				Строка.ВидДоступа);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Функция СвойстваГруппЗначенияДоступа(ТипЗначенияДоступа, ЗаголовокОшибки)
	...
	Если СвойстваВидаДоступа = Неопределено Тогда
		ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для значений доступа типа ""%1"""
"не используются группы значений доступа.'; en = 'Access value groups are not used for access"
"values of the ""%1"" type.'"),
			Строка(ТипЗначенияДоступа));
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
"интерфейса настройки прав доступа.'; en = 'This operation is possible only for simplified"
"interface of setting access rights.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
"Ожидалось: %3; передано значение: %4 (тип %5).'; en = 'Invalid value for parameter %1 to %2."
...
"которому разрешен вход в программу.'; en = 'At least one user with logon rights must have the Administrator profile"
