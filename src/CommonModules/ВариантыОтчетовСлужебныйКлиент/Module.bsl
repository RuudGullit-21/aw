...
Процедура УстановитьВысотуСтроки(Форма, Команда, СвойстваЗаголовка = Неопределено,
	...
	Если ВысотаСтроки = Неопределено Тогда 
		...
		ПоказатьВводЧисла(Обработчик, ПараметрыВысотыСтроки.Размер, НСтр("ru = 'Высота строки'; en = 'Высота строки'"), 5);
...
Функция КонтекстноеМенюОбластиЗаголовка()
	...
	КонтекстноеМеню.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.ОткрытьЗначение, НСтр("ru = 'Открыть'; en = 'Open'"));
	...
	// Сортировать
	КонтекстноеМеню.Добавить(НаправлениеСортировкиКомпоновкиДанных.Возр, НСтр("ru = 'Сортировать по возрастанию'; en = 'Ascending order'"),, БиблиотекаКартинок.СортироватьСтрокиПоВозрастанию);
	КонтекстноеМеню.Добавить(НаправлениеСортировкиКомпоновкиДанных.Убыв, НСтр("ru = 'Сортировать по убыванию'; en = 'Descending order'"),, БиблиотекаКартинок.СортироватьСтрокиПоУбыванию);
	...
	КонтекстноеМеню.Добавить(ПодменюОформления, НСтр("ru = 'Оформить'; en = 'Complete'"),, БиблиотекаКартинок.УсловноеОформлениеКомпоновкиДанных);
