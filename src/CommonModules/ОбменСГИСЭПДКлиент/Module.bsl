...
Процедура ПоказатьОшибкуЗаполнения(Форма) Экспорт
	
	//@skip-check unknown-method-property
	Если ТипЗнч(Форма.ПереходыПоОшибкам) = Тип("ФиксированныйМассив")
		И Форма.ПереходыПоОшибкам.Количество() > 0
...
Процедура ВывестиФормуВводаДанных(Форма, Знач ИмяКоманды, ТекущиеДанные = Неопределено, ЭтоВыбор = Истина, ЕстьХранимыеДанные = Ложь, ОповещениеОЗавершении = Неопределено) Экспорт
	...
	Если ЭтоВыбор = Истина Тогда
		...
		Если СтандартнаяОбработка Тогда
			...
			ОбменСГИСЭПДКлиент.ПоказатьХранимыеДанныеЭПД(ГруппаДанных, ОтборХранимыхДанныхЭПД, Форма, ОписаниеОповещения, Отказ, ИдентификаторСтрокиРодителя);
		КонецЕсли;
		// {/УАТ}
	КонецЕсли;
...
Процедура ВводАдреса(ИмяЭлемента, Форма, ВладелецАдреса = Неопределено, ТекущиеДанные = Неопределено) Экспорт
	...
	// {УАТ}
	СтандартнаяОбработка = Истина;
	СтандартнаяОбработка = уатОбменСГИСЭПДКлиент.ВводАдреса(ПараметрыФормыВводаАдреса, ИмяЭлемента, Форма, Оповещение);
	Если СтандартнаяОбработка Тогда
