...
"%ИнформацияОбОшибке%'; en = 'An error occurred while receiving IB user name (ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()) as:"
"%ИнформацияОбОшибке%'");
		...
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Разное. Ошибка работы с пользователями'; en = 'News. Different. Error of user management'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
...
"%ИнформацияОбОшибке%'; en = 'An error occurred while receiving a user from the Users catalog by its name (%ИмяПользователяИБ%) in IB (ОбработкаНовостейПовтИсп.ПолучитьПользователяПоИмениПользователяИБ()) due to:"
"%ИнформацияОбОшибке%'");
...
"Причина:"
"%ИнформацияОбОшибке%'; en = 'Error search user (%ПолныеПрава%) by a unique identifier:"
"Short user name: %КраткоеNameПользователя%"
"User full name: %ПолноеИмяПользователя%"
"UUID: %УникальныйИдентификатор%"
"Reason:"
"%ИнформацияОбОшибке%'")
					+ Символы.ПС;
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПолныеПрава%", ?(ПолныеПрава = Истина, НСтр("ru = 'доступны полные права'; en = 'full rights are available'"), НСтр("ru = 'роль РедакторНовостей'; en = 'NewsEditor role'")));
				...
				Если НЕ НайденныйПользователь.Пустая() Тогда
					Результат.Добавить(
						Новый Структура("Пользователь, ПользовательПредставление, ПолныеПрава",
							...
							?(ПолныеПрава = Истина,
								НайденныйПользователь.Наименование + " " + НСтр("ru = '(полные права)'; en = '(full rights)'"),
								НайденныйПользователь.Наименование + " " + НСтр("ru = '(редактор новостей)'; en = '(news editor)'")),
							ПолныеПрава));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
