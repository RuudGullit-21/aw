...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		ДелоРодитель.Представление  = НСтр("ru = 'Почта'; en = 'Mail'");
...
Функция СписокДоступныхПоисков(ППДВключен, Параметры, ЭлементыФормы, ДляАдреснойКниги) Экспорт
	...
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "ПоEmail", НСтр("ru = 'По email'; en = 'By email'"), Адрес);
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "ПоДомену", НСтр("ru = 'По доменному имени'; en = 'By domain name'"), ДоменныйАдрес);
	
	Если Не ДляАдреснойКниги И (Параметры.Свойство("ТолькоEmail") И Не Параметры.ТолькоEmail) Тогда
		ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "ПоТелефону", НСтр("ru = 'По телефону'; en = 'By phone number'"), Адрес);
	КонецЕсли;
	...
	Если ППДВключен Тогда
		ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "ПоСтроке",
			НСтр("ru = 'По строке'; en = 'By line'"), ВариантыПоискаПоСтроке);
	КонецЕсли;
	
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "НачинаетсяС", НСтр("ru = 'Начинается с'; en = 'Begins with'"), Представление);
...
Функция ПолнотекстовыйПоискКонтактовПоСтроке(Знач СтрокаПоиска, НайденныеКонтакты, Знач ДляАдреснойКниги = Ложь) Экспорт
	...
	Возврат ?(КоличествоНайденныхЭлементов < 101, "", НСтр("ru = 'Уточните параметры поиска. В списке отображены не все найденные контакты.'; en = 'Specify the search parameters. list displays the found contacts.'"));
...
Процедура СформироватьСписокВыбораТипВзаимодействияТолькоПочта(Элемент)
	...
	Элемент.СписокВыбора.Добавить("ВсеПисьма", НСтр("ru = 'Все письма'; en = 'All emails'"));
	Элемент.СписокВыбора.Добавить("ВходящиеПисьма", НСтр("ru = 'Входящие'; en = 'Incoming'"));
	Элемент.СписокВыбора.Добавить("ПисьмаЧерновики", НСтр("ru = 'Черновики'; en = 'Drafts'"));
	Элемент.СписокВыбора.Добавить("ИсходящиеПисьма", НСтр("ru = 'Исходящие'; en = 'Outgoing'"));
	Элемент.СписокВыбора.Добавить("Отправленные", НСтр("ru = 'Отправленные'; en = 'Sent'"));
	Элемент.СписокВыбора.Добавить("УдаленныеПисьма", НСтр("ru = 'Удаленные'; en = 'Deleted'"));
...
Процедура УстановитьЗаголовокФормыЭлектронногоПисьма(Форма) Экспорт
	...
	Если НЕ ОбъектПисьмо.Ссылка.Пустая() Тогда
		...
		Форма.Заголовок  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокФормы,
			?(ТипЗнч(ОбъектПисьмо.Ссылка) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее"), НСтр("ru = 'Входящее'; en = 'Incoming'"), НСтр("ru = 'Исходящее'; en = 'Outgoing'")));
			
	Иначе
		Если ТипЗнч(ОбъектПисьмо.Ссылка) = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") Тогда
			Форма.Автозаголовок = Ложь;
			Форма.Заголовок = НСтр("ru = 'Исходящее письмо (создание)'; en = 'Outgoing email (create)'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьСтраницыФормыПодбораКонтактов(Форма) Экспорт
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		Если ЕстьКолонкиАдрес[ОписаниеКонтакта.Имя] Тогда
			...
			КолонкаАдрес.Заголовок = НСтр("ru = 'Адрес'; en = 'Address'");
		КонецЕсли;
...
Процедура ИнициализироватьФормуСпискаВзаимодействий(Форма, Параметры) Экспорт
	...
	Если Форма.ТолькоПочта Тогда
		Форма.Заголовок = НСтр("ru = 'Электронная почта'; en = 'Email'");
		...
		Форма.Команды.Предмет.Заголовок = НСтр("ru = 'Установить предмет переписки'; en = 'Set correspondence subject'");
		Форма.Команды.Предмет.Подсказка = НСтр("ru = 'Установить предмет переписки'; en = 'Set correspondence subject'");
		...
		Если Форма.Команды.Найти("ПредметСписок") <> Неопределено Тогда
			Форма.Команды.ПредметСписок.Заголовок = НСтр("ru = 'Установить предмет переписки'; en = 'Set correspondence subject'");
			Форма.Команды.ПредметСписок.Подсказка = НСтр("ru = 'Установить предмет переписки'; en = 'Set correspondence subject'");
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьЭлементДанныхШапкиПисьма(ЭлементРодитель, ШапкаПисьма, ТолькоПоПредставлениюОтправителя = Ложь)
	...
	Тема = ?(ПустаяСтрока(ШапкаПисьма.Тема), НСтр("ru = '<Без Темы>'; en = '<No Subject>'"), ШапкаПисьма.Тема);
...
Процедура ДополнитьТелоПисьмаПодваломВложения(ДокументHTML, Вложения) Экспорт
	...
	СтрокаКоличествоВложений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вложений: %1.'; en = 'Attachment: %1.'"), Вложения.Количество());
	ДобавитьТекстовыйУзел(ЭлементШрифт, СтрокаКоличествоВложений, Истина, Истина);
	
	Для Каждого Вложение Из Вложения Цикл 
		
		ПредставлениеВложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2).'; en = '%1 (%2).'"), Вложение.ИмяФайла, Вложение.РазмерПредставление);
...
Функция ОтправкаSMSПоДокументу(Документ) Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не выполнены настройки отправки SMS.'; en = 'Settings of SMS sending are not executed.'"),,"Объект");
...
Процедура ОтправкаSMS() Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		...
		УровеньЖурналаРегистрации.Ошибка, , ,
		НСтр("ru = 'Не выполнены настройки отправки SMS.'; en = 'Settings of SMS sending are not executed.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат;
	КонецЕсли;
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ТекстСообщения = НСтр("ru = 'Не удалось отправить : %Ссылка% по причине: %Причина%'; en = 'Failed to send : %Ссылка% reason: %Причина%'");
...
Процедура ОбновлениеСтатусовДоставкиSMS() Экспорт
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ТекстСообщения = НСтр("ru = 'Не удалось обновить статусы доставки : %Ссылка% по причине: %Причина%'; en = 'Failed to update the delivery statuses : %Ссылка% for a reason: %Причина%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Ссылка%", ВыборкаДокументы.Ссылка);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(),
			                         УровеньЖурналаРегистрации.Предупреждение,
...
Процедура ЗаполнитьСписокВыбораВремени(ПолеВводаФормы, Интервал = 3600) Экспорт
	...
	Пока НачалоЧаса(ВремяСписка) <= НачалоЧаса(ОкончаниеРабочегоДня) Цикл
		...
		Иначе
			ПредставлениеВремени = Формат(ВремяСписка, НСтр("ru = 'ДФ=ЧЧ:мм'; en = 'ДФ=ЧЧ:мм'"));
		КонецЕсли;
...
Функция СписокОтборовПоТипуВзаимодействий(ТолькоПочта) Экспорт
	...
	СписокОтборов.Добавить("ВсеПисьма", НСтр("ru = 'Все письма'; en = 'All emails'"));
	Если Не ТолькоПочта Тогда
		СписокОтборов.Добавить("Встречи", НСтр("ru = 'Встречи'; en = 'Meetings'"));
		СписокОтборов.Добавить("ТелефонныеЗвонки", НСтр("ru = 'Телефонные звонки'; en = 'Phone calls'"));
		СписокОтборов.Добавить("ЗапланированныеВзаимодействия", НСтр("ru = 'Запланированные взаимодействия'; en = 'Scheduled interactions'"));
		СписокОтборов.Добавить("СообщенияSMS", НСтр("ru = 'Сообщения SMS'; en = 'SMS'"));
	КонецЕсли;
	СписокОтборов.Добавить("ВходящиеПисьма", НСтр("ru = 'Входящие'; en = 'Incoming'"));
	СписокОтборов.Добавить("ПисьмаЧерновики", НСтр("ru = 'Черновики'; en = 'Drafts'"));
	СписокОтборов.Добавить("ИсходящиеПисьма", НСтр("ru = 'Исходящие'; en = 'Outgoing'"));
	СписокОтборов.Добавить("Отправленные", НСтр("ru = 'Отправленные'; en = 'Sent'"));
	СписокОтборов.Добавить("УдаленныеПисьма", НСтр("ru = 'Удаленные'; en = 'Deleted'"));
	Если Не ТолькоПочта Тогда
		СписокОтборов.Добавить("ИсходящиеЗвонки", НСтр("ru = 'Исходящие звонки'; en = 'Outgoing calls'"));
		СписокОтборов.Добавить("ВходящиеЗвонки", НСтр("ru = 'Входящие звонки'; en = 'Incoming calls'"));
	КонецЕсли;
...
Функция СписокСтатусов() Экспорт
	...
	СписокСтатусов.Добавить("КРассмотрению", НСтр("ru = 'К рассмотрению'; en = 'To review'"));
	СписокСтатусов.Добавить("Отложенные", НСтр("ru = 'Отложенные'; en = 'Deferred'"));
	СписокСтатусов.Добавить("Рассмотренные", НСтр("ru = 'Рассмотренные'; en = 'Reviewed'"));
...
Функция ПредставлениеПисьма(ТемаПисьма, ДатаПисьма) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 от %2'; en = '%1 from %2'"), 
...
Процедура ЗаполнитьСписокВыбораДляРассмотретьПосле(СписокВыбора) Экспорт
	...
	СписокВыбора.Добавить(15*60,    НСтр("ru = 'Через 15 мин.'; en = 'In 15 min.'"));
	СписокВыбора.Добавить(30*60,    НСтр("ru = 'Через 30 мин.'; en = 'In 30 min.'"));
	СписокВыбора.Добавить(60*60,    НСтр("ru = 'Через 1 час'; en = 'In an hour'"));
	СписокВыбора.Добавить(3*60*60,  НСтр("ru = 'Через 3 часа'; en = 'In 3 hours'"));
	СписокВыбора.Добавить(24*60*60, НСтр("ru = 'Завтра'; en = 'Tomorrow'"));
