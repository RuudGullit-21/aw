...
"Подробнее см. в журнале регистрации.'; en = 'An error occurred while logging on to 1C:ITS Portal."
...
Функция ТекстВложенияТехническаяИнформация(ВидПриложения, ДопПараметры)
	...
	Если ИмяПрограммы = "Unknown" Тогда
		ИмяПрограммы = НСтр("ru = '<Не заполнено>'; en = '<Not filled in>'");
	КонецЕсли;
...
"Вид приложения: %8;"
"Режим: %9;'; en = 'Technical information about program:"
"Program title: %1"
"Program name: %2;"
"Program version: %3;"
"Supplier: %4;"
...
"Rights for Online user support: %3;'"),
			?(Пользователи.ЭтоПолноправныйПользователь(, Ложь, Ложь), НСтр("ru = 'есть'; en = 'there are'"), НСтр("ru = 'нет'; en = 'no'")),
			?(Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь), НСтр("ru = 'есть'; en = 'there are'"), НСтр("ru = 'нет'; en = 'no'")),
			?(ИнтернетПоддержкаПользователей.ПравоЗаписиПараметровИПП(), НСтр("ru = 'есть'; en = 'there are'"), НСтр("ru = 'нет'; en = 'no'")));
...
"Регистрационный номер программного продукта: %3;'; en = 'Login for connecting online support: %1;"
"Password for connecting online support: %2;"
"Registration number of the software product: %3;'"),
			ДопПараметры.Логин,
			?(ДопПараметры.ПарольЗаполнен, НСтр("ru = 'заполнен'; en = 'filled with'"), НСтр("ru = 'не заполнен'; en = 'not filled in'")),
...
"	Доменная зона: %1'; en = 'Connection settings with Internet support servers:"
"     Domain zone: %1'"),
	...
	Если НастройкиПрокси = Неопределено Тогда
		ЗначениеНастройкиПрокси = НСтр("ru = 'не используется'; en = 'not used'");
	Иначе
	...
	Результат = Результат
		+ Символы.ПС
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Идентификатор ИБ: %1'; en = 'Identifier ID: %1'"),
...
Функция ТекстОписаниеУстановленныхРасширений()
	...
	Если ОписаниеРасширенийМассивСтр.Количество() = 0 Тогда
		ОписаниеРасширенийМассивСтр.Добавить(НСтр("ru = '<Нет>'; en = '<No>'"));
	КонецЕсли;
