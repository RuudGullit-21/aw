...
"Регламентное задание ""%1"", работающее с внешними ресурсами, отключено.'; en = 'The application was moved."
...
"Регламентное задание ""%1"" отключено.'; en = 'Infobase connection string was changed."
"Perhaps, the infobase was moved."
...
Функция УстановитьБлокировкуРаботыСВнешнимиРесурсами()
	...
	Если ИзменилосьРазделение Тогда
		ТекстСообщения = НСтр("ru = 'Информационная база была перемещена из приложения в Интернете.'; en = 'Infobase was transferred from an online application.'");
	...
	Если ПеремещенаМеждуФайловымИКлиентСервернымРежимом Тогда
		ТекстСообщения = 
			?(ЭтоФайловаяБаза, 
				НСтр("ru = 'Информационная база была перемещена из клиент-серверного режима работы в файловый.'; en = 'Infobase was transferred from the client/server mode to the file mode.'"),
				НСтр("ru = 'Информационная база была перемещена из файловый режима работы в клиент-серверный.'; en = 'Infobase was transferred from the file mode to the client/server mode.'"));
		УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
		Возврат Истина;
	КонецЕсли;
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт 
	...
	Возврат НСтр("ru = 'Работа с внешними ресурсами заблокирована'; en = 'Operations with external resources have been locked'", ОбщегоНазначения.КодОсновногоЯзыка());
