...
Процедура ПолучениеИОтправкаЭлектронныхПисем() Экспорт
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), 
		...
		НСтр("ru = 'Начато регламентное получение и отправка электронной почты'; en = 'Scheduled email receiving and sending is started'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru = 'Закончено регламентное получение и отправка электронной почты'; en = 'Scheduled receiving and sending of emails is completed'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОтправитьУведомленияОПрочтении(ДляТекущегоПользователя)
	...
	Пока ВыборкаУчетныеЗаписи.Следующий() Цикл
		...
		Пока ВыборкаПисьма.Следующий() Цикл
			...
			ПараметрыПисьма.Вставить("Тема",НСтр("ru = 'Уведомление о прочтении'; en = 'Read receipt'") + " / " +"Reading Confirmation");
...
Процедура ЗаписатьВложениеЭлектронногоПисьма(Объект, Вложение,МассивПодписей,КоличествоПустыхИменВоВложениях)
	...
	Если ЕстьПодписи Тогда
		...
		Для Каждого ПодписьВложения Из МассивПодписей Цикл
			...
			ПараметрыПодписи.Вставить("Комментарий", НСтр("ru = 'Вложение электронного письма'; en = 'Attach email'"));
...
Функция ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(
	...
	Если ПустаяСтрока(ИмяБезРасширения) Тогда
		...
		ИмяБезРасширения =
			НСтр("ru = 'Вложение без имени'; en = 'Attachment without name'") + ?(КоличествоПустыхИменВоВложениях = 0, ""," " + Строка(КоличествоПустыхИменВоВложениях + 1));
...
"Отправленное %4"
"Было прочитано %5"
"Получателем %6 <%7>'; en = 'Message from %1 < %2 >"
"Subject: %3"
"Sent %4"
"Read %5"
"By receiver %6 <%7>'");
...
Функция ТипПредопределеннойПапкиПоИмени(ИмяПредопределеннойПапки) Экспорт
	...
	Если ИмяПредопределеннойПапки = НСтр("ru = 'Входящие'; en = 'Incoming'") Тогда
		ТипПредопределеннойПапки = Перечисления.ТипыПредопределенныхПапокПисем.Входящие;
	ИначеЕсли ИмяПредопределеннойПапки = НСтр("ru = 'Исходящие'; en = 'Outgoing'")  Тогда
		ТипПредопределеннойПапки = Перечисления.ТипыПредопределенныхПапокПисем.Исходящие;
	ИначеЕсли ИмяПредопределеннойПапки = НСтр("ru = 'Удаленные'; en = 'Deleted'") Тогда
		ТипПредопределеннойПапки = Перечисления.ТипыПредопределенныхПапокПисем.Удаленные;
	ИначеЕсли ИмяПредопределеннойПапки = НСтр("ru = 'Черновики'; en = 'Drafts'") Тогда
		ТипПредопределеннойПапки = Перечисления.ТипыПредопределенныхПапокПисем.Черновики;
	ИначеЕсли ИмяПредопределеннойПапки = НСтр("ru = 'Отправленные'; en = 'Sent'")  Тогда
		ТипПредопределеннойПапки = Перечисления.ТипыПредопределенныхПапокПисем.Отправленные;
	ИначеЕсли ИмяПредопределеннойПапки = НСтр("ru = 'Нежелательная почта'; en = 'Junk email'") Тогда
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Взаимодействия'; en = 'Interactions'", ОбщегоНазначения.КодОсновногоЯзыка());
