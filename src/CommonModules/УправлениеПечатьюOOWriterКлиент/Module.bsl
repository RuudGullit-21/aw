...
Функция ПолучитьПозициюНачалаОбласти(Знач xDocument, Знач ИмяОбласти)
	...
	Если xFound = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не найдено начало области макета:'; en = 'Template area start is not found:'") + " " + ИмяОбласти;	
	КонецЕсли;
	Возврат xFound.End;
...
Функция ПолучитьПозициюОкончанияОбласти(Знач xDocument, Знач ИмяОбласти)
	...
	xFound = xDocument.findFirst(xSearchDescr);
	Если xFound = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не найден конец области макета:'; en = 'End of template area is not found:'") + " " + ИмяОбласти;	
	КонецЕсли;
