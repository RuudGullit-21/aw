...
Процедура СохранитьКомпонентуВФайл(ВнешняяКомпонентаСсылка) Экспорт 
	...
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru = 'Выберите файл для сохранения внешней компоненты'; en = 'Select the file to save external component'");
	ПараметрыСохранения.Диалог.Фильтр    = НСтр("ru = 'Файлы внешних компонент (*.zip)|*.zip|Все файлы (*.*)|*.*'; en = 'External component files (*.zip)|*.zip|All files (*.*)|*.*'");
...
Процедура СохранитьКомпонентуВФайлПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ПолученныеФайлы <> Неопределено 
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Сохранение в файл'; en = 'Save to file'"),,
			НСтр("ru = 'Внешняя компонента успешно сохранена в файл.'; en = 'External component is successfully saved to file.'"), БиблиотекаКартинок.Успешно32);
	КонецЕсли;
