...
Функция ПодсказкаВДокументе(ПараметрыПрорисовки, СостояниеПроверки) Экспорт
	...
	РекламаСервиса = Новый ФорматированнаяСтрока(
		НСтр("ru = 'Проверьте контрагента по кнопке в подменю Еще'; en = 'Check counterparty by clicking the button in submenu More'"));
	...
	ИначеЕсли СостояниеПроверки = ПредопределенноеЗначение("Перечисление.СостоянияПроверкиКонтрагентов.ПроверкаВПроцессе") Тогда
		...
		Подстроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Выполняется проверка контрагентов согласно данным ФНС'; en = 'Checking counterparties against FTS data'")));
...
"сервис ФНС временно недоступен'; en = 'Could not check the counterparties:"
"service of FTS is temporarily unavailable'")));
	...
	ИначеЕсли СостояниеПроверки = ПредопределенноеЗначение("Перечисление.СостоянияПроверкиКонтрагентов.ПроверкаВыполнена")
		...
		Если НЕ КонтрагентЗаполнен Тогда
			...
			Подстроки.Добавить(НСтр("ru = 'Проверка контрагента по базе ФНС не выполнена: не заполнен контрагент'; en = 'Counterparty was not checked in the FTS base: counterparty was not specified'"));
		...
		ИначеЕсли СостояниеКонтрагента = ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.ПустойИННИлиКПП") Тогда
			...
			Подстроки.Добавить(НСтр("ru = 'Не заполнен ИНН или КПП'; en = 'TIN or CRTR is not entered'"));
		...
		ИначеЕсли ЭтоСостояниеНедействующегоКонтрагента(СостояниеКонтрагента) Тогда
			...
			Если КонтрагентовНесколько Тогда
				// Выводим обобщенно
				Подстроки.Добавить(НСтр("ru = 'Обнаружены недействующие контрагенты по данным ФНС'; en = 'Inactive counterparties (according to FTS) are detected'"));
			Иначе
		...
		ИначеЕсли СостояниеКонтрагента = ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.КонтрагентСодержитОшибкиВДанных") Тогда
			...
			Подстроки.Добавить(НСтр("ru = 'Обнаружены ошибки в заполнении ИНН, КПП или даты документа'; en = 'Errors in TIN, CRTR or document date are detected'"));
			ЦветФона = ЦветНекорректногоКонтрагента;
		...
		ИначеЕсли СостояниеКонтрагента = ПредопределенноеЗначение("Перечисление.СостоянияСуществованияКонтрагента.КонтрагентЕстьВБазеФНС") Тогда
			...
			Если КонтрагентовНесколько Тогда
				// Выводим обобщенно 
				Подстроки.Добавить(НСтр("ru = 'Проверка контрагентов по данным ФНС выполнена успешно'; en = 'Counterparties check in FTS base was successful'"));
			Иначе
				// Выводим конкретное состояние.
				Подстроки.Добавить(Строка(СостояниеКонтрагента));
			КонецЕсли;
...
Функция СсылкаНаИнструкцию() Экспорт
	Возврат Новый ФорматированнаяСтрока(НСтр("ru = 'Подробнее о проверке'; en = 'More about check'"),,,, ПутьКИнструкции());
КонецФункции
