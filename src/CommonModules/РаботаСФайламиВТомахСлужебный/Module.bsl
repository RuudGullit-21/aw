...
"Обратитесь к администратору.'; en = 'Failed to add file ""%1"" in any of the volumes, because it does not exist."
"The file may be removed by antivirus software."
...
"в том ""%2"" (%3):"
"""%4"".'; en = 'An error occurred when adding file ""%1"""
"to volume ""%2"" (%3):"
"""%4"".'");
		
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Добавление файла'; en = 'Files. Adding a file'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
...
"""%1.%2""."
""
"Обратитесь к администратору.'; en = 'Cannot add file:"
"""%1.%2""."
""
...
Функция ПредставлениеСтатусаПроверки(Знач СтатусПроверки) Экспорт
	Если СтатусПроверки = "ОК" Тогда
		Возврат НСтр("ru = 'Целостные данные'; en = 'Integral data'");
	ИначеЕсли СтатусПроверки = "ЛишнийФайлВТоме" Тогда 
		Возврат НСтр("ru = 'Лишние файлы (есть на диске, но сведения о них отсутствуют)'; en = 'Excess files (exist on disk but no information about them)'");
...
Функция СвободныйТом(РазмерФайла)
	...
	Если Результат.Пустой() Тогда
		ВызватьИсключение НСтр("ru = 'Нет ни одного тома для размещения файла.'; en = 'There are no volumes to place a file in.'");
	КонецЕсли;
