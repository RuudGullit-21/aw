...
Функция НомераДоверенностейПоУчастникам(ДоверительИНН, ПредставительИНН, ЭлектронныйДокумент = Неопределено) Экспорт
...
Функция РезультатПроверкиДоверенности(ДанныеДоверенности, РезультатПроверкиПодписи, КонтекстДиагностики) Экспорт
	...
	ДанныеXDTO = Неопределено; // Неопределено,ОбъектXDTO
	...
	Если ДанныеXDTO <> Неопределено Тогда
		СНИЛСДоверителяФЛ = СНИЛСФизическогоЛицаБезДоверенности(ДанныеXDTO);
	КонецЕсли;
...
// Возвращаемое значение:
//  Строка
Функция СНИЛСФизическогоЛицаБезДоверенности(ДанныеДоверенностиXDTO)
	
	СНИЛС = "";
	
	ВерсияФормата = ВерсияФорматаОбъектаМЧД(ДанныеДоверенностиXDTO);
	
	Если ВерсияФормата = ФорматМЧД_2022_Версия_002() Тогда
	...
	ЗначениеСвойства = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеДоверенностиXDTO, ПутьКСвойству);
	
	Если ЗначениеСвойства = Неопределено Тогда
		Возврат СНИЛС;
	КонецЕсли;
	
	СНИЛС = СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими(" -", ЗначениеСвойства, "");
