...
Процедура ПриВключенииИспользованияПрофилейБезопасности() Экспорт
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(),
		...
		НСтр("ru = 'При включении профилей безопасности настройки прокси-сервера сброшены на системные.'; en = 'When enabling security profiles, proxy server settings were reset to default values.'"));
...
Функция ПолучитьФайлИзИнтернет(Знач URL, Знач НастройкаСохранения, Знач НастройкаСоединения,
			...
			Если ЗаписыватьОшибку Тогда
				СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1
					           ...
					           |Таймаут: %3'"),
					ТекстОшибки,
					Формат(Соединение.Защищенное, НСтр("ru = 'БЛ=Нет; БИ=Да'; en = 'BF=No; BT=Yes'")),
		...
		Исключение
			...
			Если ЗаписыватьОшибку Тогда
				СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					Формат(Соединение.Защищенное, НСтр("ru = 'БЛ=Нет; БИ=Да'; en = 'BF=No; BT=Yes'")),
					Формат(Соединение.Таймаут, "ЧГ=0"),
					Формат(Соединение.ИспользоватьАутентификациюОС, НСтр("ru = 'БЛ=Нет; БИ=Да'; en = 'BF=No; BT=Yes'")));
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Не указано место для сохранения файла.'; en = 'The location to save the file is not specified.'");
	КонецЕсли;
...
Функция ПредставлениеИнтернетПрокси(Прокси, Протокол = Неопределено)
	...
	Если Прокси.НеИспользоватьПроксиДляЛокальныхАдресов Тогда 
		Журнал.Добавить(НСтр("ru = 'Не использовать прокси для локальных адресов'; en = 'Bypass proxy for local URLs'"));
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Получение файлов из Интернета'; en = 'Get files from the Internet'", ОбщегоНазначения.КодОсновногоЯзыка());
