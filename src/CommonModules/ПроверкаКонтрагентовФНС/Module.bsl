...
Функция ПолучитьСостоянияКонтрагентов(Прокси, КонтрагентыЗапроса)
	...
	Исключение
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			...
			НСтр("ru = 'NdsRequest2'; en = 'NdsRequest2'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ДобавитьКонтрагентаВЗапросКСервису(
	...
	Исключение
		...
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			...
			НСтр("ru = 'Формирование данных для обращения к сервису ФНС'; en = 'Data formation adressed to service fts'",
...
Процедура ЗаполнитьСостоянияКонтрагентов(
	...
	Для Каждого КонтрагентОтвета Из NdsResponse.NP Цикл
		...
		Исключение
			...
			ЗаписатьОшибкуВЖурналРегистрации(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке),
				НСтр("ru = 'Обработка ответа от веб-сервиса'; en = 'Processing response from the web service'",
					ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ПроксиСервиса(Таймаут = 120)
	...
	Исключение
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			СообщениеЖурналаРегистрации,
			НСтр("ru = 'Ошибка доступа к веб-сервису'; en = 'Error access to web service'", ОбщегоНазначения.КодОсновногоЯзыка()));
